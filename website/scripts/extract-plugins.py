...
- ``plugin-catalog.json`` — ``{"entries": [raw YAML mappings], "removed": [...]}`` in the loader's own
  schema; installed Missher clients fetch this for live catalog refresh
  (``missher_cli.plugin_catalog.LIVE_CATALOG_URL``) so new entries and removals reach them without
...
def load_catalog_entries(catalog_dir: Path) -> list[dict]:
    ...
    for path in sorted(catalog_dir.glob("*.yaml")):
        ...
        entries.append({
            ...
            "requiresMissher": str(raw.get("requires_missher") or "").strip(),
            ...
            "installCommand": f"missher plugins install {name}",
        })
