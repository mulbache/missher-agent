...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/missher-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    missher_meta = (fm.get("metadata") or {}).get("missher") or {}
    tags = missher_meta.get("tags") or []
    related = missher_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `missher skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Missher Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Missher ships with a large built-in skill library copied into `~/.missher/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Missher also syncs bundled skills on `missher update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.missher/skills/` tree, it is still shipped with Missher; restore it with `missher skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with missher-agent — install via missher skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with missher-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "missher skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "missher skills install official/blockchain/solana",
        "missher skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "missher skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
