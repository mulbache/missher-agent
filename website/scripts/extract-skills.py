...
2. The unified Missher Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                missher_meta = metadata.get("missher", {})
                if isinstance(missher_meta, dict):
                    tags = missher_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
