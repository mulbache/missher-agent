...
const CATALOG_README_URL =
  "https://github.com/NousResearch/missher-agent/tree/main/plugin-catalog";
      ...
      <div className={styles.cardInner}>
        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            {plugin.requiresMissher && (
              <div className={styles.metaRow}>
                <span className={styles.metaLabel}>Requires</span>
                <span className={styles.metaValue}>
                  <code>missher {plugin.requiresMissher}</code>
                </span>
              </div>
            )}
            <div className={styles.metaRow}>
...
export default function PluginCatalogPage() {
  ...
  // (Missher desktop's Capabilities > Plugins tab) as a one-click catalog
  // picker. Site chrome is hidden via CSS and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'missher-plugin-pick', name, repo, sha, subdir, tier,
  //     installCmd }
  ...
  const pickPlugin = useCallback((plugin: CatalogPlugin) => {
    ...
    window.parent.postMessage(
      {
        type: "missher-plugin-pick",
        name: plugin.name,
        repo: plugin.repo,
        sha: plugin.sha,
        subdir: plugin.subdir || "",
        tier: plugin.tier,
        installCmd: plugin.installCommand || `missher plugins install ${plugin.name}`,
      },
      "*"
    );
  }, []);
  ...
  return (
    <Layout
      ...
      description="Browse reviewed, SHA-pinned plugins for Missher Agent"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>Missher Agent</p>
          ...
          {!data && !loadError ? (
            ...
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>
                The plugin catalog is a curated, human-reviewed list of Missher
