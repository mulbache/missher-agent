...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Missher 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Missher 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Missher](/guides/use-voice-mode-with-missher)。
...
```bash
missher gateway setup        # 交互式配置所有消息平台
```
...
```bash
missher gateway              # 在前台运行
missher gateway setup        # 交互式配置消息平台
missher gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo missher gateway install --system   # 仅 Linux：安装开机启动的系统服务
missher gateway start        # 启动默认服务
missher gateway stop         # 停止默认服务
missher gateway status       # 检查默认服务状态
missher gateway status --system         # 仅 Linux：显式检查系统服务
```
...
| `/update` | 将 Missher Agent 更新至最新版本 |
| `/help` | 显示可用命令 |
...
# 其他配对命令：
missher pairing list          # 查看待审核和已批准的用户
missher pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Missher 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.missher/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.missher/config.yaml` 中的 `display.background_process_notifications` 控制：
...
missher gateway start                 # 启动服务
missher gateway stop                  # 停止服务
missher gateway status                # 检查状态
journalctl --user -u missher-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Missher 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Missher 安装（使用不同的 `MISSHER_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.missher` 使用 `missher-gateway`；其他安装使用 `missher-gateway-<hash>`。`missher gateway` 命令会自动针对当前 `MISSHER_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
missher gateway install               # 安装为 launchd agent
missher gateway start                 # 启动服务
missher gateway stop                  # 停止服务
missher gateway status                # 检查状态
tail -f ~/.missher/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.missher.gateway.plist`。它包含三个环境变量：
...
- **MISSHER_HOME** — 将网关限定到你的 Missher 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `missher gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `MISSHER_HOME` 目录都有自己的 launchd 标签。默认的 `~/.missher` 使用 `ai.missher.gateway`；其他安装使用 `ai.missher.gateway-<suffix>`。
:::
...
| 平台 | 工具集 | 功能 |
|----------|---------|--------------|
| CLI | `missher-cli` | 完全访问 |
| Telegram | `missher-telegram` | 完整工具，包括终端 |
| Discord | `missher-discord` | 完整工具，包括终端 |
| WhatsApp | `missher-whatsapp` | 完整工具，包括终端 |
| Slack | `missher-slack` | 完整工具，包括终端 |
| Google Chat | `missher-google_chat` | 完整工具，包括终端 |
| Signal | `missher-signal` | 完整工具，包括终端 |
| SMS | `missher-sms` | 完整工具，包括终端 |
| Email | `missher-email` | 完整工具，包括终端 |
| Home Assistant | `missher-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `missher-mattermost` | 完整工具，包括终端 |
| Matrix | `missher-matrix` | 完整工具，包括终端 |
| DingTalk | `missher-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `missher-feishu` | 完整工具，包括终端 |
| WeCom | `missher-wecom` | 完整工具，包括终端 |
| WeCom Callback | `missher-wecom-callback` | 完整工具，包括终端 |
| Weixin | `missher-weixin` | 完整工具，包括终端 |
| BlueBubbles | `missher-bluebubbles` | 完整工具，包括终端 |
| QQBot | `missher-qqbot` | 完整工具，包括终端 |
| Yuanbao | `missher-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `missher-teams` | 完整工具，包括终端 |
| API Server | `missher-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `missher-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.missher/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
