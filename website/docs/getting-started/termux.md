...
description: "Run Missher Agent directly on an Android phone with Termux"
---

# Missher on Android with Termux
...
Missher Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Missher from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Missher installs are marked with install method `apt`. Missher therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade missher-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Missher runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Missher now ships a Termux-aware installer path:
...
- links `missher` into `$PREFIX/bin` so it stays on your Termux PATH
...
Missher requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `missher` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/missher" "$PREFIX/bin/missher"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `missher` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.missher/.env`.
...
### `missher doctor` says ripgrep or Node is missing
