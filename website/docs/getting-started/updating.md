...
description: "How to update Missher Agent to the latest version or uninstall it"
---
...
:::tip
`missher update` automatically detects new configuration options and prompts you to add them. If you skipped that prompt, you can manually run `missher config check` to see missing options, then `missher config migrate` to interactively add them.
:::
...
```
  → missher-gateway: draining (up to 1875s)...
...
Chat turns show their session key, model and current tool; cron jobs show the job id, name and the process running them (an external restart-safe worker on systemd installs, otherwise the gateway itself). `missher gateway status` lists the same units while the gateway is draining. To stop waiting, finish or kill the listed work, or lower `agent.restart_after_turn_timeout` in `config.yaml` (`0` enters the forced drain immediately).
...
By default `missher update` tracks `origin/main`. Pass `--branch <name>` to update against a different branch — useful for QA channels, feature branches, or release-candidate testing:

```bash
missher update --branch release-candidate
missher update --check --branch experimental   # preview behindness only
```

If your local checkout is on a different branch, Missher auto-stashes any uncommitted work, switches HEAD to the target branch, and then pulls. Branches that don't exist locally are auto-tracked from `origin/<name>` (`git checkout -B <name> origin/<name>`). Branches that don't exist anywhere fail cleanly — your stashed changes are restored before exit so you're never stranded in a weird state. The `main`-only fork-upstream sync logic is automatically skipped on non-`main` branches.

### Checkout parked on a feature branch

If the source checkout was left sitting on a feature branch (by tooling, a worktree experiment, or a manual checkout), `missher update` switches it back to the update target automatically whenever the working tree is clean:
...
commands and retry `missher update`.
...
For high-value profiles (production gateways, shared team installs) you can opt into a full pre-pull backup of `MISSHER_HOME` (config, auth, sessions, skills, pairing):

```bash
missher update --backup
```
...
`updates.pre_update_backup` is a single knob with three modes: `quick` (default — the lightweight state snapshot described above), `full` (the quick snapshot plus a complete `MISSHER_HOME` zip; can add minutes on large homes), and `off` (no pre-update backup at all — `--no-backup` does the same for a single run). Legacy boolean values still work: `true` means `full`, `false` means `off`.

:::tip Moving to a new machine instead?
Update backups protect an in-place update. If you're migrating your whole setup to different hardware, use `missher backup` + `missher import` instead — see [Exporting Missher to another machine](/reference/faq#exporting-missher-to-another-machine) and [`missher backup` vs `missher profile export`](/reference/faq#missher-backup-vs-missher-profile-export).
:::

### Windows: another `missher.exe` is running

On Windows, `missher update` will refuse to run if it detects another `missher.exe` process holding the venv's entry-point executable open — most commonly the Missher Desktop app's spawned backend, an open `missher` REPL in another terminal, or a running gateway:

```
$ missher update
✗ Another missher.exe is running:
    PID 12345  missher.exe

  Updating now would fail to overwrite ...\venv\Scripts\missher.exe because
  Windows blocks REPLACE on a running executable.

  Close Missher Desktop, exit any open `missher` REPLs, and
  stop the gateway (`missher gateway stop`) before retrying.
  Override with `missher update --force` if you've already
  confirmed those processes will not write to the venv.
```

Close the listed processes and re-run. If you're sure the concurrent process won't interfere (rare — usually only useful when an antivirus shim is mis-attributed), pass `--force` to skip the check. In that case the updater will still retry the `.exe` rename with exponential backoff and, on stubborn locks, schedule the replacement for next reboot via `MoveFileEx(MOVEFILE_DELAY_UNTIL_REBOOT)` so the update can complete.

A second, separate guard refuses to touch the venv while any process is running from its Python interpreter (the Desktop app's backend, a gateway, a Python REPL). Those processes keep native extension files (`.pyd`) locked, and a dependency sync that dies partway on an access-denied error strands the install between versions. This guard is **not** bypassed by `--force`; if you're certain the detected holders are false positives, use the explicit `missher update --force-venv`.
...
A `venv.stale.*` or `venv.failed.*` directory can remain when another process still owns a file handle. Close Missher Desktop, gateways, and Python processes using the install, then retry the install/update; parked directories are cleaned up best-effort after a successful recreation.

Expected output looks like:

```
$ missher update
Updating Missher Agent...
📥 Pulling latest code...
...
🔄 Restarting gateways...
✅ Gateway restarted
✅ Missher Agent updated successfully!
```

### Recommended Post-Update Validation

`missher update` handles the main update path, but a quick validation confirms everything landed cleanly:

1. `git status --short` — if the tree is unexpectedly dirty, inspect before continuing
2. `missher doctor` — checks config, dependencies, and service health
3. `missher --version` — confirm the version bumped as expected
4. If you use the gateway: `missher gateway status`
5. If `doctor` reports npm audit issues: run `npm audit fix` in the flagged directory

:::warning Dirty working tree after update
If `git status --short` shows unexpected changes after `missher update`, stop and inspect them before continuing. This usually means local modifications were reapplied on top of the updated code, or a dependency step refreshed lockfiles.
:::

### If your terminal disconnects mid-update

`missher update` protects itself against accidental terminal loss:

- The update ignores `SIGHUP`, so closing your SSH session or terminal window no longer kills it mid-install. `pip` and `git` child processes inherit this protection, so the Python environment cannot be left half-installed by a dropped connection.
- All output is mirrored to `~/.missher/logs/update.log` while the update runs. If your terminal disappears, reconnect and inspect the log to see whether the update finished and whether the gateway restart succeeded:

```bash
tail -f ~/.missher/logs/update.log
```

- `Ctrl-C` (SIGINT) and system shutdown (SIGTERM) are still honored — those are deliberate cancellations, not accidents.

You no longer need to wrap `missher update` in `screen` or `tmux` to survive a terminal drop.
...
Compare against the latest release at the [GitHub releases page](https://github.com/NousResearch/missher-agent/releases).
...
```bash
cd /path/to/missher-agent
# Activate the venv you created during install (outside the source tree)
export VIRTUAL_ENV="$HOME/.missher/venvs/missher-dev"
...
missher config check
missher config migrate   # Interactively add any missing options
```
...
:::warning
Rolling back may cause config incompatibilities if new options were added. Run `missher config check` after rolling back and remove any unrecognized options from `config.yaml` if you encounter errors.
:::

### Image-managed installs (Docker): the provenance marker

Published Docker images bake a small read-only marker (`/etc/missher/image-provenance.json`) that authoritatively identifies the filesystem as image-managed. `missher update`, `missher update --check`, and the dashboard's Update button all consult it before touching anything: on an image-managed install they refuse cleanly (exit code 2), print the actual update command (`docker pull nousresearch/missher-agent:latest`), and write a `refused` receipt so fleet tooling can see the attempt happened. The marker wins even when a source checkout is bind-mounted into the container — the refusal is based on what the running filesystem *is*, not what it looks like. A damaged marker still refuses (fail-closed). Nix- and apt-managed installs refuse through the same gate using the existing detection.
...
nix flake update missher-agent

# Or rebuild with the latest
nix profile upgrade missher-agent
```
...
The uninstaller gives you the option to keep your configuration files (`~/.missher/`) for a future reinstall.

:::tip Moving to a new machine rather than leaving?
Take your setup with you before removing anything: `missher backup` captures the entire `~/.missher` directory including credentials, while `missher profile export` packs a single profile with credentials excluded by design (so an export alone is not a full backup). See [`missher backup` vs `missher profile export`](/reference/faq#missher-backup-vs-missher-profile-export).
:::

### Manual Uninstall

```bash
rm -f ~/.local/bin/missher
rm -rf /path/to/missher-agent
rm -rf ~/.missher            # Optional — keep if you plan to reinstall
```
...
# Linux: systemctl --user disable missher-gateway
# macOS: launchctl remove ai.missher.gateway
```
:::
