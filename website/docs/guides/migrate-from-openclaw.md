...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Missher Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`missher claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Missher. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`missher import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `missher setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.missher/` (by default a single restore-point archive is written before apply, under `~/.missher/backups/pre-migration-*.zip`; restorable with `missher import`). |
...
| What | OpenClaw source | Missher destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.missher/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.missher/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.missher/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.missher/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Missher destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.missher/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.missher/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.missher/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.missher/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Missher skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Missher destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `missher update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.missher/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Missher config path | Mapping |
|------|---------------------|-------------------|---------|
...
Idle and daily reset timers are not imported: Missher conversations persist until an explicit `/new` or `/reset`. Advanced session settings (identity links, thread bindings, maintenance, scope and send policy) remain archived for reference.

### MCP servers

| OpenClaw field | Missher field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.missher/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Missher `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Missher path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `MISSHER_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Missher equivalent)

These are saved to `~/.missher/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Missher |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Missher has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `missher cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `missher webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `missher honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `missher skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Missher profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Missher manually via `missher config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.missher/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `missher status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart missher-gateway`

6. **Check session archives** — review archived advanced settings; idle and daily reset timers are intentionally not imported.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `missher whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `missher claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `missher config set`.

### Skills not appearing after migration

Imported skills land in `~/.missher/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `missher config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
