---
sidebar_position: 8
title: "Use Voice Mode with Missher"
description: "A practical guide to setting up and using Missher voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Missher
...
- you want Missher sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Missher.
...
## Step 1: make sure normal Missher works first
...
Missher supports both local and cloud speech stacks.
...
### If you use `missher setup`

If you choose NeuTTS in the setup wizard, Missher checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Missher transcribes and responds
...
- asking Missher to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Missher loop.

## Tuning CLI behavior

### Silence threshold

If Missher starts/stops too aggressively, tune:
...
Missher stays a normal chat bot, but can speak replies.
...
- you want Missher to function like a portable research or ops assistant
...
Missher joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Missher detects speech boundaries
- transcripts are posted in the associated text channel
- Missher responds in text and audio
...
1. get text Missher working
2. run `missher setup tts` to enable voice support
