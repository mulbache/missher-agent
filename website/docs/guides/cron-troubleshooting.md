...
description: "Diagnose and fix common Missher cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`missher gateway` for foreground, or `missher gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `missher cron tick`.
...
```bash
date
missher cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.missher/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.missher/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.missher/.env` |
...
| `local` | Write access to `~/.missher/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `missher cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Missher Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `missher skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `missher skills list`.
...
2. `~/.missher/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `missher cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Missher config directory). Verify:
```bash
ls ~/.missher/scripts/your-script.py   # Must exist
missher cron edit <job_id> --script ~/.missher/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `missher skills install <skill-name>`.
...
Jobs are stored in `~/.missher/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.missher/cron/jobs.json
chmod 600 ~/.missher/cron/jobs.json   # Your user should own it
```
...
```bash
missher cron list                    # Show all jobs, states, next_run times
missher cron run <job_id>            # Schedule for next tick (for testing)
missher cron edit <job_id>           # Fix configuration issues
missher logs                         # View recent Missher logs
missher skills list                  # Verify installed skills
```
...
1. Run the job with `missher cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.missher/logs/agent.log` for scheduler messages and `~/.missher/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/missher-agent](https://github.com/NousResearch/missher-agent) with:
