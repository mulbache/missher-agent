...
```bash
missher teams-pipeline validate
```
...
```bash
missher teams-pipeline token-health
missher teams-pipeline token-health --force-refresh
```
...
```bash
missher teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
missher teams-pipeline maintain-subscriptions
missher teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Missher cron (recommended if you already run the Missher gateway)

Missher ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.missher/scripts/`. First create the script:

```bash
mkdir -p ~/.missher/scripts
cat > ~/.missher/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec missher teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.missher/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
missher cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/missher-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Missher Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=missher
EnvironmentFile=/etc/missher/env
ExecStart=/usr/local/bin/missher teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/missher-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Missher Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now missher-teams-pipeline-maintain.timer
systemctl list-timers missher-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/missher teams-pipeline maintain-subscriptions >> /var/log/missher/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.missher/.env` at the top of a wrapper script that crontab calls.
...
```bash
missher teams-pipeline subscriptions   # should show expirationDateTime advanced
missher teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
missher teams-pipeline list
missher teams-pipeline list --status failed
missher teams-pipeline show <job-id>
```

### Replay a stored job

```bash
missher teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
missher teams-pipeline fetch --meeting-id <meeting-id>
missher teams-pipeline fetch --join-web-url "<join-url>"
missher teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `missher teams-pipeline maintain-subscriptions --dry-run`
- inspect `missher teams-pipeline list --status failed`
...
- run `missher teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `missher teams-pipeline run`
...
- [ ] `missher teams-pipeline validate` returns an OK snapshot
- [ ] `missher teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Missher cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
