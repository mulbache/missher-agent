...
Copy-paste blueprints for common automation patterns. Each blueprint uses Missher's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`missher webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `missher webhook subscribe`) |
...
```bash
missher cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/missher-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/missher-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
missher webhook subscribe github-pr-review \
  --events "pull_request" \
...
missher cron create "0 9 * * 1" \
  "Scan the NousResearch/missher-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/missher-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (missher_cli/commands.py, missher_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (missher_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
missher cron create "0 6 * * *" \
  "Run a dependency security audit on the missher-agent project.

1. cd ~/.missher/missher-agent && source .venv/bin/activate
...
```bash
missher webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
missher webhook subscribe alert-triage \
...
```python title="~/.missher/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Missher-Monitor/1.0"})
...
missher cron create "every 30m" \
  ...
  --script ~/.missher/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
missher cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Missher Agent development." \
...
```bash
missher webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
missher webhook subscribe stripe-payments \
...
```bash
missher cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the missher-agent codebase.
