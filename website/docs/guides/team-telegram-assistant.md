...
This tutorial walks you through setting up a Telegram bot powered by Missher Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Missher Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.missher/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Missher itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Missher Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_missher_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Missher Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.missher/.env`:
...
```
[Gateway] Starting Missher Gateway...
...
# View live logs
journalctl --user -u missher-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `missher gateway install` to update the plist.
:::
...
```bash
missher gateway stop && missher gateway start
```
...
missher pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.missher/.env`:
...
Control how much detail the bot shows when using tools. In `~/.missher/config.yaml`:
...
Customize how the bot communicates by editing `~/.missher/SOUL.md`:

For a full guide, see [Use SOUL.md with Missher](/guides/use-soul-with-missher).
...
```bash
# From the CLI
missher cron list          # View all scheduled jobs
missher cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u missher-gateway` (Linux) or `~/.missher/logs/gateway.log` (macOS) |
| Cron job output | `~/.missher/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.missher/cron/jobs.json` |
| Pairing data | `~/.missher/pairing/` |
| Session history | `~/.missher/sessions/` |
