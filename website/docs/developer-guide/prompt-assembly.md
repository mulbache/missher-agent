...
description: "How Missher builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Missher deliberately separates:
...
2. **context** — caller-supplied `system_message`, project context files (`.missher.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`), then the worktree-dependent git workspace snapshot, operator instructions and platform hints
...
the first host-info paragraph after the rendered `# Missher runtime environment` boundary, with a
...
```
# Layer 1: Agent Identity (from ~/.missher/SOUL.md)
You are Missher, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance Missher
...
`SOUL.md` lives at `~/.missher/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_missher_home() / "SOUL.md"
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_missher_md(cwd_path)       # 1. .missher.md / MISSHER.md (walks to git root)
    ...
    # SOUL.md from MISSHER_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.missher.md`, `MISSHER.md` | CWD up to git root | Missher-native project config |
...
- **YAML frontmatter stripped** — `.missher.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Missher concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.missher.md` / `MISSHER.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Missher already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.missher/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.missher/MEMORY.md` and `~/.missher/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.missher.md`, `MISSHER.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Missher.
- Ephemeral overlays such as `MISSHER_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Missher assembles prompts for everyone, change Python and treat it as a code contribution
