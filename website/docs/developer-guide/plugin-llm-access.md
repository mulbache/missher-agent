...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Missher already has applies. The plugin never sees a
...
missher-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Missher provider id (e.g. "openrouter")
...
using Missher built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Missher provider
        # (must be one Missher already knows about — same names as
        # `missher model` and config.yaml model.provider).
        ...
        # plugin sends — Missher does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.missher/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Missher' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Missher subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/missher-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/missher-agent/blob/main/tests/agent/test_plugin_llm.py)
