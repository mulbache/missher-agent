...
description: "How Missher recovers state.db when the FTS index or the file itself is corrupt"
---
...
`missher doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `missher doctor --fix` while the gateway
is still up. Next steps:

```bash
missher gateway stop
MISSHER_HOME="$HOME/.missher" missher sessions recover --source "$HOME/.missher/state.db" --inspect-only
# if recoverable:
MISSHER_HOME="$HOME/.missher" missher sessions recover --source "$HOME/.missher/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
missher gateway stop
MISSHER_HOME="$HOME/.missher" missher sessions repair --check-only
MISSHER_HOME="$HOME/.missher" missher sessions repair
```
