...
has to be thrown away or summarized. Missher has always done this in one batch:
...
Missher folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Missher's normal session flush is
...
With `micro_compact` unset or `false` Missher behaves exactly as it always has:
...
Defaults to `$MISSHER_HOME/logs/agent.log`. It reports passes, outcome mix, net
