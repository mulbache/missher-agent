...
Missher includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Missher and does not require a Missher observability plugin. Missher remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Missher execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `missher-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Missher `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `MISSHER_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `MISSHER_NEMO_RELAY_ATOF_*` and `MISSHER_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Missher does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Missher requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Missher
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Missher installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Missher process so configured interceptors can
...
Collection remains off unless Missher policy enables it:
...
opt-in. Set `MISSHER_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Missher does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Missher reports the error and does not invoke
...
retain the same `session_id` and add `missher.session.segment` plus
`missher.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Missher
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Missher process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Missher logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Missher
process and share the same policy boundary.

Run profiles in separate Missher processes when they require different trust
...
Missher core owns one Relay host and one isolated Relay session scope per Missher
...
marks do not require Missher plugin registration. Shared-metrics marks must
...
```text
Missher turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Missher shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Missher sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Missher used for the logical call, such as
...
New calls use `missher.model_route.count`. The previous
`missher.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `missher.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Missher
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Missher processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `missher.task_run`, parented to
the owning Missher session. The start counter contains only bounded execution
...
provider attempts for the same Missher API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Missher tool-call
...
cancellations. Active task ownership follows the task ID if Missher rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`missher.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Missher
...
tool or plugin names. Missher does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `missher.tool_approval` marks and
...
Successful skill mutations emit `missher.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `missher.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Missher derives reuse and patch-generation
...
```text
$MISSHER_HOME/telemetry/shared_metrics/metrics.sqlite3
$MISSHER_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Missher version,
...
schema field name, its current scope is one `MISSHER_HOME`, so it is more
...
Deleting `$MISSHER_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `MISSHER_HOME`. To reset it, stop Missher
processes and remove `$MISSHER_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Missher CLI turn against the deterministic local model server:
...
`$MISSHER_HOME/telemetry/shared_metrics/outbox/` to the Missher telemetry ingest
...
Removing `$MISSHER_HOME/telemetry/shared_metrics` still resets local identity,
