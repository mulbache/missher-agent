...
Missher runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.missher/plugins/`, registering a backend the user selects
...
| `missher setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `missher status` / `missher doctor` | `doctor_checks()` |
...
```python title="~/.missher/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.missher"    # where synced cache files land, or None
...
```yaml title="~/.missher/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
missher plugins enable acmebox
missher config set terminal.backend acmebox
```
