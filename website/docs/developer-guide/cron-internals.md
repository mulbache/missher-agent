...
description: "How Missher stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `missher_cli/cron.py` | CLI `missher cron` subcommands |
...
Jobs are stored in `~/.missher/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`missher cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
The same store fields drive every topology: a standalone `missher -p X gateway
...
In CLI mode, cron jobs only fire when `missher cron` commands are run or during active CLI sessions.
...
```python
# ~/.missher/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.missher/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `missher cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `missher cron` CLI provides direct job management:

```bash
missher cron list                    # Show all jobs
missher cron create                  # Interactive job creation (alias: add)
missher cron edit <job_id>           # Edit job configuration
missher cron pause <job_id>          # Pause a running job
missher cron resume <job_id>         # Resume a paused job
missher cron run <job_id>            # Trigger immediate execution
missher cron remove <job_id>         # Delete a job
```
