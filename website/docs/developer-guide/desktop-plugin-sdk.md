...
title: "Desktop Plugin SDK (@missher/plugin-sdk)"
description: "Extend the native Missher Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Missher Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `MissherPlugin`.
It imports one module — `@missher/plugin-sdk` — and gets everything: the app's
...
`$MISSHER_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Missher. This page is the **native
desktop app** (`missher desktop`) SDK — the `@missher/plugin-sdk` module and
`$MISSHER_HOME/desktop-plugins/`. The **web dashboard** (`missher dashboard`) has
its own, unrelated plugin system on `window.__MISSHER_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Missher Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$MISSHER_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$MISSHER_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `MissherPlugin` contract, appear in **Capabilities → Plugins**,
...
Create `$MISSHER_HOME/desktop-plugins/hello/plugin.js` (that's `~/.missher/...`
...
```javascript
// ~/.missher/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@missher/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Missher' }),
      jsx('div', {
...
The only importable specifiers are `@missher/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `MissherPlugin`:

```ts
interface MissherPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `missher.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@missher/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@missher/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@missher/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@missher/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@missher/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@missher/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@missher/plugin-sdk'
...
`window.missher.send('get-price eth')` (or a declarative
`<button data-missher-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@missher/plugin-sdk'
...
for requests; `targetProfile` is the backend Missher profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Missher
...
ctx.os.notify({
  ...
  // Body click → focus Missher + navigate. Same vocabulary as OS deep links:
  activate: 'missher://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'missher://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `missher://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `missher://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@missher/plugin-sdk'
...
```javascript
import { queryClient } from '@missher/plugin-sdk'
...
copies that half into `$MISSHER_HOME/desktop-plugins/<id>/` beside a
`.missher-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.missher/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`missher plugins update`, or
...
a package into `~/.missher/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.missher/plugins` is not reachable as a filesystem —
...
the `missher://` scheme — a plain anchor on your website or README:
...
`missher-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Missher plugin and declare it in a
`manifest.json`:
...
missher-agent codebase directly (`missher_state`, `missher_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.missher/`)
...
(`missher.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `MissherPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@missher/plugin-sdk` alias.
2. It's still lint-fenced to `@missher/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@missher/plugin-sdk`, `react`,
...
| Plugin contract | `MissherPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `MissherOpenTarget`, `Contribution` |
...
### Agents: the `missher-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`missher-desktop-plugins`** skill — it carries the same contract as this page in
...
`$MISSHER_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `missher logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@missher/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.missher/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.missher/logs/errors.log` for
