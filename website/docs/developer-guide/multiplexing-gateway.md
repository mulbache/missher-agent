...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_missher_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`MISSHER_HOME`, `MISSHER_PROFILE`, proxy settings,
...
reload helper: `missher_cli.env_loader.load_missher_dotenv` skips the
...
## The MISSHER_HOME override

`missher_constants.py` holds a context-local override consulted by
`get_missher_home()` before the `MISSHER_HOME` env var. Everything that resolves
...
automatically. `get_process_missher_home()` exists for the few machine-level
assets that must not follow the override. `missher_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `missher_cli/profiles.py`
...
DB handles are resolved at call time through the active MISSHER_HOME override —
...
MISSHER_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `missher_home_key()`. |
...
- `agent/secret_scope.py`, `missher_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `missher_cli/profiles.py`
