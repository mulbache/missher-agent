...
Missher Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `missher plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Missher has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.missher/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|missher|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `missher`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
excluded because they already unlock the wider window), Missher raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$MISSHER_HOME`
...
```bash
missher config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
missher config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Missher keeps the
...
`gpt-5.4-900k`). These are Missher-side aliases: the suffix is stripped before
...
thread context, so Missher' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Missher
  records the resulting compaction events (compression counters, session
  events). Set `missher` to let Missher' compression threshold initiate
  app-server compaction, or `off` to disable Missher-initiated automatic
  compaction entirely (codex may still compact natively).

Missher' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Missher' summary compressor.
...
encrypted `compaction` output item. Missher captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Missher
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Missher can avoid; user-facing docs for `/model`, fallback providers, and
