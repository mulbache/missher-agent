...
description: "Wire contract between the Missher gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **Missher gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `missher gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`missher gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `missher` CLI.
