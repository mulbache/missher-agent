...
| Variable | Meaning |
|----------|---------|
| `MISSHER_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `MISSHER_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `MISSHER_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Missher itself — they're private to these helpers. The variables Missher *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `missher --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_missher_root)" || { echo "htui: not in a Missher checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$MISSHER_MAIN_CHECKOUT/.venv/bin/python" -m missher_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `MISSHER_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_missher_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `MISSHER_TUI_DIR` are mutually exclusive
`MISSHER_TUI_DIR` points Missher at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `missher --tui --dev` exits with an error. Run `unset MISSHER_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_missher_root)" || { echo "hgui: not in a Missher checkout" >&2; return 1; }
  deps="${MISSHER_GUI_DEPS_CHECKOUT:-$MISSHER_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _missher_link_deps "$desktop" "$deps/apps/desktop"
    _missher_link_deps "$root" "$deps"
  else
  ...
  trap '_missher_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    MISSHER_DESKTOP_MISSHER_ROOT="$root" \
    MISSHER_DESKTOP_PYTHON="$MISSHER_MAIN_CHECKOUT/.venv/bin/python" \
    MISSHER_DESKTOP_IGNORE_EXISTING=1 \
    MISSHER_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `MISSHER_DESKTOP_MISSHER_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `missher`. |
| `MISSHER_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `MISSHER_DESKTOP_IGNORE_EXISTING` | Ignores any `missher` on `PATH` so it can't shadow the worktree. |
| `MISSHER_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Missher checkout.
_missher_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/missher_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_missher_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'missher_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `missher --tui --dev` and the `MISSHER_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NousResearch/missher-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `MISSHER_*` variable Missher reads
