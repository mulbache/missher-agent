...
The ACP adapter wraps Missher' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
missher acp / missher-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.missher/.env
  -> configure stderr logging
  -> construct MissherACPAgent
...
- `allow_once` -> Missher `once`
- `allow_always` -> Missher `always`
- reject options -> Missher `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Missher tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["missher-acp"])
...
Instead it reuses Missher' runtime resolver:

- `acp_adapter/auth.py`
- `missher_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Missher provider/credentials. It also always advertises a terminal setup auth method (`missher-setup`, args `--setup`) so first-run ACP clients can open Missher' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.missher/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `missher-acp` toolset definition
- `missher_cli/main.py` — `missher acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `missher-acp` script
