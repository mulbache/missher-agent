...
sidebar_position: 1
title: "Architecture"
description: "Missher Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Missher Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ missher_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # MissherCLI facade — mixins in missher_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── missher_state.py           # SQLite session/state database facade (+ missher_state_*.py siblings)
├── missher_constants.py       # MISSHER_HOME, profile-aware paths
...
│
├── missher_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `missher` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # missher skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # missher tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # missher gateway start/stop
│
...
```text
User input → MissherCLI.process_input()
...
Three discovery sources: `~/.missher/plugins/` (user), `.missher/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `missher plugins` or `config.yaml`.
...
Exposes Missher as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`missher -p <name>`) gets its own MISSHER_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
