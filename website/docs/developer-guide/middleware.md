...
Missher middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Missher'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Missher calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Missher retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Missher guardrails,
...
- `telemetry_schema_version`: currently `missher.observer.v1`
- `middleware_schema_version`: currently `missher.middleware.v1`
...
Missher stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Missher runs
them as a nested chain in registration order. Middleware failures are fail-open:
Missher logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Missher applies middleware in this order:
...
For each tool call, Missher applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Missher pre-execution path against the effective arguments:
...
For isolated local testing, use one `MISSHER_HOME` for plugin enablement and the
agent run:

```bash
export MISSHER_HOME=/tmp/missher-middleware-test
mkdir -p "$MISSHER_HOME"
missher plugins enable <plugin-name>
missher chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run missher plugins enable <plugin-name>
uv run missher chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["missher_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/missher-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Missher expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Missher treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Missher preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Missher does not convert downstream
