...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Missher Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Missher tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Missher core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Missher Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_MISSHER_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `missher_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `missher_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `missher chat -q "Use the weather tool for London"`
