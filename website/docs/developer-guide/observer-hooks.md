...
# Missher Observer Hooks

Missher observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Missher' planner, model providers, memory, tool registry,
...
Missher also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "missher.observer.v1"
```

Hook callbacks are fail-open. Missher catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Missher only builds
...
The native NeMo Relay SDK integration maps Missher session, turn, LLM, and tool
