...
Plugins can launch and supervise fresh Missher child sessions without importing
...
active agent turn fails closed with `No active Missher parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Missher can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Missher's existing unsafe-tool block remains enforced.
