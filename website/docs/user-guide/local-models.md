...
Missher can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; Missher does the rest.
...
2. Click **Install runtime**. Missher downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with Missher, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How Missher picks what to download
...
Models ship in several quality grades (quantizations). Missher picks the
...
guarantees. Below 4-bit the quality loss is too severe, so Missher never
...
Local models live or die by memory placement, so Missher manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, Missher deliberately places the
...
- **Memory fit includes the launch configuration**, not just the model file:
  ...
  count. For multi-token prediction (MTP), Missher uses smaller batches when
...
- **Conversation compression follows a growth check.** If a larger window
  ...
  Missher compresses instead of claiming a window the server did not receive.
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — Missher
...
If a llama-server is already running on your machine, Missher detects it
...
[Run Missher Locally with Ollama](/guides/local-ollama-setup) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with Missher.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; Missher updates it with
                     # each release after re-validation
```

Models and runtime builds live under the Missher home directory
