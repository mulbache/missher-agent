...
When running Missher inside Docker, the default `network_mode: host` gives the
...
The Missher [SECURITY.md](https://github.com/NousResearch/missher-agent/blob/main/SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ missher-agent │   │ missher-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   MISSHER_UID=$(id -u) MISSHER_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=missher,missher-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://missher-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](https://github.com/NousResearch/missher-agent/blob/main/SECURITY.md) — Missher trust model and vulnerability reporting
- [Docker](/user-guide/docker) — running Missher in a container
- [Egress proxy](iron-proxy.md) — credential-injection firewall for the sandbox
- [docker-compose.yml](https://github.com/NousResearch/missher-agent/blob/main/docker-compose.yml) — default compose configuration
