...
When Missher Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.missher/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.missher/config.yaml` (switchable per session
via `/personality`).
...
If you use Missher' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.missher/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Missher Agent's deliverable
