...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Missher's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Missher instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
missher memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `missher` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Missher syncs them back at session init.
...
The `missher honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `missher memory setup honcho` (or run `missher memory setup` and pick it from the list); the `missher honcho` subcommand then appears on the next invocation.

```bash
missher memory setup honcho    # Configure Honcho directly (works before activation)
missher honcho status          # Connection status, config, and key settings
missher honcho setup           # Redirects to `missher memory setup` (post-activation alias)
missher honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
missher honcho peer            # Show or update peer names + dialectic reasoning level
missher honcho mode            # Show or set recall mode (hybrid/context/tools)
missher honcho tokens          # Show or set token budget for context and dialectic
missher honcho identity        # Seed or show the AI peer's Honcho identity
missher honcho sync            # Sync Honcho config to all existing profiles
missher honcho peers           # Show peer identities across all profiles
missher honcho sessions        # List known Honcho session mappings
missher honcho map             # Map current directory to a Honcho session name
missher honcho enable          # Enable Honcho for the active profile
missher honcho disable         # Disable Honcho for the active profile
missher honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `missher honcho`

If you previously used the standalone `missher honcho setup`:
...
No re-login or re-setup needed. Run `missher memory setup` and select "honcho" — the wizard detects your existing config.
