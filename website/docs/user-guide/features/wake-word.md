...
The wake word turns Missher into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Missher listens in the background for a spoken
trigger phrase. Say it, and Missher starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Missher backend (for example a
...
Missher supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with Missher, so
...
```bash
cd ~/.missher/missher-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `missher` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.missher/config.yaml`,
...
wake_word:
  ...
  openwakeword:
    model: hey_missher         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
default) to let Missher choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Missher installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Missher surfaces, and `surface` picks which one
...
| `cli` | Only the classic `missher` CLI. |
| `tui` | Only `missher --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Missher surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Missher does not silently fail over to another open surface.
...
(`model: hey_missher`) is the default. To wake on something else, the easiest
...
command (`missher -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.missher/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("missher" qualifies) beat common words like
"hello" or "stop".
:::
...
wake_word:
  ...
  phrase: "hey hermes"
  porcupine:
    keyword: ~/.missher/wakewords/hey_missher.ppn
```

Set your access key in `~/.missher/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `missher tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `missher-agent[wake]`).
...
but the phrase never fires. Missher detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Missher
backend (it may appear as your terminal, `python`, or Missher), then toggle the
...
```bash
missher config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
missher config set wake_word.input_device null
```
