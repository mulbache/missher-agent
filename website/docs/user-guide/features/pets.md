...
Missher can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<MISSHER_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Missher renders the sprite at full fidelity when your
...
missher pets list
missher pets list cat

# Install a pet and make it active in one step
missher pets install boba --select
...
## `missher pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `missher pets list [query] [--limit N]` |
| List installed pets | `missher pets list --installed` |
| Install a pet | `missher pets install <slug> [--select] [--force]` |
| Set the active pet | `missher pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `missher pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `missher pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `missher pets off` |
| Remove an installed pet | `missher pets remove <slug>` |
| Diagnose setup | `missher pets doctor` |

`missher pets show` flags:
...
Beyond installing pre-made pets from the gallery, Missher can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `missher pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `missher tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `MISSHER_PET_IMAGE_PROVIDER` env var (e.g. `MISSHER_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Missher is
...
Run `missher pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Missher uses its own
  profile-scoped `<MISSHER_HOME>/pets/` instead — install through `missher pets`.

## See also

- The [`missher-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-missher-agent.md)
