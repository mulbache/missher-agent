...
fills the password through Missher. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Missher generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `missher vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Missher
...
in, Missher picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Missher hands the master password to the manager's
...
Prefer not to use a detected manager? `missher vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `missher vault add`), bound to the checkout site,
...
- **CLI**: `missher vault list`, `missher vault add`, `missher vault rm <handle>`,
  `missher vault sources`.

Items live encrypted under `~/.missher/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `missher chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Missher: not in
