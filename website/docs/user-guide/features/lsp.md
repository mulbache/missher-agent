...
Missher runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Missher
...
1. Missher captures a baseline of current diagnostics for the file.
...
…). Missher auto-detects the binary on PATH or in
`<MISSHER_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Missher at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<MISSHER_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`missher lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Missher installs both packages together when you
run `missher lsp install typescript` or auto-install fires on first
use.

## CLI

```
missher lsp status          # service state + per-server install status
missher lsp list            # registry, optionally --installed-only
missher lsp install <id>    # eagerly install one server
missher lsp install-all     # try every server with a known recipe
missher lsp restart         # tear down running clients
missher lsp which <id>      # print resolved binary path
```

`missher lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <MISSHER_HOME>/lsp/bin
...
When `install_strategy: auto`, Missher installs binaries into
`<MISSHER_HOME>/lsp/bin/`. NPM packages land in
`<MISSHER_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Missher-owned and is
...
**single process** per Missher process: the first Python project spawns
...
**`missher lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<MISSHER_HOME>/lsp/bin/`. Run
`missher lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `missher lsp status`**
...
When `missher lsp status` shows a `Backend warnings` section, install
...
`~/.missher/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.missher/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `missher lsp restart` to clear the set;
