...
Missher Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Missher integration. It
...
**Fresh installs already have the driver.** The Missher installer
...
- **`missher tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
missher computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `missher computer-use
status` to verify the install.

Already have cua-driver? Missher reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `missher update`, and the first
`computer_use` call of a session, Missher checks the local version and
...
selected with `MISSHER_CUA_DRIVER_CMD` stays
under your control, so Missher reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Missher autodetection is a
planned cua-driver follow-up, so currently point Missher at that directory or
...
interface. The built-in toolset provides Missher actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `missher computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Missher host identity. |
...
```
missher -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.missher/config.yaml`.

## Permission modes and logged-in browser profiles

Missher maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Missher session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Missher approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Missher risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.missher/cua-manifest.yaml
```
...
for the format). Missher launches a private runtime with
...
identity instead of resetting with every Missher build), and Missher verifies
...
resetting or closing the Missher session, cancellation cleanup, or process exit
closes that transport session. Missher also stops private runtimes that it
launched for bounded or unrestricted access. One Missher
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Missher host identity.
...
## `missher computer-use doctor` — your first triage stop

`missher computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ missher computer-use doctor
...
Missher run declares a public cua-driver **session name** (something like
`missher-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Missher disables it automatically where it is a known failure
...
Missher keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Missher-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Missher
autodetection is a planned cua-driver follow-up, so currently point Missher at
...
These are **platform deep dives, not duplicates of the Missher skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Missher
...
image capture also saves a bounded, shareable copy under Missher' image cache
...
Missher applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.missher/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Missher applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Missher agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Missher agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Missher warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Missher from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Missher passes the
...
```
MISSHER_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
MISSHER_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Missher disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Missher sets
...
When it's on, `missher computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Missher at a binary you built from source instead
of the published release. Missher resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`MISSHER_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Missher testing (see notes).
...
### Option B — point Missher straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`MISSHER_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
MISSHER_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
MISSHER_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Missher is using your build

- `missher computer-use status` prints the resolved binary path and
  version.
- `missher computer-use doctor` confirms the binary is reachable and
...
- **Missher spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Missher YOLO, Missher instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Missher
  change needed either way.
- **Local builds skip the version check.** Missher warns when the
...
**First action when anything's off: run `missher computer-use doctor`.**
...
Run `missher computer-use install` to fetch the cua-driver binary, or
run `missher tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `missher computer-use doctor` will
...
- **Missher-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Missher `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Missher autodetection is a
  planned follow-up; currently point Missher at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Missher among them)
