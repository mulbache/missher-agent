...
Missher supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.missher/config.yaml`:
...
Or set the env var: `MISSHER_KANBAN_DISPATCH_IN_GATEWAY=false`
