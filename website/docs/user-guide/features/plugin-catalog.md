...
description: "Browse and install reviewed, SHA-pinned Missher plugins from the curated catalog"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Missher plugins you
can install by name with a single command:

```bash
missher plugins install <name>
```
...
[`plugin-catalog/`](https://github.com/NousResearch/missher-agent/tree/main/plugin-catalog)
directory of the missher-agent repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `missher plugins install` |
...
| `requires_missher` | Minimum Missher version, e.g. `>=0.19` (optional) |
...
The catalog name and the plugin's own manifest name can differ; `missher
...
```bash
missher plugins install touchdesigner
missher plugins enable td
```
...
skill, so one install gives Missher code, dependency, container and IaC scanning
plus the workflow for using it; the catalog name and manifest name match:

```bash
missher plugins install snyk
missher plugins enable snyk
```

### Updating a catalog install

`missher plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `missher plugins list` shows catalog
...
(`https://missher-agent.nousresearch.com/docs/api/plugin-catalog.json`).
`search`/`install`/`update` fetch it at most every six hours and cache it under
`~/.missher/cache/`, so new entries and removals reach installed clients without
updating Missher. Offline, the copy shipped with your checkout is used. Removals
from the in-tree list and the live list are always both enforced.

### Custom git URLs are different

`missher plugins install <git-url>` still works for any repository, but it
...
[plugin-catalog README](https://github.com/NousResearch/missher-agent/tree/main/plugin-catalog);
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with Missher
- [Build a Missher Plugin](/developer-guide/plugins) — write your own
