...
title: "Plugins"
description: "Extend Missher with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Missher has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Missher
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Missher Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.missher/plugins/` with a `plugin.yaml` and Python code:

```
~/.missher/plugins/my-plugin/
...
Start Missher — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.missher/plugins/hello-world/plugin.yaml`**
...
**`~/.missher/plugins/hello-world/__init__.py`**

```python
"""Minimal Missher plugin — registers a tool and a hook."""
...
Drop both files into `~/.missher/plugins/hello-world/`, restart Missher, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Missher does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.missher/plugins/` are disabled by default. Enable them only for trusted repositories by setting `MISSHER_ENABLE_PROJECT_PLUGINS=true` before starting Missher.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `missher <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `missher plugins install` |
| Distribute via pip | `[project.entry-points."missher_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Missher — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.missher/plugins/` | Personal plugins |
| Project | `.missher/plugins/` | Project-specific plugins (requires `MISSHER_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `missher_agent.plugins` entry_points | Distributed packages |
| Nix | `services.missher-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Missher also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.missher/plugins/model-providers/<name>/` override bundled model providers of the same name (last-writer-wins in `register_provider()`), so you can replace a built-in provider profile without any repo edits. Memory providers resolve the other way round: for `~/.missher/plugins/memory/<name>/` the **bundled** provider wins on a name collision (bundled, then user, then project, then entry points; first seen wins), so a user memory provider needs its own unique name.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `missher plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.missher/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
missher plugins                    # interactive toggle (space to check/uncheck)
missher plugins enable <name>      # add to allow-list
missher plugins disable <name>     # remove from allow-list + add to disabled
```

After `missher plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
missher plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Missher checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `missher plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`missher plugins install <source> --force --ref <new-commit>`. The
profile-local install metadata contains no config values, environment values,
secrets, or capability grants.

The same pin is available in Missher Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `missher plugins list` prints
...
`missher plugins install` clones non-interactively (it never prompts for a
username or password), so a private repo needs a credential Missher can find on
...
before. The same resolution applies to `missher plugins update`, catalog MCP
installs from git, and profile distributions fetched from a git URL.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Missher' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.missher/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.missher/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Missher tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Missher runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Missher never
materializes the prompt on another surface.

Missher still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Missher that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.missher/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 27 lifecycle events currently accepted by `missher_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Missher has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.missher/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Missher also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Missher Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Missher Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Missher Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `missher <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `MISSHER_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Missher auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `missher skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.missher/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `missher plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
missher plugins                               # unified interactive UI
missher plugins list                          # table: enabled / disabled / not enabled
missher plugins search <term>                 # search the Missher plugin catalog
missher plugins install <name>                # install a catalog entry (repo @ reviewed pinned SHA)
missher plugins install user/repo             # install from Git, then prompt Enable? [y/N]
missher plugins install user/repo --enable    # install AND enable (no prompt)
missher plugins install user/repo --no-enable # install but leave disabled (no prompt)
missher plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
missher plugins remove my-plugin              # uninstall
missher plugins enable my-plugin              # add to allow-list
missher plugins disable my-plugin             # remove from allow-list + add to disabled
missher plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Missher Desktop registers the `missher://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
missher://plugin/install?repo=owner/repo            # main install link
missher://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
missher://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Missher and shows a **confirmation dialog** — the repo id,
...
Plugins → Install from Git**. Legacy `missher://plugin-agent/…` and
`missher://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `missher-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="missher://plugin/install?repo=owner/repo&enable=1">Install in Missher</a>
```

MCP servers have the equivalent link form — see
[Add to Missher link](/reference/mcp-config-reference#add-to-missher-link).
...
When a plugin declares capabilities, `missher plugins install` (and
`missher plugins enable`) shows the list with one-line risk descriptions and
...
granted, `missher plugins update` surfaces the additions and asks again. New
...
`missher plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
missher plugins capabilities             # all plugins with declared/granted capabilities
missher plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Missher has not reviewed the plugin's code. Only install
...
### Discovering plugins — the Missher plugin catalog

`missher plugins search <term>` searches the **Missher plugin catalog** — the
curated, SHA-pinned catalog maintained in the missher-agent repository
(`plugin-catalog/`). Matching covers entry names, descriptions, and declared
tools:

```bash
missher plugins search telegram    # search the catalog
missher plugins browse             # browse every entry
missher plugins info <name>        # full details for one entry
```
...
recorded so `missher plugins update` can re-pin when the catalog moves:

```bash
missher plugins install <catalog-name>
```
...
A **plugin pack** is a declarative, shareable YAML file (`missher-pack.yaml`)
...
```bash
missher plugins pack show ./missher-pack.yaml     # dry-run review
missher plugins pack install ./missher-pack.yaml  # review → confirm → install
missher plugins pack export > missher-pack.yaml   # snapshot the current install
missher plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `missher plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `missher plugins install`.
...
provenance (installed via `missher plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`missher skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `missher plugins install` and `missher plugins update` runs a static
...
On `missher plugins update`, a dangerous verdict on the updated tree
...
Running `missher plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `missher plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Missher reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Missher rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Missher can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Missher drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Missher checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Missher' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
