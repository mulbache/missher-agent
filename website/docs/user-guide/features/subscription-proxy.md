...
chat completions — use your Missher-managed provider subscription as their
...
| Use case | "Use Missher as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Missher stores
the refresh token in `~/.missher/auth.json` — the same place all Missher
...
Model:      Hermes-4-70B    # or Missher-4.3-36B, Hermes-4-405B
```
...
interface in `missher_cli/proxy/adapters/`.
...
If you see `not logged in`, run `missher portal`. If you see
...
`missher_cli/proxy/adapters/<provider>.py` and registering it in
