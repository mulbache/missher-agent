...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `missher tools`.
...
Missher Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `missher tools` and persists in `config.yaml`.
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `missher setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the image-gen backend via `missher tools`.

If the managed gateway returns `HTTP 4xx` for a specific model, that model isn't yet proxied on the portal side — the agent will tell you so, with remediation steps (switch to FAL.ai in `missher tools` with your own `FAL_KEY` for direct access, or pick a different model).
:::
...
`image_gen.provider` is the single selection key: `nous` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: nous`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via missher tools), but FAL_KEY is not set. Run 'missher tools' to change it.` rather than silently rerouting. Change providers via `missher tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `nous` when `true`, but never written anymore.)

`max_parallel_requests` defaults to `4`. Missher clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no Missher
...
only for now; responses are saved to `$MISSHER_HOME/cache/images/`.

## FAL: GPT Image 2.5

Select **GPT Image 2.5 Flare** or **GPT Image 2.5 Sunburst** under
`missher tools` → Image Generation → FAL.ai. The model IDs are:
...
```bash
missher config set image_gen.provider fal
missher config set image_gen.model openai/gpt-image-2.5/flare/text-to-image
```
...
Both accept up to 16 source images. Missher pins quality to `medium`, matching its
...
Select them through `missher tools` → Image Generation → OpenAI, or set:

```bash
missher config set image_gen.provider openai
missher config set image_gen.openai.model gpt-image-2.5-flare
```
...
as a tool the chat model may call, and Missher cannot force the call — the
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Missher materializes them to the local cache so delivery still works after expiry
