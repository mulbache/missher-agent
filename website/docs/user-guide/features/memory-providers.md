...
Missher Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
missher memory setup      # interactive picker + configuration
missher memory status     # check what's active
missher memory off        # disable external provider
```

You can also select the active memory provider via `missher plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.missher/config.yaml`:
...
When a memory provider is active, Missher automatically:
...
**Setup Wizard:**
```bash
missher memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `missher honcho setup` command still works (it now redirects to `missher memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$MISSHER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$MISSHER_HOME/honcho.json` > `~/.missher/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/missher-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/missher).
...
:::tip Migrating from `missher honcho`
If you previously used `missher honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Missher profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Missher profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Missher profile. Host key `missher` → default; `missher.<profile>` for others. |
...
```bash
missher profile create coder --clone
```

`--clone` creates a `missher.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Missher profile, creates host blocks for any profile without one, inherits settings from the default `missher` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `missher memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/missher-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/missher).
...
missher memory setup    # select "openviking"
# Or manually:
missher config set memory.provider openviking
```

`missher memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.missher/.env`, and for named profiles use
`~/.missher/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Missher sends no peer ID and writes
...
Set `memory.openviking.agent: missher` to restore the old peer-scoped writes.
...
Missher sends `User-Agent: openviking-memory-missher/<version>` on OpenViking
...
**Setup (Platform):**
```bash
missher memory setup    # select "mem0" → "Platform"
# Or manually:
missher config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.missher/.env
```

**Setup (OSS):**
```bash
missher memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
missher memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
missher memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
missher memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
missher memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.missher/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.missher/.env
```
...
**Config:** `$MISSHER_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.missher/.env`.
...
| `user_id` | `missher-user` | User identifier |
| `agent_id` | `missher` | Agent identifier |
...
**Switching modes:** Re-run `missher memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
missher memory setup    # select "hindsight"
# Or manually:
missher config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.missher/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p missher ui start`

**Config:** `$MISSHER_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `missher` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Missher Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/missher-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
missher memory setup    # select "holographic"
# Or manually:
missher config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.missher-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$MISSHER_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
missher memory setup    # select "retaindb"
# Or manually:
missher config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.missher/.env
```
...
# Then configure Missher
missher memory setup    # select "byterover"
# Or manually:
missher config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$MISSHER_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=missher), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
missher memory setup    # select "supermemory"
# Or manually:
missher config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.missher/.env
```
...
Before running `missher memory setup`, set `base_url` in
`$MISSHER_HOME/supermemory.json`:
...
Then run `missher memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$MISSHER_HOME/supermemory.json`
...
| `container_tag` | `missher` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `missher-{identity}` → `missher-coder`) to isolate memories per Missher profile
...
| **Requires** | `pip install missher-memori` + `missher-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install missher-memori
missher-memori install
missher config set memory.provider memori
missher memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `missher-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$MISSHER_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$MISSHER_HOME/` so each profile has its own credentials
