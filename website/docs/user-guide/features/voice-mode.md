...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Missher Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Missher Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Missher](../../guides/use-voice-mode-with-missher.md).
...
1. **Missher Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `missher model` or set your preferred provider credentials in `~/.missher/.env`
3. **A working base setup** — run `missher` to verify the agent responds to text before enabling voice

:::tip
The `~/.missher/` directory and default `config.yaml` are created automatically the first time you run `missher`. You only need to create `~/.missher/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `missher setup --portal` wires both up at once.
:::
...
cd ~/.missher/missher-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.missher/missher-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.missher/missher-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.missher/missher-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`missher chat`) and the **TUI** (`missher --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.missher/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
missher                # Start the interactive CLI
```
...
1. Start the CLI with `missher` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.missher/config.yaml` (default: `ctrl+b`).
:::
...
When Missher Desktop is connected to a **remote gateway**, audio does not need to be relayed through the gateway at all. At voice-session start the desktop fetches the active profile's resolved STT/TTS settings (provider, model, language/voice, and credential) from the gateway over the authenticated REST channel (`GET /api/audio/voice-config`) and then calls the providers **directly**:
...
Requirements: an OpenAI API key (`OPENAI_API_KEY`, `VOICE_TOOLS_OPENAI_KEY`, or `voice.gpt_live.api_key`). The voice layer is billed by OpenAI at **$0.05 per minute of session time** (idle time counts); the Missher turn is billed on its own provider as always. The mode is also in Settings → Voice → *Voice Chat Mode*.
...
```bash
missher gateway        # Start the gateway (connects to configured platforms)
missher gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@missherbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.missher/.env`:
```bash
...
```bash
missher gateway        # Start with existing configuration
```
...
If you are running Missher inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.missher/.env`
...
- Review gateway logs: `tail -f ~/.missher/logs/gateway.log`
