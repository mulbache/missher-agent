...
A **profile distribution** packages a complete Missher agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Missher has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `missher profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `missher profile update` pulls new versions | Re-send the file |
...
| Made with | `missher profile install` / `update` | `/export` and `/import`, or `missher profile export` / `import` |
...
Before distributions, sharing a Missher agent meant sending someone:
...
```bash
missher profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `missher profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Missher in 2026, that's already true.
...
:::caution
**Missher does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `missher profile install` or `missher profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
missher profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.missher/profiles/research-bot/SOUL.md
...
Create `~/.missher/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
missher_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.missher/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.missher/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `missher profile update research-bot` will pull the latest.
...
```bash
missher profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.missher/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
missher profile install github.com/you/research-bot

# Full HTTPS
missher profile install https://github.com/you/research-bot.git

# SSH
missher profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
missher profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
missher profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
missher profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
missher profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
missher profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Missher distribution.
...
```bash
cp ~/.missher/profiles/research-bot/.env.EXAMPLE ~/.missher/profiles/research-bot/.env
...
```bash
missher profile info research-bot
```
...
`missher profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
missher profile update research-bot
```
...
```bash
missher profile delete research-bot
```
...
Path:    ~/.missher/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `missher profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.missher/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
missher profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `missher profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.missher/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/missher-polymarket-trader.git
git push -u origin main --tags

# Anyone
missher profile install github.com/you/missher-polymarket-trader --alias
```
...
You built Missher-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
missher profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
missher profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `missher profile info <name>` and hold off on updates until you're ready.
:::
...
missher profile info research-bot | grep Version
...
```yaml
# ~/.missher/profiles/research-bot/local/my-overrides.yaml
...
missher profile delete research-bot --yes
missher profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
missher profile update research-bot --force-config --yes
```
...
missher profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.missher/profiles/forked-research-bot/
...
missher profile install ~/.missher/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
missher profile delete research-bot-test --yes
missher profile install ~/.missher/profiles/research-bot --name research-bot-test
```
...
Without `-o`, the CLI and TUI place the archive in Missher's managed
`profile-exports/` directory under the default Missher home, not in the current
...
source file. If the Missher home itself lives inside a Git checkout (some
Docker/custom deployments), the archive goes to `~/.missher-profile-exports/`
...
```bash
missher profile export research-bot
missher profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
missher profile import ./research-bot.tar.gz
missher profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `missher -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.missher`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.missher`) is exported through an allow-list — only known Missher artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.missher/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `missher -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `missher profile` — not a parallel command tree.
...
- Reserved profile names (`missher`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`missher profile export` / `import`](../reference/profile-commands.md#missher-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Missher](../guides/use-soul-with-missher.md) — authoring personalities
