...
# Connecting Desktop to Many Missher Instances

Register every Missher backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Missher Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Missher gateway…"**) deep-links straight to
...
list of Missher gateways. Its intro says it plainly: *"Manage this device and
every Missher gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Missher runtime managed by this app." | automatic |
| **Remote gateway** | "A Missher gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Missher install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Missher Cloud** | "A hosted instance discovered through your Missher Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Missher Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Missher Cloud** kind in the add-connection
...
The Missher Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **Missher Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Missher Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `missher serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Missher agents
...
connection is registered) dispatches `missher update` to every eligible
...
- **Missher Cloud** instances are skipped with a *"Managed by Missher Cloud"*
...
panel, ⌘K **Update Missher**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `missher serve` is running on the remote host, the
...
- **"Update Missher Desktop to chat with agents on other connections"** — the
