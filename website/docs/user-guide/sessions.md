...
Missher Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.missher/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Missher stores session history so it can resume conversations, but it does not
...
Missher explicitly injects for that turn.
...
For example, if a user sends an image and asks Missher to make a meme from it,
Missher may inspect that image once with vision and run an image-processing
...
`missher sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `missher sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`missher` or `missher chat`) |
...
missher -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
missher -c "my project"   # → resumes "my project #3"
```
...
missher --resume 20250305_091523_a1b2c3d4
missher -r 20250305_091523_a1b2c3d4

# Resume by title
missher --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
missher --resume latest

# Or with the chat subcommand
missher chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `missher sessions list`.
...
missher --resume latest --in ./my-project

# Works with the TUI too
missher --tui --resume latest --in ./my-project
```
...
```bash
missher --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`missher sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
missher sessions list --workspace my-project
missher sessions list --workspace ~/code/missher-agent
```

### Conversation Recap on Resume

When you resume a session, Missher displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Missher session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.missher/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `missher -r "<title>"` from the shell) and pick up where the platform left off.
...
Missher automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `missher sessions list` or `missher sessions browse`.
...
```bash
missher sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Missher creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`missher -c "my project"`), it automatically picks the most recent session in the lineage.
...
Missher provides a full set of session management commands via `missher sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
missher sessions list

# Filter by platform
missher sessions list --source telegram

# Show more sessions
missher sessions list --limit 50
```
...
`missher sessions export` is one surface for every export format, selected with `--format`:
...
missher sessions export backup.jsonl

# Export sessions from a specific platform
missher sessions export telegram-history.jsonl --source telegram

# Export a single session
missher sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Point at a directory (existing, or ending in /) and the file is named for you:
# ~/exports/missher_session_20250305_091523_a1b2c3d4.jsonl
missher sessions export ~/exports/ --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
missher sessions export backup.jsonl --redact
```
...
missher sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
missher sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
missher sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
missher sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `missher-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
missher sessions export --format trace

# One session to a local trace file
missher sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
missher sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.missher/session-exports`).

```bash
# Export one session to Markdown
missher sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
missher sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
missher sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
missher sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
missher sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
missher sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
missher sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
missher sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
missher sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
missher sessions pin 20250305_091523_a1b2c3d4
missher sessions pin 20250305 20250306

# Remove the pin
missher sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
missher sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
missher sessions pinned --json > pinned-sessions.json
```
...
missher sessions prune

# Custom age threshold — bare numbers are days
missher sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
missher sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
missher sessions prune --newer-than 5h

# Explicit window with absolute timestamps
missher sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
missher sessions prune --source telegram
missher sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
missher sessions prune --newer-than 5h --title "smoke test"   # title substring
missher sessions prune --older-than 30 --max-messages 3        # tiny sessions
missher sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
missher sessions prune --model gpt-5 --older-than 1w           # by model (substring)
missher sessions prune --provider openrouter --older-than 60   # by billing provider
missher sessions prune --branch feature/old-experiment         # by git branch
missher sessions prune --user 12345678 --chat-type group       # by messaging origin
missher sessions prune --max-tokens 500 --older-than 7         # by token usage
missher sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
missher sessions prune --newer-than 5h --dry-run

# Skip confirmation
missher sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `missher sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `missher sessions prune` keeps the 90-day cutoff. Every
...
`missher sessions archive` takes the same filters as `prune` but soft-hides
...
missher sessions archive --newer-than 5h

# Archive by title substring, preview first
missher sessions archive --title "dry run" --dry-run
missher sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `missher sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`missher sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
missher sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`missher insights`](/reference/cli-commands#missher-insights).
...
`missher sessions repair-routing` finds message-bearing session rows with no
...
missher sessions repair-routing
...
missher sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
missher sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.missher/state.db ~/.missher/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Missher and
continue it here. Missher reads Claude Code's session logs
...
missher sessions import

# Limit to one tool, or point at a specific file
missher sessions import --from claude
missher sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
missher --resume @claude
missher --resume @codex
```

`missher sessions import` creates a new Missher session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `missher --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**Missher Desktop** has the same importer in the command palette (**Import
session**). It lists the logs on the machine the
connected backend runs on — not the computer running the app — shows a
read-only preview, and **Continue in Missher** copies the conversation into the
...
When Missher cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Missher uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Missher in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.missher/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.missher/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.missher/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.missher/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.missher/sessions/sessions.json` is a **legacy mirror** of it,
...
`missher sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.missher/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `missher sessions list`, the cause is
`state.db` not receiving them — run `missher sessions repair` and watch for a
...
`*.jsonl` files in `~/.missher/sessions/`. They are no longer written or
read by Missher. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Missher process in the same `MISSHER_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.missher/config.yaml`:
...
`in its tip segment`). `missher sessions export` still works for such sessions.
...
missher sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `missher sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
