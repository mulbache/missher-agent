...
sidebar_position: 12
title: "Google Chat"
description: "Set up Missher Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Missher Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Missher
...
> Run `missher gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Missher can read (e.g.,
`~/.missher/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a missher restart)
...
- Principal: `missher-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Missher calls
...
- **App name**: whatever you want users to see ("Missher" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/missher-chat-events`.
...
`ADDED_TO_SPACE` event that Missher uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Missher

Add the Google Chat section to `~/.missher/.env`:
...
On Docker / hosted images `/opt/missher/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `MISSHER_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "Missher is thinking…" marker, then
...
`~/.missher/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Missher is thinking…").
...
Thread support: when a user replies inside a thread, Missher detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Missher session.
...
3. Download the JSON. Move it onto the host that runs Missher.
4. Register the client with Missher (run under the profile you want it scoped to):
...
missher -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Missher home (e.g.
`~/.missher/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.missher/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.missher/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Missher isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `missher gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Missher will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.missher/google_chat_user_tokens/<sanitized_email>.json` (filesystem
