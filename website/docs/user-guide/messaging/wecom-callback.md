...
Connect Missher to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Missher supports two WeCom integration modes:
...
> Run `missher gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Missher decrypts the message, queues it for the agent
...
(Use `missher gateway start` only after `missher gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Missher and the token the
...
in `~/.missher/.env` values around `=` will also break signature checks. After
fixing, restart `missher gateway run`.
...
Check `missher gateway run` logs for the bound host/port. If the adapter bound to
