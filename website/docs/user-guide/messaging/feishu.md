...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Missher Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Missher Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Missher opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Missher responds to every message. |
| Group chats | Missher responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Missher will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `missher gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Missher runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Missher behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Missher starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Missher responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Missher responds to @mentions from any user in any group. |
| `allowlist` | Missher only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Missher ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Missher read all group traffic without requiring an @mention:
...
Missher auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Missher ignores messages sent by other bots. Enable bot-to-bot messaging when you want Missher to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Missher. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `missher update --gateway` needs confirmation, the adapter records the selected answer in Missher's `.update_response` file and replaces the card inline with a resolved state.
...
:::warning
Without a published card callback, Feishu will still successfully *send* interactive cards (sending only requires `im:message:send` permission), but clicking any button returns error 200340. The card appears to work — the error only surfaces when a user interacts with it, and the click never reaches Missher (nothing is logged), because Feishu rejects it before delivering the callback.
:::

Error codes 200672 / 200673 indicate the callback *did* reach Missher and Feishu rejected the response; if you see them, please file an issue with the matching `gateway.log` lines.

## Document Comment Intelligent Reply

Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Missher reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.missher/feishu_comment_rules.json` (pairing grants in `~/.missher/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
You can invite the Missher Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, Missher can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends Missher the meeting invitation event.
- Missher extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, Missher drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so Missher can reply to the inviter.
...
|---------|---------|---------|
| Quiet period | `MISSHER_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `MISSHER_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `MISSHER_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `MISSHER_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.missher/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `MISSHER_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `MISSHER_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `MISSHER_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `MISSHER_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `MISSHER_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `MISSHER_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `missher gateway setup` |
| `Another local Missher gateway is already using this Feishu app_id` | Only one Missher instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `missher-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
