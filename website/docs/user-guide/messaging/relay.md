...
sidebar_position: 30
title: "Missher Relay"
description: "Connect Missher to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Missher Relay (Connector)
...
Missher Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.missher/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.missher/.env`:
...
`missher gateway enroll` refuses to run in managed/hosted installs — there the
...
`missher gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.missher/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
token. Re-login with `missher auth add nous` (or `missher setup`) and retry.
...
startup. Restart it (`missher gateway restart`).
