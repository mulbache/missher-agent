...
description: "Chat with Missher from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Missher from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Missher](/guides/use-voice-mode-with-missher).
...
saved by `missher gateway setup` can enable a credential-based platform without a
...
```mermaid
flowchart TB
    subgraph Gateway["Missher Gateway"]
...
For group chats, hooks, and automation flows, Missher supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Missher keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Missher does not hide failures just because the text resembles a silence token.
...
```bash
missher gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
missher gateway              # Run in foreground
missher gateway setup        # Configure messaging platforms interactively
missher gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo missher gateway install --system   # Linux only: install a boot-time system service
missher gateway start        # Start the default service
missher gateway stop         # Stop the default service
missher gateway status       # Check default service status
missher gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.missher/config.yaml"
gateway:
...
```bash
missher gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Missher sends heartbeats
...
| `/update` | Update Missher Agent to the latest version |
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.missher/config.yaml`:
...
missher pairing approve telegram XKGH5N7P

# Other pairing commands:
missher pairing list          # View pending + approved users
missher pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Missher appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.missher/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.missher/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `MISSHER_HOME`:

- `~/.missher/status_phrases.yaml` or any `*.yaml` in `~/.missher/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to MISSHER_HOME
...
Off by default. When enabled, Missher prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.missher/config.yaml`:
...
```bash
MISSHER_BACKGROUND_NOTIFICATIONS=result
```
...
missher gateway start                 # Start the service
missher gateway stop                  # Stop the service
missher gateway status                # Check status
journalctl --user -u missher-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo missher gateway install --system
sudo missher gateway start --system
sudo missher gateway status --system
journalctl -u missher-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Missher installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit missher-gateway` (or `sudo systemctl edit missher-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `missher update`. When `missher update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart missher-gateway` command (it never blocks on an interactive password prompt).
...
```bash
missher gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `missher update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo missher update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/missher-gateway`:

```
missher ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed missher-gateway*, /usr/bin/systemctl --no-ask-password start missher-gateway*, /usr/bin/systemctl --no-ask-password restart missher-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Missher will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Missher installations on the same machine (with different `MISSHER_HOME` directories), each gets its own systemd service name. The default `~/.missher` uses `missher-gateway`; other installations use `missher-gateway-<hash>`. The `missher gateway` commands automatically target the correct service for your current `MISSHER_HOME`.
:::

### macOS (launchd)

```bash
missher gateway install               # Install as launchd agent
missher gateway start                 # Start the service
missher gateway stop                  # Stop the service
missher gateway status                # Check status
tail -f ~/.missher/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.missher.gateway.plist`. It includes three environment variables:
...
- **MISSHER_HOME** — scopes the gateway to your Missher installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `missher gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `MISSHER_HOME` directory gets its own launchd label. The default `~/.missher` uses `ai.missher.gateway`; other installations use `ai.missher.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `missher-cli` | Full access |
| Telegram | `missher-telegram` | Full tools including terminal |
| Discord | `missher-discord` | Full tools including terminal |
| WhatsApp | `missher-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `missher-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `missher-slack` | Full tools including terminal |
| Google Chat | `missher-google_chat` | Full tools including terminal |
| Signal | `missher-signal` | Full tools including terminal |
| SMS | `missher-sms` | Full tools including terminal |
| Email | `missher-email` | Full tools including terminal |
| Home Assistant | `missher-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `missher-mattermost` | Full tools including terminal |
| Matrix | `missher-matrix` | Full tools including terminal |
| DingTalk | `missher-dingtalk` | Full tools including terminal |
| Feishu/Lark | `missher-feishu` | Full tools including terminal |
| WeCom | `missher-wecom` | Full tools including terminal |
| WeCom Callback | `missher-wecom-callback` | Full tools including terminal |
| Weixin | `missher-weixin` | Full tools including terminal |
| BlueBubbles | `missher-bluebubbles` | Full tools including terminal |
| QQBot | `missher-qqbot` | Full tools including terminal |
| Yuanbao | `missher-yuanbao` | Full tools including terminal |
| Microsoft Teams | `missher-teams` | Full tools including terminal |
| API Server | `missher-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `missher-webhook` | Full tools including terminal |
| Raft | `missher-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.missher/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.missher/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
