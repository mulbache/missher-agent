...
Connect Missher to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Missher — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Missher runs it in a
...
missher photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Missher Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.missher/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.missher/.env`
...
(device token, dashboard project id) lives in `~/.missher/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Missher
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Missher replies with a pairing code. Approve it with:

```bash
missher pairing approve photon <CODE>
```

Use `missher pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.missher/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.missher/.env`):
...
By default Missher responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Missher` and
`@Missher agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Missher will reply.

## Status & troubleshooting

```bash
missher photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Missher uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run missher photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `missher photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `missher photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `missher photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Missher sends images, voice
  ...
  media.
- **Native polls are supported.** Missher sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Missher, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Missher-sent messages
...
- **Message effects are supported.** Missher sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `missher send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<missher-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Missher is set
...
| `PHOTON_MENTION_PATTERNS` | Missher wake words  | JSON list / comma / newline regex patterns for group mentions |
