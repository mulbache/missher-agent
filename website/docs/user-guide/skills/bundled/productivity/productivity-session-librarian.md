...
| List/filter by metadata (age, source, cost, tokens, workspace) | `missher sessions list` / `stats` via terminal |
| Rename | `missher sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `missher sessions archive <filters>` |
| Delete (destructive) | `missher sessions delete` / `missher sessions prune <filters>` |
| Export before deleting anything valuable | `missher sessions export --session-id <id> --format md` |
...
`missher sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `missher sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `missher sessions list`
