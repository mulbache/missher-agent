...
4. Store the token in `${MISSHER_HOME:-~/.missher}/.env` (or via `missher setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Missher. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${MISSHER_HOME:-~/.missher}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
