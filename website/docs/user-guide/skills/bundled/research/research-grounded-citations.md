...
Ledger location: `$MISSHER_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `MISSHER_CITATION_LEDGER`.

## How to Run

```bash
S=~/.missher/skills/research/grounded-citations/scripts/sources.py
...
`missher skills install official/social-media/reddit-reading` or
`missher skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `MISSHER_CITATION_LEDGER`) if their
