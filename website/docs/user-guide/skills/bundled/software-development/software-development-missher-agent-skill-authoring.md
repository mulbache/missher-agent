---
title: "Missher Agent Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "Missher Agent Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Missher Agent Skill Authoring
...
| Path | `skills/software-development\missher-agent-skill-authoring` |
| Version | `2.0.0` |
| Author | Missher Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `missher-agent`, `conventions`, `skill-md` |
...
# Authoring Missher-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.missher/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the missher-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with missher-agent
...
- Don't use for: personal skills in `~/.missher/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `missher skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Missher Agent
license: MIT
...
- Credit the **human first**, then "Missher Agent" as secondary collaborator: `Ben Barclay (benbarclay), Missher Agent`.
- Never `author: Missher Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Missher Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.missher/skills/`.
...
|---|---|
| Missher tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.missher/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Missher Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.missher.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Missher tools
