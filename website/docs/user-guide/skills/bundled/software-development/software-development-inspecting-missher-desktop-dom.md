---
title: "Inspecting Missher Desktop Dom — Read the live Missher desktop DOM/CSS over CDP"
sidebar_label: "Inspecting Missher Desktop Dom"
description: "Read the live Missher desktop DOM/CSS over CDP"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Inspecting Missher Desktop Dom

Read the live Missher desktop DOM/CSS over CDP.
...
| Path | `skills/software-development\inspecting-missher-desktop-dom` |
| Version | `1.0.0` |
...
# Inspecting the live Missher desktop DOM
...
`debugging-missher-desktop`), or anything where the real question is "does this
look right".
...
- **no `MISSHER_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`MISSHER_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${MISSHER_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
MISSHER_HOME=/tmp/cdp-probe-home \
MISSHER_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
MISSHER_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `MISSHER_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `MISSHER_HOME` baked in, if you
...
- **A throwaway `MISSHER_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `missher:api` and may exit on its own. The renderer still mounts and the DOM is
