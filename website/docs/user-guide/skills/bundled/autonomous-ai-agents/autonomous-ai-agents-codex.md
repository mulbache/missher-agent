...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`missher-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-missher-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Missher terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Missher itself, `model.provider: openai-codex` uses Missher-managed Codex
OAuth from `~/.missher/auth.json` after `missher auth add openai-codex`. For the
...
## Missher Gateway Caveat

When invoking the Codex CLI from a Missher gateway/service context (for example,
