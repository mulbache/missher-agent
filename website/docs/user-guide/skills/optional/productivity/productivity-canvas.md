...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Missher Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${MISSHER_HOME:-~/.missher}/.env`:
...
```bash
CANVAS="python $MISSHER_HOME/skills/productivity/canvas/scripts/canvas_api.py"
