...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/productivity/siyuan` |
...
3. Store it in `${MISSHER_HOME:-~/.missher}/.env`:
   ```
...
```yaml
# In ~/.missher/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
