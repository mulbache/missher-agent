...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${MISSHER_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${MISSHER_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${MISSHER_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client missher
```
...
```bash
PUBLISH="${MISSHER_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client missher
```
...
```bash
DRIVE="${MISSHER_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `missher`)    |
