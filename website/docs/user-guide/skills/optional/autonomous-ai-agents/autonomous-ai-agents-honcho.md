---
title: "Honcho — Configure and troubleshoot Honcho memory for Missher"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Missher"
---
...
Configure and troubleshoot Honcho memory for Missher.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `missher skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Missher

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Missher profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/missher#running-honcho-locally-with-missher

### Verify

```bash
missher honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Missher profile's AI peer
...
Honcho models conversations as interactions between **peers**. Missher creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Missher instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Missher run |
| `global` | Single session across all directories |

Manual override: `missher honcho map my-project-name`
...
Each Missher profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block missher.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `missher.coder` host block in `honcho.json`
...
```bash
missher honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Missher when Honcho memory is active.
...
Config file: `$MISSHER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `missher honcho setup`. Ensure `memory.provider: honcho` is in `~/.missher/config.yaml`.

### Memory not persisting across sessions
Check `missher honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `missher profile create <name> --clone`. For existing profiles: `missher honcho sync`.
...
| Command | Description |
|---------|-------------|
| `missher honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `missher honcho status` | Show resolved config, connection test, peer info for active profile |
| `missher honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `missher honcho disable` | Disable Honcho for the active profile |
| `missher honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `missher honcho peers` | Show peer identities across all profiles |
| `missher honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `missher honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `missher honcho sessions` | List known directory-to-session-name mappings |
| `missher honcho map <name>` | Map current working directory to a Honcho session name |
| `missher honcho identity` | Seed AI peer identity or show both peer representations |
| `missher honcho sync` | Create host blocks for all Missher profiles that don't have one yet |
| `missher honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Missher + Honcho |
| `missher memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `missher memory status` | Show active memory provider and config |
| `missher memory off` | Disable external memory provider |
