...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/autonomous-ai-agents/dynamic-workflow` |
| Path | `optional-skills/autonomous-ai-agents/dynamic-workflow` |
| Version | `2.0.0` |
| Author | Teknium + Missher Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `orchestration`, `fan-out`, `subagents`, `delegation`, `verification`, `migration`, `audit`, `research`, `campaign` |
| Related skills | [`missher-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-missher-agent), [`simplify-code`](/docs/user-guide/skills/bundled/software-development/software-development-simplify-code) |
...
for serial chains. For a refactor or fix campaign on missher-agent itself, load
`missher-agent` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `MISSHER_HOME`,
