...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Missher Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Missher terminal calls (desktop app flow only)
...
### Service Account (recommended for Missher)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${MISSHER_HOME:-~/.missher}/.env` (the skill will prompt for this on first load).
...
## Missher Execution Pattern (desktop app flow)

Missher terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/missher-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/missher-op.sock"
