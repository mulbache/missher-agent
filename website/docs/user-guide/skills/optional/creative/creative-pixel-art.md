...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.missher/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.missher/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Missher installs package this)
