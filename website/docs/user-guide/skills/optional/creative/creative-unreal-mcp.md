...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Missher MCP catalog. The
...
Two halves, in this order: the editor side must be up before Missher connects.
...
### One-time, Missher side

    missher mcp install unreal-engine
...
`~/.missher/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Missher — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Missher connects
...
2. Start the Missher session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Missher
they appear as:

| Missher tool | Purpose |
|---|---|
...
then happens at `missher mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Missher runs batched calls
...
- **Start order matters.** Editor + server up first, then the Missher
...
- **Timeouts on long operations.** Missher' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.missher/config.yaml` for
