---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Missher"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Missher"
---
...
Import an OpenClaw setup (memories, skills) into Missher.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `missher skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Missher Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Missher`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Missher Migration

Use this skill when a user wants to move their OpenClaw setup into Missher Agent with minimal manual cleanup.
...
```bash
missher claw migrate              # Full interactive migration
...
missher claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `missher setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_missher.py` to:

- import `SOUL.md` into the Missher home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Missher memory entries
- merge OpenClaw command approval patterns into Missher `command_allowlist`
- migrate Missher-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Missher working-directory configuration
- copy OpenClaw skills into `~/.missher/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Missher workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.missher/tts/`
- archive non-secret docs that do not have a direct Missher destination
...
- `scripts/openclaw_to_missher.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.missher/skills/migration/openclaw-migration/scripts/openclaw_to_missher.py`

Do not guess a shorter path like `~/.missher/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.missher/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Missher-compatible secrets, currently:
...
Missher CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Missher. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Missher workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Missher skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.missher/skills/migration/openclaw-migration/scripts/openclaw_to_missher.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.missher/skills/migration/openclaw-migration/scripts/openclaw_to_missher.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.missher/skills/migration/openclaw-migration/scripts/openclaw_to_missher.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.missher/skills/migration/openclaw-migration/scripts/openclaw_to_missher.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.missher/skills/migration/openclaw-migration/scripts/openclaw_to_missher.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.missher/skills/migration/openclaw-migration/scripts/openclaw_to_missher.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Missher unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Missher targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Missher destination. Unsupported auth blobs must still be reported as skipped.
...
- Missher persona state imported
- Missher memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.missher/skills/openclaw-imports/`
