...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/mcp/fastmcp` |
...
| Related skills | [`missher-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-missher-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Missher or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Missher. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.missher/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.missher/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Missher integration, either:

- configure the server in `~/.missher/config.yaml` using the `native-mcp` skill, or
...
### Missher cannot see the deployed server

The server-building part may be correct while the Missher config is not. Load the `native-mcp` skill and configure the server in `~/.missher/config.yaml`, then restart Missher.
