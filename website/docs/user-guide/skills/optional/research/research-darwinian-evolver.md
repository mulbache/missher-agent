...
| | |
|---|---|
| Source | Optional — install with `missher skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Missher Agent |
| License | MIT |
...
import upstream classes into Missher itself.
...
```bash
mkdir -p ~/.missher/cache/darwinian-evolver && cd ~/.missher/cache/darwinian-evolver
...
```bash
cd ~/.missher/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.missher/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.missher/skills/research/darwinian-evolver
DE_DIR=~/.missher/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Missher core.
   Custom driver scripts under `~/.missher/skills/...` are user-side and fine.
