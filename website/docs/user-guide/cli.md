...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Missher Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Missher Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `missher setup --portal` — and you're ready to `missher chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Missher also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `missher --tui` — see the [TUI](tui.md) guide.
:::
...
missher chat --query-file prompt.txt
missher chat --query-file - < prompt.txt

# With a specific model
missher chat --model "anthropic/claude-sonnet-4"

# With a specific provider
missher chat --provider nous        # Use Nous Portal
missher chat --provider openrouter  # Force OpenRouter

# With specific toolsets
missher chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
missher -s missher-agent-dev,github-auth
missher chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
missher --continue             # Resume the most recent CLI session (-c)
missher --resume <session_id>  # Resume a specific session by ID (-r)
missher --resume latest        # Resume the most recent session (same as -c)
missher --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
missher chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
missher -w                         # Interactive mode in worktree
missher -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`missher -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
missher worktree list              # audit: age, size, verdict, reason per tree
missher worktree list --json       # machine-readable audit (trees, external trees, branches)
missher worktree prune             # remove safe trees + delete merged branches
missher worktree prune --dry-run   # show the plan without changing anything
missher worktree prune --older-than 7   # only reap trees idle for 7+ days
missher worktree prune --trees-only     # leave local branches alone
missher worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running missher session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.missher/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `missher -w` for days — no longer accumulate merged scratch trees
...
The `missher plugins` commands manage native Missher plugins and portable Agent
...
missher plugins disable <plugin-name>
missher plugins update <plugin-name>
missher plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Missher currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Missher CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Missher CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `MISSHER_YOLO_MODE` is on (either `missher --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Missher to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.missher/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Missher supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Missher replies |
...
```bash
missher -s missher-agent-dev,github-auth
missher chat -s github-pr-workflow -s github-auth
```

Missher loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.missher/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.missher/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Missher..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Missher also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Missher recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Missher — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Missher is working, Missher prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Missher to the background — just like any terminal process. The shell prints a confirmation:

```
Missher Agent has been suspended. Run `fg` to bring Missher Agent back.
```
...
Resume this session with:
  missher --resume 20260225_143052_a1b2c3
...
```bash
missher --continue                          # Resume the most recent CLI session
missher -c                                  # Short form
missher -c "my project"                     # Resume a named session (latest in lineage)
missher --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
missher --resume "refactoring auth"         # Resume by title
missher --resume latest                     # Resume the most recent session (same as -c)
missher --resume latest --in ./my-project   # Latest session for ./my-project's workspace
missher -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `missher sessions rename <id> <title>` from the command line. Use `missher sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Missher's SQLite state database under `~/.missher/state.db`. The database keeps:
...
Missher immediately confirms the task and gives you back the prompt:
