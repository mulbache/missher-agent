...
Run multiple independent Missher agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Missher home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Missher state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same Missher home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
```bash
missher profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Missher profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `missher setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::

### Blank profile

```bash
missher profile create mybot
```
...
```bash
missher profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `missher profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
missher profile create work-backup --clone-from coder --clone-all
```
...
parks the duplicate adapter (and `missher gateway migrate --multiplex` refuses with one
duplicate-credential blocker per platform). Configure the new profile's own bots with
`missher -p <name> setup` or the dashboard Messaging page.

```bash
missher profile create twin --clone --clone-channels   # keep the source's bots anyway
```
...
without a clone flag. `missher profile list` prints the same warning for any existing
...
The alias works with every missher subcommand — it's just `missher -p <name>` under the hood.
...
```bash
missher -p coder chat
missher --profile=coder doctor
missher chat -p coder -q "hello"    # works in any position
```

### Sticky default (`missher profile use`)

```bash
missher profile use coder
missher chat                   # now targets coder
missher tools                  # configures coder's tools
missher profile use default    # switch back
```

Sets a default so plain `missher` commands target that profile. Like `kubectl config use-context`.
...
- **`missher profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Missher its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Missher was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.missher/profiles/coder/.env

# Edit assistant's tokens
nano ~/.missher/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates missher-gateway-coder systemd/launchd service
assistant gateway install     # creates missher-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `missher profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `missher -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.missher/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `missher profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`missher update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
missher profile list           # show all profiles with status
missher profile show coder     # detailed info for one profile
missher profile rename coder dev-bot   # rename (updates alias + service)
missher profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
missher profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.missher` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
missher profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `missher profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.missher/profile.yaml`; remove that line to
...
```bash
missher profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `missher profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.missher`). To remove everything, use `missher uninstall`.
:::
...
```bash
# Bash
eval "$(missher completion bash)"

# Zsh
eval "$(missher completion zsh)"
```
...
Profiles use the `MISSHER_HOME` environment variable. When you run `coder chat`, the wrapper script sets `MISSHER_HOME=~/.missher/profiles/coder` before launching missher. Since 119+ files in the codebase resolve paths via `get_missher_home()`, Missher state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `MISSHER_HOME`.
...
isolated by `MISSHER_HOME`, not by changing `HOME`. Container backends still use
`{MISSHER_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `MISSHER_HOME` is the profile boundary. It controls Missher config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Missher
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Missher keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Missher launches tool
subprocesses with `HOME={MISSHER_HOME}/home`; you then need to initialize or link
...
Missher also exposes `MISSHER_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.missher` itself. No migration needed — existing installs work identically.
...
missher profile export coder
missher profile import ./coder.tar.gz --name coder
```
