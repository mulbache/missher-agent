...
title: "Git Worktrees"
description: "Run multiple Missher agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Missher Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Missher so each session has a clean, isolated working directory.

## Why Use Worktrees with Missher?

Missher treats the **current working directory** as the project root:

- CLI: the directory where you run `missher` or `missher chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.missher/config.yaml`
...
Missher creates `.worktrees/my-experiment/` inside the repo (branch
`missher/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `missher-<id>`
...
git worktree add ../repo-feature feature/missher-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/missher-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Missher there:

```bash
cd ../repo-feature

# Start Missher in the worktree
missher
```
...
git worktree add ../repo-experiment-a feature/missher-a
git worktree add ../repo-experiment-b feature/missher-b
```
...
Each Missher process:

- Works on its own branch (`feature/missher-a` vs `feature/missher-b`).
...
- Missher checkpoint data under `~/.missher/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Missher experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/missher-checkpoints-docs`, `feature/missher-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Missher from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `missher -w` (Automatic Worktree Mode)

Missher has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `missher/missher-<hash>`).
...
```bash
missher -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `missher -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Missher session its own clean checkout.
