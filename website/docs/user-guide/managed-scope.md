...
`~/.missher/config.yaml`, `~/.missher/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/missher`:

```text
/etc/missher/
├── config.yaml     # managed config layer (wins over ~/.missher/config.yaml)
└── .env            # managed env layer (wins over ~/.missher/.env + shell)
```
...
The location can be relocated with the `MISSHER_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `MISSHER_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Missher.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export MISSHER_MANAGED_DIR=/opt/org/missher-policy
```

:::warning
A user who can set `MISSHER_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `missher doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/missher/config.yaml` (managed) | `/etc/missher/.env` (managed) |
| 2 | `~/.missher/config.yaml` (user) | `~/.missher/.env` (user) |
...
```bash
missher config        # shows a header naming the managed source + the pinned keys
missher doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Missher refuses and names the source:

```bash
$ missher config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/missher/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `missher config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/missher

# Pin some config values for every user on this machine
sudo tee /etc/missher/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/missher/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/missher
sudo chmod 0644 /etc/missher/config.yaml /etc/missher/.env
```

Changes take effect on the next Missher start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`missher doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Missher as `root`), managed scope is advisory.
