...
If you only run one Missher agent, you don't need this page — see
...
[Connecting Desktop to Many Missher Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Missher agents that should all
...
(`ai.missher.gateway-<name>.plist`) or systemd user service
(`missher-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
missher profile create coder
missher profile create personal-bot
missher profile create research
...
```bash
missher config set gateway.multiplex_profiles true
missher gateway restart
```

Equivalently, in the default profile's `~/.missher/config.yaml`:
...
You do **not** run `missher gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `missher gateway run`, `start`,
...
loop. `missher -p coder gateway stop` refuses the same way (exit 78) when coder has no
gateway of its own — there is nothing to stop but the multiplexer, which
`missher gateway stop` on the default profile takes down for every served profile.
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.missher/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `missher webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`missher webhook ls` shows the
...
A standalone `missher -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ missher -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: missher gateway status
...
`missher gateway status` and `missher status` on the default profile list the same
...
home). `missher status` on the default profile reports the multiplexer and lists
the profiles it serves (`Serves: coder, research`); `missher -p coder status`,
`missher -p coder gateway status` and `missher -p coder cron status` all report
...
`HINDSIGHT_BANK_ID`, `MISSHER_HONCHO_HOST`) are read from the routed profile's
...
`MISSHER_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`MISSHER_HOME`, without the default profile's secrets in their environment, and
...
| Dashboard actions (`missher -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `MISSHER_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
| Cron `.env` tuning (`MISSHER_CRON_TIMEOUT`, `MISSHER_MODEL` fallback, `MISSHER_CRON_MAX_PARALLEL`, prefill file), worker / Bot Chat child env | The profile's own `.env`; children never inherit the default profile's `.env` settings or bridged `TERMINAL_*` policy | Cron defaults / model refusal, exactly as a standalone `missher -p <name> gateway run` |
...
archived or deleted instead — `missher profile delete <name>`, or move the
...
multiplexer started as `missher -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`missher profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `missher -p <name> gateway
...
adapters the same way, and `missher profile rename` unroutes the old name before
...
`~/.local/bin/missher-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: missher-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    missher gateway "$action"
  else
    missher -p "$profile" gateway "$action"
  fi
}
...
```bash
missher-gateways start      # start every configured profile
missher-gateways stop       # stop every configured profile
missher-gateways restart    # restart all
missher-gateways status     # status across all
missher-gateways list       # delegates to `missher gateway list`
```

:::tip
The `default` profile is targeted with `missher gateway <action>` (no `-p`),
not `missher -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `missher -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.missher.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/missher-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.missher.gateway.plist` /
`missher-gateway.service`.
...
```bash
# Default profile
tail -f ~/.missher/logs/gateway.log
tail -f ~/.missher/logs/gateway.error.log

# Named profile
tail -f ~/.missher/profiles/<name>/logs/gateway.log
tail -f ~/.missher/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.missher/logs/gateway.log ~/.missher/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
missher logs -f                  # follow default profile
missher -p coder logs -f         # follow one profile
missher logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
missher profile list             # profiles + model + gateway state
missher-gateways status          # full status across every profile
launchctl list | grep missher    # macOS — PIDs and labels
systemctl --user list-units 'missher-gateway-*'   # Linux — units
```
...
```
~/.missher/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.missher/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
missher config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.missher/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=missher --why="gateways running" \
  sleep infinity &
...
`missher-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.missher/.env ~/.missher/profiles/*/.env
```
...
```bash
missher gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
missher gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
missher gateway migrate --standalone            # roll back to per-profile gateways
```

### What `missher update` does
...
service) and `gateway.multiplex_profiles` is off, `missher update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `missher gateway migrate --multiplex --yes`) and prints what it did. This
...
install that is already multiplexing is left alone. `missher update` also does
nothing when no secondary profile runs its own gateway — it never flips modes
on an install where nothing was running.

### Boundaries `missher update` never crosses on its own

The unattended hook only folds profiles that share **one UNIX user, one service
domain and one `profiles/` tree** — the shape `missher profile create` produces.
...
| `MISSHER_HOME` outside `<default home>/profiles/` | a unit pinning `MISSHER_HOME=/opt/missher/profiles/emma` |

In that case `missher update` prints the boundary it found plus
`missher gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `missher gateway migrate --multiplex --dry-run`
...
```bash
missher config set gateway.auto_multiplex_migration false
```

`missher update` then leaves per-profile gateways exactly as they are, with no
...
`missher gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `missher gateway migrate --multiplex` with
...
`missher profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `missher profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.missher/gateway_migration.json` for rollback.
...
above). `missher profile create` confirms this when the live multiplexer picked the
profile up; it prints the `missher gateway restart` reminder only when it could not
reach the multiplexer (for example, a gateway started from an older build).

### Rollback

```bash
missher gateway migrate --standalone
```
...
with `missher config set gateway.multiplex_profiles false && missher gateway restart`
...
flag, stop the per-profile tasks, `missher gateway restart`). The dashboard's
System page offers the same migration as a button when the preflight finds an
eligible install.

## Updating the code

`missher update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `missher gateway start` after a previous `missher gateway stop`. The
...
```bash
ps -ef | grep "missher_cli.*-p <profile>"
cat ~/.missher/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.missher.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.missher.gateway-<profile>.plist

# Linux
systemctl --user restart missher-gateway-<profile>.service
```

### Health check

```bash
missher doctor                  # default profile
missher -p <profile> doctor     # one profile
```
