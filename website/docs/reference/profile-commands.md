...
This page covers all commands related to [Missher profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `missher profile` without a subcommand shows help.
...
```bash
missher profile use <name>
```

Sets `<name>` as the active profile. All subsequent `missher` commands (without `-p`) will use this profile.
...
missher profile use default
```

## `missher profile create`

```bash
missher profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `missher profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `missher update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.missher`), use `missher skills opt-out` / `missher skills opt-in`. |
...
missher profile create work2 --clone-from work

# Clone everything from a specific profile
missher profile create work2-backup --clone-from work --clone-all
```

## `missher profile describe`

```bash
missher profile describe [<name>] [options]
```
...
missher profile describe researcher

# Set it explicitly
missher profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
missher profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
missher profile describe --all --auto
```

## `missher profile delete`

```bash
missher profile delete <name> [options]
```
...
```bash
missher profile delete mybot
missher profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.missher`) cannot be deleted — use `missher uninstall` to remove everything.
:::

## `missher profile show`

```bash
missher profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Missher home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ missher profile show work
Profile: work
Path:    ~/.missher/profiles/work
...
## `missher profile alias`

```bash
missher profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Missher installation.
...
```bash
missher profile alias work
# Creates/updates ~/.local/bin/work

missher profile alias work --name mywork
# Creates ~/.local/bin/mywork

missher profile alias work --remove
# Removes the wrapper script
```

## `missher profile rename`

```bash
missher profile rename <old-name> <new-name>
```
...
```bash
missher profile rename mybot assistant
# ~/.missher/profiles/mybot → ~/.missher/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `missher profile export`

```bash
missher profile export <name> [options]
```
...
```bash
missher profile export work
# Creates work.tar.gz in the current directory

missher profile export work -o ./work-2026-03-29.tar.gz
```
...
## `missher profile import`

```bash
missher profile import <archive> [options]
```
...
```bash
missher profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

missher profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `missher -p telemetry`). |
...
missher profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
missher profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
missher profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
missher profile install ./telemetry/
```

### `missher profile update`

```bash
missher profile update <name> [--force-config] [--yes]
```
...
### `missher profile info`

```bash
missher profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Missher version, author, env var requirements, the source URL/path, and
...
`missher profile list` also shows the distribution name and version in a
`Distribution` column, and `missher profile show <name>` / `delete <name>`
...
missher profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
missher profile install https://github.com/your-org/internal-assistant.git
```
...
`missher_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Missher version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Missher can clone from.
3. Tell recipients to run `missher profile install <your-repo-url>`.
...
## `missher -p` / `missher --profile`

```bash
missher -p <name> <command> [options]
missher --profile <name> <command> [options]
```

Global flag to run any Missher command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
missher -p work chat -q "Check the server status"
missher --profile dev gateway start
missher -p personal skills list
missher -p work config edit
```

## `missher completion`

```bash
missher completion <shell>
```
...
- `missher profile <TAB>` — subcommands (list, use, create, etc.)
- `missher profile use <TAB>` — profile names
- `missher -p <TAB>` — profile names
