...
def _profile_user_agent() -> str:
    """Return a ``missher-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from missher_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"missher-cli/{_ver}"
    except Exception:
        return "missher-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; missher_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.missher/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # missher-cli UA so the catalog endpoint is reachable.
