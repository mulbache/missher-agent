...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with missher-agent)
2. User plugins: ``$MISSHER_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``missher_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$MISSHER_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$MISSHER_HOME/plugins/model-providers/`` if it exists."""
    try:
        from missher_constants import get_missher_home

        d = get_missher_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$MISSHER_HOME/plugins/`` if it exists.

    This is where ``missher plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple MISSHER_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_missher_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``missher_agent.plugins``
    ...
    * **Provider targets only.** The ``missher_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$MISSHER_HOME``) keep their
    ...
    try:
        from missher_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$MISSHER_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``missher plugins install`` at
          ``$MISSHER_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``missher_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$MISSHER_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with missher-agent.
    ...
    # 2. User plugins — under $MISSHER_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``missher plugins install`` / the plugin index.
    #     Those clone into $MISSHER_HOME/plugins/<name>/ — flat, NOT under
