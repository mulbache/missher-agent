...
- OpenViking server running and reachable from Missher

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Missher can identify older servers that expose the legacy status-only health
...
connection values into Missher, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.missher/.env`; for a named profile use
...
OpenViking's server config is separate from Missher:
...
Missher-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Missher lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Missher sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Missher also sends `User-Agent: openviking-memory-missher/<version>` on
OpenViking requests. This standard harness identifier contains the Missher
...
does not ask for a peer ID. Without a configured peer, Missher sends neither
...
order remains environment, linked OpenViking config, then Missher YAML. To use
...
on the old implicit `missher` peer now use user memory for new writes. Without
...
Set `agent: missher` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `missher-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Missher
...
Use the same OpenViking profile and credentials as Missher for manual recovery.
...
Missher built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Missher action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Missher
...
file URIs, such as
`viking://user/default/peers/missher/memories/preferences/mem_abc123.md` (any
