"""Interactive ``missher memory setup`` wizard for the OpenViking provider.

Pure UI flow: prompts, menus, and persistence of the chosen connection (Missher
``.env`` only, or mirrored to an ``ovcli.conf.<name>`` profile that Missher then
...
def _save_missher_only_config(*, config: dict, provider_config: dict, env_path: Path, values: dict) -> None:
    ov = _ov()
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Missher session to activate.\n")
...
def _run_create_profile_setup(*, prompt, select, cancelled, config: dict, provider_config: dict, env_path: Path) -> bool | object:
    ...
    save_choice = select("  Save OpenViking config",
                         [("Keep in Missher only", "write values only to Missher .env"),
    ...
    _save_missher_only_config(config=config, provider_config=provider_config, env_path=env_path, values=values)
    _print_openviking_ready("Connection saved to Missher .env.")
    return True


def run_setup(missher_home: str, config: dict) -> None:
    """Entry point for ``OpenVikingMemoryProvider.post_setup``."""
    from missher_cli.config import save_config
    from missher_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

    env_path = Path(missher_home) / ".env"
