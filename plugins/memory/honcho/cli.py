"""``missher honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
from missher_cli.config import cfg_get
from utils import read_json_or_empty

RULE = "─" * 40
REASONING_LEVELS = ("minimal", "low", "medium", "high", "max")
_RETRY_HINT = "  Re-run 'missher honcho setup' to retry, or choose an API key instead.\n"
...
def _local_config_path() -> Path:
    ...
    return get_missher_home() / "honcho.json"
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "missher" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'missher honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'missher honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `missher update`; no output, no exceptions."""
...
def cmd_enable(args) -> None:
    ...
    if not _resolve_api_key(cfg, block, env=False):
        ...
        setup = "missher honcho setup" + (f" --target-profile {profile}" if profile != "default" else "")
...
def _resolve_effective_identity_mapping(cfg: dict, missher_host: dict) -> tuple[bool, dict, str, bool, bool]:
    ...
    pin_sources = (missher_host.get("pinUserPeer"), missher_host.get("pinPeerName"),
                   cfg.get("pinUserPeer"), cfg.get("pinPeerName"))
    pin = bool(next((v for v in pin_sources if v is not None), False))

    def _inherit(key):
        if key in missher_host:
            return missher_host.get(key), False
        val = cfg.get(key)
...
def _apply_runtime_prefix(missher_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        missher_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(missher_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(missher_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {missher_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {missher_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(missher_host, current_pin, current_aliases, current_prefix,
    ...
    _scrub_identity_mapping(missher_host)
    missher_host["pinUserPeer"] = pin
    if pin:
        return
    ...
    while entry := _prompt("    alias", default="").strip():
        ...
        if rid and peer:
            aliases[rid] = peer
    if aliases:
        missher_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(missher_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, missher_host: dict, current_peer: str, new_host: bool) -> None:
    ...
    current_pin, current_aliases, current_prefix, aliases_from_root, prefix_from_root = (
        _resolve_effective_identity_mapping(cfg, missher_host))
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Each gateway account (a Telegram user, a Discord user, ...)\n"
             ...
             "Running the Missher gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
            ("\n  No gateway platforms connected — nothing to map.", "Configure anyway? (y/N)"))
        print(notice)
        if not _yes(_prompt(question, default="n")):
            return

    peer_target = missher_host.get("peerName") or current_peer or "user"
    ai_peer_label = missher_host.get("aiPeer") or cfg.get("aiPeer") or "missher"
    ...
    print("\n  This step covers the HUMAN mapping only. Each account using the\n"
          ...
          "  Missher profile brings its own AI peer to the gateway.\n"
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(missher_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(missher_host)  # each shape starts from a clean slate
        missher_host["pinUserPeer"] = shape == "single"
        if shape == "single":
    ...
    _echo_identity_mapping(missher_host)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if missher_cli internals move."""
    try:
        from missher_cli.auth import _can_open_graphical_browser, _is_remote_session
        return _is_remote_session(), _can_open_graphical_browser()
    except Exception:
        return False, True


def _apply_grant_to_host(cfg: dict, missher_host: dict, cred) -> None:
    ...
    missher_host["apiKey"] = cred.access_token
    missher_host["oauth"] = cred.oauth_block()
    ...
    if cred.consent_peer_name:  # default the peer prompt to the consent name
        missher_host["peerName"] = cred.consent_peer_name
    print("  Authorized — token saved. Let's finish configuring.\n")


def _setup_local_auth(cfg: dict, missher_host: dict) -> None:
    ...
    if new_url := _prompt("Base URL", default=cfg.get("baseUrl") or "http://localhost:8000"):
        cfg["baseUrl"] = new_url
    current_host_key = missher_host.get("apiKey", "")
    ...
    if new_local_key:
        missher_host["apiKey"] = new_local_key
...
def _setup_device_login(cfg: dict, missher_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        ...
        print(" approved")
        _apply_grant_to_host(cfg, missher_host, cred)
        return True
    return False


def _setup_browser_login(cfg: dict, missher_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="missher-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, missher_host: dict, write_path: Path) -> bool:
    ...
    existing_oauth = OAuthCredential.from_host_block(missher_host)
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(cfg, missher_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(cfg, missher_host, write_path)
    # A leftover grant on the host block would shadow the pasted key.
    stale_grant = existing_oauth is not None or is_oauth_access_token(missher_host.get("apiKey"))
    current = ("" if stale_grant else missher_host.get("apiKey", "")) or cfg.get("apiKey", "")
    ...
    if not key:
        print("\n  No API key configured. Get yours at https://app.honcho.dev\n"
              "  Run 'missher honcho setup' again once you have a key.\n")
        return False
    missher_host.pop("oauth", None)
    missher_host["apiKey"] = key
    return True
...
def _choice_step(missher_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        missher_host[key] = new
    elif fallback is not None:
        missher_host[key] = fallback


def _setup_tuning(cfg: dict, missher_host: dict) -> None:
    ...
    _choice_step(missher_host, "observationMode", _pref(missher_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(missher_host, cfg, "writeFrequency", "async")))
    missher_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")

    _menu("Recall mode", *(f"{m:<7} -- {desc}" for m, desc in _MODES.items()))
    raw_recall = _pref(missher_host, cfg, "recallMode", "hybrid")
    _choice_step(missher_host, "recallMode", raw_recall if raw_recall in _MODES else "hybrid", "Recall mode", _MODES)

    missher_host["recallSync"] = _yes(_prompt(
        "Wait for current-query recall (bounded by request timeout, default 5s)? (y/N)",
        default="y" if missher_host.get("recallSync", cfg.get("recallSync", False)) else "n"))

    current_ctx_tokens = _pref(missher_host, cfg, "contextTokens")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(missher_host, cfg, "dialecticCadence") or "2"))
    if (val := _first_parsed([new_dialectic], int, None)) is None:
        missher_host["dialecticCadence"] = 2
    elif val >= 1:
        missher_host["dialecticCadence"] = val
    ...
    _choice_step(missher_host, "dialecticReasoningLevel", _pref(missher_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(missher_host, "sessionStrategy", _pref(missher_host, cfg, "sessionStrategy", "per-session"),
...
def _setup_wizard(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives Missher persistent cross-session memory.\n  Config: {write_path}")
    ...
    missher_host = cfg.setdefault("hosts", {}).setdefault(_host_key(), {})
    _migrate_pin_key(cfg)  # canonicalize legacy pinPeerName before detection/writes
    _migrate_pin_key(missher_host)
    # Taken before the prompts populate the block: an existing install must not default to pinning every account.
    new_host = not any(k in missher_host or k in cfg for k in (*_IDENTITY_MAPPING_KEYS, "peerName", "workspace", "enabled"))
    ...
    if is_local:
        _setup_local_auth(cfg, missher_host)
    elif not _setup_cloud_auth(cfg, missher_host, write_path):
        return

    # --- 3. Identity ---
    current_peer = missher_host.get("peerName") or cfg.get("peerName", "")
    for key, label, default in (
        ("peerName", "Your name (user peer)", current_peer or os.getenv("USER", "user")),
        ("aiPeer", "AI peer name", _pref(missher_host, cfg, "aiPeer", "missher")),
        ("workspace", "Workspace ID", _pref(missher_host, cfg, "workspace", "missher")),
    ):
        if new := _prompt(label, default=default):
            missher_host[key] = new

    _setup_identity_mapping(cfg, missher_host, current_peer, new_host)
    print("\n  For a gateway with many users and agents, run\n"
          "  'missher honcho peers map' to map accounts interactively.")

    _setup_tuning(cfg, missher_host)
    missher_host["enabled"] = True
    missher_host.setdefault("saveMessages", True)
    _write_config(cfg)
    ...
    try:  # auto-enable Honcho as memory provider in config.yaml
        ...
        missher_config = load_config()
        missher_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(missher_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}\n  Run: missher config set memory.provider honcho")
  ...
  Other commands:
    missher honcho status     -- show full config
    missher honcho mode       -- change recall/observation mode
    missher honcho tokens     -- tune context and dialectic budgets
    missher honcho peer       -- update peer names
    missher honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active Missher profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("missher.work") readable.
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: missher honcho setup\n")
        return
...
def _state_db_path() -> Path:
    ...
    if _profile_override and _profile_override not in {"default", "custom"}:
        try:
            from missher_cli.profiles import get_profile_dir
            return get_profile_dir(_profile_override) / "state.db"
        except Exception:
            pass
    return get_missher_home() / "state.db"
...
def _classify_workspace_peers(
    ...
    missher_hosts = {hostk for _, hostk, _ in profile_rows}
    ...
    # Host blocks that are not Missher profiles: other apps sharing the config.
    for hostk, block in (cfg.get("hosts") or {}).items():
        if hostk in missher_hosts or not isinstance(block, dict):
            continue
...
def cmd_peers_map(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    missher_host = _host_block(cfg, host)
    pin, aliases, prefix, aliases_from_root, _ = _resolve_effective_identity_mapping(cfg, missher_host)
    peer_name = missher_host.get("peerName") or cfg.get("peerName") or ""

    if pin:
        print("\n  pinUserPeer is on: every gateway account resolves to peer")
        print(f"  '{peer_name or '(peerName not set)'}' and aliases have no effect.")
        print("  Turn the pin off with 'missher honcho setup' to use per-account peers.")
    ...
    workspace = (
        ...
        or missher_host.get("workspace") or cfg.get("workspace") or host
    )
...
def cmd_peer(args) -> None:
    ...
    def show(missher, cfg):
        print(f"""
Honcho peers
{RULE}
  User peer:   {_pref(missher, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(missher, cfg, 'aiPeer') or _host_key()}
    Missher' identity in Honcho. Seed with 'missher honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(missher, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(missher, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: missher honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(missher, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(missher, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(missher, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Missher asks Honcho's AI peer a question
  ...
  Set with: missher honcho tokens [--context N] [--dialectic N]
""")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'missher honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'missher honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    missher honcho identity --show        — show both peer representations
    missher honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Missher + Honcho."""
    ...
    print("\nHoncho migration: OpenClaw native memory → Missher\n" + "─" * 50)
    print("""
...
""")
    if has_key:
        ...
        print("""  Honcho is a cloud memory service that gives Missher persistent memory
  across sessions. You need an API key to use it.

  1. Get your API key at https://app.honcho.dev
  2. Run:  missher honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'missher honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'missher honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  missher honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'missher'
  with Honcho configured and no prior session history.
  (Missher calls migrate_memory_files() on first session init.)

  If you want to migrate them now without starting a session:""")
        print("    missher honcho migrate  — this step handles it interactively\n" * len(user_files), end="")
        if has_key:
            _offer("  Upload user memory files to Honcho now?", _migrate_upload, user_files)
        else:
            print("  Run 'missher honcho setup' first, then re-run this step.")
    else:
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Missher, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'missher honcho setup' first, then seed manually:")
            print("\n".join(f"    missher honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  missher honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Missher:   per-session by default — each run gets its own session
              Map a custom name:  missher honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. missher honcho setup              — configure API key (required)\n"
              "  2. missher honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. missher honcho status             — verify Honcho connection
  2. missher                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. missher honcho identity --show    — verify AI peer representation
  4. missher honcho tokens             — tune context and dialectic budgets
  5. missher honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to missher memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Missher Honcho", cmd_migrate, ()),
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":  # honcho setup goes through the unified memory-provider path
        print("\n  Honcho is configured via the memory provider system.\n  Running 'missher memory setup'...\n")
        from missher_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``missher honcho`` argparse subcommand tree on the ``missher honcho`` parser."""
