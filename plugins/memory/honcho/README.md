...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/missher>
...
```bash
missher memory setup honcho   # configure Honcho directly (works on a fresh install)
missher memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
missher config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.missher/.env
```

> `missher honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `missher memory setup honcho`.
...
Set `"recallSync": true` in `$MISSHER_HOME/honcho.json` (at the root or under
`hosts.missher`), or enable **Current-query recall** in the memory settings or
`missher honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `missher model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MISSHER_HOME/honcho.json` | Profile-local (isolated Missher instances) |
| 2 | `~/.missher/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Missher profile: `missher` (default) or `missher_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `missher honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Missher session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`missher-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`missher`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-missher-agent`.
...
- **`per-session`** — Missher session ID (timestamp + hex). Every `missher` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Missher profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`missher`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Missher profile: `missher` (default) or `missher_<profile>` (e.g. `missher -p coder` -> host key `missher_coder`). Older `missher.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `MISSHER_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `missher-agent`) |
...
|---------|-------------|
| `missher memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `missher honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `missher memory setup`) |
| `missher honcho status` | Show resolved config for active profile |
| `missher honcho enable` / `disable` | Toggle Honcho for active profile |
| `missher honcho mode <mode>` | Change recall or observation mode |
| `missher honcho peer --user <name>` | Update user peer name |
| `missher honcho peer --ai <name>` | Update AI peer name |
| `missher honcho tokens --context <N>` | Set context token budget |
| `missher honcho tokens --dialectic <N>` | Set dialectic max chars |
| `missher honcho map <name>` | Map current directory to a session name |
| `missher honcho sync` | Create host blocks for all Missher profiles |
