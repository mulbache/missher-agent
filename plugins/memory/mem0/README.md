...
```bash
missher memory setup    # select "mem0"
```
...
Behavioral settings live in `$MISSHER_HOME/mem0.json` (set them via `missher memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.missher/.env`.
...
| `user_id` | `missher-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Missher session and call `mem0_search` — it connects to your server.
...
```bash
missher memory setup mem0 --mode oss \
...
```bash
missher memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$MISSHER_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.missher/mem0_qdrant"}}
  }
}
```
...
```bash
missher memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$MISSHER_HOME/mem0.json`).
