...
from missher_constants import get_missher_home  # noqa: F401 — patched by tests
...
_PGVECTOR_CONTAINER, _PGVECTOR_IMAGE, _PGVECTOR_PASSWORD = "missher-pgvector", "pgvector/pgvector:pg17", "missher"
...
def _prompt_api_key(label: str, env_var: str, missher_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    if not existing:
        from agent.secret_scope import load_env_file

        existing = load_env_file(Path(missher_home) / ".env").get(env_var, "")
...
def _persist_provider_config(missher_home: str, config: dict, provider_config: dict, env_writes: dict[str, str], label: str, key_line: str, server: str | None = None) -> None:
    ...
    Mem0MemoryProvider().save_config(provider_config, missher_home)
    if env_writes:
        _write_env(Path(missher_home) / ".env", env_writes)
    if server:
...
def _setup_platform(missher_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    provider_config = read_json_or_empty(Path(missher_home) / "mem0.json")
    print("\n  Configuring mem0:\n")
    env_writes = _api_key_writes(flags, "Mem0 Platform API key", url="https://app.mem0.ai")
    for key, desc, default in (("user_id", "User identifier", "missher-user"), ("agent_id", "Agent identifier", "missher")):
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(f"\n  ⚠ MEM0_HOST is set in your environment ({os.environ['MEM0_HOST']}). It overrides platform mode — remove it from ~/.missher/.env (or unset it) or Missher will keep routing to the self-hosted server.")
    _persist_provider_config(missher_home, config, provider_config, env_writes, "mem0", "API keys saved to .env")
...
def _setup_selfhosted(missher_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt("User identifier", default=provider_config.get("user_id") or "missher-user")
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "missher")
    if flags.get("dry_run"):
    ...
    _persist_provider_config(missher_home, config, provider_config, env_writes, "mem0 (self-hosted)", "API key saved to .env", server=host)
...
def _finish_oss(missher_home: str, config: dict, oss_config: dict, env_writes: dict[str, str], user_id: str, agent_id: str, pgvector_config: dict | None = None) -> None:
    """Shared OSS tail: write secrets + mem0.json, install deps, activate, check, summarize."""
    from utils import read_json_or_empty
    if env_writes:
        _write_env(Path(missher_home) / ".env", env_writes)
    config_path = Path(missher_home) / "mem0.json"  # merge-write, plain text (platform path uses save_config's 0600 atomic write)
...
def _setup_oss(missher_home: str, config: dict, flags: dict[str, str]) -> None:
    """OSS mode — non-interactive when --mode was given, otherwise curses pickers."""
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(missher_home, config)
        return
    ...
    if flags.get("dry_run"):
        ...
        print("  [dry-run] No files written.\n")
        return
    _finish_oss(missher_home, config, oss_config, env_writes, flags.get("user_id") or os.getenv("USER", "missher-user"), "missher")
...
def _configure_model_provider(kind: str, registry: dict, missher_home: str, env_writes: dict[str, str], llm: tuple[str, dict] | None = None) -> tuple[str, dict, str, str | None]:
    ...
    if pdef["needs_key"]:
        if llm is None or pid != llm[0]:
            if key := _prompt_api_key(pdef["label"] if llm is None else f"{pdef['label']} embedder", pdef["env_var"], missher_home):
...
def _setup_oss_interactive(missher_home: str, config: dict) -> None:
    env_writes: dict[str, str] = {}
    llm_id, llm_def, llm_model, llm_url = _configure_model_provider("LLM", LLM_PROVIDERS, missher_home, env_writes)
    embedder_id, _, embedder_model, embedder_url = _configure_model_provider("Embedder", EMBEDDER_PROVIDERS, missher_home, env_writes, llm=(llm_id, llm_def))
    ...
    user_id = _input("User ID", os.getenv("USER", "missher-user"))
    agent_id = _input("Agent ID", "missher")
    flags = {
    ...
    _finish_oss(missher_home, config, oss_config, env_writes, user_id, agent_id, pgvector_config)
...
def _install_provider_deps(llm_id: str, embedder_id: str, vector_id: str) -> None:
    ...
    for dep in sorted(deps):
        ...
        try:
            # Environment-aware install: sealed hosted venvs redirect to the durable data-volume target instead of /opt/missher.
...
def post_setup(missher_home: str, config: dict) -> None:
    """Entry point for `missher memory setup`: routes on --mode (platform / selfhosted / oss), else shows a picker.
    ...
    handler(missher_home, config, flags)
