...
_DEFAULT_CONTAINER_TAG = "missher"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=missher"
...
def _load_supermemory_config(missher_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $missher_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if missher_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(missher_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, missher_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(missher_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "missher"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Missher writes into the "Missher" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "missher", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, missher_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(missher_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._missher_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `missher memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, missher_home):
        ...
        _save_supermemory_config(sanitized, missher_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from missher_constants import get_missher_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_missher_home())))}

    def post_setup(self, missher_home: str, config: dict) -> None:
        from missher_cli.config import save_config
        from missher_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, missher_home=missher_home)
        ...
        status = _probe_supermemory_connection(api_key, missher_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from missher_constants import get_missher_home
        self._missher_home = kwargs.get("missher_home") or str(get_missher_home())
        self._session_id, self._turn_count, self._session_turns = session_id, 0, []
        config = _load_supermemory_config(self._missher_home)
