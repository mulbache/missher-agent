...
- Hosted: API key from [app.supermemory.ai/integrations?connect=missher](http://app.supermemory.ai/integrations?connect=missher)
...
Before running `missher memory setup`, add the local endpoint to
...
Then run `missher memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$MISSHER_HOME/supermemory.json`
...
| `container_tag` | `missher` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `missher-{identity}` → `missher-coder`). |
...
`https://api.supermemory.ai`. Missher resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: missher`, and document writes stamp
`metadata.sm_source: missher`. This is a **functional routing key, not telemetry**:
it groups Missher-written memories into a dedicated "Missher" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Missher profile:

```json
{
  "container_tag": "missher-{identity}"
}
```

For a profile named `coder`, this resolves to `missher-coder`. The default profile resolves to `missher-default`. Without `{identity}`, all profiles share the same container.
