...
Config: $MISSHER_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Missher Agent and the User"
...
def _load_config() -> dict:
    """$MISSHER_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    (legacy, shared), else environment variables."""
    for path in (get_missher_home() / "hindsight" / "config.json", Path.home() / ".hindsight" / "config.json"):
    ...
    return {
        ...
        "banks": {"missher": {"bankId": get_secret("HINDSIGHT_BANK_ID", "") or "missher",
                             "budget": os.environ.get("HINDSIGHT_BUDGET", "mid"), "enabled": True}},
    }


def _event_timestamp() -> str:
    """Configured Missher event time with an explicit UTC offset."""
    event_time = _missher_now()
    # missher_time.now() is aware; guard a replacement clock emitting offset-less dates.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._bank_id_template = "missher", "mid", ""
    ...
    def save_config(self, values, missher_home):
        """Merge *values* into $MISSHER_HOME/hindsight/config.json."""
        from utils import atomic_json_write
        config_path = Path(missher_home) / "hindsight" / "config.json"
    ...
    def post_setup(self, missher_home: str, config: dict) -> None:
        ...
        run_setup(self, missher_home, config)
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "missher"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: missher-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Missher Agent and the User"},
    ...
    def _new_embedded_client(self):
        ...
        logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                     cfg.get("profile", "missher"), llm_provider)
        ...
        kwargs = dict(profile=cfg.get("profile", "missher"), llm_provider=llm_provider,
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        banks = cfg_get(cfg, "banks", "missher", default={})
        self._bank_id_template = cfg.get("bank_id_template", "") or ""
        self._bank_id = _resolve_bank_id_template(
            self._bank_id_template,
            fallback=cfg.get("bank_id") or banks.get("bankId", "missher"),
    ...
    def _start_embedded_daemon(self) -> None:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            msg = ("Hindsight local_embedded mode cannot run as root "
                   ...
                   "memory daemon. Run Missher as a non-root user, or switch "
                   "to cloud / local_external mode via 'missher memory setup'.")
            logger.warning(msg)
            # Also print: otherwise the user would only see Missher get sluggish.
            with contextlib.suppress(Exception):
                # Surface to the terminal too — a daemon that never starts would otherwise fail silently and
                # the user would only see Missher get sluggish. (issue #13125)
    ...
    def _daemon_start_worker(self) -> None:
        import traceback
        log_path = get_missher_home() / "logs" / "hindsight-embed.log"
        ...
        try:
            ...
            profile = self._config.get("profile", "missher")
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # See NousResearch/missher-agent#5820.
    ...
    def on_session_switch(self, new_session_id: str, *, parent_session_id: str = "",
        ...
        document. See missher-agent#6672.
