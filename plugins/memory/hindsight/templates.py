...
catalog, filter to the ``missher`` integration, apply a manifest via
...
def fetch_missher_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``missher`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "missher" in (e.get("integrations") or [])]
...
def run_template_step(*, api_url: str, bank_id: str, api_key: str | None, select, cancelled, log=print) -> str | None:
    ...
    try:
        entries = fetch_missher_templates()
