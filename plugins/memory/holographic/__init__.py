"""missher-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $MISSHER_HOME/config.yaml under plugins.missher-memory-store: db_path ($MISSHER_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from missher_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "missher-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, missher_home):
        """Write config to config.yaml under plugins.missher-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *missher_home* is the active profile already.
        from missher_cli.config import save_config
        save_config({"plugins": {"missher-memory-store": dict(values)}}, merge_existing=True)

    def get_config_schema(self):
        from missher_constants import display_missher_home
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_missher_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _missher_home = str(get_missher_home())
        db_path = self._config.get("db_path", _missher_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $MISSHER_HOME so paths resolve to the active profile
            db_path = db_path.replace("$MISSHER_HOME", _missher_home).replace("${MISSHER_HOME}", _missher_home)
