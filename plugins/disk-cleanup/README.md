...
Auto-tracks and cleans up ephemeral files created during Missher Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MISSHER_HOME` and `/tmp/missher-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MISSHER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `MISSHER_HOME` or `/tmp/missher-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MISSHER_HOME/disk-cleanup/` is itself excluded
- `$MISSHER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
