...
models from :func:`missher_cli.models._fetch_deepinfra_models_by_tag`; no ids hardcoded).
...
class DeepInfraImageGenProvider(StaticImageGenProvider):
    ...
    def generate(self, prompt: str, aspect_ratio: str = DEFAULT_ASPECT_RATIO, **kwargs: Any) -> Dict[str, Any]:
        ...
        if not api_key:
            return fail(
                "DEEPINFRA_API_KEY not set. Run `missher tools` → Image "
                "Generation → DeepInfra to configure, or `missher setup` "
                "to add the key.",
                "auth_required")
        ...
        from missher_cli.models import deepinfra_base_url
