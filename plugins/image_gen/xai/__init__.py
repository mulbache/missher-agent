...
from tools.xai_http import (
    build_xai_storage_options, missher_xai_user_agent, maybe_mark_xai_storage_notice_seen,
...
def _fetch_live_models(creds: Optional[Dict[str, Any]] = None) -> Dict[str, Dict[str, Any]]:
    ...
    response = requests.get(
        ...
        headers={"Authorization": f"Bearer {api_key}", "User-Agent": missher_xai_user_agent()},
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_factory(provider_name, aspect_ratio)(
                "No xAI credentials found. Configure xAI OAuth in `missher model` or set XAI_API_KEY.",
                "missing_api_key")
        ...
        headers = {
            ...
            "User-Agent": missher_xai_user_agent(),
        }
        base_url = _base_url(creds)
        storage_options = build_xai_storage_options(
            "image_gen", filename_prefix="missher-xai-image", extension="png")
