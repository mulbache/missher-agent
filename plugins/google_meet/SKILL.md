...
missher meet install                 # pip deps + Chromium (transcribe only)
missher meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
missher meet auth                    # optional; skips guest-lobby wait
missher meet setup                   # preflight checks
```

`missher meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or MISSHER_MEET_REALTIME_KEY in ~/.missher/.env
```
...
pip install playwright websockets && python -m playwright install chromium
missher plugins enable google_meet
missher meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
missher meet node approve my-mac ws://<mac-ip>:18789 <token>
missher meet node ping my-mac                   # confirm reachable
```

Run `missher meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Missher agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `missher meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `MISSHER_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `missher_meet_src`). |
...
Local:
```
$MISSHER_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$MISSHER_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `missher meet node approve`.
