...
Let the missher agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where missher runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `missher meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `missher meet` CLI |
...
| `cli.py` | `missher meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$MISSHER_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `missher meet node {run,list,approve,remove,status,ping}` |
...
missher meet install                                      # pip + Chromium
missher meet setup                                        # preflight
missher meet auth                                         # optional
missher meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
missher meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.missher/.env
missher meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
missher meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
missher meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.missher/.env
missher meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, missher will **not** switch your system audio input automatically — the
...
missher plugins enable google_meet
missher meet node run --display-name my-mac --host 0.0.0.0 --port 18789
