...
``dashboard.oauth.self_hosted.{issuer,client_id,scopes,client_secret}`` or ``MISSHER_DASHBOARD_OIDC_*``.
"""
...
from missher_cli.dashboard_auth import LoginStart, ProviderError, Session
...
def _settings() -> dict:
    ...
    issuer = setting("MISSHER_DASHBOARD_OIDC_ISSUER", "issuer")
    client_id = setting("MISSHER_DASHBOARD_OIDC_CLIENT_ID", "client_id")
    if not issuer or not client_id:
        raise SkipRegistration(
            "Self-hosted OIDC dashboard auth is not configured. Set both an issuer and "
            "a client_id — either as env vars (MISSHER_DASHBOARD_OIDC_ISSUER + "
            "MISSHER_DASHBOARD_OIDC_CLIENT_ID) or under "
    ...
    return {
        ...
        "scopes": setting("MISSHER_DASHBOARD_OIDC_SCOPES", "scopes") or _DEFAULT_SCOPES,
        # Credential: canonical home is the env var / ~/.missher/.env. Empty ⇒ public client.
        "client_secret": setting("MISSHER_DASHBOARD_OIDC_CLIENT_SECRET", "client_secret")}
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('missher_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('missher_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('missher_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('missher_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
