...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, MISSHER_HOME).
        ...
        try:
            ...
            missher_home = Path(get_missher_home()).resolve()
        except Exception:
            missher_home = Path.home().joinpath(".missher").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), missher_home)):
...
def is_connected(config) -> bool:
    """Surface in ``missher status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``missher status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``missher setup line`` wizard (writes ``~/.missher/.env``); CLI helpers are lazy-imported."""
    from missher_cli.config import get_env_value, save_env_value
    from missher_cli.cli_output import print_header, print_info, prompt
