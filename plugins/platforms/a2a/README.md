# A2A — Agent-to-Agent protocol for Missher
...
peer (another Missher, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
missher gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Missher serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.missher/a2a_audit.jsonl`.
- Conversations persist to `~/.missher/a2a_conversations/` — they survive context
