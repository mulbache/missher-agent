...
Small Node helper that bridges Missher Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Missher is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Missher-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Missher can receive inbound messages
...
The Missher plugin's `missher photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Missher is Python, so replies go through this sidecar
