// Missher Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Missher): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Missher -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Missher-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `missher photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Missher can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "missher-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Missher'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Missher allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Missher, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-missher-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Missher supplied them so a known-good
