...
// `message.content.text` before Missher can see it. We rewrite the two inbound
...
const MARKER = "Missher patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Missher to patch; keep the legacy v8 path below for older pinned installs.
    if (
