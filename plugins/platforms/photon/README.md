...
This plugin connects Missher Agent to iMessage (and other Spectrum
...
TypeScript-only, Missher runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Missher channel,
...
Run `missher photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.missher/.env` (the same place every other
...
Management metadata lives in `~/.missher/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `missher photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Missher
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Missher coalesces
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage read
  after forwarding it to Missher, so the sender sees `Read` without waiting for
  a model/tool turn. Inbound receipts for Missher-sent messages are consumed as
  presence telemetry and never create an agent turn. Set
  `PHOTON_READ_RECEIPTS=false` to keep messages at `Delivered`.
- **Native polls are supported.** Missher posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `missher send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<missher-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `missher photon status`, a DM and a group roundtrip,
