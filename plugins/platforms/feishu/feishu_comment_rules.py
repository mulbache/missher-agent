...
(enabled/policy/allow_from) falling back independently. Config ~/.missher/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.missher/feishu_comment_pairing.json."""
...
RULES_FILE = get_missher_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_missher_home() / "feishu_comment_pairing.json"
...
def _rules_file() -> Path:
    ...
    it, else live profile-scoped MISSHER_HOME — the multiplexed gateway serves every profile from
    one process, so the import-time constant would apply the launch profile's rules everywhere."""
    return RULES_FILE if RULES_FILE != _RULES_FILE_AT_IMPORT else get_missher_home() / "feishu_comment_rules.json"


def _pairing_file() -> Path:
    return PAIRING_FILE if PAIRING_FILE != _PAIRING_FILE_AT_IMPORT else get_missher_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("missher_cli.env_loader", fromlist=["load_missher_dotenv"]).load_missher_dotenv()
    except Exception:
        pass
