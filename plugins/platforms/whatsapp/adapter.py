...
from missher_constants import (find_node_executable, get_missher_dir, with_missher_node_path)
...
def _is_allowed_bridge_path(url: str) -> bool:
    """Absolute bridge path resolves (symlinks included) inside a Missher cache dir — a rogue bridge could hand back /etc/passwd."""
    try:
...
def check_whatsapp_requirements() -> bool:
    """Node.js (Missher-managed first, so a bad system Node on PATH can't break Windows) is available."""
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path = Path(extra.get("session_path", get_missher_dir("platforms/whatsapp/session", "whatsapp/session")))
    ...
    def _ensure_bridge_deps(self, bridge_dir: Path) -> bool:
        ...
        _dep_stamp = bridge_dir / "node_modules" / ".missher-pkg-hash"  # holds the package.json hash of the last install
        ...
        # Missher-managed portable Node's npm.cmd first (Windows), then PATH.
        ...
        try:  # Default 300s accommodates slow systems like an Unraid NAS.
            install_result = subprocess.run([_npm_bin, "install", "--silent"], cwd=str(bridge_dir), timeout=env_int("WHATSAPP_NPM_INSTALL_TIMEOUT", 300),
                                            env=with_missher_node_path(), **_RUN_TEXT)
        ...
        self._set_fatal_error("whatsapp_npm_install_failed", f"WhatsApp bridge npm install failed{detail}. Run `cd {bridge_dir} && {_npm_bin} install` "
                              "manually, then restart `missher gateway`.", retryable=False)
        return False
    ...
    def _bridge_env(self) -> dict:
        ...
        # with_missher_node_path() copies os.environ when called with no arg: under a multiplexed secondary
        ...
        bridge_env = with_missher_node_path()
        ...
        # Without these the bridge hardcodes ~/.missher/{image,audio,document}_cache (wrong under MISSHER_HOME/profiles/cache layout).
        img_dir, audio_dir, _video_dir, doc_dir = _cache_dirs()
        bridge_env.update(MISSHER_IMAGE_CACHE_DIR=str(img_dir), MISSHER_AUDIO_CACHE_DIR=str(audio_dir), MISSHER_DOCUMENT_CACHE_DIR=str(doc_dir))
        return bridge_env
    ...
    async def _wait_for_bridge(self) -> bool:
        ...
        if data.get("status") != "connected":
            ...
            if connected is None:
                ...
                print(f"[{self.name}]   If session expired, re-pair: missher whatsapp")
        return True
    ...
    def _preflight(self) -> bool:
        ...
        checks = (
            (check_whatsapp_requirements, ("[%s] Node.js not found. WhatsApp requires Node.js.", self.name),
             "whatsapp_node_missing", "Node.js is not installed — install Node.js and re-run `missher gateway`."),
            ...
            (creds_path.exists, ("[%s] WhatsApp is enabled but not paired (no creds.json at %s). Pair from the dashboard or run "
                                 "`missher whatsapp`; remove WHATSAPP_ENABLED from your .env to disable.", self.name, creds_path),
             "whatsapp_not_paired", "WhatsApp enabled but not paired — pair from the dashboard or run `missher whatsapp`."),
        )
...
# ── Plugin glue: register(ctx) plus the hooks for gateway/run.py, gateway/config.py, missher_cli/gateway.py, send_message_tool.py.
...
def interactive_setup() -> None:
    ...
    from missher_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success
...
def _is_connected(config) -> bool:
    ...
    if config is not None and getattr(config, "enabled", False) and (getattr(config, "extra", {}) or {}):
        return True
    # Via missher_cli.gateway.get_env_value (not os.getenv) so setup-status callers that patch it observe the same value.
