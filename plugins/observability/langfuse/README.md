...
This plugin ships bundled with Missher but is **opt-in** — it only loads when
...
missher tools  # → Langfuse Observability
...
Set these in `~/.missher/.env` (or via `missher tools`):
...
missher chat -q "hello"              # then check Langfuse for a "Missher turn" trace
```

Generation observations include the Missher system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `MISSHER_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
MISSHER_LANGFUSE_ENV=production       # environment tag
MISSHER_LANGFUSE_RELEASE=v1.0.0       # release tag
MISSHER_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
MISSHER_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
MISSHER_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
MISSHER_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`MISSHER_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
missher plugins disable observability/langfuse
```
