"""Per-plugin persistent storage: ``<missher home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<missher home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `missher plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<missher home>/plugin-data/<name>/``; resolves ``get_missher_home()`` on
    ...
    root = get_missher_home() / "plugin-data" / _validate_name(name)
