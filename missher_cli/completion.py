"""Shell completion script generation for missher CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_missher_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Missher Agent bash completion
# Add to ~/.bashrc:
#   eval "$(missher completion bash)"

_missher_profiles() {{
    local profiles_dir="$HOME/.missher/profiles"
...
_missher_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_missher_profiles)" -- "$cur"))
        return
    fi
...
complete -F _missher_completion missher
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef missher
# Missher Agent zsh completion
# Add to ~/.zshrc:
...
_missher_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.missher/profiles" ]]; then
        profiles+=($HOME/.missher/profiles/*(N/:t))
    fi
...
_missher() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_missher_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'missher command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Missher Agent fish completion",
        "# Add to your config:",
        "#   missher completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __missher_profiles",
        "    echo default",
        "    if test -d $HOME/.missher/profiles",
        "        for d in $HOME/.missher/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c missher -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c missher -f -s p -l profile"
        " -d 'Profile name' -xa '(__missher_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c missher -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__missher_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
