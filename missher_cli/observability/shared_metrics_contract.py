"""Bounded product contract for the first Missher shared-metrics slice."""
...
SCHEMA_KEY = "missher.metrics.schema_version"
SCHEMA_VERSION = "missher.metrics.event.v2"
MODEL_CALL_SCOPE = "missher.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "missher.task_run"
TOOL_CALL_SCOPE = "missher.tool_call"
CLIENT_ACTIVE_MARK = "missher.client.active"
TOOL_APPROVAL_MARK = "missher.tool_approval"
SKILL_LIFECYCLE_MARK = "missher.skill.lifecycle"
SKILL_LOAD_MARK = "missher.skill.load"
SUBSCRIBER_NAME = "missher.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "missher.client.active"
LEGACY_MODEL_CALL_METRIC = "missher.model_call.count"
MODEL_ROUTE_METRIC = "missher.model_route.count"
TASK_STARTED_METRIC = "missher.task_run.started"
TASK_FINISHED_METRIC = "missher.task_run.finished"
TOOL_CALL_METRIC = "missher.tool_call.count"
TOOL_APPROVAL_METRIC = "missher.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "missher.skill.lifecycle.count"
SKILL_LOAD_METRIC = "missher.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "missher_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Missher installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    missher_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(missher_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "missher_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("missher_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Missher logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "missher.call_role"
    )
    call_role = (metadata or {}).get("missher.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from missher_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Missher terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "missher-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Missher registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Missher tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Missher."""
