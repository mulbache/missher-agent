...
from missher_cli.config import redact_key
from missher_cli.web_deps import late
from missher_cli.web_server_files import _path_is_under
from missher_cli.web_server_gateway import _restart_gateway_after
from missher_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from missher_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
from missher_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("missher_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "missher_cli.web_server_memory")
...
save_config = late("save_config", "missher_cli.config")


def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str) -> dict:
    """Spawn a dashboard-profile ``missher <argv>`` action; spawn failure -> 500."""
...
# --- Webhooks: same JSON store as the CLI (missher_cli.webhook); the adapter
...
async def list_webhooks():
    import missher_cli.webhook as wh
...
# --- Gateway lifecycle: spawn the real `missher gateway <verb>` so behaviour
...
async def start_gateway(profile: Optional[str] = None):
    from missher_cli.web_server_gateway import multiplexed_profile_refusal
    # The spawned `missher -p X gateway start` would refuse with exit 78 into an action log nobody reads;
    ...
    with http_failure("Failed to spawn gateway start", 500, "Failed to start gateway"):
        proc = _spawn_missher_action(_gateway_subcommand(profile, "start"), "gateway-start")
...
async def stop_gateway(profile: Optional[str] = None):
    ...
    with http_failure("Failed to spawn gateway stop", 500, "Failed to stop gateway"):
        proc = _spawn_missher_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    ...
    def _run():
        try:
            ...
            pool.add_entry(PooledCredential(
                ...
                # every added account into the latest login: a second ``missher auth add openai-codex``
            ...
            # (mirrors `missher auth add`).
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from missher_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    the same RemovalStep registry as ``missher auth remove``: each source cleans
...
def _dashboard_backup_dir() -> Path:
    return get_missher_home() / "backups"
...
async def run_backup(body: BackupRequest):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir() / f"missher-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
...
async def list_hooks():
    ...
    def _run():
        from missher_cli.config import load_config as _load_config
        from agent import shell_hooks

        valid_events = []
        with contextlib.suppress(Exception):
            from missher_cli.plugins import VALID_HOOKS
...
async def create_hook(body: HookCreate):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from missher_cli.plugins import VALID_HOOKS as valid_hooks
...
async def list_checkpoints():
    ...
    cp_dir = get_missher_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
