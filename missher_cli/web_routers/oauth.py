...
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "missher_cli.web_server_oauth")
...
_nous_plain_poller = late("_nous_plain_poller", "missher_cli.web_server_oauth")
_nous_promotion_poller = late("_nous_promotion_poller", "missher_cli.web_server_oauth")
_minimax_poller = late("_minimax_poller", "missher_cli.web_server_oauth")
_xai_device_poller = late("_xai_device_poller", "missher_cli.web_server_oauth")
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Missher "
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from missher_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from missher_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from missher_cli.auth import _save_codex_tokens
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        if status_fn is not None:
            return status_fn()
        from missher_cli import auth as hauth
...
async def _start_nous_device_code(profile: Optional[str]) -> Dict[str, Any]:
    ...
    from missher_cli.auth import PROVIDER_REGISTRY, _request_device_code
    from missher_cli.web_server_profiles import _config_profile_scope, _profile_scope
    pconfig = PROVIDER_REGISTRY["nous"]
    portal_base_url = (
        os.getenv("MISSHER_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL") or pconfig.portal_base_url
    ).rstrip("/")
...
async def _start_xai_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from missher_cli.auth import _xai_oauth_request_device_code
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Missher still
    # OWNS its credential (the PKCE file ~/.missher/.anthropic_oauth.json and its
    # credential-pool entry, written by `missher auth add anthropic`), so it is
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Missher — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``missher model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        from missher_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"missher auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
def _clear_anthropic_auth() -> bool:
    """Clear only the Missher-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_missher_oauth_file
        oauth_file = _get_missher_oauth_file()
...
async def disconnect_oauth_provider(provider_id: str, request: Request, profile: Optional[str] = None):
    ...
    def _run():
        ...
        try:
            from missher_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
