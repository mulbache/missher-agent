"""Gateway restart/drain, Missher update and background-action status dashboard routes.

Extracted from ``missher_cli.web_server``; helpers/state that tests monkeypatch on
...
from missher_cli.config import format_docker_update_message, recommended_update_command_for_method
from missher_cli.web_deps import LateState, late
from missher_cli.web_server_gateway import _ACTION_LOG_FILES
from missher_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "missher_cli.config")
get_missher_home = late("get_missher_home", "missher_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "missher_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "missher_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "missher_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "missher_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import missher_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== missher-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Missher updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== missher update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``missher gateway restart`` in the background."""
...
async def gateway_migrate_plan():
    ...
    from missher_cli.gateway_migrate import build_migration_plan
...
async def gateway_migrate():
    """Run ``missher gateway migrate --multiplex --yes`` detached; the CLI re-runs the preflight and
    refuses (exit 1 into the action log) when blocked, so the UI should gate on the plan first."""
    from missher_cli.web_server_gateway import _spawn_missher_action
    with http_failure("Failed to spawn gateway migrate", 500, "Failed to start gateway migration"):
        proc = _spawn_missher_action(["gateway", "migrate", "--multiplex", "--yes"], "gateway-migrate")
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("missher-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "missher-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/missher/update")
async def update_missher():
    """Kick off ``missher update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("missher-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "missher-update", "already_running": True}
        action_id = _ACTION_IDS.get("missher-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn missher update", 500, "Failed to start update"):
        proc = _spawn_missher_action(["update"], "missher-update", env_overrides={"MISSHER_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "missher-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Missher is managed by Termux APT; run `pkg upgrade missher-agent`.",
}


@router.get("/api/missher/update/check")
async def check_missher_update(force: bool = False):
    """Report whether a Missher update is available, without applying it.
    ...
    try:
        from missher_cli.banner import check_for_updates, upstream_commits_behind

        if force:
            with contextlib.suppress(OSError):
                (get_missher_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "missher-update":
        ...
        if durable_update_action_id:
            marker = f"=== missher-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``missher update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``missher update`` run since #91283, including
...
@status_router.get("/api/missher/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `missher update` run recorded).")
