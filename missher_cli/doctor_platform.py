"""Host-platform checks for missher doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``missher_cli/doctor.py``, which re-exports every name so ``missher_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from missher_cli.config import is_nix_install_method, recommended_update_command_for_method
from missher_cli.doctor_report import (
    Finding, _fail_and_issue, _section, check_bool, check_fail, check_info, check_ok, check_warn, doctor_check,
    warn_on_error,
)
from missher_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from missher_cli.doctor import PROJECT_ROOT
    from missher_cli.config import detect_install_method
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Missher containers", "apt": f"run `{cmd}`"}.get(method, "run `missher update`")
    return f"({action}; fixed versions: 3.51.3+ / 3.50.7 / 3.44.6 — see https://sqlite.org/wal.html#walresetbug)"


def _missher_database_paths(missher_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Missher-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from missher_cli.backup import _QUICK_STATE_FILES
    entries = [(name, missher_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((missher_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(missher_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``missher_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from missher_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_journal_modes(missher_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from missher_cli.doctor import MISSHER_HOME
    from missher_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    try:
        databases = _missher_database_paths(missher_home if missher_home is not None else MISSHER_HOME)
    except Exception as exc:
        check_warn(f"Could not list Missher databases: {exc}")
        return
    exposed = []
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs missher_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from missher_cli import __version__ as init_version
    except Exception:
        return
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != missher_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'missher update', or set missher_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from missher_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-missher", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
    ...
    if not profiles:
        return check_info("No per-profile gateways registered yet — create one with `missher profile create <name>`")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `missher doctor --fix`, or `{pip_cmd}`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from missher_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `missher update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `missher desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.nousresearch.missher` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Missher once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Missher.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Missher.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored Missher-Setup launcher.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Missher.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from missher_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Missher "
                   ...
                   "    then enable your terminal (and Missher.app if you use Desktop), and restart them once. "
                   "With Missher' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from missher_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `missher doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from missher_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Missher already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "missher" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(missher not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    link = link_dir / "missher"
    if link.is_symlink():
        target, expected = link.resolve(), venv_bin.resolve()
        if target == expected:
            return check_ok(f"{display}/missher → correct target")
        check_warn(f"{display}/missher points to wrong target", f"(→ {target}, expected → {expected})")
        if not should_fix:
            return f.issues.append(f"Broken symlink at {display}/missher — run 'missher doctor --fix'")
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/missher exists (non-symlink)")
    else:
        check_fail(f"{display}/missher not found", "(missher command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/missher symlink — run 'missher doctor --fix'")
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/missher → {venv_bin}")
    f.fixed += 1
