"""Multi-provider authentication system for Missher Agent.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.missher/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``missher_cli.auth.<name>`` stays the public/patchable surface."""
...
from missher_cli.config import (
    get_missher_home, get_config_path, read_raw_config, require_readable_config_before_write)
from missher_constants import OPENROUTER_BASE_URL, missher_home_key, secure_parent_dir
...
from missher_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from missher_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from missher_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from missher_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from missher_cli.auth_nous import (  # noqa: F401  re-exported
...
from missher_cli.auth_minimax import (  # noqa: F401  re-exported
...
from missher_cli.auth_xai import (  # noqa: F401  re-exported
...
from missher_cli.auth_codex import (  # noqa: F401  re-exported
...
from missher_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from missher_cli.auth_openrouter import _openrouter_pkce_login  # noqa: F401  re-exported
from missher_cli.auth_qwen import (  # noqa: F401  re-exported
...
from missher_cli.auth_constants import (  # noqa: F401  re-exported
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ...
    # is a 401), so there is no secret to configure. Select via `missher model` / `/model free`.
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.missher/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from missher_cli.config import get_env_value_prefer_dotenv
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$MISSHER_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from missher_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.missher/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $MISSHER_HOME/.env (e.g. MISSHER_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``missher auth``."""
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `missher model` to re-authenticate."
...
# ── Auth Store — persistence layer for ~/.missher/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_missher_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch MISSHER_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".missher" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set MISSHER_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_missher_root()
    except Exception:
        return None
    return None if _same_path(get_missher_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".missher" / "auth.json"
        try:
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``missher auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``missher auth add`` in the profile shadows global)."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``missher auth reset``) and skip the
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Missher flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Missher.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "missher_pkce", "manual"})
...
def _explicit_env_credentials_present(normalized: str) -> bool:
    ...
    if pconfig is None:
        from missher_cli.providers import get_provider
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Missher flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Missher-scoped routing config is present.
    ...
    Missher-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Missher-initiated flow, or Missher-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``missher logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        from missher_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""
        lines = ["Config issue detected — run 'missher doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from missher_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run missher setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `missher auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `missher auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `missher auth list` showing the credential while requests go out
...
def _config_model_provider() -> Tuple[Any, Optional[str]]:
    ...
    dashboard's Ink chat parked every session on Setup Required while ``missher chat`` worked
    (#108383)."""
    try:
        from missher_cli.config import load_config
        model_cfg = (load_config() or {}).get("model")
        ...
        if base_url:
            from missher_cli.runtime_provider import _config_base_url_trustworthy_for_bare_custom
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.missher/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.missher/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'missher model' for available providers, "
                "or run 'missher doctor' to diagnose config issues.")
    ...
    raise AuthError(
        "No inference provider configured. Run 'missher model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.missher/.env.",
...
# Keyed by missher_home_key(): the resolution itself is profile-scoped (_auth_file_path reads the
# per-turn MISSHER_HOME override a multiplex gateway sets), so a single slot would hand profile A's
...
def _nous_portal_base_url(state: Dict[str, Any]) -> str:
    """MISSHER_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted operator override and wins
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        if not state:
            raise _nous_err("Missher is not logged into Nous Portal.", relogin=True)
        ...
        with _nous_shared_store_lock(timeout_seconds=lock_timeout):
            from missher_cli.anon_auth import is_guest_state, refresh_guest_state
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("missher_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``missher logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
def _provider_env_base_url(pconfig: ProviderConfig) -> str:
    if pconfig.id == "actual":
        from missher_cli.providers import normalize_provider
...
def _provider_is_keyless(provider_id: str) -> bool:
    """MissherOverlay keyless flag — the same source the provider catalog and GUI contract tests use."""
    try:
        from missher_cli.providers import MISSHER_OVERLAYS
        return bool(getattr(MISSHER_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Missher can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from missher_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: MISSHER_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / MISSHER_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``missher_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``missher doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from missher_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            info.update(
                ...
                hint=(
                    ...
                    "is skipped here. Run `missher doctor` to verify token acquisition."
                ) if installed else (
                    "azure-identity not installed. Install with: "
                    "pip install azure-identity  (or rely on Missher' "
...
def _update_config_for_provider(
    ...
    *default_model*, when given, is written as ``model.default`` in the same step so the gateway
    ...
    from missher_cli.config import clear_model_endpoint_credentials
...
def login_command(args) -> None:
    """Deprecated: use 'missher model' or 'missher setup' instead."""
    print("The 'missher login' command has been removed.\nUse 'missher auth' to manage credentials,\n"
          "'missher model' to select a provider, or 'missher setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not target:
        ...
        return
    if target == "nous":
        from missher_cli.anon_auth import FREE_TIER_NOT_SIGNED_IN, is_guest_state
    ...
    if target == "nous":
        ...
        from missher_cli.auth_nous import _clear_shared_nous_state
    ...
    if not should_reset_config:
        ...
        print("Missher will use OpenRouter for inference.")
    else:
        print("Run `missher model` or configure an API key to use Missher.")
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('missher_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('missher_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('missher_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('missher_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('missher_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NOUS_INFERENCE_INVOKE_SCOPE': ('missher_cli.auth_constants', 'NOUS_INFERENCE_INVOKE_SCOPE'),
    'NOUS_SHARED_STORE_FILENAME': ('missher_cli.auth_nous', 'NOUS_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('missher_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('missher_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('missher_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('missher_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('missher_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('missher_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('missher_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('missher_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('missher_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nous_oauth_pure': ('missher_cli.auth_nous', 'refresh_nous_oauth_pure'),
}
