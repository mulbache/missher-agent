"""Directory initialization and storage diagnostics for the active Missher home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool) -> None:
    from missher_cli.config import _secure_dir
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize Missher directory {path}: {exc}. "
            ...
            "Missher has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from missher_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize Missher home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from missher_cli.config import ConfigIssue

    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"Missher storage is unavailable: {exc}",
            "Check the reported path, link target, mount and permissions; keep config.yaml unchanged.",
        )
    return ConfigIssue("error", "Could not load config.yaml", "Run 'missher setup' to create a valid config")
