...
Pure leaf: imports nothing from ``missher_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because missher_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("missher_cli.auth.httpx.Client")
...
DEFAULT_NOUS_CLIENT_ID = "missher-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from missher_cli import __version__ as _MISSHER_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _MISSHER_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"missher-cli/{_MISSHER_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://missher-agent.nousresearch.com/docs/user-guide/features/spotify"
...
OAUTH_OVER_SSH_DOCS_URL = "https://missher-agent.nousresearch.com/docs/guides/oauth-over-ssh"
