"""``missher profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``missher_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _render_distribution_plan(plan) -> None:
    ...
    from missher_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.missher_requires:
        print(f"  Requires: Missher {mf.missher_requires}")
...
def _profile_status(args):
    """Bare ``missher profile`` — show current profile status."""
    from missher_constants import display_missher_home
    from missher_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    profile_name = get_active_profile_name()
    dhh = display_missher_home()
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → missher -p {p.name}")
    print()
...
def _shared_credential_warnings(profiles) -> list:
    ...
    from missher_cli.profile_channels import shared_channel_credentials, shared_credential_warning
...
def _profile_use(args):
    from missher_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.missher)" if name == "default" else f"Switched to: {name}")
...
def _print_channel_clone_notice(name: str, source_label: str, clone_channels: bool, clone_flag: str) -> None:
    from missher_cli.profile_channels import (
...
def _profile_create(args):
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  missher profile alias {name} --name <custom>")
            print(f"  Or access via flag:     missher -p {name} chat")
        else:
    ...
    if served is not None and normalize_profile_name(name) in {normalize_profile_name(p) for p in served}:
        ...
        print("  missher gateway restart    Serve this profile from the running multiplexed gateway")
    else:
...
def _profile_delete(args):
    from missher_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_missher_home),
    everything else to its named directory."""
    from missher_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from missher_constants import get_missher_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from missher_cli import profile_describer as _pd
    if all_flag:
...
def _profile_show(args):
    ...
    if dist_name:
        ...
        print(f"  (run `missher profile info {name}` for full manifest)")
    if alias_name:
        print(f"Alias:   {alias_name} → missher -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_rename(args):
    from missher_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from missher_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from missher_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="missher_dist_preview_") as tmp:
        ...
        if plan.has_cron:
            print(
                ...
                f"  Review them with:  missher -p {plan.manifest.name} cron list"
            )
        print(f"\n  Use with:      missher -p {plan.manifest.name} chat")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")


def _profile_update(args):
    from missher_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from missher_cli.profiles import get_profile_dir, normalize_profile_name
    try:
        ...
        if current is None:
            _die(
                ...
                "Only profiles installed via `missher profile install` can be updated."
            )
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  missher -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
_INFO_FIELDS = (
    ...
    ("author", "Author:       "),
    ("license", "License:      "),
    ("missher_requires", "Requires:     Missher "),
    ("source", "Source:       "),
...
def _profile_info(args):
    from missher_cli.profile_distribution import describe_distribution, DistributionError
    try:
...
# Order mirrors the original if/elif chain; None = bare ``missher profile``.
PROFILE_ACTIONS = {
