"""Persistent session goals — the Ralph loop for Missher.
...
# (schema DDL, FTS tables, first missher_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per MISSHER_HOME (profile switches pick the right DB); None on any failure.
    ...
    if cached is not None and _registry_tore_down(cached):
        # ``missher profile delete`` force-closes every handle under the profile home
        # (``missher_state_registry.close_all_under``) before rmtree; a same-name recreate in this
