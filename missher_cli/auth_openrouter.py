"""OpenRouter OAuth PKCE login (``missher auth add openrouter --type oauth``).
...
def _openrouter_loopback_code(auth_url_params: Dict[str, str], *, open_browser: bool, timeout_seconds: float) -> str:
    ...
    print(f"Open this URL to authorize Missher with OpenRouter:\n  {auth_url}\n\nDocs: {OPENROUTER_OAUTH_DOCS_URL}")
...
def _openrouter_pkce_login(*, open_browser: bool = True, timeout_seconds: float = 300.0) -> Dict[str, Any]:
    ...
    if _is_remote_session():
        code = _openrouter_headless_code(f"{OPENROUTER_AUTH_URL}?{urlencode({**params, 'key_label': 'missher-agent'})}")
    else:
