"""Cross-process mutual exclusion for in-flight Missher updates.
...
MARKER_NAME = ".missher-update-in-progress"

# Set by an orchestrating updater (Tauri `missher-setup --update`) to its own pid before
# spawning `missher update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "MISSHER_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Missher is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Missher home (never the context-local profile override): the Rust
    updater resolves ``$MISSHER_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_missher_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``missher update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Missher update is already running (PID {holder.pid}, "
