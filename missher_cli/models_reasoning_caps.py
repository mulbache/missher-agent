...
Split out of ``missher_cli.models``. OpenRouter and
...
``missher_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `missher -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_missher_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._MISSHER_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``missher_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from missher_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from missher_cli.models_profile_cache import profile_slot_set
