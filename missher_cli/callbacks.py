...
from missher_cli.banner import cprint, _DIM, _RST
...
def _secret_result(var_name: str, value: str) -> dict:
    ...
    cprint(f"\n{_DIM}  ✓ Stored secret in {display_missher_home()}/.env as {var_name}{_RST}")
    return {
        **stored,
        "skipped": False,
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    in ~/.missher/.env and never exposed to the model.
    """
