...
def _strip_missher_path_lines(content: str) -> str:
    """Drop the ``# Missher Agent`` marker (+ its PATH line) and any missher PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Missher Agent' in line or '# missher-agent' in line:
            skip_next = True
            continue
        if skip_next and ('missher' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'missher' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Missher PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_missher_path_lines(content)
...
def remove_wrapper_script():
    """Remove the missher wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a missher_cli / missher-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('missher_cli' in content or 'missher-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("missher", "missher-acp", "missher-agent"))
...
def remove_node_symlinks(missher_home: Path) -> list:
    ...
    node_dir = (missher_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from missher_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from missher_cli.gateway import (
...
def _remove_launchd_gateway() -> bool:
    ...
    from missher_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from missher_cli import gateway_windows as gw
...
# MISSHER_HOME / MISSHER_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\missher\git\{cmd,bin,usr\bin}, ...\missher\node), PortableGit + Node copies
...
def _missher_path_markers(missher_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Missher-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(missher_home).rstrip("\\/")
    subs = ("missher-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(missher_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Missher-owned entries from User-scope PATH in the registry (see ``_missher_path_markers``)."""
    markers = tuple(m.lower() for m in _missher_path_markers(missher_home, include_managed_bin=include_managed_bin))
...
def remove_missher_env_vars_windows() -> list[str]:
    """Delete MISSHER_HOME and MISSHER_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("MISSHER_HOME", "MISSHER_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(missher_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``missher_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (missher_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``missher`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from missher_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from missher_constants import get_default_missher_root
        bin_dir = get_default_missher_root() / "bin"
    except Exception as e:
...
def _is_default_missher_home(missher_home: Path) -> bool:
    """Return True when ``missher_home`` points at the default (non-profile) root."""
    try:
        from missher_constants import get_default_missher_root
        return missher_home.resolve() == get_default_missher_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``missher -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current MISSHER_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m missher_cli.main` (the `missher` wrapper may be half-gone).
    missher_invocation = [sys.executable, "-m", "missher_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                missher_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's MISSHER_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``missher uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from missher_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    missher_home = get_missher_home()
    summary = gui_install_summary(missher_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ☤ Missher Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Missher Chat GUI installation was found.")
        print(f"  Checked: {missher_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Missher agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(missher_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Missher agent at {missher_home / 'missher-agent'}")
        print(f"  • Your config, sessions, and secrets under {missher_home}")
        print()
    ...
    print()
    uninstall_gui(missher_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Missher agent is still installed. Run 'missher' to use the CLI,")
    print("or 'missher uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.missher/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, missher_home=missher_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie MISSHER_HOMEs and systemd units behind.
    is_default_profile = _is_default_missher_home(missher_home)
    ...
    print()
    _print_box("│            ☤ Missher Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {missher_home / 'config.yaml'}")
    print(f"  Secrets: {missher_home / '.env'}")
    print(f"  Data:    {missher_home / 'cron/'}, {missher_home / 'sessions/'}, {missher_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, MISSHER_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Missher data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Missher code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, missher_home=missher_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, missher_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Missher PATH entries from shell configs / Windows User PATH")
    print("  • Missher wrapper scripts and Missher-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Missher config/data: {missher_home}")
    else:
        print(f"  • Missher config/data: {missher_home}")
        profiles = _discover_named_profiles() if _is_default_missher_home(missher_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$MISSHER_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: missher_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; missher\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_missher_home(missher_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(missher_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Missher-owned PATH entries in User environment"),
        (windows, "Removing MISSHER_HOME / MISSHER_GIT_BASH_PATH User env vars...",
         remove_missher_env_vars_windows, "Removed User env var: {}", "No Missher-set User env vars to remove"),
        (True, "Removing missher command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows missher launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows missher launchers found"),
        (True, "Removing Missher-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(missher_home), "Removed {}", "No Missher-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE MISSHER_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from missher_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(missher_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(missher_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.missher/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(missher_home)
    else:
        log_info(f"Keeping configuration and data in {missher_home}")
    ...
    print()
    print("Thank you for using Missher Agent! ☤")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://missher-agent.nousresearch.com/install.ps1)",
    False: "  curl -fsSL https://missher-agent.nousresearch.com/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m missher_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``missher_constants`` + ``missher_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m missher_cli.uninstall")
    parser.add_argument(
