"""Status command for missher CLI."""
...
from missher_cli.auth import AuthError, resolve_provider
from missher_cli.colors import Colors, color
from missher_cli.config import get_env_path, get_env_value, get_missher_home, load_config
from missher_cli.models import provider_label
from missher_cli.runtime_provider import resolve_requested_provider
from missher_cli.vercel_auth import describe_vercel_auth
from missher_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from missher_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `missher status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `missher resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'missher-agent[vercel]')")
        ...
        # (fail-soft — never break `missher status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from missher_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "missher gateway status   # from the default profile")
            return
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from missher_cli.gateway_multiplex_served import served_profile_ingress_urls
        ...
        if snapshot.has_process_service_mismatch:
            ...
            _kv("Start with:", "missher gateway")
...
def _render_cron(ctx):
    ...
    jobs_file = get_missher_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from missher_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_missher_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from missher_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'missher doctor' for detailed diagnostics", "  Run 'missher setup' to configure"),
            Colors.DIM)
    print()


# Print order of `missher status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Missher Agent components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('missher_cli.nous_account', 'get_nous_portal_account_info'),
