"""Configuration management for Missher Agent: config.yaml / .env loading, saving,
validation, migration, and the ``missher config`` command."""
...
from missher_cli import managed_scope
from missher_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from missher_cli.secret_prompt import masked_secret_prompt
# Re-export from missher_constants — canonical definition lives there.
from missher_constants import get_missher_home, get_process_missher_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (missher config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    from missher_cli.config_backups import backup_config
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  missher config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Missher restarts through them), PATH (fix tool lookup
...
# and Missher runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``MISSHER_*`` overall is NOT blocked — many integration credentials use that prefix
# (MISSHER_LANGFUSE_PUBLIC_KEY, MISSHER_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Missher runtime location
    "MISSHER_HOME", "MISSHER_PROFILE", "MISSHER_CONFIG", "MISSHER_ENV",
    "MISSHER_CONFIG_PATH", "MISSHER_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "MISSHER_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "MISSHER_COPILOT_ACP_COMMAND", "MISSHER_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "MISSHER_YOLO_MODE", "MISSHER_ACCEPT_HOOKS", "MISSHER_REDACT_SECRETS",
    "MISSHER_INTERACTIVE", "MISSHER_EXEC_ASK", "MISSHER_GATEWAY_SESSION",
    "MISSHER_CRON_SESSION", "MISSHER_SINGLE_QUERY_SESSION",
    "MISSHER_SESSION_KEY", "MISSHER_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Missher runtime location and security policy (MISSHER_HOME, MISSHER_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.missher/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean MISSHER_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "MISSHER_TOOL_PROGRESS_MODE",
    ...
    "MISSHER_LANGFUSE_ENV", "MISSHER_LANGFUSE_RELEASE", "MISSHER_LANGFUSE_SAMPLE_RATE",
    "MISSHER_LANGFUSE_MAX_CHARS", "MISSHER_LANGFUSE_CAPTURE", "MISSHER_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "MISSHER_ACP_AUTH_METHOD", "MISSHER_ACP_AUTO_APPROVE", "MISSHER_COPILOT_ACP_COMMAND",
    "MISSHER_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# MISSHER_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
# Explicit opt-out (``MISSHER_MANAGED=false``): without this a bool-shaped value became a package
# manager literally named "false" and is_managed() blocked `missher update` (#12864).
_MANAGED_FALSE_VALUES = frozenset({"false", "0", "no", "off"})


def get_managed_system() -> Optional[str]:
    """Return the package manager owning this install, if any.
    Signals: MISSHER_MANAGED env var (systemd service) or a ``.managed`` marker file in
    MISSHER_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("MISSHER_MANAGED", "").strip().lower() or None
    managed_marker = get_missher_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Missher is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Missher through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Missher was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$MISSHER_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because MISSHER_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``missher update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``nousresearch/missher-agent`` image bakes a ``docker`` stamp into
    ``/opt/missher`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of missher_cli/), NOT of $MISSHER_HOME,
    ...
    method = _install_method_stamp(get_missher_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``missher_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull nousresearch/missher-agent:latest",
    "apt": "pkg upgrade missher-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "missher update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (missher_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Missher.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``missher update`` doesn't apply inside the Docker container.

Missher Agent runs as a published image (nousresearch/missher-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/missher-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate missher-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/missher-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/missher-agent/tags
  • Your config and session history live under ``$MISSHER_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``missher update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Missher installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Missher installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Missher.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from MISSHER_HOME/.container-mode.
    ...
    already inside the container, or when MISSHER_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("MISSHER_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_missher_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "missher-agent"),
        "exec_user": info.get("exec_user", "missher"),
        "missher_bin": info.get("missher_bin", "/data/current-package/bin/missher")}


# ---- Config paths / MISSHER_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_missher_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / MISSHER_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("MISSHER_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_missher_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read MISSHER_UID / MISSHER_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns MISSHER_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Missher commonly set these to map the in-container user to a host user so
    ...
    return _env_int("MISSHER_UID"), _env_int("MISSHER_GID")


def _chown_to_missher_uid(path) -> None:
    """Chown ``path`` to ``MISSHER_UID:MISSHER_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_missher_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_missher_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply MISSHER_UID/GID ownership. No-op when managed;
    in a container only an explicit MISSHER_HOME_MODE is applied. MISSHER_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse MISSHER_HOME to a served subdirectory without
    directory listings.

    Also applies ``MISSHER_UID``/``MISSHER_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    explicit_mode = os.environ.get("MISSHER_HOME_MODE", "").strip()
    ...
    # start (#10757). An explicit MISSHER_HOME_MODE is the operator's choice and is still applied.
    if _is_container() and not explicit_mode:
        _chown_to_missher_uid(path)
        return
    try:
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or MISSHER_CONTAINER / MISSHER_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("MISSHER_CONTAINER") or os.environ.get("MISSHER_SKIP_CHMOD")
...
_MISSHER_HOME_ENSURED: set = set()
_MISSHER_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_missher_home():
    """Ensure the ~/.missher directory skeleton exists with secure permissions.
    ...
    from missher_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _MISSHER_HOME_ENSURED and home.is_dir():
        return
    from missher_cli.config_home import initialize_home
    initialize_home(home, _MISSHER_HOME_SUBDIRS, _MISSHER_HOME_ENSURED)


# ---- Config loading/saving ----

from missher_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from missher_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use missher_cli.config.<name>)
...
# Back-compat re-exports — :mod:`missher_cli.personality` owns personality/overlay semantics.
from missher_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``missher config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `missher config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `missher update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (missher_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # missher_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'missher tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from missher_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Missher won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'missher doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_missher_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from missher_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ missher config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from missher_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from missher_cli.toolset_validation import validate_platform_toolsets
        from missher_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: missher config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.missher/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_missher_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``missher config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``missher status``/``dump``) read ``name`` and
...
# the MISSHER_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Missher host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `missher config get`): nothing loaded yet in this process, so
        ...
        from missher_cli.config_backups import load_newest_good_backup
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_missher_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_missher_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — missher auth) — OpenAI Codex
#   nous         (OAuth — missher auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.missher/config.yaml.
...
def load_env() -> Dict[str, str]:
    """Load ~/.missher/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.missher/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.missher/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``MISSHER_CUSTOM_`` prefix keeps the
    ...
    return f"MISSHER_CUSTOM_{slug}_API_KEY" if slug else "MISSHER_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.missher/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from missher_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.missher/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Missher (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.missher/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Missher-managed credential preferring ``~/.missher/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale MISSHER_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("MISSHER_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale MISSHER_MAX_ITERATIONS={env_ghost} "
                    f"(run 'missher doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from missher_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ☤ Missher Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from missher_cli.auth import get_anthropic_key
    ...
    print(color("  missher config edit     # Edit config file", Colors.DIM))
    print(color("  missher config set <key> <value>", Colors.DIM))
    print(color("  missher setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``MISSHER_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("MISSHER_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`missher cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `missher config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``missher config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``missher config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    missher config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    missher config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_missher_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Missher may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `missher config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from missher_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: missher config get <key> [--json]", [
    "missher config get model", "missher config get terminal.backend",
    "missher config get skills.config --json"], None)
_USAGE_SET = ("Usage: missher config set [--force] <key> <value>", [
    "missher config set model anthropic/claude-sonnet-4", "missher config set terminal.backend docker",
    "missher config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: missher config unset <key>", [
    "missher config unset model", "missher config unset terminal.backend",
    "missher config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'missher config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  missher config           Show current configuration
  missher config edit      Open config in editor
  missher config get <key>          Print a resolved config value
  missher config set <key> <value>   Set a config value
  missher config unset <key>        Remove a config value
  missher config check     Check for missing/outdated config
  missher config migrate   Update config with new options
  missher config path      Show config file path
  missher config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``missher config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``missher_cli/`` — i.e. the git checkout for source
    installs, ``/opt/missher`` inside the published image. It is a property of
    the running interpreter, NOT of ``$MISSHER_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$MISSHER_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/missher`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('missher_cli.route_identity', 'normalize_route_base_url'),
}
