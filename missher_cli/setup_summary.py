...
resolved through the module object so test patches on ``missher_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from missher_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'missher setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'missher setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'missher tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("missher setup", "          Re-run the full wizard"), ("missher setup model", "    Change model/provider"),
    ("missher setup terminal", " Change terminal backend"), ("missher setup gateway", "  Configure messaging"),
    ("missher setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("missher config", "         View current settings"), ("missher config edit", "    Open config in your editor"),
    ("missher config set <key> <value>", ""))
_READY_ROWS = (
    ("missher", "              Start chatting"), ("missher gateway", "      Start messaging gateway"),
    ("missher doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'missher setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `missher tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'missher setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via missher auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, missher_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Missher cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    missher model            (pick any provider/model)",
              "    missher setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'missher setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {missher_home}/cron/, sessions/, logs/")):
...
import missher_cli.setup as _setup  # noqa: E402  (bottom: missher_cli.setup imports this module)
