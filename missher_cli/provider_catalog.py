...
The provider list shown by ``missher model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`missher_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `missher model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``missher model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("missher_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("missher_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    MISSHER_OVERLAYS = _safe_import("missher_cli.providers", "MISSHER_OVERLAYS", {})
    try:
