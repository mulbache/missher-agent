"""``missher logs`` subcommand parser."""
...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Missher log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    missher logs                    Show last 50 lines of agent.log
    missher logs -f                 Follow agent.log in real time
    missher logs errors             Show last 50 lines of errors.log
    missher logs gateway -n 100     Show last 100 lines of gateway.log
    missher logs gui -f             Follow gui.log in real time
    missher logs desktop -f         Follow desktop.log (Electron app boot/backend)
    missher logs --level WARNING    Only show WARNING and above
    missher logs --session abc123   Filter by session ID
    missher logs --component tools  Only show tool-related lines
    missher logs --since 1h         Lines from the last hour
    missher logs --since 30m -f     Follow, starting from 30 min ago
    missher logs list               List available log files with sizes
""")
