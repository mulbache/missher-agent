"""``missher debug`` subcommand parser."""
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Missher Agent. Use 'missher debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    missher debug share              Upload debug report (asks for confirmation)
    missher debug share --yes        Skip confirmation (for scripts/CI)
    missher debug share --lines 500  Include more log lines
    missher debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    missher debug share --local      Print report locally (no upload)
    missher debug share --no-redact  Disable upload-time secret redaction
    missher debug share --nous       Upload to Nous-internal storage (private)
    missher debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'missher debug share'")
