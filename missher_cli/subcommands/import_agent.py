"""``missher import-agent`` subcommand parser (logic lives in ``missher_cli/agent_import.py``)."""

from __future__ import annotations

from typing import Callable

from missher_cli.subcommands._shared import add_yes_flag
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent", help="Import a Claude Code or Codex CLI setup into Missher",
        description="One-command import of another coding agent's setup into Missher. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Missher equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'missher setup' for those.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Overwrite existing Missher items on name conflicts (default: skip)")
