"""Git plumbing for ``missher update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
_BAR = "=" * 68
_UPSTREAM_ADD_CMD = "git remote add upstream https://github.com/NousResearch/missher-agent.git"
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from missher_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/missher-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/missher-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && missher update\n"
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/missher-agent.git",
    "git@github.com:NousResearch/missher-agent.git",
    "https://github.com/NousResearch/missher-agent",
    "git@github.com:NousResearch/missher-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/missher-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_missher_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from missher_constants import get_missher_home
        (get_missher_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from missher_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        "\nℹ Your fork is not tracking the official Missher repository.\n"
        "  This means you may miss updates from NousResearch/missher-agent.\n"
    )
    ...
    if not _add_upstream_remote(git_cmd, cwd):
        ...
        return False
    print("  ✓ Added upstream: https://github.com/NousResearch/missher-agent.git")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from missher_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped MISSHER_HOME), then profile home as a fallback for custom layouts.

    The Missher-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped MISSHER_HOME (``<root>/profiles/<name>``), so a profile-scoped ``missher update`` must look
    there (monerostar review, #87876).
    """
    from missher_cli.update_cmd import get_default_missher_root, get_missher_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_missher_root(), Path(get_missher_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from missher_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``missher update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``missher update``, which ships with the checkout
