...
from missher_cli.config import DEFAULT_CONFIG, get_process_missher_home
from missher_cli.web_models import MCPServerCreate
from missher_cli.web_server_gateway import _ACTION_LOG_FILES
from missher_cli.web_server_mcp import _normalize_mcp_server_create
...
@contextmanager
def _missher_home_scope(path) -> Any:
    """Scope ``load_config``/``save_config`` (anything resolving ``get_missher_home()`` at call
    time) to ``path`` for the block via the context-local MISSHER_HOME override."""
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_missher_home().resolve()
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_missher_home()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    """Write MCP server entries into ``profile_dir``'s config.yaml (MISSHER_HOME-scoped).
    ...
    from missher_cli.mcp_config import _save_bearer_auth_token
    written = 0
...
def _profile_scope(profile: Optional[str]):
    ...
    Two seams: (1) ``load_config``/``save_config`` resolve ``get_missher_home()`` at call
    ...
    ``get_missher_home()`` so writes land in the live home even when the import-time binding
    is stale (test isolation, late MISSHER_HOME override). Yields the profile dir for a named
    ...
    with _config_profile_scope(profile) as scoped:
        profile_dir = get_missher_home() if scoped is None else scoped
        modules = (_skills_tool, _skill_mgr)
        with _SKILLS_PROFILE_LOCK:
            saved = [(m.MISSHER_HOME, m.SKILLS_DIR) for m in modules]
            for m in modules:
                m.MISSHER_HOME, m.SKILLS_DIR = profile_dir, profile_dir / "skills"
            try:
                yield scoped
            finally:
                for m, (home, skills_dir) in zip(modules, saved):
                    m.MISSHER_HOME, m.SKILLS_DIR = home, skills_dir


@contextmanager
def _config_profile_scope(profile: Optional[str]):
    """Await-safe, config-only profile scope: touches ONLY the task-local MISSHER_HOME
    ...
    with _missher_home_scope(profile_dir):
        yield None if profile_dir.resolve() == get_process_missher_home().resolve() else profile_dir
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    a fresh ``missher`` subprocess whose ``_apply_profile_override()`` reads ``-p`` from argv —
...
def _hub_action_name(verb: str, key: str) -> str:
    """Unique per-skill hub action name (+ registered log file): ``_spawn_missher_action``
