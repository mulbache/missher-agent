...
Split out of ``missher_cli/auth.py`` and re-exported there; origin helpers are imported lazily
inside each function so ``missher_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from missher_cli.auth_constants import _decode_jwt_claims
...
def _is_pkce_row(row: Dict[str, Any]) -> bool:
    return str(row.get("source") or "").endswith("missher_pkce")
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``missher profile create
    ...
    """
    from missher_cli.auth import _same_path, _save_auth_store
    ...
    try:
        ...
        root_auth_path = get_default_missher_root() / "auth.json"
...
# the heal for the life of ONE process, so every fresh `missher` invocation and every new worker
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``missher auth list`` / ``missher auth status`` print them so the user sees the consolidation.
    """
    from missher_cli.auth import _oauth_heal_notices
...
def heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    row with no root counterpart (an independent ``missher -p <p> auth add`` grant, or the only
...
class _HealPass:
    ...
    def heal_pool_rows(self) -> None:
        ...
        for row in self.p_rows:
            ...
            # (the ``missher auth`` PKCE shape); a profile missher_pkce-family row is its copy.
    ...
    def heal_profile_singleton(self, profile_singleton: Optional[Path]) -> None:
        if profile_singleton is None or not profile_singleton.exists():
            return
        from missher_cli.auth import _is_same_auth_store
    ...
    def sync_root_singleton_with_pkce_row(self) -> None:
        """Keep root's singleton and its ``missher_pkce``-seeded pool row in step.
        ...
        pkce_idx = next(
            (i for i, r in enumerate(self.r_rows)
             if _is_oauth_pool_payload(r) and r.get("source") == "missher_pkce"), None)
        if pkce_idx is None:
            return
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # ~/.missher/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".missher" / "auth.json"):
            return None
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            if summary["adopted"] and root_singleton is not None and singleton_row is not None:
                from agent.anthropic_credentials import _write_missher_oauth_credentials
                _write_missher_oauth_credentials(
