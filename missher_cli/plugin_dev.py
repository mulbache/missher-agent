"""Runtime-backed validation behind ``missher plugins doctor``: every manifest/import/registration
...
def _doctor_runtime(plugin_path: Path):
    ...
    MISSHER_HOME with outbound socket connects blocked.
    """
    stack = ExitStack()
    try:
        # The temp dir enters the stack FIRST so any failure below (ENOSPC / KeyboardInterrupt in
        # copytree) removes it instead of stranding a missher-plugin-doctor-* directory.
        home = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="missher-plugin-doctor-")))
        ...
        stack.enter_context(patch.dict(os.environ, {
            "MISSHER_HOME": str(home),
            "MISSHER_BUNDLED_PLUGINS": str(bundled),
            "MISSHER_ENABLE_PROJECT_PLUGINS": "0",
        }, clear=False))
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(f"Missher discovery found no valid plugin manifest under {copied}")
        if len(manifests) != 1:
...
def _is_plugin_module(name: str) -> bool:
    return name == "missher_plugins" or name.startswith("missher_plugins.")
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        candidates.append(get_missher_home() / "plugins" / raw)
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".missher" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    import re as _re
    from missher_cli.plugins import SUPPORTED_MANIFEST_VERSION
    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Missher supports "
    ...
    if missing:
        report.warning(
            ...
            + " — Missher never auto-installs plugin dependencies; install manually: pip install "
            + " ".join(f"'{m}'" for m in missing))

    schema = getattr(manifest, "config_schema", {}) or {}
    if schema:
        from missher_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Missher' real scanner and registration path."""
    try:
