"""`missher checkpoints` CLI subcommand.
...
from missher_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: missher checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``missher checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `missher checkpoints` → status
...
_PLUGIN_COMPAT_LAZY = {
    'cmd_list': ('missher_cli.plugins_cmd', 'cmd_list'),
}
