...
from missher_cli.session_schema_history import SCHEMA_HISTORY, reachable_physical_layouts

from missher_state_ids import SESSION_ID_PATTERN  # timestamp prefix: strongest sentinel for schema-less rows
...
# 3.50.7 and 3.44.6 — the same version gate missher_state applies to the
...
# caught exactly this shell converting a live Missher state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Missher database.
#
# The predicate lives in missher_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from missher_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Missher database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="missher-recover-probe-")
...
def _type_conflicts(value: Any, declared: str) -> bool:
    ...
    non-numeric string in an INTEGER column, but Missher never writes one, so
...
# The closed sets SessionDB writes (missher_state.py: request/claim/complete/
