"""``missher plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``MISSHER_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "MISSHER_VALIDATE_JSON:"
...
def _requires_missher_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_missher`` spec.

    Unlike :func:`missher_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from missher_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_missher(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_missher") or "").strip()
    if not spec:
        report.add("requires_missher", True, "not declared")
        return
    if _requires_missher_spec_valid(spec):
        report.add("requires_missher", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_missher",
            False,
            f"requires_missher spec {spec!r} does not parse "
...
# from missher so a hostile plugin only sees a bare interpreter.
_PROBE_SCRIPT = r"""
...
def _run_capability_probe(plugin_dir: Path) -> Tuple[Optional[dict], str]:
    ...
    *error* is a human-readable failure description otherwise.
    """
    with tempfile.TemporaryDirectory(prefix="missher-validate-") as scratch:
        env = dict(os.environ)
        env["MISSHER_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_missher(report, manifest)
