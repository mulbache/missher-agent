"""Slash command registry for the Missher CLI and gateway.
...
from missher_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``missher_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Missher config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Missher works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .missher/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("busy", "Control how messages behave while Missher is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.missher/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Missher Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Missher Agent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``missher skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('missher_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('missher_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('missher_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('missher_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('missher_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('missher_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('missher_cli.commands_platforms', 'telegram_menu_max_commands'),
}
