"""Messaging-platform setup wizards (Telegram, BlueBubbles, webhooks) and the ``missher setup
gateway`` flow. setup.py re-exports the public names, and tests monkeypatch prompt/print/env
helpers on missher_cli.setup, so those are imported lazily per function."""
...
def _profile_name_from_missher_home(missher_home) -> str | None:
    """Return the active profile name when MISSHER_HOME is a profile dir."""
    return missher_home.name if missher_home.parent.name == "profiles" else None


def _setup_telegram_auto_result():
    """Attempt automatic Telegram bot creation via managed QR onboarding."""
    from missher_cli.setup import get_missher_home
    try:
        from missher_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    profile_name: str | None = None
    with contextlib.suppress(Exception):
        profile_name = _profile_name_from_missher_home(Path(get_missher_home()))
...
def declines_reconfigure(label: str, question: str, *env_vars: str) -> bool:
    ...
    """
    from missher_cli.setup import get_env_value, print_info, prompt_yes_no
...
def save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from missher_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from missher_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from missher_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from missher_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from missher_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from missher_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _obtain_telegram_token():
    ...
    from missher_cli.setup import _info, print_error, prompt
...
def _setup_telegram():
    ...
    from missher_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
    ...
    _info(None, "📬 Home Channel: where Missher delivers cron job results,",
...
def _setup_bluebubbles():
    ...
    from missher_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
    print_header("BlueBubbles (iMessage)")
    if declines_reconfigure("BlueBubbles", "Reconfigure BlueBubbles?", "BLUEBUBBLES_SERVER_URL"):
        return
    _info("Connects Missher to iMessage via BlueBubbles — a free, open-source",
...
def _setup_webhooks():
    ...
    from missher_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
    ...
    print()
    _info("   Full guide: https://missher-agent.nousresearch.com/docs/user-guide/messaging/webhooks/", None)
    ...
    _info(f"   1. Define webhook routes in {_dhh()}/config.yaml",
          ...
          "   https://missher-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes",
          None,
          # Printed twice upstream; kept verbatim for output parity.
          "   Open config in your editor:  missher config edit",
          "   Open config in your editor:  missher config edit")
...
def _warn_missing_home_channels() -> None:
    ...
    from missher_cli.setup import get_env_value, _info, print_warning
    missing_home = [
    ...
    _info("   Without a home channel, cron jobs and cross-platform",
          ...
          *(f"     missher config set {plat.upper()}_HOME_CHANNEL <channel_id>" for plat in missing_home))
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from missher_cli.setup import print_error, prompt_yes_no
...
def setup_gateway(config: dict):
    ...
    from missher_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from missher_cli.gateway import _all_platforms, _platform_status, _configure_platform
    print_header("Messaging Platforms")
    _info("Connect to messaging platforms to chat with Missher from anywhere.",
    ...
    if not selected:
        print_info("No platforms selected. Run 'missher setup gateway' later to configure.")
    ...
    # mode (cron keeps running; adapters come up once tokens are added via `missher import` /
    # `missher setup gateway`). Gating it on messaging config left install-then-import machines
    # with cron jobs and bot tokens but no process to serve them.
    from missher_cli.gateway import _is_service_running, supports_systemd_services, ensure_gateway_service
