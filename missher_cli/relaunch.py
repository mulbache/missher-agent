"""Unified self-relaunch for Missher CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``missher sessions browse`` / post-setup relaunch keep the user's mode."""
...
from missher_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched missher."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_missher_bin() -> Optional[str]:
    """Missher entry point: ``sys.argv[0]`` if a real executable, else ``which missher``, else ``None``
    (caller falls back to ``python -m missher_cli.main``).
    ...
    lose the venv. Falling through to ``sys.executable -m missher_cli.main`` keeps the venv.
    """
    argv0 = sys.argv[0]
    ...
    def _is_unsafe_python_launcher(p: str) -> bool:
        ...
        from missher_cli.linux_desktop_entry import _needs_interpreter
...
def build_relaunch_argv(
    ...
    """Construct an argv list for replacing the current process with missher."""
    bin_path = resolve_missher_bin()
    argv = [bin_path] if bin_path else [sys.executable, "-m", "missher_cli.main"]
...
def relaunch(
    ...
    """Replace the current process with a fresh missher invocation.
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``missher`` not on
            # PATH yet (install hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nMissher relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run missher.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
