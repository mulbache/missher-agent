...
from missher_cli.platforms import PLATFORMS
from missher_cli.toolset_scope import toolset_allowed_for_platform

_NO_TOOLS = "the agent will have no tools on this platform. Run `missher tools` to reconfigure."


def _platform_default_toolset(platform: object) -> str:
    info = PLATFORMS.get(platform)
    return info.default_toolset if info is not None else f"missher-{platform}"
...
def _platform_default_is_valid(
    ...
    # missher-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    Reports: a toolset name ``is_valid_toolset`` rejects (suggesting ``missher-<platform>`` when that
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `missher tools` to configure explicitly.")
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `missher tools` to reconfigure.")
    return warnings
