# missher_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `MissherCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`missher_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`MissherCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.missher/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`missher_cli/curses_ui.py`; example
`missher_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`missher_cli/commands.py`)
...
`missher_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`missher_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`missher tools/setup`, most subcommands, `missher_cli/config.py`, merges `DEFAULT_CONFIG`);
  `missher_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `missher send`, doctor,
  `missher_time`/`missher_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
## Skin engine (`missher_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.missher/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`missher update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `missher update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `missher-gateway*` unit /
  `ai.missher.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.missher/logs/update_receipts/`
...
`_apply_profile_override()` in `missher_cli/main.py` sets `MISSHER_HOME` before any module import, so
every `get_missher_home()` scopes to the active profile (rules in root). Profiles are independent
...
cron all go through `mkdir_under_missher_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `missher_constants.missher_home_key()`, never a single flag.
Migration from per-profile gateways: `missher_cli/gateway_migrate.py` (`missher gateway migrate
...
other service domain / UNIX user / MISSHER_HOME outside `profiles/` — notices for the explicit command,
...
## Nous free tier (`missher_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `missher` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`missher auth upgrade`), the CLI `/login` handler and the desktop
