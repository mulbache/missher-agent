"""Top-level argparse construction for the missher CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``missher_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``MISSHER_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/missher_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``missher --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``missher_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    missher                        Start interactive chat
    missher chat -q "Hello"        Single query mode
    missher --tui                  Launch the modern TUI (or set display.interface: tui)
    missher --cli                  Force the classic REPL (overrides display.interface: tui)
    missher -c                     Resume the most recent session
    missher -c "my project"        Resume a session by name (latest in lineage)
    missher --resume <session_id>  Resume a specific session by ID
    missher --resume latest        Resume the most recent session (same as -c)
    missher --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    missher setup                  Run setup wizard
    missher logout                 Clear stored authentication
    missher auth add <provider>    Add a pooled credential
    missher auth list              List pooled credentials
    missher auth remove <p> <t>    Remove pooled credential by index, id, or label
    missher auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    missher auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    missher auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    missher model                  Select default model
    missher fallback [list]        Show fallback provider chain
    missher fallback add           Add a fallback provider (same picker as `missher model`)
    missher fallback remove        Remove a fallback provider from the chain
    missher config                 View configuration
    missher config edit            Edit config in $EDITOR
    missher config set model gpt-4 Set a config value
    missher gateway                Run messaging gateway
    missher -s missher-agent-dev,github-auth
    missher -w                     Start in isolated git worktree
    missher gateway install        Install gateway background service
    missher sessions list          List past sessions
    missher sessions browse        Interactive session picker
    missher sessions rename ID T   Rename/title a session
    missher logs                   View agent.log (last 50 lines)
    missher logs -f                Follow agent.log in real time
    missher logs errors            View errors.log
    missher logs --since 1h        Lines from the last hour
    missher debug share             Upload debug report for support
    missher console                Open the safe Missher command console
    missher update                 Update to latest version
    missher dashboard              Start web UI dashboard (port 9119)
    missher dashboard --stop       Stop running dashboard processes
    missher dashboard --status     List running dashboard processes

For more help on a command:
    missher <command> --help
"""
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via MISSHER_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `missher setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to MISSHER_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.missher/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``missher -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/missher_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Missher Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also MISSHER_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.missher/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Missher itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="missher", description="Missher Agent - AI assistant with tool-calling capabilities",
