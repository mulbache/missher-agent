"""Plugin catalog — curated, Nous-approved Missher plugins shipped with the repo.
...
an installed Missher sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://missher-agent.nousresearch.com/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "maintainer": self.maintainer, "tier": self.tier, "requires_missher": self.requires_missher,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_missher=str(data.get("requires_missher") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_missher_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``MISSHER_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_missher:
        bits.append(f"Requires Missher {entry.requires_missher}.")
    return " ".join(bits)
