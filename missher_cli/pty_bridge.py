"""PTY bridge for `missher dashboard` chat tab.
...
``missher_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/missher-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "MISSHER_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Missher Agent supports Windows only via WSL.")
