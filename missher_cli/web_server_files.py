...
_MANAGED_FILES_ROOT_ENV = "MISSHER_DASHBOARD_FILES_ROOT"
...
def _default_missher_root_is_opt_data() -> bool:
    raw = os.environ.get("MISSHER_HOME", "").strip()
    if not raw:
        return False
    try:
        from missher_constants import get_default_missher_root

        root = get_default_missher_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``missher update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the missher-webui image), where
    ...
    if _default_missher_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Missher
    # root actually IS /opt/data or MISSHER_DASHBOARD_FILES_ROOT is set.
    if _default_missher_root_is_opt_data():
