...
Split out of :mod:`missher_cli.plugins`; validation warns and never fails a load.
"""
...
from missher_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``missher``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "missher", "depends",
    "requires_missher",
}

# Highest manifest schema version this Missher understands.
...
def _plugins_debug() -> bool:
    from missher_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Missher "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `missher plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `missher plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Missher version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_missher_error``.
    requires_missher: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Missher never auto-installs these (isolation design for the install seam
...
# ── requires_missher version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_missher_version() -> str:
    """Installed ``missher-agent`` distribution version, else ``missher_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("missher-agent")
    except Exception:
...
def requires_missher_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_missher`` rejects the running version."""
    if not manifest.requires_missher:
        return None
    current = running_missher_version()
    if version_satisfies(manifest.requires_missher, current):
        return None
    return f"requires missher {manifest.requires_missher}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_missher=str(data.get("requires_missher") or "").strip(),
