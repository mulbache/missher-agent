...
Split out of ``missher_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from missher_cli.kanban_db import Task
...
# ``MISSHER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Missher profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``missher kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``missher_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from missher_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Missher profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Missher ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``MISSHER_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``missher -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``missher kanban tail`` shows why the task looks stuck.
...
def _module_missher_argv() -> list[str]:
    """Interpreter-bound Missher CLI invocation (``missher_cli.main`` is the
    console-script target — there is no top-level ``missher`` package)."""
    return [sys.executable, "-m", "missher_cli.main"]


def _absolute_missher_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Missher shim."""
...
def _missher_path_argv(path: str) -> list[str]:
    """argv for a resolved Missher executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_missher_argv()
    return [_absolute_missher_path(path)]


def _resolve_missher_argv() -> list[str]:
    """Resolve the ``missher`` invocation as argv for ``Popen``: ``$MISSHER_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("missher")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    missher_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_missher_bin``; local because
    ``missher_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("MISSHER_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _missher_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _missher_path_argv(resolved_env_bin)
        return _module_missher_argv()

    missher_bin = _safe_which_no_cwd("missher") if _kb._IS_WINDOWS else shutil.which("missher")
    if missher_bin:
        return _missher_path_argv(missher_bin)
    return _module_missher_argv()
...
def _resolve_worker_cli_toolsets(missher_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``MISSHER_KANBAN_TASK`` is set.
    """
    if not missher_home:
        return None
    try:
        ...
        token = set_missher_home_override(missher_home)
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(missher_home)))
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for MISSHER_HOME=%r (%s)",
            missher_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, missher_home: Optional[str]) -> list[str]:
    """Build the ``missher -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_missher_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped MISSHER_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(missher_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `missher kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``missher -p <profile> chat -q ...`` subprocess.
    ...
    ``MISSHER_KANBAN_DB`` / ``MISSHER_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from missher_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # MISSHER_PROFILE (set below) instead.
        profile_home = None
    ...
    # Inject MISSHER_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_missher_home() falls back to the DEFAULT
    # profile root because `missher -p` applies its override before
    # missher_constants is imported.
    if profile_home:
        env["MISSHER_HOME"] = profile_home
    ...
    if task.tenant:
        env["MISSHER_TENANT"] = task.tenant
    env["MISSHER_KANBAN_TASK"] = task.id
    env["MISSHER_KANBAN_WORKSPACE"] = workspace
    ...
    env["MISSHER_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["MISSHER_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["MISSHER_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["MISSHER_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["MISSHER_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["MISSHER_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `missher -p` rewrites MISSHER_HOME (symlink / Docker layouts).
    env["MISSHER_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["MISSHER_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["MISSHER_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["MISSHER_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads MISSHER_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["MISSHER_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping MISSHER_TUI covers
    # older missher builds on PATH that predate the flag's precedence.
    env.pop("MISSHER_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("MISSHER_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`missher` executable not found on PATH. "
            "Install Missher Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``missher kanban dispatch`` — the standalone
...
from missher_cli import kanban_db as _kb  # noqa: E402
from missher_cli import kanban_db_connect as _kbc  # noqa: E402
from missher_cli import kanban_db_workspace as _kbw  # noqa: E402
