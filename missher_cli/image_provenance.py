"""Image-authored deployment provenance for immutable Missher runtimes.

The image bakes ``/etc/missher/image-provenance.json`` outside ``$MISSHER_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/missher/image-provenance.json")
