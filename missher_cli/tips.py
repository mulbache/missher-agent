...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or missher -c.",
    ...
    "Ctrl+Z suspends Missher to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "missher -c resumes your most recent CLI session. missher -c \"project name\" resumes by title.",
    "missher -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "missher -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "missher chat -t web,terminal enables only specific toolsets for a focused session.",
    "missher chat -s github-pr-workflow preloads a skill at launch.",
    "missher chat -q \"query\" runs a single non-interactive query and exits.",
    "missher chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "missher chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "missher --yolo bypasses all dangerous command approval prompts for the entire session.",
    "missher chat --source telegram tags the session for filtering in missher sessions list.",
    "missher -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "missher doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "missher dump outputs a compact setup summary — great for bug reports.",
    "missher config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "missher config edit opens config.yaml in your default editor.",
    "missher config check scans for missing or stale configuration options.",
    "missher sessions browse opens an interactive session picker with search.",
    "missher sessions stats shows session counts by platform and database size.",
    "missher sessions prune --older-than 30 cleans up old sessions.",
    "missher skills search react --source skills-sh searches the skills.sh public directory.",
    "missher skills check scans installed hub skills for upstream updates.",
    "missher skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "missher skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "missher mcp add github --command npx adds MCP servers from the command line.",
    "missher mcp serve runs Missher itself as an MCP server for other agents.",
    "missher auth add lets you add multiple API keys for credential pool rotation.",
    "missher completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "missher logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "missher backup creates a zip backup of your entire Missher home directory.",
    "missher profile create coder creates an isolated profile that becomes its own command.",
    "missher profile create work --clone copies your current config and keys to a new profile.",
    "missher update syncs new bundled skills to ALL profiles automatically.",
    "missher gateway install sets up Missher as a system service (systemd/launchd).",
    "missher memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "missher webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: missher tools disables unused tools, missher skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "missher models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as missher -w).",
    ...
    "MISSHER_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Missher tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'missher profile create coder' creates the 'coder' command.",
    "missher profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Missher prints a resume command with session ID and stats.",
    "missher sessions export backup.jsonl exports all sessions for backup or analysis.",
    "missher -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "missher skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .missher/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: missher cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Missher runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "missher gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Missher via a local macOS server.",
    ...
    "Context files (.missher.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Missher loads project context from .missher.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "missher mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "missher mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "missher mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — missher tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.missher/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "missher chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "missher plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "missher acp runs Missher as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MISSHER_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "missher auth add nous or missher auth add openai-codex sets up OAuth-based providers.",
    ...
    "missher status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.missher/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Missher your own.",
    ...
    "Before a gateway session reset, Missher auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from missher_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.missher/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Missher state via MISSHER_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "MISSHER_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in MISSHER_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.missher/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'missher profile create ls' would be rejected.",
    "missher profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "missher claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MISSHER_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via missher tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.missher/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.missher/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Missher auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Missher config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Missher is working.',
    ...
    '/reload reloads ~/.missher/.env into the running session — pick up new API keys without restarting.',
    ...
    "MISSHER_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.missher/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.missher/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'missher curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "missher curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'missher curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'missher auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    # --- TUI & Dashboard ---
    'MISSHER_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "MISSHER_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'missher dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'missher dashboard embeds the full Missher TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.missher/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.missher/dashboard-plugins/ — no npm build required.',
    ...
    'MISSHER_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'MISSHER_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'MISSHER_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'MISSHER_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.missher/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.missher/skills/ so drop-in skills appear without restarting the session.',
    ...
    'missher -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'missher chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'missher chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'missher chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "missher chat --source tool tags programmatic chats so they don't clutter missher sessions list.",
    'missher dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'missher sessions rename <ID> "new title" renames any past session; missher sessions delete <ID> removes one.',
    'missher import restores a full Missher backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'missher fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'missher pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'missher setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'missher status --deep runs the full health sweep across every component; plain missher status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'MISSHER_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'MISSHER_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.missher/plugins/ — trust-gated by design.',
    "MISSHER_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'MISSHER_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'MISSHER_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'MISSHER_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'MISSHER_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'MISSHER_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'MISSHER_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'MISSHER_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'MISSHER_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'MISSHER_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'MISSHER_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Missher block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from missher sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.missher/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Missher is
