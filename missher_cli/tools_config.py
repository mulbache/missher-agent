"""Unified tool configuration for Missher Agent."""
...
from missher_cli.cli_output import print_info as _print_info
from missher_cli.colors import Colors, color
from missher_cli.config import cfg_get, load_config, save_config, get_env_value
from missher_cli.nous_subscription import (
    NousSubscriptionFeatures, apply_nous_managed_defaults, get_nous_subscription_features)
from missher_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from missher_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS, toolset_allowed_for_platform as _toolset_allowed_for_platform)
# Re-exports: keep ``missher_cli.tools_config.X`` callers and test patch targets resolving.
from missher_cli.tools_config_cua import (  # noqa: F401
...
from missher_cli.tools_config_post_setup import (  # noqa: F401
...
from missher_cli.tools_config_providers import (  # noqa: F401
...
from missher_cli.tools_config_mcp import (  # noqa: F401
...
# OFF by default for new installs (still in _MISSHER_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `missher tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from missher_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from missher_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``missher tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``missher-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"missher-{platform}"
...
_LANGFUSE_PUBLIC = ("MISSHER_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("MISSHER_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Missher routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Missher, text-only (no screenshots)",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; MISSHER_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("MISSHER_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from missher_cli.plugins import get_portable_mcp_server_names_nowait
...
#: "declined": saving ``missher tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[missher-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (missher-acp, missher-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``missher-cli`` would re-enable disabled toolsets). A mixed list (``[missher-cli, spotify]``) still expands the
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``missher-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[missher-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[missher-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `missher config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``missher tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("missher-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``missher`` for ``missher-cli`` → no
    ...
    if named and not any(validate_toolset(t) for t in named) and platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "name(s): %s) - tools will be unavailable. Run `missher tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (missher-cli, ...) resolve to ALL tools and would silently override the user's
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from missher_cli.setup import setup_telemetry
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``missher tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `missher tools` / `missher setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    if getattr(args, "summary", False):
        ...
        return
    print(color("☤ Missher Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://missher-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    if first_install:
    ...
    print(color(f"  Tool configuration saved to {display_missher_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'missher' or gateway restart.", Colors.DIM))
    print()
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('missher_cli.nous_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    ...
    'format_nous_portal_entitlement_message': ('missher_cli.nous_account', 'format_nous_portal_entitlement_message'),
