"""Per-profile view of ``missher_cli.models``' module-level catalog slots.
...
files. In a multiplexed gateway every turn runs under a MISSHER_HOME override, so a single module slot
...
def profile_slot_get(module: Any, attr: str, default: Any = None) -> Any:
    ...
    return _SLOTS_BY_HOME.get((missher_home_key(), attr), default)
...
def profile_slot_set(module: Any, attr: str, value: Any) -> None:
    if get_missher_home_override() is None:
        ...
        _SLOTS_BY_HOME[(missher_home_key(), attr)] = value
