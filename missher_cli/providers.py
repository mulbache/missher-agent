"""Single source of truth for provider identity in Missher Agent."""
...
# -- Missher overlay: metadata models.dev doesn't provide ----------------------

@dataclass(frozen=True)
class MissherOverlay:
    """Missher-specific provider metadata layered on top of models.dev."""
...
MISSHER_OVERLAYS: Dict[str, MissherOverlay] = {
    "moa": MissherOverlay(auth_type="virtual", base_url_override="moa://local"),
    "openrouter": MissherOverlay(is_aggregator=True, base_url_env_var="OPENROUTER_BASE_URL"),
    "nous": MissherOverlay(auth_type="oauth_device_code", base_url_override="https://inference-api.nousresearch.com/v1"),
    "openai-codex": MissherOverlay(transport="codex_responses", auth_type="oauth_external",
                                  base_url_override="https://chatgpt.com/backend-api/codex"),
    "openai-api": MissherOverlay(transport="codex_responses", base_url_override="https://api.openai.com/v1",
                                base_url_env_var="OPENAI_BASE_URL"),
    "xai-oauth": MissherOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": MissherOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="MISSHER_QWEN_BASE_URL"),
    "lmstudio": MissherOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": MissherOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": MissherOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": MissherOverlay(transport="anthropic_messages", extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN")),
    "zai": MissherOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": MissherOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": MissherOverlay(extra_env_vars=("STEPFUN_API_KEY",),
                             base_url_override="https://api.stepfun.ai/step_plan/v1",
                             base_url_env_var="STEPFUN_BASE_URL"),
    "minimax": MissherOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_BASE_URL"),
    "minimax-oauth": MissherOverlay(transport="anthropic_messages", auth_type="oauth_external",
                                   base_url_override="https://api.minimax.io/anthropic"),
    "minimax-cn": MissherOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_CN_BASE_URL"),
    "deepseek": MissherOverlay(base_url_env_var="DEEPSEEK_BASE_URL"),
    "alibaba": MissherOverlay(base_url_env_var="DASHSCOPE_BASE_URL"),
    "alibaba-coding-plan": MissherOverlay(base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL"),
    "vercel": MissherOverlay(is_aggregator=True),
    "opencode": MissherOverlay(is_aggregator=True, base_url_env_var="OPENCODE_ZEN_BASE_URL"),
    "opencode-go": MissherOverlay(is_aggregator=True, base_url_env_var="OPENCODE_GO_BASE_URL"),
    "opencode-free": MissherOverlay(is_aggregator=True, base_url_override="https://opencode.ai/zen/v1", keyless=True),
    "kilo": MissherOverlay(is_aggregator=True, base_url_env_var="KILOCODE_BASE_URL"),
    "huggingface": MissherOverlay(is_aggregator=True, base_url_env_var="HF_BASE_URL"),
    "novita": MissherOverlay(is_aggregator=True, base_url_env_var="NOVITA_BASE_URL"),
    "xai": MissherOverlay(transport="codex_responses", base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "nvidia": MissherOverlay(base_url_override="https://integrate.api.nvidia.com/v1", base_url_env_var="NVIDIA_BASE_URL"),
    "xiaomi": MissherOverlay(base_url_env_var="XIAOMI_BASE_URL"),
    "tencent-tokenhub": MissherOverlay(base_url_env_var="TOKENHUB_BASE_URL"),
    "tencent-tokenplan": MissherOverlay(transport="anthropic_messages",
                                       base_url_override="https://api.lkeap.cloud.tencent.com/plan/anthropic",
                                       base_url_env_var="TOKENPLAN_BASE_URL"),
    "arcee": MissherOverlay(base_url_override="https://api.arcee.ai/api/v1", base_url_env_var="ARCEE_BASE_URL"),
    "gmi": MissherOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": MissherOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": MissherOverlay(transport="chat_completions", extra_env_vars=("ACTUAL_API_KEY",),
                            base_url_override="https://api.actual.inc/v1", base_url_env_var="ACTUAL_BASE_URL"),
    "upstage": MissherOverlay(extra_env_vars=("UPSTAGE_API_KEY",), base_url_override="https://api.upstage.ai/v1",
                             base_url_env_var="UPSTAGE_BASE_URL"),
    "nebius-token-factory": MissherOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": MissherOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": MissherOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": MissherOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": MissherOverlay(auth_type="vertex"),
}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "missher", "user-config"
...
def _overlay_pdef(canonical, ov: MissherOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Missher overlay;
    Missher-only overlay (nous, openai-codex, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = MISSHER_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or MissherOverlay()
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "missher")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # MISSHER_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Missher registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from missher_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
...
def _llamacpp_pdef() -> Optional[ProviderDef]:
    ...
    try:
        from missher_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
