"""Implementation of the ``missher bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  missher bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``missher bundles`` argparse tree (called from missher_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `missher bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``missher bundles <subcommand>`` to the right handler."""
