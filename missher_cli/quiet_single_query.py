"""Quiet ``missher chat -Q`` helpers: bind this session's key and resume nested notifies.

Bot Mode delivers a local DM as ``missher -p <bot> chat -Q --query-file``. Interactive
...
``MISSHER_SESSION_KEY`` and never woke the recipient. Quiet also printed and exited
