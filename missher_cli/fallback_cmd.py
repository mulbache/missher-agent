"""missher fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `missher model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  missher fallback add\n")
        return
    print()
    ...
    print("  Docs: https://missher-agent.nousresearch.com/docs/user-guide/features/fallback-providers\n")


def cmd_fallback_add(args) -> None:
    """Launch the same picker as `missher model`, then append the selection to the chain."""
    from missher_cli.main import _require_tty, select_provider_and_model
    from missher_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `missher model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `missher fallback list` to view, or `missher fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``missher fallback [subcommand]``."""
