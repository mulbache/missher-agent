"""``missher logs`` — view and filter Missher log files.

``missher logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``missher logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_missher_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Missher runs — try 'missher chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_missher_home()}/logs/{filename}{filter_desc} ({mode}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_missher_home()}/logs/")
        return

    print(f"Log files in {display_missher_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'missher chat' to generate logs)")
