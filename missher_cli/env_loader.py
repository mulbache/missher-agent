"""Helpers for loading Missher .env files consistently across entrypoints."""
...
# Once-per-process guards: load_missher_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `missher
...
# MISSHER_HOME paths already pulled external secrets for: load_missher_dotenv() runs at import time from
...
# Behavioral routing keys a parent Missher process injects into child env that silently redirect a profile
...
def get_secret_source_values(missher_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``missher_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(missher_home).resolve()), {}))


def hydrate_profile_secret_sources(missher_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(missher_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_missher_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
        ...
        local_env["MISSHER_HOME"] = str(home)
...
def reset_secret_source_cache(missher_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``missher_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    (#102041)."""
    if missher_home is None:
        _APPLIED_HOMES.clear()
        _SECRET_SOURCES.clear()
        _SECRET_SOURCE_VALUES_BY_HOME.clear()
        return
    home_key = str(Path(missher_home).resolve())
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `missher setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from missher_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_missher_dotenv(
    *,
    missher_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Missher env files: ``~/.missher/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(missher_home) if missher_home else get_process_missher_home()
    ...
    if is_multiplex_active() and get_missher_home_override() is not None:
        ...
        if load_external_secrets:
            from missher_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``missher update`` retry just completed a deferred dependency
    ...
    # ~/.missher/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.missher/.env (e.g. written by an older
    # `missher setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_missher_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_missher_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Missher reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and missher_logging reuse one parse instead of 3-4.
    if home_path == _process_missher_home():
        try:
...
def _process_missher_home() -> Path:
    """The MISSHER_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_missher_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``missher_constants.get_process_missher_home()`` is the override-immune
