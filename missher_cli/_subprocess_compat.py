...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_MISSHER_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Missher's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Missher shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_missher(text: str) -> bool:
    r"""True when *text* names Missher at a path-segment / token boundary.

    A bare ``"missher" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\smisshera\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("missher", ".missher"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_missher(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Missher code."""
    try:
        import psutil
        ...
        return _text_names_missher(f"{command} {executable}")
    except Exception:
        return False


def pid_is_missher(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Missher process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_missher(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_missher`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``missher update`` on slow-WMI
