"""Streamlined setup flows: the Nous Portal one-shot (`missher portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``missher_cli.setup`` take effect."""
...
logger = logging.getLogger("missher_cli.setup")
...
def _blank_slate_done(config: dict, missher_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from missher_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         missher skills opt-in --sync",
          "  Add MCP servers:     missher mcp add", *extra, "  Tune agent settings: missher setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from missher_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``missher model``. False when cancelled or failed (the
    message is already printed)."""
    from missher_cli.setup import _info
    try:
        from missher_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``missher setup --portal`` / ``missher portal``)."""
    from missher_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ☤ Missher Setup — Nous Portal (one-shot)             │")
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print_info("  You can retry later with `missher portal`.")

    if not _run_nous_flow(config, context="`missher portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `missher portal`."),
    ...
    _info("  Run `missher portal info` to inspect routing.", "  Run `missher` to start chatting.")


def _run_first_time_quick_setup(config: dict, missher_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'missher setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `missher import`).
        from missher_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    missher setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  missher setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Missher.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Missher update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``missher-agent`` skill
    ...
    try:
        ...
        from missher_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "missher-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("missher-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``missher setup agent``."""
...
def _run_blank_slate_setup(config: dict, missher_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `missher-agent` skill is always kept so the agent",
          "can help you drive and configure Missher itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, missher_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `missher update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, missher_home, "  Enable tools:        missher tools", "  Enable plugins:      missher plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, missher_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `missher-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`missher update` runs from re-injecting them. Opt back in any",
              "time with `missher skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `missher tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `missher plugins list` / `missher plugins install`.",
         "No plugins enabled. Add later with `missher plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `missher mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `missher mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, missher_home, "  Enable more tools:   missher tools")


def _run_quick_setup(config: dict, missher_home):
    ...
    from missher_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'missher setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Missher to messaging apps to chat from anywhere.",
              "You can configure these later with 'missher setup gateway'.")
