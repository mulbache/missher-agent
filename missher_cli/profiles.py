"""Profile management for multiple isolated Missher instances."""
...
from missher_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
from missher_constants import clear_named_profile_deleted, mark_named_profile_deleted, named_profile_is_deleted
...
# stay visible; containers persist HOME state here). See missher_constants.get_subprocess_home().
...
# (``~/.missher``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "missher-agent", ".worktrees", "profiles", "bin", "node_modules",
})

# Per-profile history excluded from --clone-all for ANY source: SQLite session store
# (+wal/shm, can reach many GB), session dirs, `missher backup` archives, quick-backup
...
# Marker written by `missher profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `missher update` all-profile sync, the
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this Missher profile.\n"
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_missher_home().resolve():
...
# Allow-list for ``export_profile("default")``: when MISSHER_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Missher profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``missher_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"missher", "default", "test", "tmp", "root", "sudo"})

# Missher subcommands that cannot be used as profile names/aliases
_MISSHER_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the missher root (NOT the current MISSHER_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_missher_home() / "profiles"


def _get_default_missher_home() -> Path:
    """Default (pre-profile) MISSHER_HOME: ``~/.missher``, or MISSHER_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from missher_constants import get_default_missher_root
    return get_default_missher_root()


def _get_active_profile_path() -> Path:
    return _get_default_missher_home() / "active_profile"
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a Missher-generated wrapper (contains ``missher -p``)."""
    try:
        return "missher -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: missher profile create {canon}")
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its MISSHER_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_missher_home()
    ...
    # profiles/missher may still exist and must keep resolving.
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _MISSHER_SUBCOMMANDS:
        return f"'{canon}' conflicts with a missher subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            missher_exe = shutil.which("missher") or "missher"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(missher_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `missher doctor --fix` surfaces
    ...
    with contextlib.suppress(Exception):
        ...
        from missher_cli.config import check_config_version, migrate_config
        token = set_missher_home_override(str(profile_dir))
        try:
...
# wrappers are a few hundred bytes with the ``missher -p X`` needle near the top; the wrapper
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `missher model` sets it later
        import yaml
        from missher_constants import get_missher_home
        from missher_cli.config import read_user_config_raw
        source = get_missher_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating MISSHER_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from missher_cli.gateway import named_profile_served_by_running_multiplexer
...
# Deliberately tiny and separate from ``config.yaml`` (user-facing Missher config, ~5000
...
def list_profiles() -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_missher_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """``(profile_name, missher_home)`` pairs a gateway should serve — the single chokepoint
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_missher_home())]
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from missher_cli.auth import strip_cloned_single_use_oauth_grants
...
def create_profile(
    ...
    makes two gateways fight over one bot (``missher_cli.profile_channels``; callers list what
    was left behind with ``channel_platforms_configured(source_dir)``).
    ``no_skills`` creates an empty profile and writes a marker so ``missher update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.missher).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and clone_channels:
        from missher_cli.profile_channels import clone_channels_refusal
    ...
    try:
        ...
        if source_dir is not None and not clone_channels:
            from missher_cli.profile_channels import strip_channel_settings
    ...
    # `missher -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def _finish_profile_layout(profile_dir: Path, *, no_skills: bool, clone_all: bool,
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `missher -p <name> auth add`)
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from missher_cli.default_soul import DEFAULT_SOUL_MD
        _seed_file_if_missing(profile_dir / "SOUL.md", DEFAULT_SOUL_MD)

    # Opt-out marker read by seed_profile_skills() and `missher update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`missher profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `missher profile describe` works later
...
def _notify_multiplexer(canon: str) -> None:
    from missher_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
...
def _live_default_multiplexer() -> bool:
    ...
    try:
        from missher_cli.gateway_multiplex_served import recorded_served_profiles
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches MISSHER_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MISSHER_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_missher_home() / ".env"
...
_MISSHER_ARGV_MARKERS = ("missher_cli.main", "missher-gateway", "tui_gateway")
...
# `findOnPath('missher')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "missher".
...
# argv[1] is matched against exact names, not ``startswith("missher")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("missher-notes.py").
_MISSHER_CONSOLE_SCRIPT_NAMES = frozenset({"missher", "missher-agent", "missher-acp"})


def _is_missher_argv(argv: list) -> bool:
    """True for a Missher process: entrypoint marker in argv, executable named ``missher*``,
    or a python interpreter directly exec'ing a known ``missher`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _MISSHER_ARGV_MARKERS) or exe_name.startswith("missher"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _MISSHER_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Missher *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`missher -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_missher_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by MISSHER_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("MISSHER_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.missher).\nTo remove everything, use: missher uninstall")
    ...
    with contextlib.suppress(Exception):
        from missher_state_registry import close_all_under as _close_session_dbs_under
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``MISSHER_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # MISSHER_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("MISSHER_HOME")
    try:
        os.environ["MISSHER_HOME"] = str(profile_dir)
        from missher_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("MISSHER_HOME", None)
        if old_home is not None:
            os.environ["MISSHER_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's missher_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded MISSHER_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from MISSHER_HOME: ``"default"`` when unset or ``~/.missher``, the
    name under ``~/.missher/profiles/<name>``, ``"custom"`` for any other path."""
    from missher_constants import get_missher_home
    resolved = get_missher_home().resolve()
    if resolved == _get_default_missher_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when MISSHER_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_missher_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point MISSHER_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/missher-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".missher-profile-exports", Path(tempfile.gettempdir()) / f"missher-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    MISSHER_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Missher file the user may have lying alongside MISSHER_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``missher
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.missher (dir name ".missher"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: missher profile import <archive> --name <name>"
        )
    ...
    # target ~/.missher itself.
    canon = _canon_valid(inferred_name)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.missher). "
            "Specify a different name: missher profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise FileExistsError(f"Profile '{canon}' already exists at {profile_dir}")
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="missher_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"missher_{old_name}"
    legacy_old_host = f"missher.{old_name}"
    new_host = f"missher_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_missher_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``missher_<old>`` or legacy ``missher.<old>``
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    # ``mkdir_under_missher_home`` does not refuse it) and the periodic reconcile re-adopts the
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MISSHER_HOME path string. Called early in the CLI entry
    point, before missher modules are imported, to set MISSHER_HOME.

    When MISSHER_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match Missher-owned PYTHONPATH
