"""On-demand supply-chain audit for Missher Agent installs.
...
def _discover_plugins(missher_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.missher/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, missher_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = missher_home or Path(get_missher_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `missher security audit`."""
    ...
    components = _discover_components(missher_home=home, **skips)
    ...
    try:
        findings = run_audit(missher_home=home, components=components)
