...
Every Missher process that may need the free tier runs this once at boot (``missher serve`` on a
...
(:func:`missher_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
def start_background_bootstrap() -> threading.Thread:
    """``missher serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
