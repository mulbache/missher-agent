...
# Missher WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: MISSHER_TINI_SHIM_TARGET overrides the `/init` path (and
# MISSHER_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${MISSHER_TINI_SHIM_TARGET:-/init}"
WRAPPER="${MISSHER_TINI_SHIM_WRAPPER:-/opt/missher/docker/main-wrapper.sh}"
