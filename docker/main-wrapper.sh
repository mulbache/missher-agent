#!/bin/sh
# shellcheck shell=sh
# /opt/missher/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching MISSHER_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `missher` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `missher <args>` (subcommand passthrough)
#
# Drop to missher via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${MISSHER_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${MISSHER_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export MISSHER_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset MISSHER_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid missher "$@"; exec "$@"; }
...
# an arbitrary non-root, non-missher UID, the bootstrap was skipped and the
# baked image dirs (owned by the missher build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u missher)" ]; then
    cat >&2 <<EOF
[missher] ERROR: container started with --user $cur_uid (an arbitrary, non-missher UID) — not supported.
...
# to the missher user's home before dropping privileges so libraries that
...
_missher_orig_cwd="${MISSHER_ORIG_CWD:-$PWD}"
...
# the user's command so `missher chat` starts in the Docker -w
# directory, not /opt/data.
cd "$_missher_orig_cwd"
...
# Missher subcommand pass-through.
drop missher "$@"
