...
# 5. Symlinks the 'missher' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☤ Missher Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/missher-uv-install.$$.log")"
...
if is_termux; then
    # Missher currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Missher requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink missher into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up missher command..."

MISSHER_BIN="$SCRIPT_DIR/venv/bin/missher"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MISSHER_BIN" "$COMMAND_LINK_DIR/missher"
echo -e "${GREEN}✓${NC} Symlinked missher → $COMMAND_LINK_DISPLAY_DIR/missher"
...
# ============================================================================
# Seed bundled skills into ~/.missher/skills/
# ============================================================================

MISSHER_SKILLS_DIR="${MISSHER_HOME:-$HOME/.missher}/skills"
mkdir -p "$MISSHER_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.missher/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MISSHER_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  missher status        # Check configuration"
if is_termux; then
    echo "  missher gateway       # Run gateway in foreground"
else
    echo "  missher gateway install # Install gateway service (messaging + cron)"
fi
echo "  missher cron list     # View scheduled jobs"
echo "  missher doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m missher_cli.main setup
fi
