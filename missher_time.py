"""Timezone-aware clock for Missher.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``MISSHER_TIMEZONE`` env var, then ``timezone`` in ``~/.missher/config.yaml``, else server-local
...
# MISSHER_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``MISSHER_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("MISSHER_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without missher_cli importable.
        try:
