"""ACP auth helpers — detect and advertise Missher authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "missher-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Missher runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Missher.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Missher credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Missher using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Missher provider", type="terminal", args=["--setup"],
        description=("Open Missher' interactive model/provider setup in a terminal. "
                     "Use this when Missher has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Missher can resolve any runtime provider credentials."""
