"""ACP session manager — maps ACP sessions to Missher AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.missher/state.db``) so they
...
from missher_constants import get_missher_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when Missher runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["missher-acp"]) if n]
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Missher agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Missher AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.missher/state.db``."""
    ...
    def _get_db(self):
        ...
        error in a minimal test env). ``MISSHER_HOME`` is resolved here, not via the import-time
        ...
        if self._db_instance is None:
            try:
                ...
                self._db_instance = acquire(get_missher_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": _expand_acp_enabled_toolsets(["missher-acp"], mcp_server_names=configured_mcp_servers),
        ...
        # picked up by MissherACPAgent._schedule_mcp_late_refresh.
        try:
