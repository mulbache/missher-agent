"""CLI entry point for the missher-agent ACP adapter.

Loads ``~/.missher/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: missher acp / missher-acp
"""

# IMPORTANT: missher_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See missher_bootstrap.py for full rationale.
try:
    import missher_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``missher update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Missher modules.
...
def _load_env() -> None:
    """Load .env from MISSHER_HOME (default ``~/.missher``)."""
    from missher_cli.env_loader import load_missher_dotenv

    missher_home = get_missher_home()
    loaded = load_missher_dotenv(missher_home=missher_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", missher_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="missher-acp", description="Run Missher Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Missher version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Missher provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.missher/node/ "
...
def _print_version() -> None:
    ...
    print(missher_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import MissherACPAgent  # noqa: F401

    print("Missher ACP check OK")


def _run_setup() -> None:
    from missher_cli.main import main as missher_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "missher", "model"]
        missher_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting missher-agent ACP adapter")
    ...
    import acp
    from .server import MissherACPAgent
    ...
    if os.environ.get("MISSHER_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
