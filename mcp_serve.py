"""
Missher MCP Server — expose messaging conversations as MCP tools (`missher mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Missher-specific
channels_list. Client config: {"mcpServers": {"missher": {"command": "missher", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("missher.mcp_serve")
...
def _missher_home() -> Path:
    try:
        ...
        return Path(os.environ.get("MISSHER_HOME", Path.home() / ".missher"))


def _get_sessions_dir() -> Path:
    return _missher_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_missher_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_missher_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Missher analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Missher MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("missher", instructions=(
        "Missher Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Missher MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
