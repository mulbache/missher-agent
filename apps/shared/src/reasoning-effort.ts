/** Missher' reasoning levels, in ascending order — mirrors the backend's
 *  VALID_REASONING_EFFORTS (missher_constants.py). `none` is not a level: it's
 *  thinking disabled. */
...
/** Missher' built-in level when neither the surface nor the profile config
