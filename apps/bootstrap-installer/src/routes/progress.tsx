...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Missher' : 'Setting up Missher Agent'

  const description = isUpdate
    ? 'Missher is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Missher installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="missher-fade-in flex h-full flex-col">
