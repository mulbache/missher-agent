...
export const $missherHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.missher/logs/bootstrap-installer.log')
    $missherHome.set('~/.missher')
  ...
  try {
    const [logPath, missherHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_missher_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $missherHome.set(missherHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Missher" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Missher-Setup.exe --update);
...
export async function launchMissherDesktop(): Promise<void> {
  ...
  await invoke('launch_missher_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Missher repository' },
