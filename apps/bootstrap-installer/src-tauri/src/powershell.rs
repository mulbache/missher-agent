...
/// read side does not see EOF until the last of them closes it. `missher update`
...
/// stranded with it.
///
/// Same bound `Invoke-MissherStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `missher_home_override` propagates to the child as $MISSHER_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, missher_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = missher_home_override {
        cmd.env("MISSHER_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, missher_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = missher_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_missher_home() {
    ...
    /// child. `missher update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
