...
//! MISSHER_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for MISSHER_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub missher_home: Option<String>,
}
...
/// Spawn the locally-built Missher desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_missher_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_missher_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Missher desktop at {}. The desktop build step \
             may have been skipped or failed. Run `missher desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Missher desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Missher
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_missher_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Missher.exe"),
            ("win-arm64-unpacked", "Missher.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Missher.app/Contents/MacOS", "Missher"),
            ("mac-arm64/Missher.app/Contents/MacOS", "Missher"),
        ]
    } else {
        &[("linux-unpacked", "missher")]
    };
...
pub(crate) fn resolve_missher_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_missher_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Missher.app/Contents/MacOS/Missher -> .../Missher.app
...
/// path so a bare re-open just opens Missher instead of re-running setup.
pub(crate) fn missher_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".missher-bootstrap-complete").exists()
        && resolve_missher_desktop_exe(install_root).is_some()
}
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    // writeFileAtomic(). missher_is_installed() only checks existence, so a
    // partial direct write would incorrectly enable the launcher fast path.
    let tmp_path = install_root.join(".missher-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_missher_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Missher desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_missher_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::missher_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $MissherHome which Stage-Repository clones
    // the repo INTO at $MissherHome\missher-agent. Mirrors missher_constants.
    let missher_home = args
        .missher_home
        .clone()
        .unwrap_or_else(|| crate::paths::missher_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&missher_home).join("missher-agent");
    ...
    // Copy ourselves to MISSHER_HOME/missher-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_missher_home() {
        tracing::warn!(?err, "failed to copy installer into MISSHER_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    missher_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, missher_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Missher"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Missher.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Missher.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("missher");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Missher.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_missher_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_missher_desktop_app(&root)
    ...
    #[test]
    fn resolve_missher_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_missher_desktop_app(&root).is_none(),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".missher-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".missher-bootstrap-complete");
        let tmp_path = root.join(".missher-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    #[test]
    fn missher_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".missher-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".missher-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".missher-bootstrap-complete.tmp").exists(),
