...
//!
//! Mirrors `missher_constants.get_missher_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\missher
//!   macOS:   ~/.missher
//!   Linux:   ~/.missher  (override via $MISSHER_HOME)
//!
//! NOTE (macOS): Python's get_missher_home(), scripts/install.sh, and the
//! Electron desktop's resolveMissherHome() ALL use ~/.missher on macOS — there
...
/// Returns the canonical Missher home directory, respecting $MISSHER_HOME if set.
pub fn missher_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("MISSHER_HOME") {
    ...
    {
        // %LOCALAPPDATA%\missher — matches scripts/install.ps1's $MissherHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("missher");
        }
    }

    // macOS + Linux + fallback: ~/.missher (matches Python get_missher_home(),
    // install.sh, and the Electron desktop's resolveMissherHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".missher");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".missher")
}

pub fn log_dir() -> PathBuf {
    missher_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    missher_home().join("bootstrap-cache")
}
...
/// MISSHER_HOME so it survives repo checkout deletion (unlike anything under
/// missher-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\missher\missher-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_missher`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under MISSHER_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves MISSHER_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    missher_home().join(".missher-update-in-progress")
}
...
pub fn copy_self_to_missher_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to MISSHER_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_MISSHER_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".missher-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under MISSHER_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[missher-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("MISSHER_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_missher_home() -> String {
    missher_home().to_string_lossy().into_owned()
}
