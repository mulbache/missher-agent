...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'missher:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.missherDesktop?.saveImageFromUrl) {
        if (await window.missherDesktop.saveImageFromUrl(src)) {
