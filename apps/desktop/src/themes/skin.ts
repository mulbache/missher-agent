/**
 * Missher skin → DesktopTheme converter.
 *
 * A "skin" is the CLI/TUI theme unit: a YAML file in `$MISSHER_HOME/skins/` (or a
 * built-in) resolved by `missher_cli/skin_engine.py` and pushed to every surface
 ...
 * skin Missher authors from a prompt lights up all three surfaces from one file.
 *
...
import { ensureContrast, mix } from '@missher/shared/color'
import type { MissherSkin, SkinColors } from '@missher/shared/skin'
 ...
 */
export function skinToDesktopTheme(skin: MissherSkin): DesktopTheme | null {
