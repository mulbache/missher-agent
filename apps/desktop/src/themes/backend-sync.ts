/**
 * Live skin sync from the Missher backend.
 *
 * The backend resolves the active skin (built-in or `$MISSHER_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Missher
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { MissherSkin } from '@missher/shared/skin'
...
const BACKEND_THEMES_KEY = 'missher-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: MissherSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as MissherSkin)
