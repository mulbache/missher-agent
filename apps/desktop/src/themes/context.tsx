...
import { ensureContrast, mix, parseColor } from '@missher/shared/color'
...
const SKIN_KEY = 'missher-desktop-theme-v2'
const MODE_KEY = 'missher-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'missher-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'missher-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'missher-desktop-active-profile-v1'
...
// mix / ensureContrast live in @missher/shared/color (shared with the TUI);
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.missherTheme = skinName
  root.dataset.missherMode = rendered
  ...
  window.missherDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('missher-boot-background', chromeBg)
    window.localStorage.setItem('missher-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.missherThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.missherDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Missher authoring/activating a skin from a
