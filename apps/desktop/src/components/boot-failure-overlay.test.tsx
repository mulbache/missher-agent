...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Missher gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.missherDesktop
  Object.defineProperty(window, 'missherDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'missherDesktop', { configurable: true, value: original })
}
