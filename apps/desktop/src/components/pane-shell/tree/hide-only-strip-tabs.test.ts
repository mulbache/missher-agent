...
function sessionsBotsTree() {
  ...
  registerPane('missher-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'missher-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('missher-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('missher-bots:pane')
    expect($hiddenStripTabs.get()).toContain('missher-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('missher-bots:pane')

    expect(setStripTabHidden('missher-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('missher-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('missher-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('missher-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('missher.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('missher-bots:pane')
    ...
    revealTreePane('missher-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('missher-bots:pane')
    expect(window.localStorage.getItem('missher.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'missher-bots:pane', title: 'missher-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'missher-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
