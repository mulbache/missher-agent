...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Missher Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Missher docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Missher Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Missher docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://missher-agent.nousresearch.com/docs' },
    ...
    expect(view.status).toBe('error')
    expect(view.title).toBe('Failed to open missher-agent.nousresearch.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://missher-agent.nousresearch.com/docs' },
        result: { success: true, url: 'https://missher-agent.nousresearch.com/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened missher-agent.nousresearch.com/docs')
  })
})
