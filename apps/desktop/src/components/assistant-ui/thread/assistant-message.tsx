...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's MISSHER_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.missherDesktop?.logsRoot?.()
      ...
      const result = await window.missherDesktop?.openDir?.(root)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {window.missherDesktop?.logsRoot && (
