...
const MISSHER_REF_TYPES = WIRE_REFERENCE_KINDS
type MissherRefType = (typeof MISSHER_REF_TYPES)[number]
...
const MISSHER_DIRECTIVE_RE = referenceRe()
...
export const missherDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(MISSHER_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...missherDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Missher directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.missherDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.missherDesktop?.readFileDataUrl(id)
