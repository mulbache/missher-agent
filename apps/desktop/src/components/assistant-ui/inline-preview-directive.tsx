 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.missher.send(prompt)` (or declarative
 * `data-missher-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'missher-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'missher-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `missher.send(prompt)`.
 ...
 *  wires `data-missher-send` so declarative HTML works with zero script:
 *  `<button data-missher-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.missher={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-missher-send]"):null;' +
    'if(el)send(el.getAttribute("data-missher-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `missher serve` — renders live.)
