import type { GatewayWsUrlResult } from '@missher/shared'
import type { TranslucencyState } from '@missher/shared/translucency'

import type { MissherNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    missherDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`missher --tui
      ...
      // Missher Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new MISSHER_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: MissherApiRequest) => Promise<T>
      notify: (payload: MissherNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<MissherReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<MissherReadFileTextResult>
      selectPaths: (options?: MissherSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<MissherPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<MissherPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<MissherPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: MissherActiveWork) => void
      setTitleBarTheme?: (payload: MissherTitleBarTheme) => void
       ...
       *  (MISSHER_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (MISSHER_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh MISSHER_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      readDir: (path: string) => Promise<MissherReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<MISSHER_HOME>/desktop-plugins),
      ...
      /** LOCAL `<MISSHER_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<MissherGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<MissherGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<MissherGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<MissherRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: MissherReviewScope, baseRef?: null | string) => Promise<MissherReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: MissherReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<MissherReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<MissherRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<MissherPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: MissherTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<MissherTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: MissherWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: MissherPreviewFileChanged) => void) => () => void
...
export interface MissherTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface MissherTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `missher update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  missherRoot?: string
...
export interface MissherConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface MissherTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface MissherActiveWork {
  count: number
  titles: string[]
}

export interface MissherWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Missher Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Missher Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Missher Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the missher gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Missher Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface MissherApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface MissherPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface MissherReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface MissherPreviewWatch {
  id: string
  path: string
}
...
export interface MissherGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface MissherGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface MissherGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface MissherRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface MissherRepoStatus {
  branch: null | string
...
// turn began.
export type MissherReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface MissherReviewFile {
  path: string
  added: number
  removed: number
...
export interface MissherReviewList {
  files: MissherReviewFile[]
...
export interface MissherReviewPr {
  url: string
  state: string
  number: number
}
...
export interface MissherBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface MissherRepoPullRequests {
  ghReady: boolean
  prs: MissherBranchPullRequest[]
}
...
export interface MissherPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface MissherReviewShipInfo {
  ghReady: boolean
  pr: MissherReviewPr | null
}

export interface MissherReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface MissherReadDirResult {
  entries: MissherReadDirEntry[]
  error?: string
}

export interface MissherPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface MissherSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
