import { applyDocumentLocale, isRecord } from '@missher/shared/i18n'
import { createContext, type ReactNode, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'

import { getMissherConfigRecord, type MissherConfigRecord, saveMissherConfig } from '@/missher'
...
export interface I18nConfigClient {
  getConfig: () => Promise<MissherConfigRecord>
  saveConfig: (config: MissherConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.missherDesktop?.api) {
    ...
    return getMissherConfigRecord(undefined, { includeDefaults: false })
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.missherDesktop?.api) {
    ...
    return saveMissherConfig(config, undefined, { preserveLanguage: true })
  }
}

export function getConfigDisplayLanguage(config: MissherConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: MissherConfigRecord, locale: Locale): MissherConfigRecord {
    ...
    const loadLocale = () => {
      ...
      return configClient
        ...
        .then(async config => {
          ...
          const machineProfile = await window.missherDesktop?.getMachineProfile?.().catch(() => null)
