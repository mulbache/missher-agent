...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Missher.
 *
 ...
 * that into an ordinary Missher turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Missher keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Missher) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await missherApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Missher; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await missherApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("Missher is running the tests…"). */
