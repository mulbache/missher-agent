...
const UPDATE_BACKEND = 'Update the Missher backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.missherDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Missher Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.missherDesktop.openExternal(authUrl)
    let pollFailures = 0
