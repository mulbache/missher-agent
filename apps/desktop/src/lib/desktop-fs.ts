import { missherApi } from '@/api/client'
import type {
  MissherConnection,
  MissherReadDirResult,
  MissherReadFileTextResult,
  MissherSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: MissherConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: MissherConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<MissherReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<MissherReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<MissherReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<MissherReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.missherDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: MissherSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
