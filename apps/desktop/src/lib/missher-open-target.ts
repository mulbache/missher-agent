 ...
 * `missher://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `missher://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `missher://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type MissherOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const MISSHER_PROTOCOL = 'missher:'
...
export function normalizeMissherOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('missher://') || trimmed.startsWith(`${MISSHER_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // missher://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // missher://index-network/intent/1 → /index-network/intent/1
...
export function resolveMissherOpenPath(target: MissherOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeMissherOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeMissherOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeMissherOpenString(target.path)
 ...
 * (`missher://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromMissherDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveMissherOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveMissherOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `missher://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromMissherDeepLink('open', name, params)
}
