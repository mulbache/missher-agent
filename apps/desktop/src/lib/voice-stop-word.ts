...
// phrase (optionally addressed to Missher), so a real turn that merely contains
...
// Optional address prefixes so "missher stop" / "ok stop" / "hey hermes, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey hermes', 'hey hermes,', 'missher', 'missher,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("missher") is not a stop command on its own.
      continue
    }
 ...
 * to Missher). Returns false for anything that merely contains "stop" as part of
