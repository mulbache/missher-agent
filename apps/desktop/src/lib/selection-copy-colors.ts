...
// Missher is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.missherMode
 ...
 * the only carriers of Missher' palette — are dropped so the paste target's
