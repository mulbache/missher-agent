...
describe('isVoiceStopCommand', () => {
  ...
  it('matches stop commands addressed to Missher', () => {
    for (const phrase of ['missher stop', 'hey hermes stop', 'hey hermes, stop', 'ok stop', 'okay stop']) {
      expect(isVoiceStopCommand(phrase)).toBe(true)
    }
  })
  ...
  it('does not match bare address words or empty input', () => {
    for (const phrase of ['', '  ', 'missher', 'hey hermes', 'ok', 'okay', 'hey']) {
      expect(isVoiceStopCommand(phrase)).toBe(false)
    }
  })
