...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>Missher Missher</p>')
    performScopedFind(surface, 'missher', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'missher', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>missher</p>')
    performScopedFind(surface, 'missher', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">Missher</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>missher</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'missher')).toBe(true)
  })
