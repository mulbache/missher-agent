...
/** Why a known Missher command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Missher profile', surface: action('profile') },
  {
    name: '/skin',
 ...
 * `missher_cli/commands.py::desktop_surface_registry` by
...
function isKnownMissherSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Missher' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownMissherSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Missher and were never asked for. It is
