...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __missherWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__missherWatch || (win.__missherWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('missher-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('missher-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __missherWatchTag?: number }).__missherWatchTag
