import { JsonRpcGatewayClient } from '@missher/shared'

import type { MissherApiRequest } from '@/global'
...
// "Timed out connecting to Missher backend" that hangs the UI (#48504).
//
...
export class MissherGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Missher gateway connection closed',
      connectErrorMessage: 'Could not connect to Missher gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Missher gateway is not connected',
...
// process to route them to that host (missher:api's registry branch). Null
...
export function missherApi<T>(request: MissherApiRequest): Promise<T> {
  return window.missherDesktop.api<T>({ ...connectionScoped(), ...request })
}
