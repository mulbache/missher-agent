...
const missherApi = vi.mocked(client.missherApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    missherApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'missher-pi' })

    await deleteSession('sess-1', { connectionId: 'missher-pi', profile: 'tommy' })

    expect(missherApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'missher-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((missherApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('omits the profile from the body when none is given', async () => {
    ...
    const req = missherApi.mock.calls[0][0] as { body: Record<string, unknown> }
