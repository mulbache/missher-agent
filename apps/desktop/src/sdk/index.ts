/**
 * @missher/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__MISSHER_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, missherApi, type MissherGateway } from '@/missher'
...
import type { PaginatedSessions, UsageStats } from '@/types/missher'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Missher profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.missherDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.missherDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and MissherGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (missher-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.missherDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Missher Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.missherDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Missher Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Missher Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (missher-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.missherDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('Missher Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return missherApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return missherApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Missher gateway unavailable')
    }
 ...
 *  "MISSHER AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { MissherGateway } from '@/missher'
...
export type { MissherOpenTarget } from '@/lib/missher-open-target'
...
/** Compact labels for the reasoning levels exported from @missher/shared, so a
...
export type { StatusResponse } from '@/types/missher'
/** Public SDK name for the shared gateway wire event; kept stable for plugins. */
export type { GatewayEvent as RpcEvent } from '@missher/shared'
/** THE compact-number formatter — every user-facing count/token figure goes
 *  through here (1230 → "1.2k", 1_500_000 → "1.5M"). Don't hand-roll `/1000`. */
export { compactNumber } from '@missher/shared'
/** Missher' reasoning levels, so a plugin surfacing a thinking depth uses the
...
export { contrastRatio } from '@missher/shared/color'
