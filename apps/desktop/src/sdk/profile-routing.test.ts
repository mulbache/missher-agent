...
vi.mock('@/missher', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), missherApi: vi.fn() }))
...
const { deleteProfile, missherApi } = await import('@/missher')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its MISSHER_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { missherDesktop: unknown }).missherDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(missherApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(missherApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(missherApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
