...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `MissherPlugin` and
...
`$MISSHER_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`missher-desktop-plugins` skill.
