...
Bundled Missher Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@missher/plugin-sdk`, `react`, and
...
`$MISSHER_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Missher theme
...
Stream availability, regional restrictions, and metadata vary. Missher is not
