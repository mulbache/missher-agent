...
import { host, LruCache } from '@missher/plugin-sdk'
...
// to (local, remote URL, SSH, Missher Cloud, docker) must be able to find
...
// missher_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['missher-bots']?.title || profile?.display_name || ''),
