...
import { forgetSessionUnread, host, queryClient } from '@missher/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['missher-bots']
...
/** Permanently delete a bot's Missher profile, then remove plugin-local state
 ...
 * directory (missher-agent#52279). That is the "can't delete a bot" error. */
