...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Missher". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `missher profile rename
  ...
  // never generic "Missher" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Missher (the agent it is)
