...
import * as sdk from '@missher/plugin-sdk'
import { host } from '@missher/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['missher-bots'].chat) were
 ...
 *  click path's tile-staleness probe (missher-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Missher Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (missher-agent#89617) — clicking Retry
  ...
  // until an app restart (missher-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Missher Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['missher-bots']
