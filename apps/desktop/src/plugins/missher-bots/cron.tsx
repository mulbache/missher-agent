/**
 * Routines: the Missher cron jobs scoped to the bot you're chatting with — the
...
// missher -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `missher -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Missher-native schedule string; Advanced exposes it raw.
...
/** Compose the Missher schedule string from picker state. */
