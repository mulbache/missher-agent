...
import { atom, host } from '@missher/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'missher-bots-groups'
...
/** The v3 envelope stored under the default profile's `missher-bots-groups`
 ...
 *  clients (Missher Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Missher"
 *  (community report, Aug 21 2026: renamed default still read "Missher is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Missher (matching displayName and the
 *  @missher handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`missher profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Missher' : trimmed
}
