...
describe('routing', () => {
  ...
  it('resolves @missher to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@missher take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @missher', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @missher,/)
    ...
    expect(peer).toMatch(/group chat with @missher/)
  })
