 ...
 *    a `MissherPlugin` registers automatically (vite glob — drop a folder in).
 *    `missher-bots` (Bot Mode) ships in-tree and is ON by default; other
 ...
 *  - RUNTIME: the on-disk doors (`<missher home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<missher home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: MissherPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default MissherPlugin export — skipped`)
