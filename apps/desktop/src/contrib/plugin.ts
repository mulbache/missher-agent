 ...
 * `MissherPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/missher'
...
export type { PluginRestOptions } from '@/missher'
 ...
 *  under `missher.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.missherDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Missher — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `missher://index-network/intent/1`),
...
export interface MissherPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `missher.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.missherDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.missherDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.missherDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.missherDesktop
