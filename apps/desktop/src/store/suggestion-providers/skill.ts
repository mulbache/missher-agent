 ...
 *  Working in `~/www/missher-agent`, the draft says "missher-agent" constantly
 *  — a "Use skill: missher-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`missher-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/missher-agent-dev`, `plugin:skill`). */
