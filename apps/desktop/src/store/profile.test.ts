...
const { getProfiles } = await import('@/missher')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/missher/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<MissherConnection> = {}): MissherConnection =>
  ({ baseUrl: 'https://missher-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as MissherConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { missherDesktop: { getConnection } })
