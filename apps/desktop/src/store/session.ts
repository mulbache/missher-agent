import type { ConnectionState } from '@missher/shared'
...
import type { SessionInfo, UsageStats } from '@/types/missher'
...
const COMPOSER_MODEL_KEY = 'missher.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'missher.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'missher.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'missher.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'missher.desktop.composer.fast'
...
function composerScopeForConnection(connection: MissherConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'missher.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'missher.desktop.lastRoute'
...
function workspaceCwdKey(connection: MissherConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.missherDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.missherDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<MissherConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'missher.desktop.sessionOwnerHints.v1'
 ...
 * MissherConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<MissherConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useMissherConfig.
