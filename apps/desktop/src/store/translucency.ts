 ...
 * `@missher/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-missher-glass]`
 * block in styles.css).
 */
...
} from '@missher/shared/translucency'
 ...
 * sets `missherDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.missherDesktop?.glassSupported === 'boolean'
    ? window.missherDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.missherDesktop?.translucencySupported === 'boolean'
    ? window.missherDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-missher-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-missher-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-missher-glass-on', glassLive)
  root.toggleAttribute('data-missher-glass', glassOn)
  root.toggleAttribute('data-missher-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-missher-glass-scope', scope)
  } else {
    root.removeAttribute('data-missher-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.missherDesktop?.setTranslucency?.(state)
  })
