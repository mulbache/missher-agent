...
import { checkMissherUpdate, getActionStatus, updateMissher } from '@/missher'
...
import type { BackendUpdateCheckResponse } from '@/types/missher'
...
const UPDATE_TOAST_SNOOZE_KEY = 'missher:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'missher:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'missher:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateMissher'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.missherDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkMissherUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.missherDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `missher update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'missher update',
        command: result.command ?? 'missher update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== missher-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateMissher()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'missher update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkMissherUpdate(true)
...
const FOCUS_RECHECK_KEY = 'missher.updates.last-passive-check'
