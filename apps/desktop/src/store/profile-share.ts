 ...
 *
 * The archive is the CLI's own `missher profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/missher'
...
import type { ProfileDesktopOverlay } from '@/types/missher'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Missher profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.missherDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.missherDesktop?.selectPaths?.({
