...
describe('composer model persistence scope', () => {
  ...
  it('keeps inferred local-primary connections on the historical bare keys', () => {
    ...
    window.localStorage.setItem('missher.desktop.composer.model', 'legacy-model')
    window.localStorage.setItem('missher.desktop.composer.provider', 'legacy-provider')
    ...
    expect(window.localStorage.getItem('missher.desktop.composer.model')).toBe('next-model')
    expect(window.localStorage.getItem('missher.desktop.composer.model.registry.local.default')).toBeNull()
  })
...
describe('session owner hints', () => {
  ...
  it('ignores malformed persisted entries and never throws on hydrate', () => {
    window.localStorage.setItem(
      'missher.desktop.sessionOwnerHints.v1',
      JSON.stringify([
        'junk',
        ['no-route', null],
...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('missher.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('missher.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  ...
  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('missher.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('missher.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('remembered session id (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    ...
    localStorage.setItem('missher.desktop.lastSessionId', 'legacy-session')
    ...
    expect(localStorage.getItem('missher.desktop.lastSessionId')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('missher.desktop.lastSessionId.profile.research%2Fops')).toBe('ops-session')
...
describe('remembered route (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    localStorage.setItem('missher.desktop.lastRoute', '/skills')
    ...
    expect(localStorage.getItem('missher.desktop.lastRoute')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('missher.desktop.lastRoute.profile.research%2Fops')).toBe('/cron')
    expect(getRememberedRoute('research')).toBeNull()
  })
