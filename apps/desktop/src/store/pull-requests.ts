...
import type { MissherBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/missher'
...
export const $pullRequestsByBranch = atom<Record<string, MissherBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'missher.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('missher.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: MissherBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
