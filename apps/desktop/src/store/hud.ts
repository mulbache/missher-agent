 ...
 * composer with the reply scrolling above it, so Missher can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.missherDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.missherDesktop?.hud
...
export function resetHudLayout(): void {
  void window.missherDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.missherDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.missherDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
