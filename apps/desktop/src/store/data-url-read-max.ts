 ...
 * (`missher:data-url-read-max:*`). The default and the clamp are shared with
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.missherDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.missherDesktop?.dataUrlReadMax) {
