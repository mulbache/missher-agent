...
import type { MissherConnection } from '@/global'
import { MissherGateway, setApiRequestConnection } from '@/missher'
...
function dialProfile(
  desktop: NonNullable<typeof window.missherDesktop>,
  profile: string,
...
const isOpen = (gateway: MissherGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: MissherConnection) => void
  /**
...
interface Secondary {
  ...
  connection: MissherConnection | null
  gateway: MissherGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: MissherGateway | null
  ...
  $gateway: ReturnType<typeof atom<MissherGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('missher.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<MissherGateway | null>(null),
...
export function setPrimaryGateway(gateway: MissherGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<MissherConnection, 'connectionId'> | null): void {
...
export function activeGateway(): MissherGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the missher module (null
...
function publishActiveConnection(connection: MissherConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new MissherGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: MissherGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Missher gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.missherDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Missher Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<MissherGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Missher gateway is not connected" on the Sessions `+` action (#88880).
