 ...
 *
 * Local models ship on main behind `--local` (either `missher desktop --local`
 * or the flag on Missher.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.missherDesktop?.localModelsEnabled === true
)
