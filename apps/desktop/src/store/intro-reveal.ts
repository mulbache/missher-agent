 ...
 *
 * This store is the only writer of missher-intro-reveal-seen-v1. First-run eligibility is guest onboarding
...
const SEEN_KEY = 'missher-intro-reveal-seen-v1'
...
export function isIntroRevealEnabled(): boolean {
  return isOnboardingEnabled() && window.missherDesktop?.skipIntro !== true
}
...
export function startIntroReveal(): void {
  ...
  void window.missherDesktop?.introReveal?.open({ hideMain: true }).catch(finishIntroReveal)
}
...
export function finishIntroReveal(): void {
  ...
  void window.missherDesktop?.introReveal?.close({ showMain: true }).catch(() => undefined)
}

export function installIntroRevealBridgeListeners(): () => void {
  const bridge = window.missherDesktop?.introReveal
