...
vi.mock('@/missher', () => ({
  getMissherConfigRecord: mocks.getMissherConfigRecord,
...
afterEach(() => {
  ...
  mocks.getMissherConfigRecord.mockReset()
...
it('shows the toast with Sign in + Disable, then stays quiet for a day and re-nudges after it', async () => {
  ...
  mocks.getMissherConfigRecord.mockResolvedValue(servers)
...
it('coalesces reconnects during a sweep into one fresh follow-up sweep', async () => {
  ...
  mocks.getMissherConfigRecord.mockReturnValueOnce(first).mockResolvedValue({ mcp_servers: {} })

  startMcpHealthChecker()
  mocks.gatewayState.set('open')
  await flush()
  expect(mocks.getMissherConfigRecord).toHaveBeenCalledTimes(1)
  ...
  await flush()
  await flush()
  expect(mocks.getMissherConfigRecord).toHaveBeenCalledTimes(2)
})
