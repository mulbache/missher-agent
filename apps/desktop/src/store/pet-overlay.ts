 ...
 * app's bounds and stays visible while Missher is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'missher.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'missher.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.missherDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.missherDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.missherDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.missherDesktop?.petOverlay?.close()
}
