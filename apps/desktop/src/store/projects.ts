...
import type { MissherGitBaseBranch, MissherGitBranch } from '@/global'
import { getMissherConfig, missherApi, type MissherGateway } from '@/missher'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/missher'
...
const PROJECT_SCOPE_KEY = 'missher.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Missher profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await missherApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<MissherGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<MissherGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Missher
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getMissherConfig(context.profile))
...
// repo, returning where Missher should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<MissherGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<MissherGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.missherDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.missherDesktop?.writeClipboard?.(path)
  }
}
