...
import type { MissherGitWorktree, MissherRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: MissherGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, MissherRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, MissherGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<MissherRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<MissherGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<MissherRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<MissherGitWorktree[]>>()
const $noRepoStatus = atom<MissherRepoStatus | null>(null)
const $noWorktrees = atom<MissherGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<MissherRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<MissherGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: MissherRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: MissherGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<MissherRepoStatus | null>
  seq: number
  target: string
}
