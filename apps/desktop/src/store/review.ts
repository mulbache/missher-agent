...
import type { MissherReviewFile, MissherReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Missher' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'missher.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'missher.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'missher.desktop.reviewTreeMode'
const SELECTED_KEY = 'missher.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<MissherReviewFile[]>([])
...
export const $reviewShipInfo = atom<MissherReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['missherDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: MissherReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly MissherReviewFile[], path: string): MissherReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.missherDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.missherDesktop?.openExternal?.(url)
    }
