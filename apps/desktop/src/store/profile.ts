import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@missher/shared'
import { atom, batch, computed } from 'nanostores'

import type { MissherConnection } from '@/global'
import { getProfiles, missherApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/missher'
...
import type { ProfileInfo } from '@/types/missher'
...
// /api/profiles/active `current`). "default" is the root ~/.missher. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'missher.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'missher.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await missherApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new MISSHER_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.missherDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<MissherConnection | null> {
  const getConnection = window.missherDesktop?.getConnection
...
export async function ensureGatewayProfile(profile: string | null | undefined): Promise<void> {
  ...
  // #89206 split-brain — observed live as atom 'default' over a missher-setup
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<MissherConnection | null> {
  const getConnectionFor = window.missherDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'missher.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.missherDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.missherDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.missher) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.missherDesktop?.touchBackend?.(target).catch(() => undefined)
}
