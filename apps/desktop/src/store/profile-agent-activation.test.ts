...
const agentConn = (over: Partial<MissherConnection> = {}): MissherConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as MissherConnection

const localConn = (over: Partial<MissherConnection> = {}): MissherConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as MissherConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<MissherConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<MissherConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { missherDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<MissherConnection>(() => undefined))
