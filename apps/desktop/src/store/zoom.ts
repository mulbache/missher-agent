...
export function setZoomPercent(percent: number): void {
  window.missherDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.missherDesktop?.zoom) {
  void window.missherDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.missherDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
