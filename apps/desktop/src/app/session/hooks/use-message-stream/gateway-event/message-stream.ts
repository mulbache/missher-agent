import type { BillingBlock } from '@missher/shared'
 ...
 * billing-specific toast — never the generic "Missher error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Missher reported an error',
