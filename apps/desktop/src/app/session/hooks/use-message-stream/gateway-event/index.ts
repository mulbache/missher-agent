...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshMissherConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshMissherConfig — two REST calls (config + defaults) per event, per
