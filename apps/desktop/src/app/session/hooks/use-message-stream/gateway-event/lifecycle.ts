import type { GatewayEvent } from '@missher/shared'
...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'setup.ready') {
    // The boot bootstrap (missher_cli/free_tier_bootstrap.py) resolved the
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (Missher activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as MissherSkin | undefined, { apply: true })
    }
