...
import { getMissherConfig } from '@/missher'
...
import { useMissherConfig } from './use-missher-config'

vi.mock('@/missher', () => ({
  getMissherConfig: vi.fn(),
  getMissherConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'missher.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getMissherConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getMissherConfig>>)

describe('useMissherConfig refreshMissherConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Missher' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useMissherConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshMissherConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useMissherConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getMissherConfig>>>()
    vi.mocked(getMissherConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useMissherConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshMissherConfig(true)
    })
    expect(getMissherConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getMissherConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getMissherConfig>>>()
    vi.mocked(getMissherConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useMissherConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshMissherConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getMissherConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getMissherConfig>>>()
    vi.mocked(getMissherConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshMissherConfig(true)
      refreshC = result.current.refreshMissherConfig(true)
    })
