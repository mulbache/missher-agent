...
import { getMissherConfig, getMissherConfigDefaults } from '@/missher'
...
interface MissherConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useMissherConfig({ activeSessionIdRef }: MissherConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshMissherConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getMissherConfig(), getMissherConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Missher' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshMissherConfig, sttEnabled, voiceMaxRecordingSeconds }
}
