...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/missher'
...
export function SkillsView({
  ...
  const registryBridge = window.missherDesktop?.connections
  const rosterBridge = window.missherDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Missher (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill Missher ships but
