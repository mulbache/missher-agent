...
function setWorkspaceTransfer(transferring: boolean): void {
  window.missherDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.missherDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.missherDesktop?.hud?.beginMove?.()
}
 ...
 * (see missher:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.missherDesktop?.hud?.endMove?.()
      }
