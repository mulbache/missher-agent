...
const MISSHER_PATHS_MIME = 'application/x-missher-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(MISSHER_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(MISSHER_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.missherDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (missher --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.missherDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[missher-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.missherDesktop?.terminal?.write(sessionId, `${command}\r`)
