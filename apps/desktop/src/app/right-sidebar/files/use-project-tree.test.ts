...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { missherDesktop: { readDir: typeof readDir } }).missherDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): MissherReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { missherDesktop: unknown }).missherDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: MissherReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MissherReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: MissherReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: MissherReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: MissherReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MissherReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.missherDesktop is missing', async () => {
    delete (window as unknown as { missherDesktop?: unknown }).missherDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: MissherReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: MissherReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: MissherReadDirResult) => void) | undefined
