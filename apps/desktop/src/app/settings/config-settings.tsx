...
import { getElevenLabsVoices, getMissherConfigSchema, saveMissherConfig } from '@/missher'
...
import type { ConfigFieldSchema, MissherConfigRecord } from '@/types/missher'

import { missherConfigCacheWriter, useMissherConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<MissherConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useMissherConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => missherConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['missher-config-schema'] : ['missher-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getMissherConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `missher config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<MissherConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveMissherConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: MissherConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: MissherConfigRecord) => {
