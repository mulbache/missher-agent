...
import { type ProfileScope, saveMissherConfigRecord } from '@/missher'
 ...
 * the default browser's profile (cookies/logins) into a Missher-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useMissherConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await saveMissherConfigRecord({ browser: { use_real_profile: on } }, profile)
        notify({
          kind: 'info',
