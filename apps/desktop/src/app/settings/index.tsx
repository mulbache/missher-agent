...
import { getMissherConfigDefaults, getMissherConfigRecord, saveMissherConfig } from '@/missher'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getMissherConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'missher-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveMissherConfig(await getMissherConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
