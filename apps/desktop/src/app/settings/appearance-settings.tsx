...
function ResumeLastSessionSetting() {
  ...
  const configQuery = useMissherConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void saveMissherConfig(setNested({}, 'display.resume_last_session', on))
      .then(result => {
        if (!result.ok) {
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.missherDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
