...
import type { MissherConfigRecord, SessionInfo } from '@/types/missher'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.missherDesktop) {
      return
    }

    let alive = true

    void getMissherConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        ...
        await saveMissherConfig({ sessions: { auto_archive: autoArchive, auto_archive_days: archiveDays } })
      } catch (err) {
...
// / Program Files), which buried any files Missher wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.missherDesktop` is
    ...
    const settings = window.missherDesktop?.settings
