...
import { setMissherConfigCache, useMissherConfigRecord } from '../hooks/use-config-record'
...
export function VoiceProviderFields({ section, providerKey }: { section: 'tts' | 'stt'; providerKey: string }) {
  ...
  const { data: loadedConfig } = useMissherConfigRecord()

  const { data: schemaResponse } = useQuery({
    queryKey: ['missher-config-schema'],
    queryFn: () => getMissherConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  const [baseline, setBaseline] = useState<MissherConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveMissherConfig(diffConfig(baseline ?? {}, config))
        .then(() => {
          setBaseline(config)
          setMissherConfigCache(config)
        })
