...
const getMissherConfigSchema = vi.fn()
const saveMissherConfig = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/missher', () => ({
  getMissherConfigRecord: () => getMissherConfigRecord(),
  getMissherConfigSchema: () => getMissherConfigSchema(),
  saveMissherConfig: (config: unknown, profile?: string) => saveMissherConfig(config, profile),
...
beforeEach(() => {
  ...
  getMissherConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getMissherConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await vi.waitFor(() => expect(saveMissherConfig).toHaveBeenCalledTimes(1))
      expect(saveMissherConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveMissherConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveMissherConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
