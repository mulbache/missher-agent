 ...
 * agent sources (local runtime + any number of remote gateways / Missher Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.missherDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.missherDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `missher update` to every eligible source; per-connection results
