...
import type { ToolsetConfig } from '@/types/missher'
...
beforeEach(() => {
  ...
  getMissherConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveMissherConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveMissherConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
