...
import { disconnectOAuthProvider, listOAuthProviders } from '@/missher'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/missher'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.missherDesktop?.terminal)
...
//      in missher_cli/provider_catalog.py) — the SAME provider identity
//      `missher model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Missher can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Missher can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Missher never deletes creds another tool owns behind a silent API call.
