import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@missher/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as MissherGateway | null
  const gatewayRef = useRef<MissherGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.missherDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<MissherGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as MissherGateway | null
      }),
    []
  )
