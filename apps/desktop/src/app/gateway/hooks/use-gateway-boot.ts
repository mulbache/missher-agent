...
import type { DesktopBootProgress, MissherConnection } from '@/global'
import { MissherGateway } from '@/missher'
...
export function primaryRuntimeConnectionId(connection: Pick<MissherConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.missherDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: MissherGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: MissherConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Missher...". Once the
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Missher…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Missher backend'
        )
        ...
        // as an opaque "Could not connect to Missher gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshMissherConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMissherConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startMissher re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new MissherGateway()
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Missher Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen())) {
          throw new Error('Missher gateway is not connected')
        }
    ...
    async function boot() {
      ...
      try {
        ...
        // reaches the recovery affordance instead of hanging "Starting Missher…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMissherConfig(),
          ...
          // retry it), not brick boot behind the "Missher couldn't start"
