...
vi.mock('@/missher', async importOriginal => {
  const actual = await importOriginal<typeof MissherModule>()

  class FakeMissherGateway {
  ...
  return { ...actual, MissherGateway: FakeMissherGateway }
})

import type * as MissherModule from '@/missher'
import type { MissherGateway } from '@/missher'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as MissherGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as MissherGateway, 'default')
  $gateway.set(primary as unknown as MissherGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.missherDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as MissherGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { missherDesktop: unknown }).missherDesktop = { getConnection }
    $gateway.set(dropped)
