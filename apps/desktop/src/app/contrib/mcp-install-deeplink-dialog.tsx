...
import { getMissherConfigRecord, saveMcpServers } from '@/missher'
...
import { setMissherConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `missher://mcp/install` deep links. The payload is
 ...
 * confirming lets Missher spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getMissherConfigRecord())
      ...
      setMissherConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
