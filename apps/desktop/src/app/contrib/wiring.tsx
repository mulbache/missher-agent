...
import { getLatestSessionMessages } from '@/missher'
...
import { useMissherConfigRecord } from '../hooks/use-config-record'
...
import { useMissherConfig } from '../session/hooks/use-missher-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // Without this pin, the owner ladder sends session.create to missher-setup
  ...
  const { refreshMissherConfig, sttEnabled, voiceMaxRecordingSeconds } = useMissherConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('missher:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('missher:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshMissherConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshMissherConfig])
  ...
  const configRecord = useMissherConfigRecord()
