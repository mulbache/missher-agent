...
import { pathFromMissherDeepLink, resolveMissherOpenPath } from '@/lib/missher-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Missher" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.missherDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.missherDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.missherDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.missherDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // missher://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.missherDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future missherDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveMissherOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.missherDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (missher://index-network/intent/1, missher://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromMissherDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.missherDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.missherDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.missherDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.missherDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
