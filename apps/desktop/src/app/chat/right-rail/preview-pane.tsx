        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.missherDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.missherDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Missher to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.missherDesktop?.watchPreviewFile ||
      !window.missherDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.missherDesktop.onPreviewFileChanged(payload => {
    ...
    void window.missherDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.missherDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.missherDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:missher-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.missherDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.missherDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.missherDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.missherDesktop?.openExternal(currentUrl)
                : undefined
            }
