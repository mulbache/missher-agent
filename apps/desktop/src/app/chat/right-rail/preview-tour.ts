...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__missherTourEngine) {
    ${driverIife}
    w.__missherTourHolder = {};
    w.__missherTourCollect = (${collectTourTargets.toString()});
    w.__missherTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__missher-tour-style')) {
    var style = document.createElement('style');
    style.id = '__missher-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__missherTourEngine(
    w.driver.js.driver,
    w.__missherTourHolder,
    ${JSON.stringify(action)},
    w.__missherTourCollect,
    document
  ));
})()`
}
