...
const preamble = () => `  var w = window;
  ...
  w.__missherActHolder = w.__missherActHolder || {};
  w.__missherAct = ${actEngineSource()};
  w.__missherWatch_fn = (${watchInPage.toString()});
  w.__missherWatchTag = ${WATCH_TAG};
  var holder = w.__missherActHolder;
  var act = function (a) { return w.__missherAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__missherWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__missherHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'MISSHER-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
