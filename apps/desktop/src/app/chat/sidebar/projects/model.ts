...
import type { SessionInfo } from '@/missher'
...
// zero Missher sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, MissherGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, MissherGitWorktree[]>>({})
