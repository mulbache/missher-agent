...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Missher doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in missher_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `missher model` uses — instead of
...
export interface MissherConfig {
  agent?: {
...
export type MissherConfigRecord = Record<string, unknown>
...
// non-desktop) Missher simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (missher.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors missher_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Missher, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Missher but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `missher tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Missher); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** missher-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** missher-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/missher/update/check` — the backend's own update state.
