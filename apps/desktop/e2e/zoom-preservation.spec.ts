...
async function readZoomPercent(): Promise<number> {
  return fixture!.page.evaluate(async () => {
    const desktop = window as unknown as {
      missherDesktop: { zoom: { get: () => Promise<{ percent: number }> } }
    }

    return (await desktop.missherDesktop.zoom.get()).percent
  })
}
...
async function setZoomPercent(percent: number): Promise<void> {
  await fixture!.page.evaluate(target => {
    const desktop = window as unknown as {
      missherDesktop: { zoom: { setPercent: (percent: number) => void } }
    }

    desktop.missherDesktop.zoom.setPercent(target)
  }, percent)
