 ...
 * packaged binary) with a real `missher serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → missher serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Missher title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Missher')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { missherDesktop?: Record<string, unknown> }).missherDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → missher serve is spawned → WS connects → config
