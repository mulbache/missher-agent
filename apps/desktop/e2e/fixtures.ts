/**
 * Shared E2E fixtures for the Missher desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → missher serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `missher desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `missher-e2e-${prefix}-${Math.random()}`))
  const missherHome = path.join(root, 'missher-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(missherHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(missherHome, 'config.yaml')
 ...
 */
export function writeEnvFile(missherHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(missherHome, '.env')
 ...
 */
function writeEmptyConfig(missherHome: string): void {
  const configPath = path.join(missherHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - MISSHER_HOME → sandbox missher-home (isolated config/sessions)
 *  - MISSHER_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - MISSHER_DESKTOP_IGNORE_EXISTING=1 → don't pick up `missher` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - MISSHER_DESKTOP_MISSHER_ROOT → REPO_ROOT (dev checkout resolution)
 *  - MISSHER_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    MISSHER_HOME: sandbox.missherHome,
    MISSHER_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    MISSHER_DESKTOP_IGNORE_EXISTING: '1',
    MISSHER_DESKTOP_MISSHER_ROOT: REPO_ROOT,
    MISSHER_DESKTOP_APP_NAME: `MissherE2E-${Date.now()}`,
    ...
    MISSHER_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated MISSHER_HOME + userData
 * @param env      - the process environment (already has MISSHER_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.missherHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.missherHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via MISSHER_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`missher serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.missherHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { MISSHER_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Missher backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Missher.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Missher.app', 'Contents', 'MacOS', 'Missher')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'missher')
}
 ...
 * progress without spawning a real Missher backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * MISSHER_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    MISSHER_DESKTOP_BOOT_FAKE: '1',
    MISSHER_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + missher-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).MISSHER_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).MISSHER_DESKTOP_MISSHER
  delete (env as Record<string, string | undefined>).MISSHER_DESKTOP_MISSHER_ROOT
