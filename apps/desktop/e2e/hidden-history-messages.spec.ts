...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.missherHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.missherHome)
  const builder = await RealSessionBuilder.start(sandbox.missherHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.missherHome, mock.url)
  fs.appendFileSync(path.join(sandbox.missherHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
