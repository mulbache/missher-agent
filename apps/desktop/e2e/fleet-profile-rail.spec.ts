 ...
 * second gateway, "Homelab", is a REAL second `missher serve` this spec spawns
 * with its own MISSHER_HOME, profiles and session token, registered in the v2
...
function findMissherBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'missher')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['missher'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('missher binary not found: create the repo venv (uv sync) or put missher on PATH')
}
...
/**
 * Spawn a second, fully real `missher serve` as the remote gateway. Its
 * session token is pinned through MISSHER_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        MISSHER_HOME: home,
        MISSHER_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote missher serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Missher-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote missher serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.missherHome, mock.url)
    ...
    seedProfiles(sandbox.missherHome, ['research'])
