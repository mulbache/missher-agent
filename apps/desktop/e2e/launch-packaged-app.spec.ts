...
/**
 * E2E smoke tests for the packaged Missher desktop app.
 *
 ...
 * (credential stripping, isolated MISSHER_HOME + userData, unique app name).
 *
...
test('window opens with the Missher title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      missherDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).missherDesktop?.hud?.open({ sessionId: null })
  )
