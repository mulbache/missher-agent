...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Missher E2E'], { cwd: repo })
...
function configureRepoCwd(missherHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(missherHome, mockUrl)
  fs.appendFileSync(path.join(missherHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(missherHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.missherHome, mock.url, repo)
