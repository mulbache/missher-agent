...
//   D=$(mktemp -d) && env -u MISSHER_SHARED_AUTH_DIR MISSHER_GUEST_ONBOARDING=1 MISSHER_SKIP_INTRO=1 \
//     MISSHER_HOME="$D/home" MISSHER_DESKTOP_USER_DATA_DIR="$D/userdata" missher desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'missher-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.MISSHER_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set MISSHER_DESKTOP_PYTHON to your installed Missher Python executable.')
...
for (const dir of ['.missher/shared', 'electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, MISSHER_HOME: path.join(sandbox, '.missher'), MISSHER_SHARED_AUTH_DIR: path.join(sandbox, '.missher/shared'),
  MISSHER_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), MISSHER_DESKTOP_CWD: path.join(sandbox, 'work'),
  MISSHER_DESKTOP_MISSHER_ROOT: root, MISSHER_DESKTOP_PYTHON: python,
  MISSHER_DESKTOP_IGNORE_EXISTING: '1', MISSHER_DESKTOP_APP_NAME: 'Missher Connector Rehearsal',
  MISSHER_GUEST_ONBOARDING: '1', MISSHER_SKIP_INTRO: '1', MISSHER_DESKTOP_DEV_SERVER: url, MISSHER_DESKTOP_CDP_PORT: '9344'
})
