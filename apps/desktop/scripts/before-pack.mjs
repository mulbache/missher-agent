 ...
 * product name (`Missher`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Missher'
 *
 * This is a hard failure with no obvious cause for the user — `missher desktop`
 ...
 * (the mac-unpacked Missher.app bundle) and Windows (win-unpacked), so we
 ...
 * Missher.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `missher desktop --build-only` (missher_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Missher.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Missher'}.exe`
