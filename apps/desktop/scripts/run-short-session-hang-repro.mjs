...
const UPSTREAM_URL = 'https://github.com/NousResearch/missher-agent.git'
...
const LEGACY_STORAGE_KEY = 'missher.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'missher.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.missherDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `missher-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const missherHome = join(sandbox, 'missher-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(missherHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(missherHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        MISSHER_DESKTOP_APP_NAME: `MissherShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        MISSHER_DESKTOP_MISSHER_ROOT: target.targetRoot,
        MISSHER_DESKTOP_IGNORE_EXISTING: '1',
        MISSHER_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'missher-short-session-ab-'))
  const prepared = []
