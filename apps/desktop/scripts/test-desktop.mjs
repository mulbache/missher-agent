...
// no longer bundles the Missher Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Missher.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Missher'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Missher.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Missher'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default MISSHER_HOME for non-sandboxed runs -- matches main.ts's
// resolveMissherHome(). On Windows it's %LOCALAPPDATA%\missher; elsewhere
// it's ~/.missher. The fresh-install sandbox launchFresh() sets its own
// MISSHER_HOME and never touches this.
const DEFAULT_MISSHER_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'missher')
  }
  return path.join(os.homedir(), '.missher')
})()
const VENV_ROOT = path.join(DEFAULT_MISSHER_HOME, 'missher-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'missher-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.MISSHER_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Missher-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Missher-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Missher-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Missher-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.MISSHER_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.MISSHER_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const missherHome = path.join(sandbox, 'missher-home')
  ...
  env.MISSHER_DESKTOP_CWD = cwd
  env.MISSHER_DESKTOP_IGNORE_EXISTING = '1'
  env.MISSHER_DESKTOP_TEST_MODE = 'fresh-install'
  env.MISSHER_DESKTOP_USER_DATA_DIR = userDataDir
  env.MISSHER_HOME = missherHome
  delete env.MISSHER_DESKTOP_MISSHER
  delete env.MISSHER_DESKTOP_MISSHER_ROOT
  ...
  console.log(`  MISSHER_HOME: ${missherHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(missherHome, 'missher-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Missher Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of missher_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'missher-agent', 'missher_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Missher
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + MISSHER_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  MISSHER_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
