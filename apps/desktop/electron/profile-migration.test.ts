 ...
 *      top of startMissher() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.missher/active_profile',
    missherHome: '/home/u/.missher',
    profilesRoot: '/home/u/.missher/profiles',
    existsSync: fs.existsSync,
    readFileSync: fs.readFileSync,
    statSync: fs.statSync,
    readdirSync: fs.readdirSync,
    isMissherProcess: () => true,
    now: () => NOW,
    writeJson: () => {},
    isValidProfileName,
    ...overrides
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.missher/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.missher/profiles', ['coder'], {
      ...fs,
      isMissherProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-missher) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live missher process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.missher/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.missher/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isMissherProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.missher/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.missher/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.missher/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.missher/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.missher/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.missher/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isMissherProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.missher/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.missher/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isMissherProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.missher/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.missher/profiles/default': { dir: true },
    '/home/u/.missher/profiles/coder': { dir: true },
    '/home/u/.missher/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.missher/profiles/default': { dir: true },
    '/home/u/.missher/profiles/some-file.txt': { dir: false },
    '/home/u/.missher/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $MISSHER_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.missher/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.missher/profiles/coder': { dir: true },
    '/home/u/.missher/profiles/writer': { dir: true },
    '/home/u/.missher/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.missher/active_profile': { content: 'coder' },
    '/home/u/.missher/profiles/coder': { dir: true },
    '/home/u/.missher/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.missher/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.missher/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.missher/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.missher/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.missher/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.missher/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isMissherProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.missher, not ~/.missher/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at missherHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.missher', '/home/u/.missher/profiles'), '/home/u/.missher/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.missher', '/home/u/.missher/profiles'),
    '/home/u/.missher/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at missherHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.missher', '/home/u/.missher/profiles'),
    '/home/u/.missher/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.missher', '/home/u/.missher/profiles'),
    '/home/u/.missher/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at missherHome', () => {
  const fs = makeFs({
    '/home/u/.missher/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.missher/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.missher/profiles', ['default', 'coder'], {
      ...fs,
      missherHome: '/home/u/.missher',
      isMissherProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.missher/profiles/<name>, never scored ~/.missher/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.missher/profiles/conduit': { dir: true },
    '/home/u/.missher/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.missher/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.missher/profiles/work': { dir: true },
    '/home/u/.missher/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.missher/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.missher/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.missher/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isMissherProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
