// The standalone desktop-plugin root (`<MISSHER_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.missher-package.json` marker
...
export const PACKAGE_MARKER = '.missher-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.missher-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every missher home the app knows about locally: the default plus each profile. */
export async function localHomes(missherHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(missherHome, 'profiles'))

  return [missherHome, ...profiles.map(name => path.join(missherHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(missherHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(missherHome, 'profiles'))) {
    const scopedRoot = path.join(missherHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(missherHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(missherHome)) {
