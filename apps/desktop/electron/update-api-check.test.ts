...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/missher-agent.git'), 'someone/missher-agent')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('nousresearch/missher-agent', 'bb/gui'),
    'https://api.github.com/repos/nousresearch/missher-agent/commits/bb%2Fgui'
  )
})
