 ...
 * console. It writes MISSHER_HOME/.missher-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Missher within 30 minutes.
...
export function handoffResultPath(missherHome: string): string {
  return path.join(missherHome, '.missher-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(missherHome)
  let raw: string
