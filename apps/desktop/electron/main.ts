...
import { buildDesktopBackendEnv, missherManagedNodePathEntries, normalizeMissherHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForMissherReady } from './backend-health'
import { backendCommandMatches, createBackendOwnership, createBackendShutdownCoordinator } from './backend-ownership'
import {
  canImportMissherCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
  shouldTrustMissherOverride,
  verifyMissherCli
} from './backend-probes'
...
import { isMissherOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvMissherCommand
} from './windows-missher-path'
import {
...
const USER_DATA_OVERRIDE = process.env.MISSHER_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.MISSHER_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.MISSHER_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with MISSHER_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[missher] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[missher] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. MISSHER_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[missher] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[missher] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via MISSHER_DESKTOP_PASSWORD_STORE, bridged by the
// `missher desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[missher] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[missher] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `missher desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Missher sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[missher] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[missher] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[missher] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[missher] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[missher] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('missher:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Missher burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[missher] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[missher] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[missher] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[missher] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// MISSHER_HOME — the user-facing root for everything Missher-related. Mirrors
// scripts/install.ps1's $MissherHome and scripts/install.sh's $MISSHER_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\missher (matches install.ps1)
//   macOS / Linux: ~/.missher (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.missher directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\missher yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// MISSHER_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// MISSHER_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.missher / %LOCALAPPDATA%\missher.
function resolveMissherHome() {
  if (process.env.MISSHER_HOME) {
    return normalizeMissherHomeRoot(process.env.MISSHER_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'missher-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a MISSHER_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\missher and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('MISSHER_HOME')

    if (fromRegistry) {
      return normalizeMissherHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'missher')
    const legacy = path.join(app.getPath('home'), '.missher')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.missher setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.missher')
}

const MISSHER_HOME = resolveMissherHome()

function pathWithMissherManagedNode(...entries) {
  const managed = missherManagedNodePathEntries(MISSHER_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_MISSHER_ROOT — the canonical mutable Missher install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_MISSHER_ROOT = path.join(MISSHER_HOME, 'missher-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_MISSHER_ROOT, 'venv')
...
// boots, going straight to `resolveMissherBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_MISSHER_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_MISSHER_ROOT, '.missher-bootstrap-complete')
...
// active-profile.json records which Missher profile the desktop launches its
// local backend as. When set, startMissher() passes `missher --profile <name>
// dashboard …`, which deterministically pins MISSHER_HOME (see
// _apply_profile_override in missher_cli/main.py) and bypasses the sticky
// ~/.missher/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors missher_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// missherDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under MISSHER_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by missher_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(MISSHER_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (missher_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.MISSHER_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.MISSHER_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.MISSHER_DESKTOP_SKIP_QUIT_CONFIRM === '1'
// Nous free tier gate, decided ONCE here and stamped onto every backend spawn
// (desktopBackendSpawnEnv) and the renderer (missher:launch-flags).
const GUEST_ONBOARDING = guestOnboardingEnabled()
const SKIP_INTRO = skipIntroEnabled()

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.MISSHER_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.MISSHER_DESKTOP_APP_NAME || 'Missher'
...
// 'missher:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.missher) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.missher')
}

// Seed the native About panel with the live Missher version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `missher update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveMissherVersion(),
...
// startMissher(); this pool only holds EXTRA profile
...
// The legacy MISSHER_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.MISSHER_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.MISSHER_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Missher backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `missher:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.MISSHER_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startMissher() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning missher serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Missher backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  missherLog.push(...lines)

  if (missherLog.length > 300) {
    missherLog.splice(0, missherLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-missher-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('missher:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the missher:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('missher:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_MISSHER_ROOT
  })
}
...
// The Tauri updater writes MISSHER_HOME/.missher-update-in-progress for the whole
...
// (`force_kill_other_missher`, taskkill /IM missher.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(MISSHER_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--missher-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `missher update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Missher will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(MISSHER_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Missher update did not finish',
        `${result.message}\n\nDetails: ${path.join(MISSHER_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Missher to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `missher` — must not shadow `missher.cmd`/`missher.exe`.
...
function unwrapWindowsVenvMissherCommand(command, backendArgs) {
  return resolveVenvMissherCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `missher serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `missher` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'missher_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `missher --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, MISSHER_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveMissherBackend step 4); execFileSync of a .cmd without
...
function isMissherSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'missher_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.MISSHER_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Missher deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the missher-import
...
// findGitBash — locate bash.exe on Windows. Resolves MISSHER_GIT_BASH_PATH
...
// normal MISSHER_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\missher\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'missher', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'missher', 'git', 'bin', 'git.exe'))
  }
...
function recentMissherLog() {
  return missherLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's missher root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_MISSHER_ROOT.
// MISSHER_DESKTOP_MISSHER_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.MISSHER_DESKTOP_MISSHER_ROOT && path.resolve(process.env.MISSHER_DESKTOP_MISSHER_ROOT),
    !IS_PACKAGED && isMissherSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isMissherSourceRoot(ACTIVE_MISSHER_ROOT) ? ACTIVE_MISSHER_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_MISSHER_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('missher:updates:progress', merged)
  }
}
...
// #105855 was a run of GitHub outages that read as a Missher bug because the
// UI hid the cause.
    ...
    const req = https.get(
      url,
      {
        headers: {
          ...
          'User-Agent': 'missher-desktop-update-check'
        },
        timeout: 10_000
      },
      res => {
...
// Windows that binary owns ALL repo mutation — running `missher update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(MISSHER_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `missher update` can write
// fresh entry points. On Windows this is the file the running backend
// `missher.exe` holds open; on POSIX it's never mandatory-locked.
function venvMissherShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'missher.exe')
    : path.join(updateRoot, 'venv', 'bin', 'missher')
}
...
// Kill only Missher-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `missher`,
...
function killMissherOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isMissherOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Missher-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `missher.exe` that
// spawned its own grandchildren (a `missher` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Missher backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Missher backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.missherBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Missher backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.missherBackendIdentity
...
// `missherProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `missher.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a missher
// REPL, a pty terminal, the gateway) keep `missher.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const missherProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\missher.exe, but the
  // backend is `python.exe -m missher_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (missherProcess && Number.isInteger(missherProcess.pid)) {
    initialPids.push(missherProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(missherProcess, {
  ...
  // `missher gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvMissherShimPath(updateRoot), MISSHER_HOME)

  // Reap Missher-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killMissherOwnedVenvDaemons(updateRoot)

  const shim = venvMissherShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentMissherProcess = backendConnectionState.getProcess()

        if (currentMissherProcess && Number.isInteger(currentMissherProcess.pid)) {
          stragglers.push(currentMissherProcess.pid)
        }
...
// `missher update`). Instead we spawn the staged Missher-Setup binary with
// --update and quit, so it can run `missher update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `missher update` → relaunch),
      ...
      // in: the MISSHER_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `missher update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `missher desktop`, never the Tauri installer that self-copies
      // missher-setup.exe into MISSHER_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `missher` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `missher update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `missher update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, missherRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(MISSHER_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Missher — this window will close and the updater will open. Don’t reopen Missher yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(MISSHER_HOME, rememberLog)
    ...
    // missher.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running missher, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Missher install open ' +
        '(a second Missher window or a terminal running missher?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvMissherShimPath(updateRoot))
      }
    ...
    // Missher processes running from this venv.  The updater normally refuses
    ...
    // `missher update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `missher update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          MISSHER_HOME,
          MISSHER_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithMissherManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `missher update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(MISSHER_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Missher update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(MISSHER_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Missher will keep running — try again, or run \`missher update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvMissher = path.join(venvBin, IS_WINDOWS ? 'missher.exe' : 'missher')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Missher launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the missher
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.missher-bootstrap-complete')),
      hasVenvMissher: fileExists(venvMissher),
...
// Python-level pre-update snapshot inside `missher update`.
function preflightStateDb(missherHome, rememberLog) {
  const stateDbPath = path.join(missherHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(missherHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(missherHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(missherHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `missher update`, swaps/relaunches the app bundle, and writes
// .missher-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'missher update', percent: null })

    return { ok: true, manual: true, command: 'missher update', missherRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      MISSHER_HOME,
      MISSHER_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithMissherManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Missher — this window will close. Don’t reopen Missher yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_MISSHER_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isMissherSourceRoot(ACTIVE_MISSHER_ROOT) &&
    fileExists(venvPython) &&
    canImportMissherCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_MISSHER_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `missher
...
function resolveWebDist() {
  const override = process.env.MISSHER_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set MISSHER_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: missher desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: missher desktop --force-build`
  )
...
function resolveMissherCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Missher.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.MISSHER_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveMissherCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'missher_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_MISSHER_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Missher at ${ACTIVE_MISSHER_ROOT}`,
    command,
    args: ['-m', 'missher_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      missherHome: MISSHER_HOME,
      pythonPathEntries: [ACTIVE_MISSHER_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_MISSHER_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveMissherBackend(backendArgs) {
  // 1. Explicit override -- MISSHER_DESKTOP_MISSHER_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.MISSHER_DESKTOP_MISSHER_ROOT && path.resolve(process.env.MISSHER_DESKTOP_MISSHER_ROOT)

  if (overrideRoot && isMissherSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Missher source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `missher` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isMissherSourceRoot.)
  if (!IS_PACKAGED && isMissherSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Missher source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_MISSHER_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\missher\\missher-agent (Windows) or ~/.missher/missher-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Missher runtime at ${ACTIVE_MISSHER_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `missher` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    MISSHER_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.MISSHER_DESKTOP_IGNORE_EXISTING !== '1') {
    let missherCommand = null
    const missherOverride = process.env.MISSHER_DESKTOP_MISSHER

    if (missherOverride) {
      const resolvedOverride = findOnPath(missherOverride)

      if (resolvedOverride) {
        missherCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(missherOverride, { isWsl: IS_WSL })) {
        missherCommand = missherOverride
      } else {
        rememberLog(`Ignoring Windows Missher override under WSL: ${missherOverride}`)
      }
    } else {
      missherCommand = findOnPath('missher')
    }

    if (missherCommand) {
      if (looksLikeDesktopAppBinary(missherCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Missher CLI: ${missherCommand}`)
        missherCommand = null
      }
    }

    if (missherCommand) {
      const unwrapped = unwrapWindowsVenvMissherCommand(missherCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `missher` shim
      ...
      const shellForProbe = isCommandScript(missherCommand)

      // MISSHER_DESKTOP_MISSHER is an explicit deployment override (used by
      ...
      if (shouldTrustMissherOverride(missherOverride) || verifyMissherCli(missherCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvMissherCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Missher CLI at ${missherCommand}`,
          command: missherCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Missher CLI at ${missherCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed missher_cli module via system Python.
  ...
  if (python) {
    ...
    // having missher_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\missher\missher-agent\venv.
    if (canImportMissherCli(python)) {
      return {
        kind: 'python',
        label: `installed missher_cli module via ${python}`,
        command: python,
        args: ['-m', 'missher_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: missher_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveMissherBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Missher Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_MISSHER_ROOT,
...
async function runEnsureRuntime(backend: any): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolveMissherBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Missher install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Missher recovery was handed off to Missher Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      missherHome: MISSHER_HOME,
      logRoot: path.join(MISSHER_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Missher install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Missher bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(MISSHER_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startMissher() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // missher:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveMissherBackend(backend.args))
  }
  ...
  if (!isMissherSourceRoot(ACTIVE_MISSHER_ROOT)) {
    throw new Error(
      `Missher install at ${ACTIVE_MISSHER_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Missher' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\missher\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `missher` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Missher on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Missher.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isMissherSourceRoot()
    // plus an importable missher_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Missher venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Missher at ${ACTIVE_MISSHER_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Missher runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----missher-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Missher backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Missher backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Missher backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Missher-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Missher-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('missher:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveMissherCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('missher:preview-file-changed', payload)
}
...
// waitForMissherReady how to read a 401 — rejected session vs gated route.
...
async function waitForMissher(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForMissherReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('missher:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('missher:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('missher:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('missher:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Missher...".
function sendPowerResume() {
  ...
  webContents.send('missher:power-resume')
}
...
// 'missher:power-battery' push below.
ipcMain.handle('missher:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('missher:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(MISSHER_HOME, app.getPath('userData'))
  } catch (err) {
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('missher:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('missher:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('missher:context-menu-spellcheck', {
...
//
// Hosted Missher gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``missher_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / missher #37247).
//     Both are set as HttpOnly cookies (``missher_session_at`` ~15 min,
//     ``missher_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startMissher → the renderer's
// single-shot boot() with no retry) surfaced as the "Missher couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Missher
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Missher Cloud agent…' : 'Sign in to Missher gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Missher backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in missher_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Missher Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:missher-remote-oauth`:
...
// convention (missher_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Missher surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.MISSHER_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Missher gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for missher_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Missher Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Missher Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Missher Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Missher Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Missher Cloud. Open Settings → Gateway, choose Missher Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Missher Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Missher Cloud session has expired. Sign in to Missher Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Missher Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every missher:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteMissherPath`/`remoteProfile` —
...
// a valid stored value (pins the root MISSHER_HOME explicitly); null means "no
...
// contains "missher", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isMissherProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('missher')
  } catch {
...
//   1. Legacy ~/.missher/active_profile (explicit CLI choice via missher profile use)
//   2. Running gateway (gateway.pid with verified liveness + missher identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(MISSHER_HOME, 'active_profile'),
    missherHome: MISSHER_HOME,
    profilesRoot: path.join(MISSHER_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.MISSHER_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.MISSHER_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Missher Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteMissherPath: (ssh || savedSsh)?.remoteMissherPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by MISSHER_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Missher Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Missher Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteMissherPath: input.sshRemoteMissherPath ?? existingBlock.remoteMissherPath,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Missher gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.MISSHER_DESKTOP_REMOTE_TOKEN,
      url: process.env.MISSHER_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      missherPath: result.missherPath,
      missherHome: result.missherHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { missherPath: result.missherPath, missherHome: result.missherHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteMissherPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteMissherPath: sshConfig.remoteMissherPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForMissher: (baseUrl, token) => waitForMissher(baseUrl, token, lease.signal, 'token'),
      ...
      // spawn command adds MISSHER_GUEST_ONBOARDING=1 only when this is on.
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        missherVersion: result.missherVersion || '',
        ...
        // per-profile pools also use bare keys. Only startMissher' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.missherVersion || 'missher (version unknown)'} at ${result.missherPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteMissherVersion: result.missherVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (MISSHER_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.MISSHER_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startMissher() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Missher
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteMissherPath: input.sshRemoteMissherPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteMissherPath || '')
          let missherPath
          let missherVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            missherPath = runtime.missherPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.missherPath])
            missherVersion = inspection.version
            supported = inspection.supported
          } else {
            missherPath = await remoteLifecycle.locateMissher(ssh, sshConfig.remoteMissherPath || '')
            missherVersion = await remoteLifecycle.probeMissherVersion(ssh, missherPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, missherPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Missher on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteMissherPath: missherPath,
            remoteMissherVersion: missherVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startMissher())
  ...
  // connect to Missher gateway". Mirror the renderer's connect here so the test
...
function resetMissherConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startMissher() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const missherProcess = backendConnectionState.invalidate()
  stopBackendChild(missherProcess)
...
// startMissher() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetMissherConnection() invalidates it.
  const missherProcess = backendConnectionState.getProcess()
  const dying = missherProcess && !missherProcess.killed ? missherProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('missher:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('missher:connections:changed', payload)
    }
  }
}
...
// startMissher() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(profile, opts: { passive?: boolean; spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startMissher()
  ...
  // spawning, so two children never share one profile's MISSHER_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteMissherPath: source.remoteMissherPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. missher:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: missherLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForMissher(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.missherPath || !state?.missherHome) {
  ...
  return {
    ...
    missherPath: state.missherPath,
    missherHome: state.missherHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteMissherPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.missherPath ? platform : await probeWindowsRemote(ssh, config.remoteMissherPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          missherPath: runtime.missherPath,
          missherHome: runtime.missherHome,
          pythonPath: runtime.python
        }
      }
    }

    const missherPath = await remoteLifecycle.locateMissher(ssh, config.remoteMissherPath || '')
    const missherHome = await remoteLifecycle.probeRemoteMissherHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, missherPath, missherHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { missherPath: state.missherPath, missherHome: state.missherHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForMissher(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startMissher() but without the boot-progress UI,
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForMissher(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited MISSHER_HOME env (see _apply_profile_override
  // step 3 in missher_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveMissherBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const missherCwd = resolveMissherCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(MISSHER_HOME, 'profiles', key))
  )
  rememberLog(`Starting Missher backend for profile "${profile}" via ${backend.label}`)
      ...
      env: desktopBackendSpawnEnv(
        {
          ...
          TERMINAL_CWD: missherCwd,
          MISSHER_DASHBOARD_SESSION_TOKEN: token,
          ...
          ...parentIdentityEnv,
          MISSHER_WEB_DIST: webDist,
          ...(readyFile ? { MISSHER_DESKTOP_READY_FILE: readyFile } : {})
        },
        GUEST_ONBOARDING
      ),
      shell: backend.shell,
  ...
  child.once('error', error => {
    rememberLog(`Missher backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Missher backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Missher backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Missher backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForMissher(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Missher backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Missher backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_missher_home() recreates the deleted profile directory.
//
...
function startMissher() {
  return localBackendLifecycle.start(runMissherStart)
}

async function runMissherStart() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Missher Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startMissher() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Missher backend', 8)
  ...
  // later missher:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Missher backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Missher backend at ${remote.baseUrl}`, 24)
      await waitForMissher(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Missher backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, missherLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.missher/active_profile file) and
    // resolves MISSHER_HOME the same way `missher -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Missher runtime', 28)

        return resolveMissherBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Missher backend via ${backend.label}`, 84)
    rememberLog(`Starting Missher backend via ${backend.label}`)
    ...
    const missherProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: missherCwd,
        env: desktopBackendSpawnEnv(
          {
            ...process.env,
            // Explicitly pin MISSHER_HOME for the child so Python's get_missher_home()
            // resolves to the SAME location our resolveMissherHome() picked. Without
            // this pin, Python falls back to ~/.missher on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\missher, which differs from C:\Users\<u>\.missher.
            // Mismatch would split config / sessions / .env / logs across two
            // directories. install.ps1 sets MISSHER_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(missherProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(missherProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, missherProcess)

    if (!processOwner) {
      stopBackendChild(missherProcess)
      await waitForBackendExit(missherProcess)
      releaseBackendChild(missherProcess)
      throw new Error('Missher backend start was superseded by a newer connection attempt.')
    }

    missherProcess.stdout.on('data', rememberLog)
    missherProcess.stderr.on('data', rememberLog)
    ...
    missherProcess.once('error', error => {
      releaseBackendChild(missherProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Missher backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Missher backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Missher backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Missher backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    missherProcess.once('exit', (code, signal) => {
      releaseBackendChild(missherProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Missher backend exit (${signal || code})`)
      ...
      rememberLog(`Missher backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Missher backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Missher backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentMissherLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Missher backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Missher backend to become ready', 90)
    await Promise.race([waitForMissher(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => missherProcess.exitCode === null && !missherProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Missher backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Missher backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed MISSHER_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startMissher() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `missher:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('missher:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Missher is
...
// pushes pet state over IPC (missher:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via missher:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Missher app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Missher icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see missher:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Missher in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('missher:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Missher can be driven
...
// left the user with NO Missher window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('missher:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('missher:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[missher] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('missher:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `missher:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, missher:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('missher:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('missher:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('missher:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'missher desktop --force-build',
  ...
  startMissher().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('missher:connection', async (_event, profile, extra) => {
...
ipcMain.handle('missher:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'missher:connection', keyed by the composite
...
ipcMain.on('missher:connection:active-route', (event, route) => {
...
// "Starting Missher…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('missher:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetMissherConnection({ soft: true }),
...
ipcMain.handle('missher:backend:touch', async (_event, profile) => {
...
ipcMain.handle('missher:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('missher:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('missher:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('missher:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('missher:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('missher:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('missher:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveMissherBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Missher is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `missher-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, MISSHER_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('missher:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('missher:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('missher:zoom:get', event => {
...
ipcMain.on('missher:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('missher:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('missher:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startMissher() call restarts the
...
ipcMain.handle('missher:bootstrap:repair', async () => {
  // Forceful repair: force the next startMissher() through the full installer
  ...
  // "bypass active runtime" path inside startMissher() and fall through
  ...
  resetMissherConnection()

  return { ok: true }
})
ipcMain.handle('missher:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('missher:bootstrap:cancel', async () => {
...
ipcMain.handle('missher:boot-progress:get', async () => bootProgressState)
ipcMain.handle('missher:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('missher:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('missher:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('missher:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('missher:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('missher:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('missher:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('missher:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('missher:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('missher:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('missher:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('missher:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('missher:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('missher:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('missher:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteMissherPath: entry.remoteMissherPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startMissher()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteMissherPath: connection.remoteMissherPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteMissherProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root MISSHER_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('missher:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// missher:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('missher:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('missher:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `missher update` to every eligible registered connection at once.
...
ipcMain.handle('missher:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/missher/update', {}, { timeoutMs: 15_000 })
...
// analogue of the missher:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('missher:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('missher:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startMissher() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('missher:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Missher Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('missher:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('missher:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('missher:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('missher:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of missher:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('missher:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('missher:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new MISSHER_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('missher:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('missher:requestMicrophoneAccess', async () => {
...
ipcMain.handle('missher:window:readBelow', async event => {
...
async function handleMissherApiRequest(request) {
  ...
  // backend calls ensure_missher_home() which recreates the profile directory,
...
ipcMain.handle('missher:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its MISSHER_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleMissherApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleMissherApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('missher:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('missher:data-url-read-max:get', () => ({
...
ipcMain.handle('missher:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('missher:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('missher:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('missher:readFileText', async (_event, filePath) => {
...
// `missher:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('missher:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('missher:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('missher:writeClipboard', (_event, text) => {
...
ipcMain.handle('missher:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('missher:readClipboard', () => clipboard.readText())

ipcMain.handle('missher:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('missher:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('missher:context-menu:edit', (event, command) => {
...
ipcMain.handle('missher:context-menu:copy-image', event => {
...
ipcMain.handle('missher:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('missher:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('missher:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('missher:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('missher:savePastedText', async (_event, payload) => {
...
ipcMain.handle('missher:saveClipboardImage', async () => {
...
ipcMain.handle('missher:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('missher:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('missher:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('missher:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('missher:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('missher:titlebar-theme', (_event, payload) => {
...
ipcMain.on('missher:native-theme', (_event, mode) => {
...
ipcMain.on('missher:translucency:support', event => {
...
// it arrives from `missher desktop --local` or directly on Missher.exe (a
// shortcut edit), and survives self-relaunches because collectRelaunchArgs
// only strips internal flags.
ipcMain.on('missher:launch-flags', event => {
  event.returnValue = {
...
ipcMain.on('missher:translucency', (_event, payload) => {
...
ipcMain.on('missher:keep-awake', (_event, on) => {
...
ipcMain.handle('missher:quick-entry:settings:get', async () => {
...
ipcMain.handle('missher:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('missher:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('missher:quick-entry:submit', {
...
ipcMain.on('missher:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('missher:quick-entry:state', payload)
  }
})

ipcMain.on('missher:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('missher:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('missher:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `missher:window:openSession`).
...
ipcMain.handle('missher:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('missher:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('missher:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('missher:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveMissherCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('missher:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveMissherCwd()
}))

ipcMain.handle('missher:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('missher:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('missher:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('missher:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('missher:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('missher:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('missher:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: missherLog.slice(-200) }))
...
ipcMain.on('missher:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (missher:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('missher:updates:check', async (_event, opts) =>
...
ipcMain.handle('missher:updates:apply', async (_event, payload) =>
...
ipcMain.handle('missher:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('missher:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Missher version (the one `release.py` bumps in
// missher_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Missher version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveMissherVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'missher_cli', '__init__.py')
...
// Renderer-bundle skew: `missher update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Missher version and push it into the native About panel
// just before showing it, so an in-place `missher update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveMissherVersion()} — app build out of date, update the desktop app`
        : resolveMissherVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('missher:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveMissherVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    missherRoot: resolveUpdateRoot(),
...
// The About page's "Restart Missher" button (shown when bundleSwapPending):
...
ipcMain.handle('missher:app:relaunch', async () => {
...
ipcMain.handle('missher:machine:profile', async () => {
...
// the actual removal via `missher uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (missher_cli/uninstall.py + missher_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_MISSHER_ROOT
  ...
  const fallback = () => ({
    missher_home: MISSHER_HOME,
    agent_installed: isMissherSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'missher_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, MISSHER_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Missher agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import missher_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_MISSHER_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_MISSHER_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `missher-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `missher-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('missher:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('missher:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('missher:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('missher:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// missher:// deep links (e.g. missher://blueprint/morning-brief?time=08:00,
// missher://mcp/install?name=NAME&config=B64 — the vendor "Add to Missher"
// button, or missher://plugin/install?repo=owner/repo). Dev
// (`MISSHER_DESKTOP_DEV_SERVER`) registers missher-dev:// instead — bare
// Electron or a stale OS handler often owns missher:// on dev machines.
...
// ---------------------------------------------------------------------------
const MISSHER_PROTOCOL = DEV_SERVER ? 'missher-dev' : 'missher'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['missher-dev', 'missher'] : ['missher']
...
function handleDeepLink(url) {
  ...
  // missher://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('missher:deep-link', payload)
...
ipcMain.handle('missher:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${MISSHER_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(MISSHER_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(MISSHER_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${MISSHER_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `missher://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startMissher), whose
...
app.whenReady().then(() => {
  ...
  // startMissher() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching missher:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Missher never keeps another app's chord hostage.
