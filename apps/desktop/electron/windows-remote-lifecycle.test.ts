...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    missherHome: 'C:\\Users\\andre\\.missher',
    python: 'C:\\Users\\andre\\.missher\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.missher-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      missherPath: 'C:\\Missher\\missher.exe',
      missherHome: 'C:\\Users\\andre\\.missher',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command missher.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          missherHome: 'C:\\Users\\alice\\.missher',
          missherPath: 'C:\\Missher\\missher.exe',
          python: 'C:\\Missher\\python.exe'
        })
      }

      if (script.includes('.missher-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('missher_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.missher\\profiles\\research')
  assert.match(script, /\.missher-update-in-progress/)
...
test('Windows probe validates Missher and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $missherHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Missher is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Missher is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Missher's\\python.exe" }, 'inspect', [
    'C:\\x y\\missher.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'missher_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Missher''s/)
  assert.match(script, /C:\\x y\\missher\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { missherPath: lock.missherPath, missherHome: lock.missherHome }
