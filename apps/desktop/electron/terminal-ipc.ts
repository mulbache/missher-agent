...
// registry, and the missher:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // MISSHER_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.MISSHER_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Missher' TUI into
    ...
    env.TERM_PROGRAM = 'Missher'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a missher/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from MISSHER_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.MISSHER_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `missher:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('missher:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('missher:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('missher:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('missher:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('missher:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('missher:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
