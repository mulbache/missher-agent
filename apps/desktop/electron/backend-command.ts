// Backend subcommand routing for the desktop-managed Missher process.
//
// The desktop app launches its own headless backend via `missher serve` — it
...
// app hasn't updated yet, or an older `missher` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Missher profile to pin via `--profile`.
 */
 ...
 * `-m missher_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `missher_cli/subcommands/dashboard.py` source registers
