...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.missher`). Default's state.db and gateway.pid live here. */
  missherHome: string
  /** Named-profile root (`~/.missher/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isMissherProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `missherHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, missherHome: string, profilesRoot: string): string {
  return name === 'default' ? `${missherHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, missherHome: string, profilesRoot: string): string {
  return name === 'default' ? `${missherHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveMissherHome(profilesRoot: string, missherHome?: string): string {
  if (missherHome) {
    return missherHome
  }

  // Tests that predate missherHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live missher process.
 ...
 *
 * `missherHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isMissherProcess'> & { missherHome?: string }
): string[] {
  const missherHome = resolveMissherHome(profilesRoot, deps.missherHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, missherHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.missherHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$MISSHER_HOME`, not `$MISSHER_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `missher --profile default` and is worse than writing nothing
