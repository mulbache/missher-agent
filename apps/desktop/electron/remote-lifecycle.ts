 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Missher dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Missher install on the remote (login-shell probe),
 ...
 *     `MISSHER_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForMissher in.
 *
 * The minted MISSHER_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.missher/desktop-ssh'
...
// Resolve the remote missher executable. An EXPLICIT path is honored strictly
...
async function locateMissher(ssh, remoteMissherPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The missher binary or wrapper script
    ...
    //     the Missher version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/NousResearch/missher-agent/issues/74411
    return candidate
  }
  ...
  if (remoteMissherPath) {
    if (await isExecutable(remoteMissherPath)) {
      return resolveLauncher(remoteMissherPath)
    }

    const err: any = new Error(
      `The Missher path you set is not an executable on the remote host: "${remoteMissherPath}". ` +
        'Check the path (it must be the full path to the `missher` binary on the remote, e.g. ' +
        '~/missher-agent/.venv/bin/missher), or clear it to auto-detect.'
    )

    err.kind = 'missher-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v missher')}`)).trim()
  ...
  candidates.push('~/.local/bin/missher')
  candidates.push('/usr/local/bin/missher')
  candidates.push('~/.missher/missher-agent/venv/bin/missher')
  ...
  const err: any = new Error(
    'Missher is not installed on the remote host (could not find a `missher` executable). ' +
      'Install it on the remote with:  curl -fsSL https://missher-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Missher path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<missher> --version`,
// e.g. "Missher Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH missher a
// connection uses, so a stale/unexpected install is visible.
async function probeMissherVersion(ssh, missherPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(missherPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Missher Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The MISSHER_HOME the remote dashboard will use (explicit env wins, else
// ~/.missher). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteMissherHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${MISSHER_HOME:-$HOME/.missher}"')).trim().split('\n').pop()

    return out || '~/.missher'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Missher home.')
...
marker=home/'.missher-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Missher checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, missherHome) {
  const home = assertSafeRemoteHome(missherHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Missher install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Missher update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Missher update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteMissherProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteMissherHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Missher profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Missher home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'missherPath', 'missherHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !missherPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(missherPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/missher`, leaving neither
      // launcher nor MISSHER_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `missher_home=os.path.expanduser(${shq(missherHome)}) if ${shq(missherHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if missher_home:\n' +
      ' expected_entries.add(os.path.join(missher_home,"missher-agent","venv","bin","missher"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.missherPath)
  const expectedHome = lock.missherHome ? expandRemotePath(lock.missherHome) : "''"
  ...
  const executableMatch = lock.missherHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.missherPath === expected.missherPath &&
    lock.missherHome === expected.missherHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.missherPath)})
missher_home=os.path.expanduser(${py(lock.missherHome)})
expected_entries={expected_path,os.path.join(missher_home,"missher-agent","venv","bin","missher")}
...
// execing Missher. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"missher-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(missherPath, profile, opts: any = {}) {
  const missher = expandRemotePath(missherPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.missherHome || '~/.missher')}/.missher-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.missherHome || '~/.missher')}/.missher-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env MISSHER_DESKTOP=1${opts.guestOnboarding === true ? ' MISSHER_GUEST_ONBOARDING=1' : ''} ${missher} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} missher-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, missherPath) {
  const missher = expandRemotePath(missherPath)

  const out = await ssh.exec(
    `help="$(${missher} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, missherPath))) {
    const err: any = new Error(
      'The remote Missher install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Missher on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(missherPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        missherHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForMissher, a token-gated
...
async function connect(deps) {
  ...
  const missherHome = await probeRemoteMissherHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, missherHome)
  const missherPath = await locateMissher(ssh, remoteMissherPath)
  log(`located missher at ${missherPath}`)
  const missherVersion = await probeMissherVersion(ssh, missherPath)

  if (missherVersion) {
    log(`remote missher version: ${missherVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Missher Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.missherPath === missherPath &&
      lock.missherHome === missherHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    missherHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, missherHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForMissher(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
