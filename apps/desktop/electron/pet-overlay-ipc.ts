...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('missher:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('missher:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('missher:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('missher:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('missher:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('missher:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('missher:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('missher:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('missher:pet-overlay:control', payload)
  })
}
