...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(MISSHER_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Missher-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.missher/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'missher-desktop-ssh')
  }

  return path.join(os.homedir(), '.missher', 'desktop-ssh')
}
