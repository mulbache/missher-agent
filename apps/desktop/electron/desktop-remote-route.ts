...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'MISSHER_DESKTOP_REMOTE_URL is set but MISSHER_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Missher backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `missher serve` the desktop never
