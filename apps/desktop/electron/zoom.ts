...
export const ZOOM_STORAGE_KEY = 'missher:desktop:zoomLevel'
 ...
 * but forgot to emit 'missher:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('missher:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `missher:zoom:changed`, which never fired —
