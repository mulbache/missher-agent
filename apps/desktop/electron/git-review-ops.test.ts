...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'missher-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'missher-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Missher Test'], { cwd: dir })
