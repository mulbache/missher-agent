 ...
 *   1. MISSHER_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\missher\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect MISSHER_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.MISSHER_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'missher', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'missher', 'git', 'usr', 'bin', 'bash.exe'))
  }
