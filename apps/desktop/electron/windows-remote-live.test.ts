...
//   MISSHER_WIN_SSH_HOST   ssh alias/host of the Windows box
//   MISSHER_WIN_SSH_USER   remote user
//   MISSHER_WIN_SSH_MISSHER absolute path to the remote missher.exe under test
const liveHost = process.env.MISSHER_WIN_SSH_HOST || ''
const liveUser = process.env.MISSHER_WIN_SSH_USER || ''
const configuredMissher = process.env.MISSHER_WIN_SSH_MISSHER || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Missher-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredMissher)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredMissher.replace('missher.exe', 'python.exe')}' -m missher_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredMissher.replace('missher.exe', 'python.exe')
        const terminate = `& '${python}' -m missher_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.missherPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m missher_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m missher_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
