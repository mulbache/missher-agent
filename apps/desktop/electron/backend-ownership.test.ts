...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'missher serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/missher serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m missher_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/missher --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Missher Runtime\\missher.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('missher chat --query serve'), false)
