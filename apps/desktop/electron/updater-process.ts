 ...
 *
 * The staged Tauri `missher-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `missher update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `missher update`, swaps/relaunches
 * the app, and writes .missher-update-result.json. With the app gone before
 * the update starts, the MISSHER_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `missher update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into MISSHER_HOME on *every* platform
 * (`missher-setup.exe` on Windows, `missher-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_missher_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `missher update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(missherHome, 'missher-setup.exe')
 ...
 *
 * `copy_self_to_missher_home` deliberately no-ops during `--update`
 ...
 * "Another Missher update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `missher update`, which
 ...
 * the installer is written to MISSHER_HOME at install/repair time, making mtime
