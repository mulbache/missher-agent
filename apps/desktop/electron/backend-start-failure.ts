 ...
 * startMissher() re-throw the cached error without re-attempting the connect —
...
/**
 * Whether a startMissher() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startMissher`, re-emits
