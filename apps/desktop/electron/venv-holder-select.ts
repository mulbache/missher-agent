 ...
 *
 * The pre-update handoff kills Missher-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `missher`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Missher-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Missher and holds venv shims mapped).
 */
export function isMissherOwnedVenvDaemon(
