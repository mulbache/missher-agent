...
export function registerHudIpc({
  ...
  ipcMain.on('missher:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('missher:hud:windowing', event => {
  ...
  ipcMain.on('missher:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('missher:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('missher:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('missher:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('missher:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('missher:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('missher:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('missher:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('missher:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('missher:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('missher:hud:close', async () => {
    closeHudWindow()
