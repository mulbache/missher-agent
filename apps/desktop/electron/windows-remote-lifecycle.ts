...
const READY_RE = /^MISSHER_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitMissherPath = '') {
  const explicit = psLiteral(explicitMissherPath)
  ...
  const script = [
    ...
    '$missherHome=$env:MISSHER_HOME',
    'if(-not $missherHome){$missherHome=Join-Path $env:LOCALAPPDATA "missher"}',
    'Assert-NoReparse $missherHome $true',
    '$candidate=[IO.Path]::Combine($missherHome, "missher-agent\\venv\\Scripts\\missher.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "missher-agent\\.venv\\Scripts\\missher.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $missherHome "missher-agent\\venv\\Scripts\\missher.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "missher-agent\\.venv\\Scripts\\missher.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command missher.exe -ErrorAction SilentlyContinue',
    ...
    '$missher=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$missher=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $missher){throw "Missher is not installed on the remote Windows host."}',
    'Assert-NoReparse $missher $false',
    'if($explicit -and $missher -ne $explicit){throw "The configured Missher path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($missher), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;missherHome=$missherHome;missherPath=$missher;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(missherHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class MissherMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(missherHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".missher-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[MissherMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, missherHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(missherHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitMissherPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitMissherPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'missher_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'missher_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'missher_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.missherHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".missher-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};missherPath=${psLiteral(reservation.missherPath)};missherHome=${psLiteral(reservation.missherHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.missherPath === 'string' &&
    typeof lock.missherHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.missherPath === runtime.missherPath &&
    lock.missherHome === runtime.missherHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteMissherPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.missherHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.missherPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Missher on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.missherPath = inspection.path
  const missherVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located missher at ${runtime.missherPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, missherPath: runtime.missherPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForMissher(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Missher SSH"', 'powershell.exe -NoLogo')
