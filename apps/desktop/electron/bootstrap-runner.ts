 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Missher Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.missher-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(missherHome) {
  return path.join(missherHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.missher/missher-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(missherHome) {
  if (!missherHome) {
    return null
  }

  const candidate = path.join(missherHome, 'missher-agent', 'scripts', installScriptName())
...
function cachedScriptPath(missherHome, commit) {
  return path.join(bootstrapCacheDir(missherHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/missher-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(missherHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(missherHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, missherHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass MISSHER_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          MISSHER_HOME: missherHome || process.env.MISSHER_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, missherHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, missherHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--missher-home', missherHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, missherHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, missherHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(missherHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, missherHome, emit })
    abortSignal?.throwIfAborted()
