...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'missher-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/missher-runner-test',
    sourceRepoRoot: null,
    missherHome: '/tmp/missher-runner-test',
    logRoot: '/tmp/missher-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'missher-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'missher-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/missher-agent',
      missherHome: '/tmp/missher'
    }),
    ['--dir', '/tmp/missher-agent', '--missher-home', '/tmp/missher', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/missher-agent', '--missher-home', '/tmp/missher', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/missher',
      missherHome: '/tmp/home'
    }),
    ['--dir', '/tmp/missher', '--missher-home', '/tmp/home', '--branch', 'main']
  )
})
