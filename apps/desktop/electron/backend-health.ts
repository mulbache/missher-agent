...
export interface MissherReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForMissherReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Missher gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForMissherReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Missher Cloud instance
 ...
 * `isReauthRequired` so startMissher latches; a bare `needsOauthLogin` (the
...
export async function waitForMissherReady(baseUrl: string, options: MissherReadyOptions): Promise<void> {
  ...
  throw new Error(`Missher backend did not become ready: ${detail}`)
}
