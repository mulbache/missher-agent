 ...
 *   - the browser sees only a minimal "return to Missher" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Missher.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('missher:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('missher:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('missher:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
