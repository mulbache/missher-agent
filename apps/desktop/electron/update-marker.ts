...
export function markerPath(missherHome) {
  return path.join(missherHome, '.missher-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(missherHome)
  let raw
 ...
 *
 * The Tauri-based missher-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(missherHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(missherHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Missher to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(missherHome, opts)
