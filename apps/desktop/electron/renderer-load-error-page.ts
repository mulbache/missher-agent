 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `missher update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `missher desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Missher couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>missher desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>missher desktop --force-build</code>, then restart the app.</p>
