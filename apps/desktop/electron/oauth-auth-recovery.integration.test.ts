...
async function fixture(beforeRefreshResponse?: () => Promise<void>) {
  const home = mkdtempSync(join(tmpdir(), 'missher-auth-recovery-'))
...
test('HTTP outage, dead-refresh and cookie coexistence reach the correct ticket/readiness/media verdicts', async () => {
  ...
  try {
    ...
    try {
      await waitForMissherReady(f.baseUrl, {
    ...
    const mediaRequest = { url: 'missher-media://remote/%2Ftmp%2Fclip.mp4', headers: new Headers(), method: 'GET' }
