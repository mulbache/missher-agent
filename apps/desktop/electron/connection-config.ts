 ...
 *     `X-Missher-Session-Token` header; WS uses `?token=`.
...
// missher_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - missher_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - missher_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / missher #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-missher_session_at', '__Secure-missher_session_at', 'missher_session_at']
const RT_COOKIE_VARIANTS = ['__Host-missher_session_rt', '__Secure-missher_session_rt', 'missher_session_rt']

// The Nous portal (NAS) does NOT use Missher gateway session cookies — it is a
...
// Keep this aligned with missher_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['missher', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startMissher latches
...
/** Which Missher profile the remote SSH dashboard should actually run as.
...
// Missher Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteMissherPath = String(entry.remoteMissherPath || '').trim()

  if (remoteMissherPath) {
    out.remoteMissherPath = remoteMissherPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Missher installation. Preserve an explicit mapping
  // when it is a valid Missher profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_missher_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `MISSHER_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process MISSHER_HOME can belong to a different
 ...
 * `missher:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://missher.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `missher:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a missher session ACCESS-token cookie
 ...
 * the Missher gateway session cookies, so cloud sign-in / discovery liveness
