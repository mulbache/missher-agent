...
// hardening, MISSHER_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('missher:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('missher:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('missher:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('missher:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<MISSHER_HOME>/desktop-plugins`,
  // resolved from the main-process MISSHER_HOME (see resolveMissherHome) — NOT from
  // the connected backend. A remote backend reports its own `missher_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(missherHome, 'profiles', profile) : missherHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(missherHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(missherHome, root)
    await reconcileUnifiedDesktopHalves(missherHome, root)

    return root
  }

  ipcMain.handle('missher:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('missher:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(missherHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(missherHome, root)
  })

  // The LOCAL logs root (`<MISSHER_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where MISSHER_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('missher:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('missher:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('missher:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('missher:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('missher:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('missher:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
