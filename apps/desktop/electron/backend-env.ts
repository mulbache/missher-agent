...
/**
 * Missher-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\missher\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$MISSHER_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_missher_node_dirs()` in missher_constants.py, which the Electron main
 * process cannot import.
 */
function missherManagedNodePathEntries(
  missherHome,
  ...
  const root = pathModule.join(missherHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const missherNodeDirs = missherManagedNodePathEntries(missherHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([missherNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeMissherHomeRoot(missherHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!missherHome) {
    return missherHome
  }

  const resolved = pathModule.resolve(String(missherHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). missher_bootstrap sets this inside the child too, but only
