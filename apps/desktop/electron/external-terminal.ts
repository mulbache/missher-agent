// Launching the Missher TUI in the user's OWN terminal emulator.
//
...
// `missher --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Missher runtime is often a venv Python invoked
//     as `python -m missher_cli.main`, not a `missher` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Missher
