 ...
 *
 * The Python side (missher_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes MISSHER_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(missherHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(missherHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(missherHome: string, userData: string): PendingNotice | null {
  const report = readReport(missherHome)
