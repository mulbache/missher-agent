  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        ...
        // if it reaches reapOrphans (see main.ts startMissher + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:missher(?:\.exe)?|missher_cli\.main|missher_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
