...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/missher', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/missher/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/missher'), 'https://gw.example.com/missher/auth/native/refresh')
})
