// Profile-delete routing logic for the `missher:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_missher_home() and
...
/** Parse a `missher:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its MISSHER_HOME.
 *
...
/**
 * Route the next `missher:api` request away from the primary/window backend
 ...
 * ensure_missher_home() recreates the directory the delete just removed.
 */
