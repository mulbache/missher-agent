/**
 * windows-missher-path.ts
 *
 * Pure, dependency-injected pieces of Windows `missher` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvMissherCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `missher` shim shadowed the real
 *      missher.cmd/missher.exe; the shim then failed the --version probe and
 ...
 *      and Missher launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvMissherCommand() — unwrapWindowsVenvMissherCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `missher`) must
 * not shadow `missher.cmd`/`missher.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Missher launcher that drives `missher update`.
...
export interface ResolveVenvMissherCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportMissherCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `missher`/`missher.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/missher(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m missher_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportMissherCli(). A
 * venv whose update died mid-`pip install` still has python.exe + missher.exe
 ...
 *
 * Returns null when `command` is not a venv missher shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvMissherCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvMissherCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^missher(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Missher at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Missher Python at ${python}`,
    command: python,
