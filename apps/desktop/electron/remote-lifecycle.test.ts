...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.missher-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Missher', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('MISSHER_HOME')) {
          return '/home/alice/.missher\n'
        }

        if (command.includes('.missher-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.missher/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.missher-update-in-progress/)
})
...
test('listRemoteMissherProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/MISSHER_HOME/, '/Users/zillajr/.missher\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteMissherProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteMissherProfiles rejects a hostile MISSHER_HOME', async () => {
  const ssh = fakeSsh([[/MISSHER_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteMissherProfiles(ssh),
    (err: any) => {
...
test('locateMissher prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/missher/, 'OK']])
  assert.equal(await locateMissher(ssh, '/opt/missher'), '/opt/missher')
})

test('locateMissher throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong missher" bug.
  const ssh = fakeSsh([
    [/command -v missher/, '/home/u/.local/bin/missher\n'],
    [/\[ -x .*\.local\/bin\/missher/, 'OK']
  ])

  await assert.rejects(
    () => locateMissher(ssh, '/bad/path/missher'),
    (err: any) => {
      assert.equal(err.kind, 'missher-not-found')
      assert.match(err.message, /\/bad\/path\/missher/)
...
test('locateMissher falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v missher/, '/home/u/.local/bin/missher\n'],
    [/\[ -x .*\.local\/bin\/missher/, 'OK']
  ])

  assert.equal(await locateMissher(ssh, ''), '/home/u/.local/bin/missher')
})

test('locateMissher preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$MISSHER_BIN" "$MISSHER_ENTRYPOINT" "$@",
  // where $MISSHER_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.missher/missher-agent/venv/bin/python\n']
  ])

  assert.equal(await locateMissher(ssh, ''), '/home/u/.local/bin/missher')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateMissher must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateMissher returns an explicit remoteMissherPath unchanged', async () => {
  // The override half of #74411: an explicit remoteMissherPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/missher changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/missher/, 'OK'],
    [/python3 -c/, '/home/u/.missher/missher-agent/venv/bin/python\n']
  ])

  assert.equal(await locateMissher(ssh, '~/.local/bin/missher'), '~/.local/bin/missher')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteMissherPath must never be rewritten')
})

test('locateMissher falls back to ~/.local/bin/missher when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateMissher(ssh, ''), '~/.local/bin/missher')
})

test('locateMissher tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/missher/, 'OK']])
  assert.equal(await locateMissher(ssh, ''), '~/.missher/missher-agent/venv/bin/missher')
})

test('locateMissher throws a missher-not-found error with an install hint', async () => {
...
test('locateMissher uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v missher/, '/x/missher'],
    [/\[ -x/, 'OK']
  ])

  await locateMissher(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.missher/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.missher/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.missher/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/missher'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/missher serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/missher'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/missher', '/Users/cd9c/.missher', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /missher-agent.*venv.*bin.*missher/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'missher wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'missher')
    const launcher = path.join(temp, 'missher launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/missher-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/missher', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('MISSHER_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/missher', 'work', {
    missherHome: '~/.missher',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.missher-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"missher-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /missher-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'missher-update-mutex-'))
  const missherPath = path.join(directory, 'missher')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *missher-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(missherPath, '', {
      missherHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { missherPath: '/x/missher', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('MISSHER_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('MISSHER_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]MISSHER_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { missherPath: '/x/missher', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nMISSHER_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'MISSHER_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'MISSHER_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.missher\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Missher Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'MISSHER_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile missherPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ missherPath: '/old/stale/missher', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteMissherPath: '/new/missher', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'MISSHER_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes missherHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/MISSHER_HOME/, '/home/alice/.missher\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'MISSHER_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"missherHome":"\/home\/alice\/\.missher"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'MISSHER_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'MISSHER_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/missher'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.missher/missher-agent/venv/bin/missher'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/missher'))
  assert.doesNotThrow(() => validateRemotePath('~/.missher/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/missher'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('missher'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/missher'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/missher\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/missher\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/missher\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/missher', '/usr/`id`/missher', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.missher/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/missher')
  assert.ok(result.includes('/usr/local/bin/missher'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/missher')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('MISSHER_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/missher', 'work', {
    tokenFilePath: `~/.missher/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.missher\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/missher', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Missher', () => {
  const cmd = buildSpawnCommand('/x/missher', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env MISSHER_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { missherPath: '/x/missher', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { missherPath: '/x/missher', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.missher/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.missher/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/missher serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/missher serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/missher'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/missher'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/missher'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/missher'), false)
})
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'missher-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.missher/.missher-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.missher/desktop-ssh/${OWNERSHIP_ID}`
