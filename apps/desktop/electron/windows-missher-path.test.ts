// Unit tests for the pure Windows `missher` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvMissherCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `missher` shim shadows
//      the real missher.cmd/missher.exe.
...
//   3. resolveVenvMissherCommand() — must probe the venv python via
//      canImportMissherCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMissher: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMissher: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMissher: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMissher: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvMissher: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvMissher: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvMissherCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportMissherCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    missherHome: '/fake/missher-home',
...
test('resolveVenvMissherCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvMissherCommand('/root/venv/Scripts/missher.exe', [], deps), null)
})

test('resolveVenvMissherCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvMissherCommand('/root/venv/Scripts/missher.cmd', [], deps), null)
})

test('resolveVenvMissherCommand: returns null when the basename is not missher/missher.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvMissherCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvMissherCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvMissherCommand('/root/venv/bin/missher.exe', [], deps), null)
})

test('resolveVenvMissherCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvMissherCommand('/root/venv/Scripts/missher.exe', [], deps), null)
})

test('resolveVenvMissherCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportMissherCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvMissherCommand('/root/venv/Scripts/missher.exe', ['serve'], deps)
...
test('resolveVenvMissherCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvMissherCommand('/root/venv/Scripts/missher.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'missher_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvMissherCommand: is case-insensitive on missher.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvMissherCommand('/root/venv/Scripts/MISSHER.EXE', [], deps))
  assert.ok(resolveVenvMissherCommand('/root/venv/SCRIPTS/missher.exe', [], deps))
})
