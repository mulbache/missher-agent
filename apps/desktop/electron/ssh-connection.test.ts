...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env MISSHER_DASHBOARD_SESSION_TOKEN=abc123deadbeef MISSHER_DESKTOP=1 missher dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /MISSHER_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /MISSHER_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Missher-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Missher-Session-Token: hdr_888'), /X-Missher-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Missher-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('missher', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v missher', 15000)
  assert.equal(args[args.length - 1], 'command -v missher')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `missher-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `missher-ssh-test-${process.pid}-${Date.now()}`)
