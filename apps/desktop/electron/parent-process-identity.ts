export type ParentWatchdogEnv = {
  MISSHER_PARENT_PID: string
  MISSHER_PARENT_START_MARKER?: string
  MISSHER_PARENT_NONCE?: string
  /** Spawn tag consumed by missher_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { MISSHER_PARENT_PID: String(pid) }
  env.MISSHER_SPAWN = spawnTag(pid, startMarker)
  ...
  env.MISSHER_PARENT_START_MARKER = startMarker
  env.MISSHER_PARENT_NONCE = nonce

  return env
}

/** Build the `MISSHER_SPAWN` tag mirrored by missher_cli/process_identity.py.
