 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `missher update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.missherHome, 'Missher home')
  const missherPath = validateRemoteValue(target.missherPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(missherPath)

  const updateCommand =
    `env MISSHER_HOME=${homeWord} ` +
    `MISSHER_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'MISSHER_UPDATE_ORIGIN_PROFILE=default ' +
    `MISSHER_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `MISSHER_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:MISSHER_HOME=${psLiteral(home)}`,
    `$env:MISSHER_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:MISSHER_UPDATE_ORIGIN_PROFILE="default"',
    `$env:MISSHER_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:MISSHER_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:MISSHER_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:MISSHER_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:MISSHER_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(missherPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.missher-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Missher updated and every managed SSH profile is ready.'
        : restoreOk
