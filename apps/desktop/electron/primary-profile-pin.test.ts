...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startMissher() boots the primary as the preference at that moment.
  ...
  // missher:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same MISSHER_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
