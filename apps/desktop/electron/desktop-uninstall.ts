 ...
 * the three user-facing uninstall modes to the `missher uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m missher_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `missher_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'missher_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Missher.app/Contents/MacOS/Missher  → …/Missher.app
 *   Windows: …\Missher\Missher.exe                 → …\Missher  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Missher.app/Contents/MacOS/Missher → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Missher.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Missher.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Missher$/i.test(dir) || /[\\/]missher-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/missher
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import missher_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, missherHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export MISSHER_HOME=${q(missherHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import missher_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Missher install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "MISSHER_HOME=${String(missherHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
