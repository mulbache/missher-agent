 ...
 * runtime, remote gateways, Missher Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@missher/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteMissherPath || '').trim() !== String(activeSsh.remoteMissherPath || '').trim() ||
...
/** Turn `ls ~/.missher/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Missher Cloud
 * instances are platform-managed — we never run `missher update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteMissherPath: input.remoteMissherPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Missher-managed names; an empty result stores nothing.
 ...
 * `remoteMissherPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Missher Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Missher Cloud' : 'Remote gateway')
