 ...
 *
 * So we delegate to `missher gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `missher update`'s
...
export function stopGatewayBeforeUpdate(
  missherCliPath: string,
  missherHome: string,
  deps: StopGatewayBeforeUpdateDeps = {}
): boolean {
  return runGatewayLifecycleCommand(missherCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(missherCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(missherCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(missherCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(missherCliPath, args)
  }

  if (!existsSync(missherCliPath)) {
    return false
  }

  try {
    exec(missherCliPath, args, {
