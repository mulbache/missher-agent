...
How to build Missher Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Missher surface can also
...
- A **runtime home change** (switching the underlying `MISSHER_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Missher — do not assume
...
`env -u NODE_ENV MISSHER_GUEST_ONBOARDING=1 MISSHER_HOME=<tmp>/.missher MISSHER_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). To use the portal stand-in, add
`MISSHER_PORTAL_BASE_URL=http://127.0.0.1:8765 MISSHER_ANON_API_SECRET=test-secret MISSHER_SHARED_AUTH_DIR=<tmp>/.missher/shared`
...
The free tier (a Nous identity with no account, `missher_cli/anon_auth.py`) reaches the renderer
