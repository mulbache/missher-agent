...
<p align="center">
  <a href="https://github.com/NousResearch/missher-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Missher Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Missher surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Missher and hear it back.</td></tr>
...
### Install with Missher (recommended)

Already have the Missher CLI? Just run:

```bash
missher desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Missher gateway or install Missher locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Missher Desktop website.](https://missher-agent.nousresearch.com/).
...
```bash
# throwaway MISSHER_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Missher Agent runtime into `MISSHER_HOME`
(`~/.missher`, or `%LOCALAPPDATA%\missher` on Windows), using the same layout as a
CLI install.
...
- **Missher Agent** runs as a headless `missher serve` process and exposes the
...
1. `MISSHER_DESKTOP_MISSHER_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `MISSHER_DESKTOP_MISSHER`, or `missher` on `PATH`
5. a system Python that can import the Missher runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Missher
...
screen offers **Connect to existing Missher** before starting the local installer.
...
terminal commands, and file operations run against the remote Missher host, not
...
transport- or Missher-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Missher-Session-Token`.
...
Boot logs land in `MISSHER_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\missher\missher-agent\.missher-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\missher\missher-agent\venv"
```

> The default Missher home on Windows is `%LOCALAPPDATA%\missher`. Set the `MISSHER_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://missher-agent.nousresearch.com/docs/)
