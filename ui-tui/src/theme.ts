import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@missher/shared/color'
import type { SkinBranding, SkinColors } from '@missher/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @missher/shared/color (the primitives,
...
// Mirrors @missher/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @missher/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Missher navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Missher look was never hand-authored: for years the
 ...
 *  answer when known (cached in MISSHER_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.MISSHER_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit MISSHER_TUI_THEME / COLORFGBG signals above still win,
...
// `MISSHER_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `MISSHER_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `MISSHER_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `MISSHER_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Missher palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.MISSHER_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.MISSHER_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.MISSHER_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into MISSHER_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into MISSHER_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Missher navy
  ...
  // the derived value already carries the identity seeds (e.g. Missher navy).
  const activeRow =
