import type { RunExternalProcess } from '@missher/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`missher ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching missher: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`missher ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
