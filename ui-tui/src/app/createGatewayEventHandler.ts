...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@missher/ink'
import { stripAnsi } from '@missher/shared/ansi'
import { relativeLuminance } from '@missher/shared/color'
import type { StreamDeltaPayload, SubagentStatus, Usage } from '@missher/shared/gateway-events'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.MISSHER_TUI_THEME : undefined

  writeBootTheme(theme, process.env.MISSHER_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to MISSHER_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit MISSHER_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the MISSHER_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.MISSHER_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.MISSHER_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a MISSHER_TUI_THEME the user
    ...
    delete process.env.MISSHER_TUI_THEME
  }
 ...
 * is cached into MISSHER_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * MISSHER_TUI_LIGHT / MISSHER_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.MISSHER_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.MISSHER_TUI_THEME || process.env.MISSHER_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.MISSHER_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.MISSHER_TUI_BACKGROUND ||
      process.env.MISSHER_TUI_THEME ||
      process.env.MISSHER_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.MISSHER_TUI_BACKGROUND || process.env.MISSHER_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.MISSHER_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: AnyGatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    ...
    setTimeout(async () => {
      ...
      // launches, `missher --tui -q "…"`) — submit LITERALLY, bypassing the
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `missher -c` /
    // `missher --tui` muscle memory and addresses the audit's "session
      ...
      case 'wake.detected': {
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: missher -p ${wakeProfile} --tui`)
