...
import { terminalBackgroundHex } from '@missher/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $MISSHER_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see MISSHER_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['MISSHER_TUI_BACKGROUND', process.env.MISSHER_TUI_BACKGROUND ?? '(unset)'],
            ['MISSHER_TUI_THEME', process.env.MISSHER_TUI_THEME ?? '(unset)'],
