...
import type { GatewayEvent } from '@missher/shared/gateway-events'
import {
  ...
  type ServerRequest,
  wireFrameText
} from '@missher/shared/json-rpc-channel'
import { reconnectBackoffDelayMs } from '@missher/shared/reconnect-backoff'
...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.MISSHER_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.MISSHER_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
// @missher/shared's JsonRpcRequestChannel; these re-exports keep the TUI's
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.MISSHER_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.MISSHER_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.MISSHER_PYTHON?.trim() || process.env.PYTHON?.trim()
...
export class GatewayClient extends EventEmitter {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.MISSHER_CWD || root
    const env = { ...process.env }
    const pyPath = env.PYTHONPATH?.trim()

    env.PYTHONPATH = pyPath ? `${root}${delimiter}${pyPath}` : root
    // Tell the gateway child where the Missher source root is so its import
    // guard can force it ahead of any same-named package in the launch cwd.
    env.MISSHER_PYTHON_SRC_ROOT = root
  ...
  start() {
    ...
    const root = process.env.MISSHER_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
