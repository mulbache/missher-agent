...
import { Box, Text } from '@missher/ink'
...
/**
 * User widget apps — Missher authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$MISSHER_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.missher/plugins/`: files under MISSHER_HOME execute
...
const widgetsDir = () => join(process.env.MISSHER_HOME?.trim() || join(homedir(), '.missher'), 'tui-widgets')
 ...
 *  every change, so a widget Missher writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Missher) ever writes must hot-load too — a 10s poll here read
