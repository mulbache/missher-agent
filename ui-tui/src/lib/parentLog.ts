...
const logDir = join(process.env.MISSHER_HOME?.trim() || join(homedir(), '.missher'), 'logs')
...
// a real ~/.missher (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('missher-tui: parent lifecycle log unavailable\n')
    }
  }
}
