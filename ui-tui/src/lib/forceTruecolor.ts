...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.MISSHER_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Missher on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via MISSHER_TUI_TRUECOLOR=1.
