 ...
 * `missher-boot-background` / `missher-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports MISSHER_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.missher matches get_missher_home()'s default.
const bootFilePath = () => join(process.env.MISSHER_HOME ?? join(homedir(), '.missher'), 'tui-theme-boot.json')

// Never touch the user's real ~/.missher from test runs (the TS suite has no
// MISSHER_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into MISSHER_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.MISSHER_TUI_THEME || env.MISSHER_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.MISSHER_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.MISSHER_TUI_BACKGROUND
  ) {
    env.MISSHER_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.MISSHER_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.MISSHER_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into MISSHER_TUI_THEME.
