import { Box, Text, useInput, useStdout } from '@missher/ink'
import { fuzzyRank } from '@missher/shared/fuzzy'
import type { ModelOptionProvider, ModelOptionsResult } from '@missher/shared/gateway-events'
import { modelSearchText } from '@missher/shared/model-search-text'
import { REASONING_EFFORTS } from '@missher/shared/reasoning-effort'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `missher model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run missher model)
          return
        }
    ...
    // filter. With Ctrl held, @missher/ink reports `ch` as the key name ('g'),
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.missher/.env)
        </Text>
