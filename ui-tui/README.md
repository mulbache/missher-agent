...
React + Ink terminal UI for Missher. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `MISSHER_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `missher setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.missher/.missher_history` or under `MISSHER_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `missher setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/missher-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external missher setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.missher/tui-parent.log
    ...
    types/
      missher-ink.d.ts            type declarations for @missher/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent MissherCLI subprocess for slash commands
```
