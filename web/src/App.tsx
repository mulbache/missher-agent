...
const SIDEBAR_COLLAPSED_KEY = "missher-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="missher-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="missher-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateMissher = status?.can_update_missher === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'missher update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "missher update";
    return (
      t.status.updateMissherConfirmMessage ??
      `This will run 'missher update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateMissherConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateMissher) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateMissher,
      runningLabel: t.status.updatingMissher,
      spin: false,
    });
  }
  ...
  return (
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the Missher gateway process. Connected channels and active sessions will reconnect afterward.")
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateMissherConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateMissherConfirmTitle ?? `${t.status.updateMissher}?`}
    />
    </>
  );
}
