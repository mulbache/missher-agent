...
// (e.g. https://mission-control.tilos.com/missher/). The Python backend
// injects ``window.__MISSHER_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__MISSHER_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const MISSHER_BASE_PATH = readBasePath();
const BASE = MISSHER_BASE_PATH;
...
declare global {
  interface Window {
    __MISSHER_SESSION_TOKEN__?: string;
    __MISSHER_BASE_PATH__?: string;
    ...
    __MISSHER_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Missher-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__MISSHER_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``missher update``, ``missher gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__MISSHER_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__MISSHER_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__MISSHER_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``missher_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__MISSHER_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Missher-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``missher_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__MISSHER_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildMissherWebSocketUrl({
...
export const api = {
  ...
  updateMissher: () =>
    fetchJSON<ActionResponse>("/api/missher/update", { method: "POST" }),
  checkMissherUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/missher/update/check${force ? "?force=true" : ""}`,
    ),
...
/** Preflight for `missher gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** missher-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``missher update``. */
  can_update_missher?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the MISSHER_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the MISSHER_HOME volume — no staleness dimension, so no
 * sampled_at. */
