...
interface MissherConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function MissherConsoleModal({ open, onClose }: MissherConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("missher> ");
  const inputPromptRef = useRef("missher> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "missher> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Missher Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="missher-console-title"
    >
      <div
        className={cn(
          themedBody,
