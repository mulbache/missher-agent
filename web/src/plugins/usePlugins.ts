...
import { api, MISSHER_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "missher:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${MISSHER_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${MISSHER_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?missher_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-missher-plugin", manifest.name);
