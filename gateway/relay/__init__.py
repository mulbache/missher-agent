"""Relay/connector support package for the Missher gateway.
...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_missher_home())
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from missher_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Missher Agent:**" and shadow the connector's
        # linked-owner fallback, which actually disambiguates. Only a customized
        # name is forwarded.
        if value == "Missher Agent":
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``missher gateway enroll``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.missher/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'missher'}"
