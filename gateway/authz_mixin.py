...
class GatewayAuthorizationMixin:
    ...
    def _adapters_for_profile(self, profile: Optional[str]) -> dict:
        ...
        BEFORE the active profile name: multiplex turns override ``MISSHER_HOME`` so
        ...
        if profile_name in profile_adapters:
            ...
            if adapters or not self._is_shared_bot_satellite(profile_name):
                return adapters
            return self._primary_adapters()
        # Identity captured at construction, not the per-turn MISSHER_HOME-derived name.
    ...
    def _authorization_home_for_source(self, source: SessionSource):
        """MISSHER_HOME whose allowlist admits *source*: the ingress-stamped transport home, else the home of
        ...
        if profile is None:
            ...
            return get_process_missher_home()  # the primary bot's home, never the per-turn override
