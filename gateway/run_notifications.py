...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``missher update --gateway`` and its watcher exchange under MISSHER_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_missher_home / ".update_pending.json",
            claimed=_missher_home / ".update_pending.claimed.json", output=_missher_home / ".update_output.txt",
            exit_code=_missher_home / ".update_exit_code",
            prompt=_missher_home / ".update_prompt.json", response=_missher_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``missher update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Missher update finished." if exit_code == 0
                        else "❌ Missher update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Missher update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Missher update finished." if exit_code == 0 else "❌ Missher update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Missher update finished successfully." if exit_code == 0 else
                        "❌ Missher update failed. Check the gateway logs or run `missher update` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _missher_home, _non_conversational_metadata
        notify_path = _missher_home / ".restart_notify.json"
    ...
    def _free_tier_startup_line(self) -> Optional[str]:
        ...
        try:
            ...
            from missher_cli.anon_auth import guest_carries_inference
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Missher is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from missher_constants import get_default_missher_root, profile_cli_selector
        from missher_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `missher` follows
        # active_profile, which may be a different database (#105887).
        profile_arg = profile_cli_selector()
        if cause == "corrupt":
            db_path = _default_db_path()
            backups_dir = get_default_missher_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `missher {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   missher {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) missher {profile_arg}sessions recover "
                ...
                f"Run `missher {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `missher {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                "recovery tools or restore a backup unless `missher doctor` confirms damage."
            )
        else:
            message = (
                f"⚠️ Session database unavailable — messages may not be persisted. "
                f"{format_session_db_unavailable()}\nRun `missher doctor` for diagnostics."
            )
    ...
    def _completion_event_scope(self, evt: dict):
        ...
        (``tools.async_delegation`` → ``get_missher_home()/state.db``) resolve from the ambient scope.
        ...
        if get_missher_home_override() == str(profile_home):
            return contextlib.nullcontext()
