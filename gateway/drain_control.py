...
(or removes) ``{MISSHER_HOME}/.drain_request.json`` and a gateway watcher reacts;
an ACTIVE marker means ``gateway_state -> "draining"``.  Two lenient staleness
signals (either suffices): epoch mismatch (MISSHER_HOME is a durable volume on
Missher Cloud, so a marker survives the restart a drain-gated action ends in and
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting MISSHER_HOME."""
    return Path(home if home is not None else get_missher_home()) / _DRAIN_REQUEST_FILENAME
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    survived a container/VM restart (MISSHER_HOME is a durable Fly volume on Missher Cloud) and the lifecycle
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    machine restart on the durable MISSHER_HOME volume, or an expired same-epoch orphan (#85433), is ignored
