"""Gateway runtime status helpers: PID/lock/marker files under ``{MISSHER_HOME}`` (one set per
...
from missher_constants import _get_platform_default_missher_home, get_missher_home
...
def record_start_and_check_storm(
    ...
    try:
        path = get_missher_home() / "gateway-starts.log"
...
def _get_process_missher_home() -> Path:
    """Launch-home MISSHER_HOME for identity files (PID, lock, status, markers):
    ``get_missher_home()`` honors the per-session ``_MISSHER_HOME_OVERRIDE`` and would misroute
    them."""
    val = os.environ.get("MISSHER_HOME", "").strip()
    return Path(val) if val else _get_platform_default_missher_home()


def _canonical_missher_home(path: Path | str) -> Path:
    """Stable absolute MISSHER_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_missher_home(left: Path | str, right: Path | str) -> bool:
    """Compare MISSHER_HOME paths with the host platform's case semantics."""
    left_c = os.path.normcase(str(_canonical_missher_home(left)))
    return left_c == os.path.normcase(str(_canonical_missher_home(right)))


def recorded_gateway_home_conflicts(
    record: Optional[dict[str, Any]], *, expected_home: Optional[Path | str] = None
) -> bool:
    """True when a persisted gateway record names a DIFFERENT MISSHER_HOME (cross-profile kill guard:
    profile B's stop must never SIGTERM profile A). ``expected_home`` overrides the comparison base.
    Legacy records without ``missher_home`` prove nothing -> False; a comparison failure fails
    closed -> True."""
    recorded_home = record.get("missher_home") if isinstance(record, dict) else None
    if not isinstance(recorded_home, str) or not recorded_home.strip():
        return False
    try:
        base = expected_home if expected_home is not None else _get_process_missher_home()
        return not _same_missher_home(recorded_home, base)
    except Exception:
        return True


# Mirrors missher_cli.profiles._PROFILE_ID_RE -- duplicated so gateway identity code
# stays import-light (missher_constants + stdlib only).
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    try:
        canonical = _canonical_missher_home(home)
    except Exception:
        return None
    if canonical.parent.name == "profiles" and _PROFILE_LABEL_RE.match(canonical.name):
        return canonical.name
    import missher_constants
    default_homes = (missher_constants.get_default_missher_root, _get_platform_default_missher_home)
    for default_home in default_homes:
        with contextlib.suppress(Exception):
            if _same_missher_home(canonical, default_home()):
                return "default"
    return None
...
def _get_runtime_status_path() -> Path:
    return _get_process_missher_home() / _RUNTIME_STATUS_FILE


def _get_lock_dir() -> Path:
    """Machine-local dir for token-scoped gateway locks; ``MISSHER_GATEWAY_LOCK_DIR`` overrides."""
    override = os.getenv("MISSHER_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "missher" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Missher gateway lifecycle subcommand from a command line, or None. No loose substring matches
    (``"gateway" in cmdline`` also matched ``gateway status`` / ``python -m tui_gateway``): needs a
    Missher entrypoint plus the ``gateway`` subcommand, or a gateway-dedicated entrypoint. Tokenizes
    ...
    if any(t == "gateway/run.py" or t.endswith("/gateway/run.py") for t in tokens):
        return "run"
    if any(b in ("missher-gateway", "missher-gateway.exe") for b in basenames):
        return "run"
    joined = " ".join(tokens)
    if "missher_cli.main" not in joined and "missher_cli/main.py" not in joined and not any(
        b in ("missher", "missher.exe") for b in basenames
    ):
        return None
    ...
    for i, token in enumerate(filtered):
        if token == "gateway":
            # Bare `missher gateway` defaults to `run`.
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    Missher-owned records / cleanup scans only; ``looks_like_gateway_command_line`` stays strict."""
    return _gateway_command_subcommand(command) in {"run", "restart"}


def _looks_like_gateway_process(pid: int) -> bool:
    """True when the live PID still looks like the Missher gateway."""
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    ``missher_cli.gateway._matches_current_profile``): a stale state file can record a PID recycled
    onto ANOTHER profile's live gateway. Named profiles carry ``-p``/``--profile <name>`` or
    ``MISSHER_HOME=`` on argv; the default gateway runs bare. Separators normalized."""
    command_lc = command.lower().replace("\\", "/")
    profile_name = _profile_name_for_home(profile_home)
    home_lc = str(profile_home).lower().replace("\\", "/")
    if profile_name is not None and profile_name != "default":
        return profile_flag_value(command_lc) == profile_name.lower() or f"missher_home={home_lc}" in command_lc
    ...
    # or a conflicting explicit MISSHER_HOME= (its absence is not disqualifying -- MISSHER_HOME usually
    # arrives via the env).
    if profile_flag_value(command_lc) is not None:
        return False
    return not ("missher_home=" in command_lc and f"missher_home={home_lc}" not in command_lc)
...
def _build_pid_record() -> dict:
    return {
        ...
        "missher_home": str(_canonical_missher_home(_get_process_missher_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    Lazy import keeps ``gateway.status`` free of ``missher_cli`` at import time. Never raises.

    A gateway keeps serving the module versions it imported at startup, so stamping the identity into
    ``gateway_state.json`` lets `missher update` (and the dashboard) prove whether a running gateway actually
...
def _pid_record_belongs_to_current_profile(record: Optional[dict[str, Any]]) -> bool:
    """True when the record's ``missher_home`` matches the current process (legacy records: True);
    another MISSHER_HOME's record must be ignored or the default gateway assumes its identity."""
    if not isinstance(record, dict):
        return False
    record_home = record.get("missher_home")
    return not record_home or _same_missher_home(record_home, _get_process_missher_home())
...
def multiplexer_liveness_for_profile(profile_dir: Path) -> Optional[tuple[int, dict[str, Any]]]:
    ...
    dashboard ladder reports it stopped while ``missher -p X status`` says running — the two must agree.
    """
    name = _profile_name_for_home(Path(profile_dir))
    if not name:
        return None
    from missher_cli.gateway import named_profile_served_by_running_multiplexer
    from missher_cli.gateway_multiplex_served import live_default_gateway_pid
    ...
    if pid is None:
        return None
    return pid, read_runtime_status(get_default_missher_root() / "gateway_state.json") or {}
...
def resolve_gateway_liveness(
    *, profile_dir: Optional[Path] = None, runtime: Any = _UNSET,
    ...
    # the multiplexer IS its gateway (mirrors `missher -p X status` / `gateway list`). Unscoped, the
    # question is about the process's OWN home — which is a named profile inside a pooled
    # `missher --profile X serve` (the Desktop's per-profile backend answers its REST without
    # `?profile=`), so it takes the same rung instead of reporting the served profile stopped.
    own_home = profile_dir if scoped else _get_process_missher_home()
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # Active-profile context: the record's missher_home must match this process so a stale record
...
def acquire_scoped_lock(
    ...
    # does not say WHICH profile). Omitted when not inferable; readers fall back to missher_home.
    profile = _profile_label_for_home(_get_process_missher_home())
    if profile:
...
# token. ``missher.service`` + ``missher- gateway.service``). See #5646.
...
def _get_takeover_marker_path(missher_home: Optional[Path] = None) -> Path:
    """Takeover marker path; ``missher_home`` is given only for a verified cross-home handoff."""
    home = _canonical_missher_home(missher_home or _get_process_missher_home())
    return home / _TAKEOVER_MARKER_FILENAME


def _get_planned_stop_marker_path() -> Path:
    return _get_process_missher_home() / _PLANNED_STOP_MARKER_FILENAME
...
def _pid_marker_names_self(target_pid: int, target_start_time: Any) -> bool:
    ...
    ``missher gateway stop`` as an unexpected exit revived by the service manager."""
...
def _consume_pid_marker_for_self(path: Path, *, ttl_s: int) -> bool:
    ...
    # cross-MISSHER_HOME --replace while ignoring a marker accidentally written into another
    ...
    target_home = record.get("target_missher_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_missher_home(target_home, our_home):
            return False
    else:
        replacer_home = record.get("replacer_missher_home")
        if replacer_home is not None and not _same_missher_home(replacer_home, our_home):
            return False
...
def write_takeover_marker(
    ...
    try:
        marker_home = _canonical_missher_home(target_home or _get_process_missher_home())
        ...
        return _write_marker(_get_takeover_marker_path(marker_home), {
            ...
            "target_missher_home": str(marker_home), "replacer_pid": os.getpid(),
            "replacer_missher_home": str(_canonical_missher_home(_get_process_missher_home())),
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    raw_home = record.get("missher_home")
    if (
        ...
        return None
    target_home = _canonical_missher_home(raw_home)
    ...
    pid_record_home = pid_record.get("missher_home")
    if (
        ...
        or not _same_missher_home(pid_record_home, target_home)
    ):
        return None
