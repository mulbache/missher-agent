...
``<MISSHER_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_missher_home() -> Path:
    """MISSHER_HOME for process-level identity files (ignore task overrides)."""
    from missher_constants import get_missher_home

    val = os.environ.get("MISSHER_HOME", "").strip()
    return Path(val) if val else get_missher_home()


def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_missher_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<MISSHER_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `missher doctor`.",
            verdict,
        )
