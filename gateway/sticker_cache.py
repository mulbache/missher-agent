...
(``~/.missher/sticker_cache.json``) so the same image is never re-analyzed.
"""
...
CACHE_PATH = get_missher_home() / "sticker_cache.json"
_CACHE_PATH_AT_IMPORT = CACHE_PATH


def _resolve_cache_path() -> Path:
    """Active profile's cache file at call time: the patched ``CACHE_PATH`` when a test changed
    it, else live profile-scoped MISSHER_HOME — under the multiplexed gateway one process serves
    every profile, so the import-time constant would pin every profile to the launch home."""
    return CACHE_PATH if CACHE_PATH != _CACHE_PATH_AT_IMPORT else get_missher_home() / "sticker_cache.json"
