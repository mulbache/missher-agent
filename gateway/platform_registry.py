...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``missher setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``missher gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved MISSHER_HOME and overlay the
        ...
        # `missher` invocation, so the import happens only when a lookup asks for it.
