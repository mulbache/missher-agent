...
``validate_media_delivery_path`` reads ``MISSHER_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``MISSHER_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``MISSHER_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``missher cron run``, ``missher send``) calls
...
_FLAG_ENVS = (("strict", "MISSHER_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "MISSHER_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "MISSHER_MEDIA_ALLOW_DIRS"
_TRUST_RECENT_SECONDS_ENV = "MISSHER_MEDIA_TRUST_RECENT_SECONDS"
_TRUTHY = frozenset({"1", "true", "yes", "on"})


def _routed_gateway_cfg() -> Optional[Dict[str, Any]]:
    """``gateway`` section of the ROUTED profile's config when a MISSHER_HOME override is active
    ...
    if not get_missher_home_override():
        return None
    try:
