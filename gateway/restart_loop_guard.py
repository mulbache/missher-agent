...
Defenses 1-2 (``_MISSHER_GATEWAY`` guard on ``missher gateway stop|restart`` /
...
persisted to ``<MISSHER_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_missher_home() / "gateway" / "restart_loop.json"
