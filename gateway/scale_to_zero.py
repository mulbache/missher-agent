...
SCALE_TO_ZERO_ENV = "MISSHER_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(missher_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under MISSHER_HOME."""
    if missher_home is None:
        from missher_constants import get_missher_home
        missher_home = get_missher_home()
    return Path(missher_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
