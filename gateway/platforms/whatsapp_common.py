...
class WhatsAppBehaviorMixin(OwnAccessPolicyMixin):
    ...
    DEFAULT_REPLY_PREFIX: str = "☤ *Missher Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/missher) is mirrored to MISSHER_HOME so npm install works."""
    import shutil
    from missher_constants import get_missher_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    missher_home_bridge = get_missher_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if missher_home_bridge.exists():
        return missher_home_bridge
    try:
        missher_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, missher_home_bridge, dirs_exist_ok=False)
        return missher_home_bridge
    except Exception:
        return install_bridge
