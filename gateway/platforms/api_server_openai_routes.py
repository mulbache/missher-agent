...
def _missher_extras(completed, is_partial, is_failed, err_msg, finish_reason: str) -> Dict[str, Any]:
    return {
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """``missher.tool.progress`` status=running; ``_``-prefixed tools stay off the wire."""
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["missher"] = {
                "completed": completed, "partial": is_partial, "failed": is_failed}
            response_headers["X-Missher-Completed"] = "false"
            response_headers["X-Missher-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)
        # Soft partial (some text, run incomplete): 200 + finish_reason="length"/Missher extras.
        response_data = {
        ...
        if is_partial or is_failed or not completed:
            response_data["missher"] = _missher_extras(
                completed, is_partial, is_failed, err_msg, finish_reason)
            response_headers["X-Missher-Completed"] = "false"
            response_headers["X-Missher-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Missher-Error"] = _redact_api_error_text(err_msg, limit=200)
        ...
        try:
            ...
            async for delta in _iter_stream_items(stream_q, agent_task, response):
                ...
                if isinstance(delta, tuple) and len(delta) == 2 and delta[0] == "__tool_progress__":
                    ...
                    await response.write(_sse_frame(delta[1], event="missher.tool.progress"))
                else:
            ...
            if finish_reason != "stop":
                ...
                finish_chunk["missher"] = _missher_extras(
                    completed, is_partial, is_failed, err_msg, finish_reason)
