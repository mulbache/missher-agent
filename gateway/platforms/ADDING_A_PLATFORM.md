...
There are two ways to add a platform to the Missher gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.missher/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Missher code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `missher gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `missher_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Missher core.
...
```python
"missher-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _MISSHER_CORE_TOOLS,
    "includes": []
},
```

And add it to the `missher-gateway` composite:

```python
"missher-gateway": {
    "includes": [..., "missher-your-platform"]
}
```
...
## 12. Status Display (`missher_cli/status.py`)
...
## 13. Gateway Setup Wizard (`missher_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ missher_cli/ toolsets.py \
  --include="*.py" -l | sort -u
