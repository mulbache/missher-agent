"""Session-scoped context variables for the Missher gateway.

Replaces the old ``os.environ``-based ``MISSHER_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "MISSHER_SESSION_PLATFORM", "MISSHER_SESSION_SOURCE", "MISSHER_SESSION_CHAT_ID",
    "MISSHER_SESSION_CHAT_TYPE", "MISSHER_SESSION_CHAT_NAME", "MISSHER_SESSION_THREAD_ID",
    "MISSHER_SESSION_USER_ID", "MISSHER_SESSION_USER_ID_ALT", "MISSHER_SESSION_USER_NAME",
    "MISSHER_SESSION_SCOPE_ID", "MISSHER_SESSION_KEY", "MISSHER_SESSION_ID",
    "MISSHER_UI_SESSION_ID", "MISSHER_SESSION_MESSAGE_ID", "MISSHER_SESSION_PROFILE",
    "MISSHER_BROWSER_CONTROL_PRINCIPAL", "MISSHER_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "MISSHER_CRON_SESSION", "MISSHER_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("MISSHER_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("MISSHER_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("MISSHER_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("MISSHER_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("MISSHER_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``MISSHER_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``MISSHER_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds MISSHER_SESSION_PLATFORM, CLI/TUI/
# desktop bind MISSHER_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``MISSHER_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("MISSHER_PLATFORM") or get_session_env("MISSHER_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("MISSHER_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See NousResearch/missher-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``MISSHER_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
