...
All hooks write atomic JSON payloads under ``<missher_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active MISSHER_HOME."""
    from missher_constants import get_missher_home
    flush_dir = get_missher_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<missher_home>/pending_messages/``), so a runtime cap rotation no longer silently
