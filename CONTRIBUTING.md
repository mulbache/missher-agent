# Contributing to Missher Agent

Thank you for contributing to Missher Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Missher to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NousResearch/missher-agent "<your terms>"
  gh search prs --repo NousResearch/missher-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/NousResearch/missher-agent/issues?q=) · [PRs (all states)](https://github.com/NousResearch/missher-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Missher itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Missher install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `missher skills browse` (labeled "official") and install it with `missher skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `missher skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.missher/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(missher_home, config)` for setup-wizard integration
...
- Integrate with `missher memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Missher ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.missher/plugins/`, project `.missher/plugins/`, or a pip entry point) — see [Build a Missher Plugin](https://missher-agent.nousresearch.com/docs/guides/build-a-missher-plugin)
...
The installer creates the Missher venv, wires the `missher` command, stamps the
install method for `missher update`, and clones the full git project into
`$MISSHER_HOME/missher-agent` (usually `~/.missher/missher-agent`). That keeps your
...
Use this only if you intentionally do not want Missher' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `missher` entrypoint from this venv; running the
system `python3 -m missher_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.missher/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.missher/config.yaml
touch ~/.missher/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.missher/.env
```

### Run

```bash
# The standard installer already put `missher` on PATH.
missher doctor
missher chat -q "Hello"
```

If you used the manual clone fallback, run `./missher` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/missher" ~/.local/bin/missher
```
...
├── cli.py                    # MissherCLI class — interactive CLI orchestrator (~4.6k LOC + missher_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (missher-cli, missher-telegram, etc.)
├── missher_state.py           # SessionDB facade (~1.4k LOC); implementation in missher_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── missher_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # MissherCLI mixins (slash commands, display, session, ...)
...
│
├── skills/                   # Bundled skills (copied to ~/.missher/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (missher-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.missher/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.missher/`)

| Path | Purpose |
|------|---------|
| `~/.missher/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.missher/.env` | API keys and secrets |
| `~/.missher/auth.json` | OAuth credentials (Nous Portal) |
| `~/.missher/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.missher/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.missher/state.db` | SQLite session database |
| `~/.missher/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
| `~/.missher/cron/` | Scheduled job data |
| `~/.missher/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`missher_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `missher sessions export` for an explicit export.
...
(for example `_MISSHER_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.missher`:
...
The user may skip setup and keep loading the skill. Missher only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `missher setup` or update `~/.missher/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Missher Agent" is the secondary collaborator. If the contributor's commit shows "Missher Agent" as author because they used Missher to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Missher tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `missher --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Missher uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.missher/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `missher_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `missher_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Missher runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the missher wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `missher_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `missher_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `missher_cli/gateway_windows.py::
...
Missher has terminal access. Security matters.
...
2. **Test manually**: Run `missher` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/missher-agent/issues)
- Include: OS, Python version, Missher version (`missher --version`), full error traceback
