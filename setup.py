...
Missher Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``MISSHER_NIX_BUILD=1`` on the
Missher package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("MISSHER_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for missher-agent is not supported.\n"
    "Missher is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://missher-agent.nousresearch.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Missher Nix derivation sets MISSHER_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``missher_state``, ``toolsets``...)
