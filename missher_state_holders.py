...
an unlinked SQLite DB/WAL/SHM generation.  ``missher_state`` supplies only the
...
_MISSHER_EXECUTABLES = frozenset({"missher", "missher-agent", "missher-acp"})
_MISSHER_PYTHON_MODULES = frozenset({"acp_adapter", "missher_cli.main"})
_MISSHER_PYTHON_SCRIPTS = frozenset({"missher_cli/main.py", "run_agent.py"})
...
def _looks_like_missher(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Missher execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _MISSHER_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _MISSHER_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _MISSHER_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the MISSHER_HOME root, so an absolute-path token
    containing a ``/.missher`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Missher home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.missher deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.missher" in normalized or normalized.endswith("/.missher"):
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # MISSHER_HOME would otherwise make every holder invisible and let maintenance proceed.
