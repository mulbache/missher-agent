 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `missher serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Missher should add
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `missher-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated MISSHER_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `missher-dev-mock-${Date.now()}`))
  const missherHome = nodePath.join(root, 'missher-home')
...
function writeMockConfig(missherHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(missherHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(missherHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.missherHome, mock.url)
  console.log(`  MISSHER_HOME: ${sandbox.missherHome}`)
  ...
  const env: Record<string, string> = {
    ...
    MISSHER_DESKTOP_MISSHER_ROOT: repoRoot,
    MISSHER_DESKTOP_APP_NAME: `MissherDevMock-${Date.now()}`,
  }
