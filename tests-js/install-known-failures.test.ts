...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/missher.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/missher.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'missher-update',
  error: 'E2E ASSERTION FAILED: missher update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('missher.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'missher-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured missher desktop spec; update completed',
      logs: { desktop: '[missher] [updates] no staged updater; surfacing manual `missher update` for CLI install at C:/install\n[missher] [updates] manual: missher update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: missher update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> missher-update (v2026.3.12 -> HEAD)'
