# Behavioral test for install.ps1's missher launcher staging (PR #92092,
...
#
# The test lifts the real Install-MissherCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $MissherHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-MissherCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-MissherCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'missher-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'missher-agent'
    ...
    $missingThrew = $false
    try {
        Install-MissherCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing missher.exe fails the launcher stage'
    ...
    $missherV1 = [byte[]](77, 90, 1)
    $missherV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'missher.exe'), $missherV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-MissherCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $missherV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'missher.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'missher-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'missher.exe'), $missherV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'missher-acp.exe'), $acp)
    Install-MissherCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $missherV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'missher.exe'))) `
        'installer refreshes an existing Missher launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'missher-acp.exe'))) `
    ...
    Install-MissherCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'missher.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'missher.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'missher.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'missher.exe')) -and $cmdBody.Contains('%*')) `
