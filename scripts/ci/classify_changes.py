...
# tree generates llms.txt — the index every LLM (Missher included, via the
# missher-agent skill) reads to learn what Missher can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/missher_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"missher_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Missher-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``missher serve`` backend, the Docker image)
