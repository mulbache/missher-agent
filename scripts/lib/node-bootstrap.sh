...
#   2. ~/.missher/node/ from a prior Missher-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.missher/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$MISSHER_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   MISSHER_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   MISSHER_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   MISSHER_HOME               (default: $HOME/.missher)
# ============================================================================

MISSHER_NODE_MIN_VERSION="${MISSHER_NODE_MIN_VERSION:-20}"
MISSHER_NODE_TARGET_MAJOR="${MISSHER_NODE_TARGET_MAJOR:-22}"
MISSHER_HOME="${MISSHER_HOME:-$HOME/.missher}"
MISSHER_NODE_AVAILABLE=false
...
# Redirect a Missher-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $MISSHER_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$MISSHER_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${MISSHER_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$MISSHER_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through missher_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$MISSHER_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$MISSHER_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$MISSHER_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $MISSHER_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$MISSHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$MISSHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $MISSHER_NODE_TARGET_MAJOR..."
    proto install node "$MISSHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $MISSHER_NODE_TARGET_MAJOR..."
    nvm install "$MISSHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$MISSHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${MISSHER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${MISSHER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${MISSHER_NODE_TARGET_MAJOR}.x/"
    local tarball=""
    # `tar xf` needs the xz binary for .tar.xz; minimal images ship tar without it (#11197).
    if command -v xz >/dev/null 2>&1; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${MISSHER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${MISSHER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $MISSHER_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $MISSHER_HOME/node/..."
    ...
    mv "$extracted" "$MISSHER_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # MISSHER_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${MISSHER_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$MISSHER_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$MISSHER_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$MISSHER_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $MISSHER_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken Missher-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$MISSHER_HOME/node/bin/$tool" \
        "$MISSHER_HOME/node/${tool}.exe" \
        "$MISSHER_HOME/node/$tool"; do
...
# The managed node runs but is below MISSHER_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$MISSHER_HOME/node/bin/node" "$MISSHER_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$MISSHER_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
# absent. Used by missher_constants.find_missher_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$MISSHER_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Missher-managed Node is broken — redownloading to $MISSHER_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        MISSHER_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$MISSHER_HOME/node/bin/node" ]; then
        export PATH="$MISSHER_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Missher-managed)"
            MISSHER_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { MISSHER_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { MISSHER_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { MISSHER_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { MISSHER_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { MISSHER_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { MISSHER_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $MISSHER_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
