"""Run a real Missher CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_missher_executable(missher_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "missher",
        Path(".venv") / "Scripts" / "missher.exe",
    ):
        candidate = missher_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("missher")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Missher executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Missher source checkout containing .venv/bin/missher",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated MISSHER_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["missher.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['missher.client.active']}"
        )
    [model] = by_name["missher.model_route.count"]
    expected_model = {
        "name": "missher.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['missher.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["missher.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['missher.task_run.started']}"
        )
    [terminal] = by_name["missher.task_run.finished"]
    ...
    [tool] = by_name["missher.tool_call.count"]
    ...
    lifecycle = by_name["missher.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["missher.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Missher development environment requires jsonschema"
        ) from exc
    ...
    if metrics["missher.client.active"] != [
        {
            "name": "missher.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['missher.client.active']}"
        )
    [model] = metrics["missher.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['missher.model_route.count']}"
        )
    [terminal] = metrics["missher.task_run.finished"]
    ...
    [tool] = metrics["missher.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["missher.skill.lifecycle.count"]
    ...
    loads = metrics["missher.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    missher_repo = args.missher_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    missher = _resolve_missher_executable(missher_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="missher-relay-shared-metrics-"))
    home = root / "missher-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["MISSHER_HOME"] = str(home)
        python_paths = [str(missher_repo)]
    ...
    (root / "missher.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "missher.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Missher exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Missher model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Missher did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Missher did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "missher.shared_metrics.v2.schema.json",
    )

    print("Missher -> NeMo Relay shared-metrics smoke test passed")
