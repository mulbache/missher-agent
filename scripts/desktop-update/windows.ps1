...
# (%MISSHER_HOME%\missher-setup.exe). That binary has no self-update path --
# copy_self_to_missher_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `missher update` refreshes
...
#     -InstallRoot <path>   repo checkout (MISSHER_HOME\missher-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Missher.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .missher-update-in-progress in place (tests)
#
...
# .missher-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim MISSHER_HOME\.missher-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining MISSHER_UPDATE_STARTED_AT from the Desktop hand-off.
# missher_cli/update_lock.py's ancestry rule lets our
# `missher update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Missher.exe pid.
try {
    Add-Type -Namespace MissherHandoff -Name Win32 -MemberDefinition @'
...
$MissherHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $MissherHome ".missher-update-in-progress"
$LogDir = Join-Path $MissherHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $MissherHome ".missher-update-result.json"
$script:Ui = $null
$script:UiStage = "Missher will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale missher-update-ui-* leftovers from interrupted
    ...
    # update marker (.missher-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "missher-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-MissherStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("missher-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [MissherHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `missher debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"missher debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [MissherHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Missher themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Missher.exe before writing the replacement,
    ...
    # DO NOT spawn Missher.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [MissherHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [MissherHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [MissherHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Missher process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [MissherHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `missher update` deliberately runs its build steps with stdout
# inherited (missher_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:MISSHER_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:MISSHER_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:MISSHER_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:MISSHER_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `missher update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (missher_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:MISSHER_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:MISSHER_UPDATE_PROGRESS_LOG
}
...
if (-not ("MissherUpdateJob" -as [type])) {
...
public static class MissherUpdateJob {
...
function Invoke-MissherStep([string]$Exe, [string[]]$MissherArgs, [string]$Tag) {
    ...
    # afterwards, where `missher debug share` picks it up.
    #
    ...
    # .missher-update-result.json, clearing .missher-update-in-progress,
    ...
    # Missher" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($MissherArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [MissherUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `missher update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [MissherUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; MISSHER_SELFTEST_FAIL=1 exercises the
# error state, MISSHER_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:MISSHER_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MISSHER_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:MISSHER_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-MissherStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Missher install to
...
#            update log, the shape of every real `missher update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:MISSHER_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MISSHER_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:MISSHER_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:MISSHER_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "missher-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "missher-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "missher-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "missher-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "missher-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "missher-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "missher-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "missher-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `missher update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-MissherStep $powershell @(
    ...
    $flood = Invoke-MissherStep $powershell @(
    ...
    $stall = Invoke-MissherStep $powershell @(
    ...
    try {
        $logstall = Invoke-MissherStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-MissherStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-MissherStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:MISSHER_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from MISSHER_HOME; pin the process directory to the
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-MissherStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for Missher to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Missher window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Missher fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing Missher files"
    $shim = Join-Path $InstallRoot "venv\Scripts\missher.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Missher install open (venv\Scripts\missher.exe locked after 20s). Nothing was changed. Close other Missher windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the missher.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\missher.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_missher_exe must first rename the running missher.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that missher.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Missher window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Missher.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m missher_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Missher installer or `missher doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "missher_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Missher windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m missher_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed missher predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-MissherStep $pythonExe $updateArgs "update"
    Write-HandoffLog "missher update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-MissherUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `missher update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "missher update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-MissherStep $pythonExe @("-m", "missher_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import missher_cli.main; from missher_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-MissherStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated Missher runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `missher desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `missher debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Missher is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Missher was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Missher."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Missher"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Missher to finish (it could not restart itself)."
