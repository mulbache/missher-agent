...
Run after changing any ``desktop=`` value or alias in ``missher_cli/commands.py``;
``tests/missher_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from missher_cli.commands import desktop_surface_registry
