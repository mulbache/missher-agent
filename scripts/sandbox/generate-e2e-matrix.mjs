 ...
 *   The artifact published on the website right now -- Missher-Setup.exe has
 ...
 * @typedef {InstallMethod | 'missher-update' | 'open-app-update' | 'missher-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; missher-desktop-app-update starts the app
 *   via `missher desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://missher.nousresearch.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Missher.exe AND
      ...
      // Website Missher-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `missher desktop`.
      { method: 'missher-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Missher-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `missher desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'missher-desktop-app-update';
}
