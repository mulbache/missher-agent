...
VERSION_FILE = REPO_ROOT / "missher_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@missher.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.missher": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level MISSHER_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (missher journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "missher.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.missher/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows missher-shim resolution + prefer --update on recovery; #52378)
    ...
    "missher@marian.local": "Schrotti77",
    ...
    "274096618+missher-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "missher@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on missher mcp add)
    ...
    "missher@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@missher-agent": "teknium1",
    ...
    "github@nadyamissher.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (MISSHER_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (MISSHER_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: missher-eval community link)
    ...
    "missheragent26@gmail.com": "missheragent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+missheragent26@users.noreply.github.com": "missheragent26",
    ...
    "vigo@missher": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.missher@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Missher runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level MISSHER_HOME state files)
    ...
    "zhchl@missher-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Missher
    ...
    # Keep the bootstrap installer (Missher-Setup.dmg CFBundleShortVersionString)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "missher@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/missher-agent",
    ...
    lines.append(f"# Missher Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Missher Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Missher Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Missher Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Missher Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Missher Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Missher Agent v{new_version} ({calver_date})' "
