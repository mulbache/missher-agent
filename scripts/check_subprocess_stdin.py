...
When Missher runs in TUI mode, the gateway child process communicates with
...
# ``get_missher_home() / "plugins"`` (user) and ``./.missher/plugins/`` (project,
# gated behind ``MISSHER_ENABLE_PROJECT_PLUGINS``) — see
# ``missher_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``missher_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import missher_constants (this script
    ...
    # plugins in get_missher_home()/plugins/ and project plugins in
    # ./.missher/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_missher_home() / "plugins"]
    if os.environ.get("MISSHER_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".missher" / "plugins")
