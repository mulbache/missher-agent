...
# changing PATH, installing software, or touching the user's Missher home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("missher-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$MissherHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -MissherHome $MissherHome -InstallDir $InstallDir
