...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[missher] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[missher`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[missher`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        MissherHome = $(if ($paths) { $paths.missher_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\missher\missher-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-missher'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-missher') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
