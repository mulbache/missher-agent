# ============================================================================
# Missher Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$MissherHome = $(if ($env:MISSHER_HOME) { $env:MISSHER_HOME } else { "$env:LOCALAPPDATA\missher" }),
    [string]$InstallDir = $(if ($env:MISSHER_HOME) { "$env:MISSHER_HOME\missher-agent" } else { "$env:LOCALAPPDATA\missher\missher-agent" }),
    ...
    # builds apps/desktop into a launchable Missher.exe.
    #
    # Why opt-in:
    #   * Missher-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Missher.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Missher.exe
    ...
    #     `missher desktop` already builds on demand.
...
# MISSHER_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[missher] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'MissherInstall.LongPath').Type) {
            Add-Type -Namespace 'MissherInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [MissherInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -MissherHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('MissherHome')) {
    $MissherHome = ConvertTo-LongPath $MissherHome
} else {
    $MissherHome = ConvertTo-LongPath $(
        if ($env:MISSHER_HOME) { $env:MISSHER_HOME } else { "$env:LOCALAPPDATA\missher" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:MISSHER_HOME) { "$env:MISSHER_HOME\missher-agent" } else { "$env:LOCALAPPDATA\missher\missher-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: MissherHome=$MissherHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:NousResearch/missher-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/missher-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Missher Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $MissherHome)) {
        New-Item -ItemType Directory -Force -Path $MissherHome | Out-Null
    }
    $envFile = Join-Path $MissherHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `missher update`'s npx
    ...
    $prefixDir = Join-Path $MissherHome "node"
...
function Install-Uv {
    # Missher owns its own uv at $MissherHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (missher_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `missher update` stay in sync.
    $managedUv = Join-Path $MissherHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $MissherHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $MissherHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $MissherHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $MissherHome "bin"
        ...
        # (missher_cli/managed_uv.py looks only at $MissherHome\bin\uv.exe).
...
# Put the Missher-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone missher-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Missher-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Missher-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("missher-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and MISSHER_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Missher belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # missher_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".missher-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Missher-managed interpreter
    ...
    #
    # Under Missher-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Missher-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Missher setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Missher uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\missher\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Missher needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Missher copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\missher\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``MISSHER_GIT_BASH_PATH`` (User scope) so Missher can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Missher-managed PortableGit install instead..."
    }

    # Download PortableGit into $MissherHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $MissherHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Missher features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Missher needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``MISSHER_GIT_BASH_PATH`` (User env scope) so Missher can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $MissherHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $MissherHome\git\usr\bin\bash.exe
    $candidates += "$MissherHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$MissherHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("MISSHER_GIT_BASH_PATH", $candidate, "User")
            $env:MISSHER_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set MISSHER_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Missher may not find Git Bash."
    Write-Info "If needed, set MISSHER_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Missher-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Missher requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Missher requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Missher-managed Node.js installation instead..."

    # Prefer a Missher-managed Node from a previous run over a too-old system one.
    $managedNode = "$MissherHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$MissherHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$MissherHome\node"
        Write-Success "Node.js $version found (Missher-managed)"
        ...
        Update-ManagedNpm "$MissherHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Missher-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $MissherHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $MissherHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\missher-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$MissherHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$MissherHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$MissherHome\node") {
                    try {
                        Rename-Item "$MissherHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Missher-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$MissherHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $MissherHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\missher-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `missher
                    # update` path (#4735).
                    ...
                    $stashName = "missher-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. missher-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``missher update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Missher behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running missher processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/missher-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/missher-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/missher-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\missher-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\missher-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `missher update` (see the notes at the shared clone-path
    ...
    # next `missher update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Missher-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Missher-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running missher process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running missher processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Missher_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (missher.exe) plus its child tree.
            & taskkill /F /T /IM missher.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM missher.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m missher_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not missher.exe.
            ...
            # Missher children re-exec through .missher-runtime, so killing only
            ...
            # user (see missher_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Missher " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `missher update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Missher (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `missher.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'missher-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install missher-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and missher.exe broken with
    ...
    # here, not 30 seconds later when the user runs `missher`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Missher processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Missher processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Missher and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register missher.exe in dist-info/RECORD but fail to
        ...
        # `missher` command while missher-agent.exe / missher-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m missher_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `missher dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\missher_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `missher dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `missher dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: missher_cli/web_server.py"
        }
    }
...
function Install-MissherCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the missher launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring missher.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "missher.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the missher command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # missher_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("missher", "missher-acp")) {
    ...
    $requiredExe = Join-Path $Destination "missher.exe"
    $requiredCmd = Join-Path $Destination "missher.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the missher command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up missher command..."
    
    if ($NoVenv) {
        $missherBin = "$InstallDir"
    } else {
        # $MissherHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `missher update`'s autostash
        ...
        # installer staged under missher-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-MissherCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $missherBin = "$MissherHome\bin"
        Install-MissherCommandLaunchers -Root $InstallDir -Destination $missherBin | Out-Null
    }
    ...
    #   missher-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The missher-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept missher via $missherBin)"
        }
    }
    
    if ($currentPath -notlike "*$missherBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$missherBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $missherBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set MISSHER_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\missher instead
    # of the Unix default ~/.missher
    $currentMissherHome = [Environment]::GetEnvironmentVariable("MISSHER_HOME", "User")
    if (-not $currentMissherHome -or $currentMissherHome -ne $MissherHome) {
        [Environment]::SetEnvironmentVariable("MISSHER_HOME", $MissherHome, "User")
        Write-Success "Set MISSHER_HOME=$MissherHome"
    }
    $env:MISSHER_HOME = $MissherHome
    
    # Update current session
    $env:Path = "$missherBin;$env:Path"
    
    Write-Success "missher command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.missher-bootstrap-complete which tells the Missher
    ...
    # Missher-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `missher update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".missher-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the MISSHER_HOME directory structure ($MissherHome, default %LOCALAPPDATA%\missher)
    New-Item -ItemType Directory -Force -Path "$MissherHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MissherHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$MissherHome\.env"
    ...
    # Create config.yaml
    $configPath = "$MissherHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Missher's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$MissherHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in missher_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $MissherHome"
    
    # Seed bundled skills into $MissherHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $MissherHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $MissherHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MissherHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $MissherHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Missher-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'missher setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\missher-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\missher-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\missher-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `missher tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $MissherHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Missher' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Missher' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'missher tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `missher
...
# paths still lazy-install via install_cua_driver() (missher_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `missher computer-use install` runs. Bounded
        ...
        # Missher' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'missher tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: missher computer-use install"
            }
        } else {
...
# the final `electron` -> `Missher` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Missher.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_missher_desktop command
    # resolves apps/desktop/release/win-unpacked/Missher.exe directly,
    ...
    # enforces the supported Node lines and prepends the Missher-managed Node to
    ...
    # The --dir mode produces an unpacked Missher.exe in
    ...
    # are NOT trying to work around). The Missher icon + product name are
    # stamped onto Missher.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\missher-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Missher.exe",
        "$desktopDir\release\win-arm64-unpacked\Missher.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Missher.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Missher icon + identity are stamped onto Missher.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, missher-agent#38216).
    ...
    #    Missher.exe. We deliberately do NOT point them at `missher desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Missher.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Missher.lnk')
        )
        ...
        # with the Missher icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.missher/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'missher setup'."
        return
    }
    ...
    # Run missher setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m missher_cli.main setup
    } else {
        python -m missher_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $missherCmd = "$InstallDir\venv\Scripts\missher.exe"
    if (-not (Test-Path $missherCmd)) {
        $missherCmd = "missher"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$MissherHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'missher whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: missher gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$MissherHome\logs\gateway.log"
            Start-Process -FilePath $missherCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$MissherHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: missher gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: missher gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$MissherHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$MissherHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MissherHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MissherHome\missher-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   missher              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   missher setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   missher config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   missher config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   missher gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   missher update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Missher repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.missher-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Missher-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Missher to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://missher-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
