...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Missher\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (missher-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "missher@nousresearch.com",
    "missher-audit@example.com",
    "nousbot@nousresearch.com",
    "missher@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "NousResearch/missher-agent",
                "--state", "merged",
