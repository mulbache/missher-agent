...
Run with a fresh HOME and MISSHER_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" MISSHER_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `MISSHER_EVAL_REPO` selects another checkout for A/B.
