...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "MISSHER_HOME": str(root / "home/.missher"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "MISSHER_NONINTERACTIVE": "1"}
    Path(env["MISSHER_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from missher_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'missher')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,missher_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,missher_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Missher\n", b"Name=Missher custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["MISSHER_HOME"]) / "config.yaml"
