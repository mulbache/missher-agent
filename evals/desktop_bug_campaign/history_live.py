...
env.update(HOME=str(home), MISSHER_HOME=str(home), MISSHER_DASHBOARD_SESSION_TOKEN='history-fixture-token', PYTHONPATH=repo, MISSHER_NONINTERACTIVE='1')
cmd=[sys.executable, '-m', 'missher_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(args.port), '--isolated']
...
try:
    for _ in range(120):
        try:
            req=urllib.request.Request(f'http://127.0.0.1:{args.port}/api/status',headers={'X-Missher-Token':'history-fixture-token'})
