...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='missher-acp-empty-'))
    missher = home / '.missher'
    missher.mkdir()
    (missher / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), MISSHER_HOME=str(missher), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               MISSHER_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(missher / 'state.db') as db:
