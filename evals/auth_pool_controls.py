        ...
        for name, provider, command, status in cases:
            ...
            with tempfile.TemporaryDirectory(prefix="auth-pool-pty-") as temp:
                ...
                env = {k: v for k, v in os.environ.items() if not any(t in k for t in
                       ("TOKEN", "API_KEY", "SECRET", "PASSWORD", "MISSHER", "PYTEST"))}
                env.update(HOME=temp, MISSHER_HOME=temp, MISSHER_SHARED_AUTH_DIR=str(home / "shared"), PYTHONPATH=str(Path(args.repo).absolute()), TERM="xterm")
