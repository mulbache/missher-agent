...
Run with the Missher venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Missher children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    from missher_cli.model_switch_providers import (
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "missher_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="missher-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".missher"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "MISSHER_HOME": str(state),
