...
Each probe runs in a FRESH subprocess with an isolated MISSHER_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "MISSHER_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "MISSHER_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("MISSHER_SESSION", "MISSHER_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "missher_cli.main", "gateway.run", "tools.registry", "missher_state", "tui_gateway.server", "missher_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "missher-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `missher --version`, `missher --help`, `missher doctor --help`, `missher config get model` (no network)
CLI = {"version": ["missher_cli/main.py", "--version"], "help": ["missher_cli/main.py", "--help"], "config_get": ["missher_cli/main.py", "config", "get", "model"], "tools_list": ["missher_cli/main.py", "tools", "--help"], "skills_help": ["missher_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_missher_default_ms", lambda: resolve_toolset("missher-default"), 200)
...
try:
    import missher_state, tempfile, uuid
    from pathlib import Path
    db=missher_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(missher_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
