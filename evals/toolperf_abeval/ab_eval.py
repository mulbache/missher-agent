...
measured in the production session DB) through `missher chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    MISSHER_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Missher home with credentials for the
...
(tracker: NousResearch/missher-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "MISSHER_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "missher_cli.main", "chat", "--query", q,
