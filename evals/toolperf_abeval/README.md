...
batch (tracker: [#77056](https://github.com/NousResearch/missher-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Missher home, same model, same tasks, same reps.
...
1. Create a dedicated Missher home with credentials for the models under test:
   ...
   integration at it; no Missher observability plugin needs to be enabled.
