...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["MISSHER_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(MissherTokenStorage("positive", missher_home=owner).get_tokens())
        ...
        override = set_missher_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from missher_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="missher-oauth-http-") as temp:
            home = Path(temp)
            (home / ".missher").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "MISSHER_HOME": str(home / ".missher"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
