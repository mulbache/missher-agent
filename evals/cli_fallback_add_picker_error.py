"""Drive the real ``missher fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from missher_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="missher_test_fallback_") as home:
        hh = Path(home) / ".missher"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "MISSHER_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "missher_cli.main", "fallback", "add"],
