"""Run the read-tool eval through the REAL Missher AIAgent.

For each task: fresh temp MISSHER_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    missher_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".missher"
    ...
    try:
        ...
        shutil.rmtree(missher_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.missher/.env; set +a  — then relaunch."
        )
