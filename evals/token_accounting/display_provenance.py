...
def child(out: Path) -> None:
    ...
    runner.session_store = SessionStore(Path(os.environ["MISSHER_HOME"]) / "sessions", runner.config)
    ...
    cli = MissherCLI(model="fixture", provider="openai-compat", api_key="fixture", base_url=base_url)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="context-display-") as home:
        ...
        if pid == 0:
            os.environ.clear()
            os.environ.update(HOME=home, MISSHER_HOME=home + "/.missher", PATH="/usr/bin:/bin", TERM="dumb", NO_COLOR="1")
            try:
                child(out)
            finally:
                sys.stdout.flush()
