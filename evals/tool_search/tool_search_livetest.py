#!/usr/bin/env python3
"""Live test harness for Missher Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any missher imports.
ORIGINAL_HOME = os.environ.get("MISSHER_HOME")
ORIGINAL_AUTH = Path.home() / ".missher" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.missher/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.missher/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="missher_ts_live_"))
    missher_home = home_dir / ".missher"
    missher_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, missher_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".missher" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, missher_home / ".env")
        ...
        load_missher_dotenv(missher_home=str(Path.home() / ".missher"))
    ...
    (missher_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return missher_home
...
def reset_module_state():
    """Drop cached modules so the new MISSHER_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "missher_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original MISSHER_HOME
    if ORIGINAL_HOME is not None:
        os.environ["MISSHER_HOME"] = ORIGINAL_HOME
    else:
