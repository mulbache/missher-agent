...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), MISSHER_HOME=str(home / "missher"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("missher-gateway*", "missher-serve*")] + [unit + ".service"]
...
unit = f"missher-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
