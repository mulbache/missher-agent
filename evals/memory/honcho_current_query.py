...
Run with isolated HOME/MISSHER_HOME and honcho-ai==2.2.0 installed (or on PYTHONPATH):
...
def prepared(sync):
    ...
    with tempfile.TemporaryDirectory() as home:
        p.save_config({"baseUrl": f"http://127.0.0.1:{server.server_port}", "workspace": "probe",
                       ...
                       "hosts": {"missher": {"recallSync": sync}}}, home)
        cfg = HonchoClientConfig.from_global_config(host="missher", config_path=Path(home) / "honcho.json")
