...
- **`forensics/`** reads a *copy* of a Missher `state.db` (plus rotated `agent.log*` and git) and
...
- A Missher checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.missher/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Missher is idle) and, optionally, the rotated `~/.missher/logs/agent.log*`.
- For `--live` probes: real credentials in `MISSHER_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.missher/logs/agent.log*"
...
Missher recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp MISSHER_HOME), main vs a branch or integration checkout:
...
Forensic lanes: five parallel Missher subagents (2026-09-04), rewritten here to take `--db`/`--root`
