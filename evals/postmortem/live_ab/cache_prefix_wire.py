...
message prefix between call N and N+1. If Missher strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--missher-home DIR]   (default MISSHER_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("MISSHER_HOME", os.path.expanduser("~/.missher"))
if "--missher-home" in sys.argv:
    os.environ["MISSHER_HOME"] = sys.argv[sys.argv.index("--missher-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.missher/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
