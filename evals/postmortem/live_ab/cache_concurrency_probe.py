...
Runs N independent Missher ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated missher-agent #104284 / #104421 and NousResearch/api#227 (2026-09-05/06,
...
Usage:
  ...
  providers: nous (Portal creds from MISSHER_HOME), openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Missher request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="missher-agent checkout to import from")
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Missher would pick for the provider/model)")
...
os.environ.setdefault("MISSHER_HOME", os.path.expanduser("~/.missher"))
import anthropic
