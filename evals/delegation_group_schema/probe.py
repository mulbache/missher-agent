...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, MISSHER_HOME=home, PATH="/usr/bin:/bin", MISSHER_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["missher-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["missher-cli"], quiet_mode=True)
