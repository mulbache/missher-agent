...
async def run(repo: str, out: str):
    ...
    for m in list(sys.modules):
        if m.startswith(("gateway", "plugins", "tools", "missher", "agent")):
            del sys.modules[m]
    ...
    r1 = await adapter.send_native_task_card_progress(
        ...
        metadata=metadata, fallback_text="Missher is working\n- terminal - ls - running")
...
def main():
    ...
    a = ap.parse_args()
    os.environ.setdefault("MISSHER_HOME", os.path.join(os.path.dirname(a.out), "missher-home-probe"))
    os.makedirs(os.environ["MISSHER_HOME"], exist_ok=True)
