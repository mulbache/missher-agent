#!/usr/bin/env python3
"""Google Workspace API CLI for Missher Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Missher-facing JSON contract and falls back to the Python client
...
from _missher_home import get_missher_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MISSHER_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Missher Agent")
