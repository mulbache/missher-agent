#!/usr/bin/env bash
# GitHub environment detection helper for Missher Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _missher_env="${MISSHER_HOME:-$HOME/.missher}/.env"; [ -f "$_missher_env" ] && grep -q "^GITHUB_TOKEN=" "$_missher_env" 2>/dev/null; then
