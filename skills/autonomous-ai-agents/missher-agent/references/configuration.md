...
Edit with `missher config edit` or `missher config set section.key value`.
Full reference: https://missher-agent.nousresearch.com/docs/user-guide/configuration
...
`missher config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `missher tools` (interactive) or `missher tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_MISSHER_CORE_TOOLS` is the default bundle most platforms inherit).
