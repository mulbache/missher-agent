...
Author widget apps for the Missher TUI (`missher --tui`): glanceable ambient
...
- The TUI must be in use (`missher --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.missher/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.missher/logs/tui_gateway_crash.log` if a widget never appears.
