...
Set via `missher model` (picker) or `missher setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://missher-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `missher auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `missher auth add openai-codex` |
| qwen-oauth | OAuth | `missher auth add qwen-oauth` |
| minimax-oauth | OAuth | `missher auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`missher auth`).
Fallback chain when the primary fails: `missher fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`missher_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   missher config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
