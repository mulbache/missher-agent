...
## Layer 1 — Is this thing a Missher plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Missher process**. Its LLM calls go through Missher's already-configured provider. | Already uses Portal if user's Missher is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Missher. That LLM call is already authenticated
via Missher's provider config. The plugin's own server auth (e.g.
...
What Missher Agent has that *feels* like OAuth — `missher login --provider nous`
opening a browser, user signs in, token lands in `~/.missher/auth.json` — is a
**Missher-specific browser flow**. Under the hood it produces a credential
Missher uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Missher's existing Portal credential out of `~/.missher/auth.json`.
...
rides on the credential Missher already holds.

Where this could live in Missher:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Missher itself.
...
1. First decide: Missher plugin (runs inside Missher) or separate app? If plugin,
   it already uses Portal via Missher's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Missher agent itself") is fully supported. Missher treats agent
...
pitch in the Tool Gateway announcement is specifically about Missher Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Missher is configured to use Portal as its provider. It is
