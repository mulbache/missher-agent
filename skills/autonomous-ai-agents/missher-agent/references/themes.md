# Themes / Skins — Author a Missher Color Theme

Author a Missher **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`missher_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.missher/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Missher itself.
...
- Write access to the Missher home dir — `~/.missher` by default, or `$MISSHER_HOME`
  / the active profile's dir. Skins live in `<missher-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `missher config set`).
...
3. `write_file` it to `<missher-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `missher config set display.skin default` (or they can `/skin default`).
...
```
missher skin set <key> <hex>      # e.g. missher skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `missher skin set background "#08201f"` changes
only the background; `missher skin use <name>` / `missher skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.missher`** when a profile is active — resolve the real home
  from `$MISSHER_HOME` first, falling back to `~/.missher`.
...
- **Never hand-edit `config.yaml` to activate.** Use `missher config set
...
- **You apply it, not the user.** `missher config set display.skin <name>` is
...
- `read_file` the written `<missher-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `missher config get display.skin` and confirm it reports `<name>`.
