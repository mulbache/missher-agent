...
For occasional contributors and PR authors. Full developer docs: https://missher-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.missher/config.yaml` (settings), `~/.missher/.env` (API keys) — both under `$MISSHER_HOME` when it is set.
...
`_MISSHER_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_missher_home()` for paths,
never hardcode `~/.missher`. For custom/local-only tools, write a plugin in
`~/.missher/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `missher_cli/commands.py`
...
- Tests auto-redirect `MISSHER_HOME` to temp dirs — never touch real `~/.missher/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/missher_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_missher_home()` from `missher_constants` for all paths (profile-safe)
