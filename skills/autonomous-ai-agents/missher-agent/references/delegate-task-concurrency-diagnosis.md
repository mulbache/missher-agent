...
**three** code paths in Missher that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Missher — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Missher
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Missher is
   refusing my value."
...
missher config get delegation.max_concurrent_children

# 2. Did Missher' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.missher/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with missher on path)
...
appears, **the cap is the model**, not Missher.
