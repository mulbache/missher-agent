...
Write plugins for the Missher desktop app: statusbar items, layout panes,
...
system (`~/.missher/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$MISSHER_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$MISSHER_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.missher-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Missher desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$MISSHER_HOME/desktop-plugins/` (usually
  `~/.missher/desktop-plugins/`).

## How to Run

1. Create `$MISSHER_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.missher/...` by default, or `~/.missher/profiles/<profile>/...` under a
...
The ONLY import surface is `@missher/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Missher
  ...
  (`missher://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.missher/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@missher/plugin-sdk`, `react`, and
