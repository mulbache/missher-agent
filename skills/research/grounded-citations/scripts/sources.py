...
Ledger path resolution (first wins):
  --ledger PATH
  $MISSHER_CITATION_LEDGER
  $MISSHER_HOME/cache/citations/ledger.json
"""
...
from _missher_home import get_missher_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("MISSHER_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_missher_home() / "cache" / "citations" / "ledger.json"
