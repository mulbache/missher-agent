#!/usr/bin/env python3
"""Canvas LMS API CLI for Missher Agent.
...
def _check_config():
    ...
    if missing:
        missher_env = os.path.join(
            os.environ.get("MISSHER_HOME", os.path.expanduser("~/.missher")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {missher_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Missher Agent"
    )
