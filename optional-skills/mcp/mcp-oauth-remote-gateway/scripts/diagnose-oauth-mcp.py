...
Usage:
  ...
  <server_name> matches the files in $MISSHER_HOME/mcp-tokens/<server>.json etc.
...
def _missher_home():
    # Prefer Missher' own resolver (profile-safe); fall back to env then ~/.missher.
    try:
        from missher_constants import get_missher_home
        return str(get_missher_home())
    except Exception:
        return os.environ.get("MISSHER_HOME") or os.path.expanduser("~/.missher")


def _tokens_dir():
    return os.path.join(_missher_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "missher-diag", "version": "1.0"}},
        },
        headers={
