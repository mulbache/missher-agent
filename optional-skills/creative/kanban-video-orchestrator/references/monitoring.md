...
missher kanban watch --tenant <project-slug>

# Snapshot of the board
missher kanban list --tenant <project-slug>
missher kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
missher kanban stats --tenant <project-slug>

# Visual dashboard (browser)
missher dashboard

# Inspect a specific task (includes comments + events)
missher kanban show <task-id>

# Follow a single task's event stream
missher kanban tail <task-id>
```

Verify available subcommands with `missher kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `missher kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `missher kanban block <id>` then `missher kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `missher kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `missher kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `missher profile list` (profile exists?), `missher status` (gateway/dispatcher up?), and verify tenant. |
...
missher kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
missher kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(missher kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
missher kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
missher kanban block <task-id>
missher kanban archive <task-id>

# Diagnose what happened
missher kanban show <task-id>      # task body, comments, recent events
missher kanban tail <task-id>      # follow the live event stream
missher kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `missher kanban create` for the director
...
while true; do
    clear
    missher kanban list --tenant <slug>
    echo "---"
    missher kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `missher kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `missher profile list` and start
