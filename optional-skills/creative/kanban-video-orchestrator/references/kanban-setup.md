...
the actual `setup.sh` that creates the project workspace, configures Missher
...
2. **Create profiles** — `missher profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.missher/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `missher kanban create` assigned to the director
...
```bash
missher profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.missher/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.missher/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Missher
...
Each profile gets a `SOUL.md` at `~/.missher/profiles/<name>/SOUL.md` that
...
```bash
missher kanban create "Direct production of <video title>" \
    --assignee director \
...
the Missher `.env` (`${MISSHER_HOME:-$HOME/.missher}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _missher_env="${MISSHER_HOME:-$HOME/.missher}/.env"
    if grep -q "^${var}=" "$_missher_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_missher_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_missher_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY missher ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY missher OPENROUTER_API_KEY || exit 1
# ...
```
