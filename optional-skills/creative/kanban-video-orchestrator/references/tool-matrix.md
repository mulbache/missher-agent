...
Maps each role archetype to the Missher skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public missher-agent
...
## Missher skills relevant to video production

### Visual / rendering skills (`missher-agent/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `missher plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`missher-agent/skills/creative/`)
...
### Audio / media skills (`missher-agent/skills/creative/` + `skills/media/`)
...
These are **not** Missher skills but external CLIs / APIs that profiles invoke.
...
## Built-in Missher tools for media review

These are native Missher tools — not invoked via terminal but through their own
...
key is present in `${MISSHER_HOME:-~/.missher}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Missher profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${MISSHER_HOME:-~/.missher}/.env` → environment variable.
...
When a new Missher-public video skill ships:
