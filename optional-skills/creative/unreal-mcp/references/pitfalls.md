...
### 1. Start order: editor first, Missher second

Missher probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Missher session.
...
restart the editor, restart the Missher session.
...
server fails to bind (Output Log) or Missher' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.missher/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Missher

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Missher' connection
lives in `~/.missher/config.yaml` via `missher mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Missher — just don't
mistake it for the Missher setup step.
...
explicitly warns against overlapping calls. Missher executes same-turn tool
...
### 9. Timeouts: Missher gives up before Unreal does

Missher' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Missher runs elsewhere (SSH backend,
