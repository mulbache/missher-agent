...
All per-subject data lives under PDD_DATA_DIR (default: $MISSHER_HOME/unbroker),
which is the same trust boundary Missher uses for .env and OAuth tokens.
"""
...
def missher_home() -> Path:
    return Path(os.environ.get("MISSHER_HOME") or (Path.home() / ".missher"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else missher_home() / "unbroker"
