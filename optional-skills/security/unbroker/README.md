...
The design is **Missher-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
missher skills install official/security/unbroker
```

Then start a new Missher session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Missher session:
...
- **PII handling.** Dossiers live under the Missher home directory (`0600`, optionally
