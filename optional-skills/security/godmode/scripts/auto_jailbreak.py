...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MISSHER_HOME", os.path.expanduser("~/.missher")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MISSHER_HOME", Path.home() / ".missher")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Missher config paths
# ═══════════════════════════════════════════════════════════════════

MISSHER_HOME = Path(os.getenv("MISSHER_HOME", Path.home() / ".missher"))
CONFIG_PATH = MISSHER_HOME / "config.yaml"
PREFILL_PATH = MISSHER_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Missher models — already uncensored, just needs clean prompt
    "missher": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "missher" in model_lower or "nous" in model_lower:
        return "missher"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Missher config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.missher/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Missher for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Missher for changes to take effect.")
