...
Usage (via cron with --no-agent):
    ...
    missher cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$MISSHER_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Missher-Watcher/1.0"})
