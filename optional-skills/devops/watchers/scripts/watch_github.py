...
Usage (via cron with --no-agent):
    ...
    missher cron create missher-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$MISSHER_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name missher-issues --repo NousResearch/missher-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Missher .env file
(``${MISSHER_HOME:-~/.missher}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Missher-Watcher/1.0",
    }
