...
is typically only in `~/.missher/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Missher,
...
`missher-custom-providers` skill, pitfall 2). Missher hits this because it forwards
