...
timeout. Config is env-only (``MISSHER_STARTUP_WATCHDOG=0``,
``MISSHER_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "MISSHER_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "MISSHER_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (missher_cli.main / missher_cli.gateway /
...
def _process_missher_home() -> Path:
    """MISSHER_HOME for diagnostic files — stdlib-only replica of the missher_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<MISSHER_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_missher_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``MISSHER_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``MISSHER_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
