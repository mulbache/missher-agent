...
# ── Voice state: MISSHER_VOICE / MISSHER_VOICE_TTS are runtime-only env flags (never config.yaml)
...
def _voice_mode_enabled() -> bool:
    return os.environ.get("MISSHER_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    return os.environ.get("MISSHER_VOICE_TTS", "").strip() == "1"
...
def _end_voice_chat(*, stop_loop: bool, stop_tts: bool) -> None:
    ...
    os.environ["MISSHER_VOICE"] = os.environ["MISSHER_VOICE_TTS"] = "0"
    if stop_loop:
        with contextlib.suppress(Exception):
            from missher_cli.voice import stop_continuous
            stop_continuous()
    if stop_tts:
        with contextlib.suppress(Exception):
...
def _(rid, params: dict) -> dict:
    ...
    from missher_cli.active_sessions import PER_SESSION_EXCLUSIVE_SUBMIT
...
def _voice_toggle_mode(rid, params: dict) -> dict:
    ...
    os.environ["MISSHER_VOICE"] = "1" if enabled else "0"
    stop_hint = ""
    if enabled:
...
def _set_voice_tts(on: bool) -> None:
    ...
    os.environ["MISSHER_VOICE_TTS"] = "1" if on else "0"
    if not on:
        _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from missher_cli.voice import start_continuous
        ...
        with contextlib.suppress(Exception):
            from missher_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        import missher_cli.voice  # noqa: F401  (a missing module must answer 5026, not die in a thread)
    except Exception as e:
