...
def summarize_server(name: str, cfg: dict) -> Dict[str, Any]:
    ...
    """
    from missher_cli.mcp_config import _oauth_tokens_present
...
def resolve_profile(rid, params, err_fn) -> Tuple[Optional[Any], Optional[dict]]:
    """Resolve the optional ``profile`` param to a MISSHER_HOME override token.
    ...
    if not profile_dir or not profile_dir.is_dir():
        return None, err_fn(rid, 4064, f"profile '{profile}' not found")
    return set_missher_home_override(str(profile_dir)), None
# ---- END PLUGIN-COMPAT ----
