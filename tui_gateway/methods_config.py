...
from missher_constants import display_missher_home as _display_missher_home
...
def _cfg_get_provider(params):
    from missher_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _missher_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_missher_home), "display": _display_missher_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's MISSHER_HOME + ``.env`` secret scope (ContextVars: concurrent checks
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from missher_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Missher provider is configured.", source)
            ...
            from missher_cli.anon_auth import route_is_welcome_host
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``missher debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from missher_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
