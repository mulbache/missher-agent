...
class ConfigProviderRef(OpenModel):
    """``missher_cli/models.py::list_available_providers`` row."""
...
class ModelPricing(Result):
    """``missher_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
...
class ModelCapabilities(Result):
    """``missher_cli/inventory.py::_apply_capabilities``."""
...
class ModelOptionProvider(OpenModel):
    """One ``missher_cli/inventory.py::build_models_payload`` provider row (the union of every field
...
class GoalContractSnapshot(Result):
    """``missher_cli/goals.py::GoalContract.to_dict``."""
