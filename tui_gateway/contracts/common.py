...
class OpenModel(Result):
    ...
    (the closed set is owned elsewhere: missher_state rows, provider inventories)."""
...
class StoredSessionRow(OpenModel):
    """One ``sessions`` row as ``missher_state`` lists it (``session.list`` / ``session.info`` rows /
    ``sessions.changed``)."""
...
class MessageReaction(OpenModel):
    """One persisted reaction row (``missher_state_messages.set_message_reaction``); ``seen`` is
