...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.missher/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class SkillBrowseItem(OpenModel):
    """``missher_cli.skills_hub.browse_skills`` row."""
...
class SkillInspectInfo(OpenModel):
    """``missher_cli.skills_hub.inspect_skill``; ``{}`` when the identifier resolves nowhere."""
...
class PluginsManageResult(Result):
    ...
    ``install`` → ``missher_cli.plugins_cmd.dashboard_install_plugin``'s ok payload; ``update`` →
    ``ok``/``unchanged``/``sha``."""
