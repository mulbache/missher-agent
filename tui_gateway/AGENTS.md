...
activate with `missher --tui` or `MISSHER_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `MISSHER_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
themselves come from `missher_cli/commands.py` (`missher_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds missher-ink + tsx --watch)
npm start         # production
npm run build     # full build (missher-ink + tsc)
