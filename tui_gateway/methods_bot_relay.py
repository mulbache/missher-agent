...
def _relay_root() -> Path:
    ...
    every MISSHER_HOME, including non-``profiles/`` subdirs of ``~/.missher``."""
    from tools.bot_mode_probe import _default_home, _missher_root
    return _missher_root(Path(_default_home()))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``missher -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "missher" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="missher-relay-dm-", suffix=".txt", text=True)
        try:
