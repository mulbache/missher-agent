"""Unified removal contract for every credential source Missher reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``missher auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.missher/.env; hint when the shell exports it."""
    from missher_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like missher_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            f"(not in ~/.missher/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Missher.",
            f"  The pool entry is now suppressed — Missher will ignore "
            f"{env_var} until you run `missher auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_missher_pkce(provider: str, removed) -> RemovalResult:
    """~/.missher/.anthropic_oauth.json is ours — delete it outright."""
    from missher_constants import get_missher_home

    result = RemovalResult()
    oauth_file = get_missher_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Missher Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `missher model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `missher auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `missher auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `missher auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="missher_pkce",
        remove_fn=_remove_missher_pkce,
        description="~/.missher/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `missher auth add qwen-oauth` to re-enable if needed.",
        ),
