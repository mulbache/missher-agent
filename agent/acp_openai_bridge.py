"""OpenAI-shape bridge shared by Missher' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Missher' tool schemas travel INTO the
...
read/edit/execute tools forwards only Missher' agent-level tools, since re-offering overlapping ones
makes Missher redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``missher_projected_messages``)
    Missher reads off the ``create`` result; a plain list would drop them on the stream path."""
