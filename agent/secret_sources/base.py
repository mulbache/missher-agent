...
env-var-shaped values at process startup, AFTER ``~/.missher/.env`` has loaded
and BEFORE the rest of Missher reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
MISSHER_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("missher_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `missher secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `missher secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `missher secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `missher secrets {name} setup` to re-authenticate.",
