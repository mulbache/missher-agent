...
Pulls API keys from BSM at startup so they need not live in ``~/.missher/.env``.
``bws`` is auto-installed into ``<missher_home>/bin/bws`` (one pinned version,
...
# <missher_home>/cache/bws_cache.json holds only secret VALUES (never the access
...
_ENCRYPTED_CACHE_INFO = b"missher-bws-encrypted-cache-v1"
...
def _missher_bin_dir() -> Path:
    """Where Missher stores its managed binaries. Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<missher_home>/bin/bws`` first, then PATH, then optional auto-install."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="missher-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
class BitwardenSource(SecretSource):
    ...
    _AUTH_HINT = (
        "Run `missher secrets bitwarden token` to paste a fresh access "
        "token (create one in the Bitwarden web app: Secrets Manager → "
        "Machine accounts → Access tokens).  Wrong region?  Re-run "
        "`missher secrets bitwarden setup` and pick EU/self-hosted."
    )
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `missher secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `missher secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `missher secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_missher_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `missher secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `missher secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `missher secrets bitwarden setup` to install."
        )
        return result
