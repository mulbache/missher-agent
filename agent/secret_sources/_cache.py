...
a cache problem must never block Missher startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_missher_dotenv()``, else ``$MISSHER_HOME``/``~/.missher``."""
    if home_path is None:
        from missher_constants import get_missher_home

        home_path = get_missher_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<missher_home>/cache/<basename>``::
