...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Missher accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and missher-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Missher deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Missher UI bridge -------------------------------------
# The app-server bypasses the Missher tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Missher tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Missher' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.missher-tools.* prefix is stripped (users see Missher tools).
_INTERNAL_MCP_SERVER = "missher-tools"
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Missher tool name for a codex item (mirrors CodexEventProjector)."""
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: Missher' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, MISSHER_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Missher' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    request without the walk. MISSHER_CODEX_SDK_TRANSFORM=1 disables."""
    if os.environ.get("MISSHER_CODEX_SDK_TRANSFORM", "").strip().lower() in {"1", "true", "yes", "on"}:
