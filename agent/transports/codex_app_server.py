...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Missher secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Missher secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = missher_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # Native shell children remain unowned. Only Missher' managed MCP tool
        ...
        owned_task = os.environ.get("MISSHER_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "MISSHER_KANBAN_DB", "MISSHER_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.missher-mcp.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.missher-mcp.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            kanban_db = spawn_env.get("MISSHER_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("MISSHER_HOME", os.path.expanduser("~/.missher")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("MISSHER_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "missher", client_title: str = "Missher Agent",
