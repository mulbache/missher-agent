...
_GLOBAL_ENV_EXACT = frozenset({
    # Missher runtime / deployment
    "MISSHER_HOME", "MISSHER_PROFILE", "MISSHER_GATEWAY_LOCK_DIR",
    "MISSHER_MAX_ITERATIONS", "MISSHER_API_TIMEOUT",
    "MISSHER_REDACT_SECRETS", "MISSHER_NOUS_TIMEOUT_SECONDS",
    "_MISSHER_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "MISSHER_KANBAN_DB", "MISSHER_KANBAN_WORKSPACES_ROOT", "MISSHER_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "MISSHER_KANBAN_",
    "MISSHER_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Missher writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``missher_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(missher_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(missher_home) / ".env")
    try:
        from missher_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(missher_home))
    except Exception:
        external_secrets = {}
