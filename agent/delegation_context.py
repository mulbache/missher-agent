...
A Missher process may itself be a Kanban dispatcher worker with MISSHER_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("missher_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("missher_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "MISSHER_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "MISSHER_KANBAN_TASK", "MISSHER_KANBAN_RUN_ID", "MISSHER_KANBAN_CLAIM_LOCK",
    "MISSHER_KANBAN_GOAL_MODE", "MISSHER_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``MISSHER_KANBAN_*`` identity gate should use."""
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("MISSHER_KANBAN_TASK")
            or (env and (env.get("MISSHER_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
