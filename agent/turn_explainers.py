...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "turn_lease": (
        "the turn was stopped because another Missher process "
    ...
    "locked": (
        ...
        "(another Missher process was writing to the state "
    ...
    "replaced": (
        ...
        "`missher doctor --fix` or in-place FTS repair — stop "
    ...
    "deleted_wal": (
        "the turn was stopped because a live Missher process held a retired "
        ...
        "Check the logs for whether Missher captured the retired generation, "
        "then read the adjacent state.db.retired-wal-*/manifest.json. If "
        "manifest.main.mode is `copied`, inspect that artifact with `missher "
    ...
    "corrupt": (
        ...
        "1. Run `missher {profile_arg}doctor --fix`\n"
        "2. Stop the gateway, then recover with:\n"
        "   missher {profile_arg}sessions recover --source {db_path} --inspect-only\n"
        "   (if it reports recoverable) missher {profile_arg}sessions recover "
    ...
    "fts_index": (
        ...
        "recovery tools or restore a backup. Run `missher {profile_arg}doctor --fix` "
        "(or restart Missher, which repairs the index on open), then "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    ...
    "Check the state database health (`missher doctor`), then "
    "send your message again."
)
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``missher_cli.config``). Any failure → True (safe default: on)."""
    try:
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # Missher-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``MISSHER_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="MISSHER_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``MISSHER_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="MISSHER_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                # the process default, and a bare `missher` follows active_profile (#105887).
                ...
                body = body.replace(
                    "{backups_dir}", str(get_default_missher_root() / "backups")
                )
