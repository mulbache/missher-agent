"""``$MISSHER_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$MISSHER_HOME/cache/<kind>/``, creating parents as needed."""
    from missher_constants import get_missher_home
    path = get_missher_home() / "cache" / kind
