...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        for raw_schema in schemas:
            ...
            if tool_name in _MISSHER_CORE_TOOLS:
                logger.warning(
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        def _collect(provider: MemoryProvider) -> None:
            for raw_schema in provider.get_tool_schemas():
                ...
                elif schema["name"] not in _MISSHER_CORE_TOOLS and schema["name"] not in seen:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        """Initialize all providers, injecting ``missher_home`` so they resolve profile-scoped paths."""
        if "missher_home" not in kwargs:
            from missher_constants import get_missher_home
            kwargs["missher_home"] = str(get_missher_home())
