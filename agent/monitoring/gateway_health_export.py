...
_OBSERVABLE_METRIC_NAMES = (
    "missher.gateway.up", "missher.gateway.state", "missher.gateway.active_agents", "missher.gateway.busy",
    "missher.gateway.drainable", "missher.gateway.restart_requested", "missher.gateway.background_work",
    "missher.gateway.background_delegations", "missher.platform.up", "missher.platform.degraded",
    "missher.cron.scheduler.heartbeat_age_seconds", "missher.cron.scheduler.last_success_age_seconds",
    "missher.cron.scheduler.catch_up_occurrences", "missher.cron.jobs.enabled", "missher.cron.jobs.running",
    "missher.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="missher-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("missher.gateway.background_work", _read_background_work_count),
            ("missher.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("missher.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("missher.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'missher-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="missher-gateway-health-export", daemon=True)
            thread.start()
