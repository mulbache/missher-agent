...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "missher.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("missher.gateway.up", int(bool(gateway_running))),
        metric("missher.gateway.active_agents", active_agents),
        metric("missher.gateway.busy", int(bool(busy))),
        metric("missher.gateway.drainable", int(bool(drainable))),
        metric("missher.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("missher.gateway.state", 1, **{"missher.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"missher.platform": str(platform), "missher.platform.state": state}
        metrics.append(metric("missher.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("missher.platform.degraded", int(is_degraded), **pattrs, **{"missher.error_code": error_code}))
        if is_degraded:
