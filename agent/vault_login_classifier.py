...
INSPECTION_STAMP_ATTR = "data-missher-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-missher-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-missher-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-missher-vault-slot]").forEach((n) => n.removeAttribute("data-missher-vault-slot"));
