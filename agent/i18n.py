"""Lightweight i18n for Missher' static user-facing strings (approval prompts, a few gateway replies).
...
Language resolution: explicit ``lang=`` > ``MISSHER_LANGUAGE`` > ``display.language`` > ``en``.
"""
...
def _locales_dir() -> Path:
    """Locale dir: ``MISSHER_BUNDLED_LOCALES`` (sealed packaging, e.g. Nix) if it exists, else ``<repo-root>/locales``.

    The source path is returned even when missing so ``_load_catalog`` can log
    the path it looked at rather than raise.
    """
    override = os.getenv("MISSHER_BUNDLED_LOCALES", "").strip()
    if override and Path(override).is_dir():
        return Path(override)
    if override:
        logger.warning(
            "MISSHER_BUNDLED_LOCALES points to a non-directory path (%s); "
...
@lru_cache(maxsize=8)
def _config_language_cached(missher_home: str) -> str | None:
...
def _config_language() -> str | None:
    ...
    return _config_language_cached(str(get_missher_home()))
...
def get_language() -> str:
    """Resolve the active language using env > config > default order. ``MISSHER_LANGUAGE`` is a
    ...
    try:
        env_lang = get_secret("MISSHER_LANGUAGE")
    except UnscopedSecretError:
        env_lang = os.environ.get("MISSHER_LANGUAGE")  # unscoped default-profile path: environ IS its own value
