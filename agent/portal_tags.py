...
Every Missher request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=missher-agent", "client=missher-client-v<__version__>"]``.
The version is read live from ``missher_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("missher_affinity_scope", default=None)
...
def missher_client_tag() -> str:
    """``client=missher-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if missher_cli is unimportable)."""
    try:
        from missher_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=missher-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=missher-agent", missher_client_tag()]
