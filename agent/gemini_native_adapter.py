...
try:
    import missher_cli as _missher_cli

    _MISSHER_VERSION = str(_missher_cli.__version__)
except Exception:
    _MISSHER_VERSION = "0.0.0"
_API_CLIENT = f"missher-agent/{_MISSHER_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Missher typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.missher/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.missher/.env (get one at https://aistudio.google.com/app/apikey), or run `missher setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Missher retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No MISSHER_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
