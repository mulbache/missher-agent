...
YAML files in ``<MISSHER_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``MISSHER_BUNDLES_DIR`` override (tests) or ``<MISSHER_HOME>/skill-bundles``."""
    override = os.environ.get("MISSHER_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_missher_home() / "skill-bundles"
...
# File-level CRUD — used by `missher bundles`.
