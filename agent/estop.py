...
``missher pause`` writes a sentinel at ``$MISSHER_HOME/ESTOP``; ``missher resume``
...
(fail safe, e.g. ``touch ~/.missher/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.missher).
from agent.file_safety import _missher_home_path as _missher_home, _missher_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `missher pause`."""
    return _missher_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (MISSHER_HOME=~/.missher/profiles/<n>) must still honor an operator's ~/.missher/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Missher is paused{tag}. New work is on hold; run `missher resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `missher resume` (%s)",
