...
def _fm_field(fm: dict[str, Any], key: str) -> Any:
    """Top-level ``key`` or ``metadata.missher.<key>``; tolerant of the string-valued
    ...
    return missher.get(key) if isinstance(missher, dict) else None
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        try:
            return json.loads((get_missher_home() / "skills" / ".usage.json").read_text(encoding="utf-8"))
        except Exception:
            return {}
...
def _memory_cards() -> list[dict[str, Any]]:
    ...
    base = get_missher_home() / "memories"
...
def build_learning_graph() -> dict[str, Any]:
    ...
    roots = [("base", Path(__file__).resolve().parent.parent / "skills"), ("profile", get_missher_home() / "skills")]
    learned_skills = {
