...
from missher_cli._subprocess_compat import IS_WINDOWS, harden_git_argv, noninteractive_git_env, windows_hide_flags
...
_SENSITIVE_MISSHER_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    home, missher_home = Path(os.path.expanduser("~")).resolve(), get_missher_home().resolve()
    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES} | {missher_home / ".env"}
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS] + [missher_home / rel for rel in _SENSITIVE_MISSHER_DIRS]
    if path in blocked_exact:
        raise ValueError("path is a sensitive credential file and cannot be attached")
    if any(_is_under(path, blocked_dir) for blocked_dir in blocked_dirs):
        raise ValueError("path is a sensitive credential or internal Missher path and cannot be attached")
