...
from missher_constants import OPENROUTER_BASE_URL, missher_home_key
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``MISSHER_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from missher_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `missher update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    with contextlib.suppress(Exception):
        from missher_cli.models import OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER, opencode_zen_free_headers
    ...
    # Missher owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Missher sees one failure.
    # Missher owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Missher sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Missher control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="missher-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_MISSHER_TOOL_CALLS)."""
    try:
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        from missher_cli.auth import resolve_api_key_provider_credentials
        from missher_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_nous:
            ...
            try:
                from missher_cli.models_pricing import _resolve_nous_pricing_credentials
        ...
        _nous_kwargs = {}
        if is_nous:
            from missher_cli.models_pricing import _NOUS_CATALOG_TTL_SECONDS
    ...
    if is_nous:
        ...
        try:
            from missher_cli.models_pricing import nous_policy_allowed_ids, restrict_to_nous_policy
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://missher-agent.nousresearch.com",
    "X-Title": "Missher Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from missher_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``MISSHER_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``MISSHER_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    env_cache = os.environ.get("MISSHER_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("MISSHER_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "MissherAgent"}
...
from missher_cli import __version__ as _MISSHER_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://missher-agent.nousresearch.com",
    "X-Title": "Missher Agent",
    "User-Agent": f"MissherAgent/{_MISSHER_VERSION}",
}

# Nous Portal attribution extra_body. Tags come from agent.portal_tags so the client= marker
# tracks missher_cli.__version__ — never inline a literal here.
...
_AUTH_JSON_PATH = get_missher_home() / "auth.json"
...
def _auth_json_path():
    ...
    from missher_cli.auth import _auth_file_path
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if base_url_hostname(url) == "api.actual.inc":
        from missher_cli.auth import normalize_actual_base_url
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from missher_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("MISSHER_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Missher ``-900k`` picker suffix.
    ...
    def create(self, **kwargs) -> Any:
        from missher_cli.providers import is_actual_route

        if is_actual_route(
            getattr(self._client, "_missher_aux_effective_provider", ""),
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Missher plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``missher_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring MISSHER_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "MISSHER_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _read_nous_auth() -> Optional[dict]:
    """Nous provider state dict from the credential pool or ~/.missher/auth.json; None when not active with tokens."""
...
def _nous_api_key(provider: dict) -> str:
    ...
    from missher_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from missher_cli.auth import _agent_key_is_usable
        pool = load_pool("nous")
    except Exception as exc:
...
def _resolve_nous_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    if pooled is not None:
        return pooled
    try:
        from missher_cli.auth import resolve_nous_runtime_credentials
        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("MISSHER_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from missher_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(_scoped_key_env("MISSHER_XAI_BASE_URL"))
    ...
    try:
        from missher_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from missher_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from missher_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            with contextlib.suppress(ImportError):
                from missher_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from missher_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from missher_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import missher_xai_default_headers
        headers = dict(missher_xai_default_headers())
    else:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from missher_cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is None and not nous:
        logger.warning("Auxiliary Nous client unavailable: no Nous authentication found (run: missher auth).")
    ...
    from missher_cli.anon_auth import GUEST_MODEL, route_is_welcome_host
    ...
    if not _aux_probe_active():
        try:
            from missher_cli.models import get_nous_recommended_aux_model
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from missher_cli import config as _cfg_mod
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        from missher_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `missher setup` or `missher model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=missher_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from missher_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from missher_cli.nous_account import get_nous_portal_account_info
...
def _refresh_copilot_credentials() -> bool:
    from missher_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_codex_credentials() -> bool:
    from missher_cli.auth import resolve_codex_runtime_credentials
...
def _refresh_nous_credentials() -> bool:
    ...
    return _creds_have_api_key(resolve_nous_runtime_credentials(
        timeout_seconds=env_float("MISSHER_NOUS_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_missher_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    from missher_cli.fallback_config import resolve_entry_api_key
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._missher_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.missher/.env value after `missher model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: missher model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.missher/.env",
...
def _discovery_chain_allowed(main_provider: str, task: Optional[str] = None) -> bool:
    ...
    logger.warning(
        ...
        "configured — refusing to guess another logged-in provider. Re-authenticate (`missher model`) "
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_missher_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "MISSHER_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Missher owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from missher_cli.model_normalize import normalize_model_for_provider
...
def _is_actual_auxiliary_route(req: _ResolveRequest, base_url: str) -> bool:
    from missher_cli.auth import normalize_actual_base_url
    from missher_cli.providers import is_actual_route
    from missher_cli.route_identity import normalize_route_base_url
...
def _wrap_transport(req: _ResolveRequest, client_obj: Any, final_model_str: str,
    ...
    if _is_actual_auxiliary_route(req, base_url_str):
        ...
        client._missher_aux_effective_provider = "actual"
        return client
    needs_codex = not (
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_missher_aux_effective_provider", effective_provider)
...
def _resolve_nous_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if client is None:
        logger.warning("resolve_provider_client: nous requested but Nous Portal not configured (run: missher auth)")
        return None, None
    ...
    from missher_cli.providers import nous_api_mode
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: missher model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: missher model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: missher doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from missher_cli.models import opencode_zen_free_runtime as _oc_free_rt
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from missher_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("missher_cli.auth not available for provider %s", provider)
        return None, None
...
def _client_cache_key(
    ...
    return (missher_home_key(), provider, async_mode, base_url or "", api_key_key, api_mode or "", runtime_key, is_vision, task_key, pool_hint, model_key)
...
def _acquire_sync_aux_semaphore(task: Optional[str]) -> Optional[threading.BoundedSemaphore]:
    ...
    if limit is None:
        return None
    from missher_constants import missher_home_key
    return _cached_semaphore(_aux_sync_semaphores, (missher_home_key(), task), limit, threading.BoundedSemaphore)
...
def _acquire_async_aux_semaphore(task: Optional[str]):
    ...
    try:
        ...
        return None
    from missher_constants import missher_home_key
    return _cached_semaphore(_aux_async_semaphores, (missher_home_key(), task, id(loop)), limit, asyncio.Semaphore)
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from missher_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Missher manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    raise RuntimeError(
                        ...
                        f"a different provider with `missher model`.")
    ...
    if client is None:
        raise RuntimeError(f"No LLM provider configured for task={task} "
                           f"provider={resolved_provider}. Run: missher setup")
