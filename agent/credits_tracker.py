...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    ``missher_cli.models``' pricing cache (filled by the model picker; a miss never fetches). Fail-open to
    ...
    if _is_nous_welcome_route(base_url):
        return True
    try:
        from missher_cli.models import _is_model_free
        from missher_cli.models_pricing import peek_cached_pricing
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning("credits header version %d unsupported, ignoring — update Missher", version_val)
            return None
        fields: dict[str, Any] = {
...
# ── Dev fixtures (MISSHER_DEV_CREDITS_FIXTURE): throwaway scaffolding to trigger any notice state
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Fixture CreditsState for MISSHER_DEV_CREDITS_FIXTURE, or None (unknown name / unset). Prod-leak guard:
    applies ONLY when MISSHER_DEV_CREDITS is also on, so a stray fixture env var never surfaces fabricated balances."""
    name = os.environ.get("MISSHER_DEV_CREDITS_FIXTURE", "").strip()
    if not name or not is_truthy_value(os.environ.get("MISSHER_DEV_CREDITS")):
        return None
