...
# `export MISSHER_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in missher_cli/main.py, gateway/run.py, and cli.py) or `MISSHER_REDACT_SECRETS=false` in
# ~/.missher/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("MISSHER_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a MISSHER_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in missher_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    try:
        ...
        raw = scope.get("MISSHER_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``missher config`` / ``status`` / ``dump``):
...
# a shell rc/profile, Missher' own ``config.yaml`` where ``missher mcp add --env`` writes
...
_MISSHER_HOME_PREFIXES = ("$MISSHER_HOME/", "${MISSHER_HOME}/")
# ``$HOME/.missher/config.yaml`` keeps the ``.missher`` segment, so stripping the prefix is
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.missher`` directory or ``$MISSHER_HOME`` (never arbitrary YAML)."""
    path = arg.strip("\"'").replace("\\", "/")
    missher_home = False
    for prefix in _MISSHER_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    return parts[-1] == "config.yaml" and (missher_home or ".missher" in parts[:-1])
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, Missher ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
