"""Language Server Protocol (LSP) integration for Missher Agent.
...
# Routed multiplex profiles (MISSHER_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``missher chat`` exit leaks pyright processes for a few seconds
