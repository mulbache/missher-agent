...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<MISSHER_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_missher_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <MISSHER_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = missher_lsp_session_dir()
    inner = (
        ...
        f"-HostName Missher -HostProfileId missher -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
...
def missher_lsp_session_dir() -> str:
    """Return (and create) the dir for PSES session/log scratch files."""
    from missher_constants import get_missher_home
    d = os.path.join(str(get_missher_home()), "lsp", "pses")
