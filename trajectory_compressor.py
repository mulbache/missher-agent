...
from missher_constants import OPENROUTER_BASE_URL, get_missher_home
from agent.retry_utils import jittered_backoff
from missher_cli.env_loader import load_missher_dotenv

# Load .env from MISSHER_HOME first, then project root as a dev fallback.
load_missher_dotenv(missher_home=get_missher_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: missher setup")
