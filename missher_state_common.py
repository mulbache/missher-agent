"""Shared constants and helpers for the SessionDB family of modules.  Lives outside missher_state so
...
def routed_sessions_setting(key: str, env_var: str) -> Any:
    ...
    carrier CLI/cron children read). Under a multiplexer a routed turn runs with a MISSHER_HOME override
...
# 'new_session' (missher_cli/cli_session_mixin.py) without a reset child row.  A compression rotation must
...
# optimized via ``missher sessions optimize-storage``. A legacy DB sits at
...
# and must not serve reads until `missher sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `missher db optimize`. This is
...
# claims progress under SQLite transaction authority).  Mirrors `missher_state_repair._cross_process_repair_lock`:
...
# `<db>.repair.lock` (offline schema surgery, minutes in VACUUM).  Lives here: mixins cannot import missher_state.

# ── Cross-process full-FTS-rebuild admission (single authority) ────────────── Several independent Missher
# processes routinely share one state.db (gateway service, the Desktop app's `missher serve` backend,
...
# authority and is intentionally multi-process. Semantics mirror `missher_state_repair._cross_process_repair_lock`
...
# indeterminate liveness still defers. It lives here (not missher_state) because the search/schema mixins
# cannot import missher_state (cycle). The lock file is `<db>.fts_rebuild.lock`, distinct from
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Missher)"
    age = ""
