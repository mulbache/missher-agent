# nix/nixosModules.nix — the NixOS module for missher-agent
#
...
#
# Container mode: missher runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $MISSHER_HOME/.env
# and read by missher at startup — no container recreation needed for env changes.
#
# Tool resolution: the missher wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.missher-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      missher-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      missherHome = "${cfg.stateDir}/.missher";
      ...
      # MISSHER_HOME via addToSystemPackages, so they can save settings through the
      ...
      # missher user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "missher-container-entrypoint" ''
        set -eu

        MISSHER_UID="''${MISSHER_UID:?MISSHER_UID must be set}"
        MISSHER_GID="''${MISSHER_GID:?MISSHER_GID must be set}"

        # ── Group: ensure a group with GID=$MISSHER_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$MISSHER_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="missher"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$MISSHER_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$MISSHER_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$MISSHER_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$MISSHER_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="missher"
          TARGET_HOME="/home/missher"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$MISSHER_UID" -g "$MISSHER_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$MISSHER_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$MISSHER_UID:$MISSHER_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure MISSHER_HOME is owned by the target user.
        ...
        if [ -n "''${MISSHER_HOME:-}" ] && [ -d "$MISSHER_HOME" ]; then
          find "$MISSHER_HOME" \! -user "$MISSHER_UID" -exec chown "$MISSHER_UID:$MISSHER_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/missher-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/missher-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/missher ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/missher
          chmod 0440 /etc/sudoers.d/missher
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$MISSHER_UID" --regid="$MISSHER_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $MISSHER_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "missher-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        missher_bin=${containerDataDir}/current-package/bin/missher
      '';

      # Default: /var/lib/missher/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the missher group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit missherHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.missher-agent =
        common.sharedOptions {
          defaultPackage = missher-agent;
          defaultPackageText = lib.literalExpression "missher-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/missher";
              description = "State directory. Contains .missher/ subdir (MISSHER_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the missher CLI to environment.systemPackages and export
                MISSHER_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.missher symlink to the service
                  stateDir. These users are automatically added to the missher group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.missher-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the missher CLI to system PATH and export MISSHER_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.missher/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.MISSHER_HOME = missherHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.missher-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.missher-agent.workingDirectory;
                optionPath = "services.missher-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.missher-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the missher user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.missher-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed missher
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure missher is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${missherHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${missherHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."missher-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${missherHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${missherHome} ${cfg.workingDirectory}
                  ...
                  find ${missherHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${missherHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${missherHome}/$_subdir"
                    chmod 2770 "${missherHome}/$_subdir"
                    find "${missherHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg missherHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${missherHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${missherHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${missherHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.missher";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${missherHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "missher-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.missher -> stateDir/.missher for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.missher-agent = {
              description = "Missher Agent Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $MISSHER_HOME/.env by the activation script. load_missher_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.missher-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.missher-agent = {
              description = "Missher Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  MISSHER_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  MISSHER_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env MISSHER_UID="$MISSHER_UID" \
                    --env MISSHER_GID="$MISSHER_GID" \
                    --env MISSHER_HOME=${containerDataDir}/.missher \
                    --env MISSHER_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/missher gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
