# nix/packages.nix — Missher Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./missher-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.missherTui;
        web = full.missherWeb;
        desktop = full.missherDesktop;

        update-npm-lockfile = full.missherNpmLib.updateNpmLockfile;
      };
    };
}
