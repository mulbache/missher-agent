# nix/web.nix — Missher Web Dashboard (Vite/React) frontend build
{ missherNpmLib, ... }:
missherNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @missher/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../missher_cli/web_dist for the
