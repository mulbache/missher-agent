# nix/desktop.nix — Missher Desktop (Electron) app build + wrapper
#
# `missherAgent` is the fully-built `.#default` package — it ships the
# `missher` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `MISSHER_DESKTOP_MISSHER` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Missher CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # MISSHER_HOME and MISSHER_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "missher-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "missher-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = missherNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "missher-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe missherNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/missher-desktop $out/bin
    cp -r ${renderer}/* $out/share/missher-desktop/
    ...
    substituteInPlace $out/share/missher-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/missher-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # MISSHER_DESKTOP_MISSHER to the absolute path of the nix-built `missher`
    # binary so the desktop's resolver step 4 ("existing Missher CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/missher-desktop \
      --add-flags "$out/share/missher-desktop" \
      --set MISSHER_DESKTOP_MISSHER "${lib.getExe missherAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/missher.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../missher_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/missher-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/missher.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/missher.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Missher Agent";
    homepage = "https://github.com/NousResearch/missher-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "missher-desktop";
  };
}
