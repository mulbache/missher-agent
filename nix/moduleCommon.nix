...
#
# `services.missher-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  missherHome, programs.missher-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "missher-config-attrs";
    description = "Missher YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $MISSHER_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Missher Agent";
      ...
      package = mkOption {
        ...
        description = "The missher-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Missher configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `missher config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $MISSHER_HOME/.env. Missher reads that file at each start, with
          load_missher_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."missher/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Missher refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Missher reads SOUL.md and memories/ from MISSHER_HOME, so put those
          files in `missherHomeFiles`.
      ...
      missherHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into MISSHER_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Missher reads SOUL.md and the memory files from MISSHER_HOME and not
          from the working directory. Declare those files here, or Missher
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the missher plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Missher discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          missher_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as missher (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in missher-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `missher gateway`.";
      };
      ...
      # ── The backend: `missher serve` or `missher dashboard` ──────────────
      # `missher serve` and `missher dashboard` are the same entry point,
      # missher_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Missher Desktop
      ...
      # controls an external gateway, with `missher gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Missher
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            MISSHER_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Missher Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."missher/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `missher` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.missher-agent.installPackage was removed. Missher now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.missher-agent.enable = ${lib.boolToString (value != false)};  # the missher CLI, and MISSHER_HOME for your shells
        programs.missher-agent.desktop.enable = true;  # the desktop application

      `services.missher-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "missher-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "missher-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "missher-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "missher-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "missher-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # MISSHER_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.missherHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Missher make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${missherHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Missher
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${missherHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${missherHome}/config.yaml
            ${run}chmod ${modes.config} ${missherHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "missher-managed" managedSystem} ${missherHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${missherHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${missherHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${missherHome}/auth.json"
        else
          ''
            if [ ! -e ${missherHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${missherHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree missherHome cfg.missherHomeFiles}
      ...
      ${run}find ${missherHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "missher-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${missherHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/missher"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps missher as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "missher-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "missher-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "missher-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          MISSHER_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export MISSHER_DASHBOARD_SESSION_TOKEN

          if [ -z "$MISSHER_DASHBOARD_SESSION_TOKEN" ]; then
            echo "missher-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "missher-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "missher-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "missher-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "missher-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "missher-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "missher-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Missher Agent web dashboard and desktop backend"
    else
      "Missher Agent backend for Missher Desktop";

  # The environment that each Missher process needs, from either module.
  #
  # managedSystem gives the value of MISSHER_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Missher an identity and a memory, use
          ${optionPath}.missherHomeFiles instead. Those files go to
          MISSHER_HOME. Missher reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of MISSHER_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
