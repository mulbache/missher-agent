...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Missher derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            missher-agent = prev.missher-agent.overrideAttrs (_old: {
              MISSHER_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $MISSHER_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$MISSHER_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        missher-agent = prev.missher-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "missher-agent-env" {
    missher-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "missher-agent-editable-env" {
    missher-agent = dependency-groups;
  };
}
