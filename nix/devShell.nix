...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      missherNpmLib = self'.packages.default.passthru.missherNpmLib;
      ...
      missherAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "missher" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../missher} $out/bin/missher
          '')
        ...
        shellHook = ''
          ${missherAgentDevShellHook}
          ${missherNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export MISSHER_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Missher Agent dev shell in $MISSHER_PYTHON_SRC_ROOT"
          echo "Ready. Run 'missher' or 'sandbox missher' to start."
        '';
      };
    };
}
