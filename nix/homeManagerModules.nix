# nix/homeManagerModules.nix — the Home Manager module for missher-agent
#
# This module is the user-level equivalent of nixosModules.default. Missher is
...
#
# `services.missher-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.missher-agent.enable and
#                                        home.sessionVariables
#   added     programs.missher-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.missher")  -> missherHome, set directly
#
# To use the module:
#   imports = [ missher-agent.homeManagerModules.default ];
#   programs.missher-agent = {
#     enable = true;          # the missher CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.missher-agent;
      ...
      # not reach it, and the application would open ~/.missher while the
      # services use missherHome. Thus the launcher carries the value itself.
      #
      # MISSHER_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        MISSHER_HOME = cfg.missherHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) MISSHER_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        MISSHER_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("MISSHER_DESKTOP_REMOTE_URL is set
      # but MISSHER_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          MISSHER_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export MISSHER_DESKTOP_REMOTE_TOKEN
        else
          echo "missher-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "missher-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.missher-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Missher needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.missher-agent` stays the authority for the state and the
      # configuration. This module reads missherHome and the backend address
      # from it, and never the reverse.
      options.programs.missher-agent = {
        enable = lib.mkEnableOption ''
          the Missher Agent command line application.

          This adds `missher` to home.packages, and exports MISSHER_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.missher-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.missher-agent.package";
          description = ''
            The missher-agent package to install.

            The default follows `services.missher-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Missher Desktop application (Electron).

            This adds `missher-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Missher
            runtime that `package` gives, and reads the MISSHER_HOME of
            `services.missher-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Missher runtime with
            the usual distribution. This module gives it the Nix package
            instead, with MISSHER_DESKTOP_MISSHER. It installs no second copy
            of Missher, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.missherDesktop;
            defaultText = lib.literalExpression "config.programs.missher-agent.package.missherDesktop";
            description = ''
              The missher-desktop package to use.

              The default follows `package`, and thus also
              `services.missher-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Missher runtime, and this module cannot make
      ...
      options.services.missher-agent =
        ...
        // {
          missherHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.missher";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.missher"'';
            description = ''
              The value of MISSHER_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.missher` to it.
              This module sets MISSHER_HOME directly. Thus an existing
              ~/.missher continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.missher-work";
          };

          # `installPackage` moved to `programs.missher-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `missher` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.missher-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.MISSHER_HOME = cfg.missherHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # MISSHER_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put MISSHER_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `missher` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.missher-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.missherAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) missherHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.missher-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.missher-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.missher-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "missher-agent";
              };
            })

            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.missher-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "missher-backend";
              };
            })
          ]
        ))
