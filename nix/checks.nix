...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      missher-agent = self'.packages.default;
      missherVenv = missher-agent.missherVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "missher-check";
                homeDirectory = "/home/missher-check";
                stateVersion = "24.11";
              };
            }
            { services.missher-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.missher-agent = programs;
              services.missher-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.missher-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.missher-agent);
      ...
      configKeys = pkgs.runCommand "missher-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${missherVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "missher-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "missher-build-package" { } ''
          echo "PASS: package built at ${missher-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "missher-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.MISSHER_TEST = "1";
                environmentFiles = [ "/run/secrets/missher-env" ];
                missherHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "missher" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "missher" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.missherAgentSetup.data;

            failures =
              lib.optional (names != [
                "missher-agent"
                "missher-backend"
              ]) "expected missher-agent + missher-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/missher gateway" (argvOf "missher-agent")
              ) "gateway process does not run `missher gateway`: ${argvOf "missher-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/missher serve" (argvOf "missher-backend")
              ) "backend process does not run `missher serve`: ${argvOf "missher-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "missher-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/missher-check/.missher" (envOf n)) names
              ) "gateway and backend must share one MISSHER_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.MISSHER_HOME or null != "/home/missher-check/.missher"
              ) "programs.missher-agent.enable must export MISSHER_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "missher-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/missher-check/.missher/SOUL.md" activation
              ) "missherHomeFiles must install into MISSHER_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads MISSHER_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "MISSHER_MANAGED=home-manager" (envOf n)) names
              ) "processes must report MISSHER_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "missher-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "missher-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/missher-check";
            ...
            cases = [
              ...
              {
                name = "missherHomeFiles needs no directory";
                ok = accepts { missherHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "missher-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one MISSHER_HOME ───────────────
        # `programs.enable` exports MISSHER_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.missher
        # while the services use the MISSHER_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/missher-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                missherHome = "/home/missher-check/.missher-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "missher-desktop") cfg.home.packages;
            ...
            # second Missher runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "missher-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.missher-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.missher-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one missher-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "MISSHER_HOME" != "/home/missher-check/.missher-work"
              ) "the launcher must carry MISSHER_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "MISSHER_HOME")})"
              ++ lib.optional (
                setValue "MISSHER_MANAGED" != "home-manager"
              ) "the launcher must report MISSHER_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "MISSHER_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "MISSHER_DESKTOP_MISSHER" != "${lib.head agentPackages}/bin/missher"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "MISSHER_DESKTOP_MISSHER")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "MISSHER_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "MISSHER_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "MISSHER_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "MISSHER_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "MISSHER_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "missher-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares MISSHER_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set MISSHER_HOME" wrapper
              ) "the launcher must carry MISSHER_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "MISSHER_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "MISSHER_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? missher-backend || cfg.launchd.agents ? missher-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "missher-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `missher`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.missher-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.missher-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "missher-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "missher-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "missher" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."missher-agent-setup".text;

            failures =
              lib.optional (names != [
                "missher-agent"
                "missher-backend"
              ]) "expected missher-agent + missher-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/missher gateway" (execOf "missher-agent")
              ) "gateway unit does not run `missher gateway`: ${execOf "missher-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/missher dashboard" (execOf "missher-backend")
              ) "backend unit does not run `missher dashboard`: ${execOf "missher-backend"}"
              ++ lib.optional (
                units.missher-agent.environment.MISSHER_HOME != units.missher-backend.environment.MISSHER_HOME
              ) "gateway and backend must share one MISSHER_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/missher/.missher/SOUL.md" activation
              ) "missherHomeFiles must install into MISSHER_HOME";
          ...
          in
          pkgs.runCommand "missher-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs missher, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.missher-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/missher serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec missher directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "missher-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "missher-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec missher, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/missher dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "missher-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $MISSHER_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                MISSHER_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "missher-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'MISSHER_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--missher-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "missher-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "missher-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${missher-agent}/bin/missher || (echo "FAIL: missher binary missing"; exit 1)
          test -x ${missher-agent}/bin/missher-agent || (echo "FAIL: missher-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${missher-agent}/bin/missher --version 2>&1 | grep -qi "missher" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "missher-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in missher missher-agent missher-acp; do
            test -x ${missher-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "missher-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking missher --help ==="
          ${missher-agent}/bin/missher --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${missher-agent}/bin/missher --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "missher-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${missher-agent}/share/missher-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${missher-agent}/share/missher-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "MISSHER_BUNDLED_SKILLS" ${missher-agent}/bin/missher || \
            (echo "FAIL: MISSHER_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: MISSHER_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${missher-agent}/share/missher-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${missher-agent}/share/missher-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "MISSHER_OPTIONAL_SKILLS" ${missher-agent}/bin/missher || \
            (echo "FAIL: MISSHER_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, MISSHER_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "missher-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${missher-agent}/share/missher-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${missher-agent}/share/missher-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "MISSHER_BUNDLED_PLUGINS" ${missher-agent}/bin/missher || \
            (echo "FAIL: MISSHER_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: MISSHER_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "missher-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${missher-agent}/share/missher-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${missher-agent}/share/missher-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${missher-agent}/share/missher-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "MISSHER_BUNDLED_LOCALES" ${missher-agent}/bin/missher || \
            (echo "FAIL: MISSHER_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: MISSHER_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + MISSHER_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && MISSHER_BUNDLED_LOCALES=${missher-agent}/share/missher-agent/locales \
            ${missherVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with MISSHER_BUNDLED_LOCALES set"; exit 1)
        ...
        # MISSHER_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "missher-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${missher-agent}/share/missher-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${missher-agent}/share/missher-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "MISSHER_OPTIONAL_MCPS" ${missher-agent}/bin/missher || \
            (echo "FAIL: MISSHER_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: MISSHER_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${missher-agent}/bin/missher mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: missher mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "missher-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${missher-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${missher-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "MISSHER_TUI_DIR" ${missher-agent}/bin/missher || \
            (echo "FAIL: MISSHER_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: MISSHER_TUI_DIR set in wrapper"
        ...
        # Verify MISSHER_NODE is set in wrapper and points to Node 26+
        # (Missher pins its toolchain to Node 26 everywhere)
        missher-node = pkgs.runCommand "missher-node-version" { } ''
          set -e
          echo "=== Checking MISSHER_NODE in wrapper ==="
          grep -q "MISSHER_NODE" ${missher-agent}/bin/missher || \
            (echo "FAIL: MISSHER_NODE not set in wrapper"; exit 1)
          echo "PASS: MISSHER_NODE present in wrapper"

          MISSHER_NODE=$(sed -n "s/^export MISSHER_NODE='\(.*\)'/\1/p" ${missher-agent}/bin/missher)
          test -x "$MISSHER_NODE" || (echo "FAIL: MISSHER_NODE=$MISSHER_NODE not executable"; exit 1)
          echo "PASS: MISSHER_NODE executable at $MISSHER_NODE"

          NODE_MAJOR=$("$MISSHER_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Missher requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All MISSHER_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify MISSHER_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "missher-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(MISSHER_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking MISSHER_MANAGED guards ==="
          check_blocked "config set" ${missher-agent}/bin/missher config set model foo
          check_blocked "config edit" ${missher-agent}/bin/missher config edit
        ...
        extra-python-packages = let
          ...
          missherWithExtra = missher-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "missher-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${missherWithExtra}/bin/missher || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${missherWithExtra}/bin/missher || \
          ...
          if grep -q "PYTHONPATH" ${missher-agent}/bin/missher; then
        ...
        extra-dependency-groups = let
          missherWithGroups = missher-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "missher-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${missherWithGroups}"
          echo "venv: ${missherWithGroups.missherVenv}"
        ...
        messaging-variant = pkgs.runCommand "missher-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.missherVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "missher-config-roundtrip" {
          ...
          merge_and_load() {
            local missher_home="$1"
            export MISSHER_HOME="$missher_home"
            ${configMergeScript} ${nixSettings} "$missher_home/config.yaml"
            ${missherVenv}/bin/python3 -c '
import json, sys
