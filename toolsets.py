...
_MISSHER_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `missher-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _MISSHER_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_MISSHER_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _MISSHER_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `missher tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``missher tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _MISSHER_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (MISSHER_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _MISSHER_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in missher_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Missher toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. missher-acp is the
    ...
    "missher-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors missher-cli; `missher tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "missher-cron": _bundle("Default cron toolset - same core tools as missher-cli; gated by `missher tools`"),
    "missher-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "missher-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "missher-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "missher-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "missher-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "missher-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "missher-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "missher-email": _bundle("Email bot toolset - interact with Missher via email (IMAP/SMTP)"),
    "missher-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "missher-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "missher-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "missher-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "missher-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "missher-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "missher-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "missher-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "missher-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _MISSHER_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "missher-sms": _bundle("SMS bot toolset - interact with Missher via SMS (Twilio)"),
    "missher-webhook": _ts("Webhook toolset - receive and process external webhook events", _MISSHER_WEBHOOK_SAFE_TOOLS),
    "missher-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "missher-telegram", "missher-discord", "missher-whatsapp", "missher-slack",
            "missher-signal", "missher-bluebubbles", "missher-homeassistant", "missher-email",
            "missher-sms", "missher-mattermost", "missher-matrix", "missher-dingtalk",
            "missher-feishu", "missher-wecom", "missher-wecom-callback", "missher-weixin",
            "missher-qqbot", "missher-webhook", "missher-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _MISSHER_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only missher-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_MISSHER_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `missher-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("missher-"):
        return []
    platform_name = name[len("missher-"):]
    try:
