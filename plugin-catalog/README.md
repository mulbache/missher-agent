# Missher Plugin Catalog

Curated, Nous-approved Missher plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`missher plugins catalog` / `missher plugins search` and installable with
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `missher-agent` repository, reviewed and merged by a maintainer. There is
...
3. **Pin maturity.** The pinned release should be **at least 2 weeks old**
   ...
   compromised release before Missher ships a pointer to it.
...
requires_missher: ">=0.19"   # optional
docs_url: ""                # optional
