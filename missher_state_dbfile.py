"""state.db file-level health helpers, split out of ``missher_state.py``.
...
holder-process classification.  Helpers that missher_state itself imports and
calls (``_connect_tracked_db`` & co) are looked up lazily from ``missher_state`` at
call time, so tests that monkeypatch ``missher_state.<name>`` keep intercepting.
"""
...
from missher_state_holders import canonical_sqlite_path
...
# shared lock (see missher_cli/sqlite_safe_read.py) so another process can treat this writer as
...
def _read_sqlite_application_id(db_path: Path) -> "Optional[int]":
    ...
    from missher_state_errors import _STATE_DB_APPLICATION_ID_OFFSET
...
def refuse_deleted_wal_generation(db_path) -> None:
    ...
    *before* ``sqlite3.connect`` so a second opener cannot mint a replacement WAL inode."""
    from missher_state import DeletedWalGenerationError
    from missher_state_errors import _DELETED_WAL_GENERATION_MSG
...
# pair that `missher sessions recover --source <dir>/state.db` can open. Above it only the 100-byte
    ...
    try:
        ...
        manifest: Dict[str, Any] = {
            ...
            "note": ("Frames in the captured WAL were committed by the retired generation. Whether they "
                     ...
                     "the copied image with `missher sessions recover --inspect-only` first."),
        }
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    without missher_cli); a real connection failure must propagate — a silent untracked retry
    would disable the guard for that connection."""
    try:
        from missher_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug("missher_cli.sqlite_safe_read unavailable; opening %s untracked "
                     "(byte-probe guard inactive in this install)", path)
        return sqlite3.connect(str(path), **kwargs)
    # Open through THIS module's sqlite3.connect so tests patching missher_state.sqlite3.connect keep control.
...
def is_zeroed_state_db(path: Path, *, probe_bytes: int = 100, force: bool = False) -> bool:
    ...
    ``missher_cli.backup.is_zeroed_sqlite_file``; this copy keeps SessionDB openable without the CLI
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error("quarantine lock for %s not acquired within 5s — refusing to "
                         ...
                         "state-snapshots via `missher snapshot list` / `missher snapshot restore <id>`.",
                         path)
            return None
        return _do_quarantine()
...
def collect_state_db_stats(db_path: Path) -> Dict[str, Any]:
    ...
    from missher_state import _connect_tracked_db
