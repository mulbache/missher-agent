...
def _missher_tags(fm: dict) -> Any:
    """``metadata.missher.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    missher_meta = metadata.get("missher", {}) if isinstance(metadata, dict) else None
    return missher_meta.get("tags", []) if isinstance(missher_meta, dict) else []
