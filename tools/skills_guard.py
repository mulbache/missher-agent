...
_MISSHER_CONFIG_FILES = r'\.missher/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.missher/\.env|\~/\.missher/\.env',
     "missher_env_access", "critical", "exfiltration", "directly references Missher secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Missher/other config (setup docs routinely say
    ...
    (_prose_modify_re(_MISSHER_CONFIG_FILES),
     "missher_config_mod", "high", "persistence", "modification language aimed at Missher configuration files (verify intent)"),
    (_shell_write_re(_MISSHER_CONFIG_FILES),
     "missher_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Missher configuration files"),
    (r'\.missher/config\.yaml|\.missher/SOUL\.md',
     "missher_config_ref", "low", "persistence", "references Missher configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Missher-native; `.clawhubignore` is honored for skills published through ClawHub.
