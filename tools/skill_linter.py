...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    missher_meta = meta.get("missher") if isinstance(meta, dict) else None
    if not isinstance(missher_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.missher.{tags, related_skills}.")
    elif "tags" not in missher_meta:
        yield _warn("missing-metadata", "metadata.missher.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("missher", "agent", "missher agent") and (
        author != "Missher Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Missher Agent' (proper caps) "
