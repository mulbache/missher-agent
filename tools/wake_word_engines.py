...
class _OpenWakeWordEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        # Default (or explicit "hey_missher") → the bundled model; built-in names / paths as-is.
...
# GigaSpeech), downloaded once under MISSHER_HOME. Keywords are tokenized at RUNTIME.
...
def _sherpa_model_root() -> Path:
    ...
    return get_missher_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        kw = tempfile.NamedTemporaryFile(mode="w", suffix=".txt", prefix="missher-kws-", delete=False,
                                         encoding="utf-8")
