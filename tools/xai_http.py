...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_missher_home() / "auth.json"
...
def missher_xai_user_agent() -> str:
    """Return a stable Missher-specific User-Agent for xAI HTTP calls."""
    try:
        from missher_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Missher-Agent/{__version__}"


def missher_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": missher_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Missher home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from missher_constants import get_missher_home
        marker_dir = get_missher_home() / "state"
...
def _xai_base_url_override() -> str:
    """``MISSHER_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    from missher_cli.config import get_env_value
    return str(get_env_value("MISSHER_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Missher-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.missher/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``MISSHER_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Missher-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``missher_cli.config.get_env_value`` so keys stored in ``~/.missher/.env`` (the standard Missher
    ...
    scoping is identical to the fallback branch, and the base URL honors ``MISSHER_XAI_BASE_URL`` /
