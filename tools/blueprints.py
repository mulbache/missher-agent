...
``metadata.missher.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.missher.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.missher.blueprint`` is a mapping with a
    ...
    missher = meta.get("missher") if isinstance(meta, dict) else None
    blueprint = missher.get("blueprint") if isinstance(missher, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.missher.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.missher.blueprint`` block) ready for ``missher skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"missher": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
