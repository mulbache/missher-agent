...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(missher_home).expanduser() if missher_home else Path.home() / ".missher") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Missher's live source checkout "
        ...
        "is pushed. To change this checkout, stop Missher, run the command externally, then restart "
        "Missher.")
