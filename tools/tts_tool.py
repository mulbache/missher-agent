...
def _get_default_output_dir() -> str:
    ...
    return str(get_missher_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    MISSHER_HOME and later scope requests to a different profile via
    ``missher_constants.set_missher_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped MISSHER_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("missher_cli.config not available, using default TTS config")
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `missher setup` to install Mistral support."),
    ...
    "neutts": (lambda: _check_neutts_available(), "NeuTTS (local)", "_generate_neutts",
               ...
               "Run missher setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."),
    "kittentts": (lambda: _importable(_import_kittentts), "KittenTTS (local, ~25MB)", "_generate_kittentts",
                  ...
                  "Run 'missher setup tts' and choose KittenTTS, or install manually: "
    ...
    "piper": (lambda: _importable(_import_piper), "Piper (local)", "_generate_piper_tts",
              ...
              "Run 'missher tools' and select Piper under TTS, or install manually: "
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("MISSHER_SESSION_PLATFORM", "").lower()
...
def _tts_schema_overrides() -> dict:
    ...
    params["properties"]["output_path"]["description"] = _output_path_description(display_missher_home())
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": _output_path_description("the profile MISSHER_HOME")
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'missher_xai_user_agent': ('tools.xai_http', 'missher_xai_user_agent'),
