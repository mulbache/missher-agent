...
is ALWAYS host-side ``$MISSHER_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
STORAGE_DIR = "/tmp/missher-results"
...
def get_spillover_dir():
    """Return $MISSHER_HOME/cache/spillover as a Path (not created)."""
    from missher_constants import get_missher_home
    return get_missher_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $MISSHER_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/missher-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "MISSHER_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
