...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("MISSHER_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Missher YOLO. Lazy import so approval-only
...
def _permanent_set() -> set:
    ...
    profile (MISSHER_HOME override) → its own set, lazily loaded from ITS ``command_allowlist``: the
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``missher chat -q``
    exports MISSHER_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    (-q) exports MISSHER_INTERACTIVE=1 but nobody answers prompts, and MISSHER_EXEC_ASK has no
    ...
    is_ask = env_var_enabled("MISSHER_EXEC_ASK")
    if _is_single_query_approval_context():
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set MISSHER_INTERACTIVE or "
                               "MISSHER_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set MISSHER_INTERACTIVE or "
                       "MISSHER_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_missher_interactive_context': ('tools.approval_context', 'reset_missher_interactive_context'),
    ...
    'set_missher_interactive_context': ('tools.approval_context', 'set_missher_interactive_context'),
}
