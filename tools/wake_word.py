...
_BUNDLED_MODEL_NAME = "hey_missher"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_missher", "hey hermes", "missher"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_missher model (.onnx/.tflite) for ``framework``."""
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from missher_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Missher backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `missher tools` "
...
# ── Process-wide singleton (mirrors missher_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_missher_root() / "runtime" / "wake-word.lock"
