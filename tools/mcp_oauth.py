...
refresh; this module supplies ``MissherTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Missher' Client ID Metadata
...
from missher_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__missher_user_skipped__"


def _get_token_dir(missher_home: str | Path | None = None) -> Path:
    """``MISSHER_HOME/mcp-tokens/`` — per-profile token directory."""
    from missher_constants import get_missher_home

    return Path(missher_home if missher_home is not None else get_missher_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "MissherTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "MissherTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``missher
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `missher mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
class MissherTokenStorage:
    """Persist OAuth state as ``MISSHER_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, missher_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._missher_home = Path(missher_home) if missher_home is not None else None
    ...
    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._missher_home) / f"{self._server_name}{suffix}"
    ...
    def _fixup_loaded_tokens(self, data: dict) -> None:
        # ``missher_issuer`` is Missher bookkeeping, not an SDK OAuthToken field: pop before validation.
        self.loaded_issuer = data.pop("missher_issuer", None)
    ...
    async def set_tokens(self, tokens: "OAuthToken") -> None:
        ...
        if self._bound_issuer:  # which authorization server granted these tokens (never sent on the wire)
            payload["missher_issuer"] = self._bound_issuer
    ...
    def stamp_issuer(self, issuer: str) -> None:
        """Backfill ``missher_issuer`` onto a pre-binding token file: adopt the currently discovered
        issuer once instead of forcing a re-login, so the *next* read is protected."""
        data = _read_json(self._tokens_path())
        if data is None or data.get("missher_issuer"):
            return
        data["missher_issuer"] = str(issuer)
        try:
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._missher_home)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Missher.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `missher mcp login <server>` later to authenticate, "
...
_SSH_HINT_LOOPBACK = (
    ...
    "\n"
    "  See: https://missher-agent.nousresearch.com/docs/guides/oauth-over-ssh\n")
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Missher' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
MissherOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, missher_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    MissherTokenStorage(server_name, missher_home=missher_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Missher only decides
# eligibility. Published from ``website/static/oauth/client-metadata.json``; the github.io origin is
# deliberate — servers MUST NOT follow redirects when fetching it, and missher-agent.nousresearch.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = "https://nousresearch.github.io/missher-agent/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "MissherTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after Missher must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "MissherTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "MissherTokenStorage | None" = None) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "Missher Agent"),
...
def _invalidate_tokens_on_client_change(
    storage: "MissherTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: missher mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "MissherTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Missher is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    global MissherOAuthClientProvider
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `missher mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if MissherOAuthClientProvider is None:
        from tools.mcp_oauth_provider import MissherProviderMixin

        MissherOAuthClientProvider = type("MissherOAuthClientProvider", (MissherProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Missher' token-endpoint fixes (see ``MissherProviderMixin``).",
            "__module__": __name__, "_missher_logger": logger})
    return MissherOAuthClientProvider(server_url=server_url, **kwargs)
