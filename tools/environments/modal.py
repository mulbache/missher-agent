...
def _snapshot_store() -> Path:
    ...
    return get_missher_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str = "/root", timeout: int = 60,
        ...
        def _create(image_spec: Any) -> None:
            async def _create_sandbox():
                app = await _modal.App.lookup.aio("missher-agent", create_if_missing=True)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .missher/ as a tar archive (sandboxes run as root, so /root/.missher)."""
        data = self._exec("tar cf - -C / root/.missher", timeout=120, fail_label="bulk download", capture=True)
