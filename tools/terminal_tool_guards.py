...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Missher tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``missher gateway restart|stop|uninstall``
    targeting missher-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_MISSHER_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (missher
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``missher gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Missher. Perform authorized "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `missher gateway restart` from a "
