...
``~/.missher/.env`` (Docker: see ``CAMOFOX_REWRITE_LOOPBACK_URLS`` below).
"""
...
from missher_cli.config import cfg_get, load_config, read_raw_config
...
def _config_cdp_url() -> str:
    ...
    try:
        from missher_cli.config import read_raw_config  # late-bound: tests patch the source module
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    share a user ID so Missher uses the same profile), or None."""
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    with _sessions_lock:
        ...
        if identity is None:
            identity = {"user_id": f"missher_{uuid.uuid4().hex[:10]}", "session_key": f"task_{task_id[:16]}"}
...
def _save_screenshot(content: bytes) -> str:
    """Write PNG bytes under ``$MISSHER_HOME/browser_screenshots`` and return the path."""
    from missher_constants import get_missher_home
    screenshots_dir = get_missher_home() / "browser_screenshots"
