...
from tools.bot_mode_probe import _default_home, _missher_root
...
# Mirrors RELAY_DELIVER_TIMEOUT_MS in apps/desktop/src/plugins/missher-bots/relay.ts; both test suites pin it.
...
# One turn in a profile's canonical Bot Chat: ``missher -p <profile> *BOT_CHAT_TURN_ARGS``.
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import missher_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("missher" if profile == "default" else profile)
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_missher_root(Path(_default_home())))
...
def _missher_cli() -> str:
    """missher CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "missher" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``missher`` entrypoint. A bare ``"missher"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("missher.exe" if sys.platform == "win32" else "missher")
    return str(sibling) if sibling.is_file() else shutil.which("missher") or "missher"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_missher_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]
...
def _delivery_child_session_env_names() -> "tuple[str, ...]":
    ...
    ``MISSHER_SESSION_*`` prefix match, which would also strip non-identity knobs
    (e.g. ``MISSHER_SESSION_STALL_TIMEOUT``)."""
...
def delivery_env(author: Optional[dict]) -> dict[str, str]:
    """Environment for one delivery turn's ``missher`` child. The dispatcher's own MISSHER_TURN_AUTHOR is
...
# Deliveries are separate ``missher`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``missher``
