...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from missher_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("MISSHER_KANBAN_DB", "MISSHER_KANBAN_BOARD"):
...
def _node_fallback(command: str) -> str:
    ...
    missher_home = os.path.expanduser(os.getenv("MISSHER_HOME", os.path.join(home, ".missher")))
    # /usr/local/bin: canonical Node location (from-source Linux, Missher Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    candidates = (os.path.join(missher_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``missher_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Missher already supervises the
