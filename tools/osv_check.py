...
# ladders, parked-server self-probes and repeated `missher mcp test` runs re-run the preflight
...
# The cache is also persisted under the Missher home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``missher_constants.get_missher_home()`` so the cache follows the
    active profile and is isolated across Missher homes. The cache directory
    is created on demand. Returns ``None`` when Missher home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "missher-agent-osv-check/1.0"},
        method="POST")
