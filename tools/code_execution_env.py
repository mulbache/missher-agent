...
# prefix, the exact-name MISSHER_ allowlist, or (Windows) an OS-essential name.
# The broad "MISSHER_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (MISSHER_BASE_URL, MISSHER_KANBAN_DB, *_WEBHOOK).
# MISSHER_RPC_SOCKET / MISSHER_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. MISSHER_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Missher code loses the Kanban mutation guard
# while still inheriting MISSHER_HOME.
_MISSHER_CHILD_ALLOWED = frozenset({
    "MISSHER_HOME", "MISSHER_PROFILE", "MISSHER_CONFIG", "MISSHER_ENV", "MISSHER_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational MISSHER_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret MISSHER_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _MISSHER_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("MISSHER_"):
            _dropped_missher.append(k)
    if _dropped_missher:
        logger.debug(
            "execute_code: dropped %d non-allowlisted MISSHER_* var(s) from the "
            ...
            len(_dropped_missher), ", ".join(sorted(_dropped_missher)),
        )
    ...
    # non-allowlisted MISSHER_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "MISSHER_KANBAN_DB", "MISSHER_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from missher_constants import apply_subprocess_home_env
    child_env = _scrub_child_env(os.environ)
    child_env["MISSHER_RPC_SOCKET"] = rpc_endpoint
    child_env["MISSHER_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; MISSHER_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — missher_time resolves the routed profile's).
    from missher_time import get_timezone_name

    _tz_name = get_timezone_name()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("MISSHER_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # PYTHONPATH: the staging dir (missher_tools.py) must always be importable even when project
    # mode changes CWD. Missher's root is added ONLY when the child runs in Missher's Python env —
    # exposing Missher's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Missher-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Missher-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Missher-
    ...
    _strip_missher_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_missher_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import missher_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Missher "
                    "environment; missher root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Missher environment.
...
def _uses_missher_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Missher's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the missher root."""
