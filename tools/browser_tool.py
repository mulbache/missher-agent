...
from missher_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Missher secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = missher_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in missher_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``missher_home_key()``): the multiplexed
...
_REAL_PROFILE_SESSION = "missher-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-missher_…``
...
# Owner Missher home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. Missher' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = missher_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_missher_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_missher_home_override': ('missher_constants', 'get_missher_home_override'),
    'missher_home_key': ('missher_constants', 'missher_home_key'),
    ...
    'node_tool_runnable': ('missher_constants', 'node_tool_runnable'),
