"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Missher.
...
delegates every real request to Missher as its *client-delegation* backend. Missher stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Missher' answer is what the voice speaks.
...
# backend (Missher) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Missher, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Missher agent: a full AI agent with tools — it can run commands, read and edit files, "
