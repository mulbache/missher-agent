...
# resolve per call in production (below) because ``get_missher_home()`` /
...
_missher_config_resolved: str | None = None
_missher_config_resolved_loaded = False
_real_missher_home_cached: str | None = None
_real_missher_home_loaded = False
...
def _missher_home_real() -> str:
    from missher_constants import get_missher_home
    return os.path.realpath(str(get_missher_home()))


def _get_missher_config_resolved() -> str | None:
    """Resolved absolute path of the Missher config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``MISSHER_HOME`` scope (#107327);
    a test may pin it via ``_missher_config_resolved`` + ``_missher_config_resolved_loaded``."""
    if _missher_config_resolved_loaded:
        return _missher_config_resolved
    try:
        ...
        # home rather than the active multiplex ``MISSHER_HOME`` — comparing
        ...
        # ``get_missher_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from missher_constants import get_missher_home
            return str((Path(str(get_missher_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_missher_home() -> str | None:
    """Realpath of the authoritative Missher home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``MISSHER_HOME`` scope (#107327);
    a test may pin it via ``_real_missher_home_cached`` + ``_real_missher_home_loaded``."""
    if _real_missher_home_loaded:
        return _real_missher_home_cached
    try:
        return _missher_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_missher_config_resolved``): re-derive from ``get_missher_home()``
        # rather than ``_expand_tilde("~/.missher")`` so the protected-instruction
        ...
        # the consumer: the ``~/.missher`` exemption is skipped, so the gate runs.
        try:
            from missher_constants import get_missher_home
            return os.path.realpath(str(get_missher_home()))
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    missher_config = _get_missher_config_resolved()
    if missher_config and missher_config in candidates:
        return (
            f"Refusing to write to Missher config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.missher/config.yaml directly or use 'missher config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .missher tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from missher_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.missher itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.missher`` component rule, which would match the home.
    real_home = _get_real_missher_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .missher config dirs (<repo>/.missher/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.missher.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".missher":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Missher mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.missher" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Missher state (a write the host never reads). Not profile isolation — that guard was
