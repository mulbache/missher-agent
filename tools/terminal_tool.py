...
processes survive cleanup, idle reaping, or Missher exit.
...
# display_missher_home imported lazily at call site (stale-module safety during missher update)
...
def _current_session_key() -> str:
    ...
    return get_session_env("MISSHER_SESSION_KEY", "")


def _current_session_profile() -> str:
    """Active session's Missher profile name, or "" (same lookup discipline as
    :func:`_current_session_key`)."""
    from gateway.session_context import get_session_env

    return get_session_env("MISSHER_SESSION_PROFILE", "")
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``missher-agent=1`` for the current
    profile — leftovers of Missher processes that died without firing
    ...
    Missher processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``missher serve``, Desktop
    ...
    env values (possibly stale from ``missher setup``); env values for omitted keys
    ...
    context-local ``MISSHER_HOME`` override (multiplexed dashboard / gateway
    ...
    with _quiet("terminal config → env fallback bridge failed"):
        from missher_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    from missher_cli.config import _is_ssh_remote_tilde_cwd
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for Missher-owned control-plane
