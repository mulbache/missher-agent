"""Harness: dashboard opt-in via MISSHER_DASHBOARD.

Today (tini): dashboard starts once when MISSHER_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``missher`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without MISSHER_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "missher dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without MISSHER_DASHBOARD"
    )
...
# whenever `MISSHER_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/missher/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``MISSHER_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "MISSHER_DASHBOARD=1",
        "MISSHER_DASHBOARD_HOST=0.0.0.0",
        "MISSHER_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"MISSHER_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and MISSHER_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``MISSHER_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
