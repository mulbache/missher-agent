...
Override the image with ``MISSHER_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("MISSHER_TEST_IMAGE", "missher-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``MISSHER_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("MISSHER_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged missher user
# ---------------------------------------------------------------------------
#
# Background: every Missher runtime path inside the container drops to UID
# 10000 (the ``missher`` user) via ``s6-setuidgid missher``. ``docker exec``
...
# fails (PermissionError swallowed) for missher, so a test that ran as root
...
# helpers below run every probe under ``-u missher`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: missher).
