...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$MISSHER_HOME" = "/opt/data" && '
        'test "$MISSHER_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_missher_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/missher/agent/message_sanitization.py")
        try:
