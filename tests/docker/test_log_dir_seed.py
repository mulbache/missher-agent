"""Runtime smoke test for Docker $MISSHER_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the missher user after container boot.
...
registered later runs its log service as the dropped missher user and
...
def test_logs_gateways_seeded_and_missher_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by missher after boot."""
    ...
    assert "logs=missher" in r.stdout, (
        f"logs/ not owned by missher: {r.stdout}"
    )
    assert "gateways=missher" in r.stdout, (
        f"logs/gateways/ not owned by missher: {r.stdout}"
    )
