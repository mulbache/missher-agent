...
def test_shim_removes_only_its_owned_profile(tmp_path, outcome):
    ...
    install = tmp_path / "missher-agent"
    install.mkdir()
    env = {**os.environ, "HOME": str(tmp_path), "MISSHER_HOME": str(tmp_path),
           "XDG_CONFIG_HOME": str(config), "TMPDIR": str(tmp_path),
           "PATH": f"{bin_dir}:/usr/bin:/bin", "MISSHER_SELFTEST_HOLD_SECONDS": "1",
           "MISSHER_UPDATE_SHIM_GRACE_SECONDS": "1", "MISSHER_SELFTEST_FAIL": "1" if outcome == "error" else ""}
    ...
    collision = tmp_path / f"missher-update-ui-{process.pid}"
    collision.mkdir()
