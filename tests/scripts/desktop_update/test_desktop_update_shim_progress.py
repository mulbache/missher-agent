...
def progress(tmp_path):
    ...
    status = tmp_path / "missher-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Missher to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `missher update`, and reports the stage that was on screen
...
n="$(cat "$MISSHER_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$MISSHER_TEST_CALLS"
for f in "$TMPDIR"/missher-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$MISSHER_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$MISSHER_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "missher-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    missher = install_root / "venv" / "bin" / "missher"
    missher.write_text(FAKE_MISSHER)
    missher.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "MISSHER_TEST_CAPTURE": str(capture),
        "MISSHER_TEST_CALLS": str(calls),
        "MISSHER_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".missher-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`missher update` is the longest wait in the hand-off and the one the
