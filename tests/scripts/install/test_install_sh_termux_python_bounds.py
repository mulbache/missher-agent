...
SETUP_MISSHER_SH = REPO_ROOT / "setup-missher.sh"
...
def _termux_env(tmp_path: Path, bin_dir: Path) -> dict[str, str]:
    ...
    env.update({
        ...
        "MISSHER_HOME": str(tmp_path / "home" / ".missher"),
...
def _copy_setup_checkout(tmp_path: Path) -> Path:
    ...
    checkout.mkdir()
    shutil.copy2(SETUP_MISSHER_SH, checkout / "setup-missher.sh")
    return checkout
...
def _run_setup(tmp_path: Path) -> subprocess.CompletedProcess[str]:
    ...
    bash = shutil.which("bash") or "/bin/bash"
    return subprocess.run(
        [bash, str(checkout / "setup-missher.sh")],
        env=env,
        input="n\n",
        text=True,
        stdout=subprocess.PIPE,
...
def test_install_stage_rejects_post_install_unsupported_default(tmp_path: Path) -> None:
    ...
    assert "Missher requires Python >=3.11,<3.14" in result.stdout
