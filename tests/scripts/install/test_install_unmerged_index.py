...
The ``missher update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m missher_cli.main gateway run`` — image name
    ``pythonw``, not ``missher.exe`` — so the ``taskkill /IM missher.exe`` guard
    ...
    # The missher.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM missher.exe' in text
