...
def test_managed_node_swap_defines_staged_and_backup_as_siblings() -> None:
    ...
    assert re.search(r'\$staged\s*=\s*"\$MissherHome\\node\.new-\$stamp"', swap), (
        "$staged must be defined as a sibling of the live tree "
        '("$MissherHome\\node.new-$stamp")'
    )
    assert re.search(r'\$backup\s*=\s*"\$MissherHome\\node\.old-\$stamp"', swap), (
        "$backup must be defined as a sibling of the live tree "
        '("$MissherHome\\node.old-$stamp")'
    )


def test_managed_node_swap_renames_stay_within_missher_home() -> None:
    swap = _swap_block()
