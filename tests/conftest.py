"""Shared fixtures for the missher-agent test suite.
...
2. **Isolated MISSHER_HOME.** MISSHER_HOME points to a per-test tempdir so
   code reading ``~/.missher/*`` via ``get_missher_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".missher"`` instead of the canonical
   ``get_missher_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No MISSHER_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox MISSHER_HOME before ANY test module is imported ──────────────────
# `missher_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_missher_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.missher/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes MISSHER_HOME, but fixtures run
...
# the REAL Missher root — capture it BEFORE the sandbox rewires MISSHER_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.missher (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("MISSHER_KANBAN_HOME", "").strip()
_PRE_SANDBOX_MISSHER_HOME = os.environ.get("MISSHER_HOME", "")


def _missher_home_points_at_production(value: str) -> bool:
    """True when a pre-set MISSHER_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    MISSHER_HOME=~/.missher``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``missher_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.missher`` — the escape vector that landed pytest
    ...
    MISSHER_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".missher").resolve()
    except Exception:
        return True
...
if _missher_home_points_at_production(os.environ.get("MISSHER_HOME", "")):
    _SESSION_MISSHER_HOME = tempfile.mkdtemp(prefix="missher-test-home-")
    os.environ["MISSHER_HOME"] = _SESSION_MISSHER_HOME
    atexit.register(shutil.rmtree, _SESSION_MISSHER_HOME, True)
...
# like a real CLI") — which used to disarm missher_state's live-DB guard in
# the child at the same moment the child lost the MISSHER_HOME redirect.
# MISSHER_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# missher_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export MISSHER_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["MISSHER_TEST_ISOLATION"] = os.environ.get("MISSHER_HOME", "") or "1"

#: MISSHER_HOME as it stood when conftest was imported - i.e. before any test
...
MISSHER_HOME_AT_CONFTEST_IMPORT = os.environ.get("MISSHER_HOME", "")
...
# MISSHER_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_MISSHER_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``missher_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Missher agent session inherits them and
    # missher_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "MISSHER_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "MISSHER_INTERACTIVE",
    "MISSHER_QUIET",
    "MISSHER_TOOL_PROGRESS",
    "MISSHER_TOOL_PROGRESS_MODE",
    "MISSHER_MAX_ITERATIONS",
    "MISSHER_SESSION_PLATFORM",
    "MISSHER_SESSION_CHAT_ID",
    "MISSHER_SESSION_CHAT_NAME",
    "MISSHER_SESSION_CHAT_TYPE",
    "MISSHER_SESSION_THREAD_ID",
    "MISSHER_SESSION_SOURCE",
    "MISSHER_SESSION_KEY",
    "MISSHER_GATEWAY_SESSION",
    "MISSHER_CRON_SESSION",
    "_MISSHER_GATEWAY",
    "MISSHER_PLATFORM",
    "MISSHER_MODEL",
    "MISSHER_INFERENCE_MODEL",
    "MISSHER_INFERENCE_PROVIDER",
    "MISSHER_TUI_PROVIDER",
    "MISSHER_MANAGED",
    "MISSHER_MANAGED_DIR",
    "MISSHER_DEV",
    "MISSHER_CONTAINER",
    "MISSHER_EPHEMERAL_SYSTEM_PROMPT",
    "MISSHER_TIMEZONE",
    "MISSHER_REDACT_SECRETS",
    "MISSHER_BACKGROUND_NOTIFICATIONS",
    "MISSHER_EXEC_ASK",
    "MISSHER_HOME_MODE",
    "MISSHER_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.missher/kanban.db instead of the per-test MISSHER_HOME.
    "MISSHER_KANBAN_DB",
    "MISSHER_KANBAN_BOARD",
    "MISSHER_KANBAN_HOME",
    "MISSHER_KANBAN_WORKSPACES_ROOT",
    "MISSHER_KANBAN_LOGS_ROOT",
    "MISSHER_KANBAN_TASK",
    "MISSHER_KANBAN_WORKSPACE",
    "MISSHER_KANBAN_RUN_ID",
    "MISSHER_KANBAN_CLAIM_LOCK",
    "MISSHER_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "MISSHER_DELEGATED_CHILD_CONTEXT",
    "MISSHER_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "missher" host.
    "MISSHER_HONCHO_HOST",
    ...
    "MISSHER_DASHBOARD_OAUTH_CLIENT_ID",
    "MISSHER_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MISSHER_HOME to per-test tempdirs so code that
    reads ``~/.missher/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral MISSHER_* vars that could change test semantics.
    for name in _MISSHER_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("MISSHER_HONCHO_HOST", "missher")

    # 3. Redirect MISSHER_HOME to a per-test tempdir. Code that reads
    #    ``~/.missher/*`` via ``get_missher_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.missher/*`` via
    #    ``Path.home() / ".missher"`` instead of ``get_missher_home()``
    #    is a bug to fix at the callsite.
    fake_missher_home = tmp_path / "missher_test"
    fake_missher_home.mkdir()
    (fake_missher_home / "sessions").mkdir()
    (fake_missher_home / "cron").mkdir()
    (fake_missher_home / "memories").mkdir()
    (fake_missher_home / "skills").mkdir()
    monkeypatch.setenv("MISSHER_HOME", str(fake_missher_home))
    ...
    # missher_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("MISSHER_TEST_ISOLATION", str(fake_missher_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("MISSHER_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. missher_state computes ``DEFAULT_DB_PATH = get_missher_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from missher_state import ...``) that
    ...
    missher_state_mod = sys.modules.get("missher_state")
    if missher_state_mod is not None and hasattr(missher_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            missher_state_mod, "DEFAULT_DB_PATH", fake_missher_home / "state.db"
        )
    ...
    monkeypatch.setenv("MISSHER_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.missher/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import missher_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path MISSHER_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_missher_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from missher_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _neutralize_git_safe_directory_read(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_safe_directory"):
        return
    try:
        from missher_cli import _subprocess_compat
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.missher. This autouse
...
# allow-list because test-level fixtures legitimately move MISSHER_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session MISSHER_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``MISSHER_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Missher root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_MISSHER_HOME and not _missher_home_points_at_production(
        _PRE_SANDBOX_MISSHER_HOME
    ):
        # MISSHER_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_missher_root().resolve()
    # No pre-existing MISSHER_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".missher").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.missher`` captured at import time. Hermetic tests that legitimately
    move MISSHER_HOME to sibling tempdirs are unaffected.

    Only patches when ``missher_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("missher_cli.kanban_db")
    _kdbc = sys.modules.get("missher_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import missher_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.missher. See #69283."
        )
...
# ``missher_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Missher root. This fixture wires the
# test-side knobs:
...
#     installs where MISSHER_HOME is not ~/.missher) into the guard's
...
# so subprocess children that import missher_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("missher_state")
    ...
    if _PRE_SANDBOX_MISSHER_HOME and not _missher_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_MISSHER_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Missher home override redirects every later
    # ``get_missher_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from missher_constants import get_missher_home_override, set_missher_home_override

        if get_missher_home_override() is not None:
            set_missher_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal missher config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``missher-gateway`` process
...
# correlated with a ``tests/missher_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> missher-gateway``
...
def _wal_is_usable() -> bool:
    """True when Missher will actually put a database into WAL mode here.

    Missher refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Missher can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Missher managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``missher_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_missher_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_missher_home`` fixture
    redirects ``MISSHER_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.missher/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``missher_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["MISSHER_VOICE_TTS"] = "1"``. The
...
#      ``missher_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``missher_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``missher_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``missher_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Missher runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.missher``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Missher falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Missher uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart missher-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting missher/python patterns are also blocked.
    """
    ...
    def _matches_missher_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _MISSHER_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_missher_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch missher-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m missher_cli.main".
                if (
                    "missher" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live missher-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting missher/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `missher update` (or the
        # equivalent `python -m missher_cli.main update`).  These commands
        ...
        if "update" in low and (
            # missher update / missher update --gateway / setsid bash -c ... missher update
            ("missher" in low and "update" in low.split())
            or
            # python -m missher_cli.main update --gateway
            ("missher_cli" in low and "update" in low.split())
            or
            # venv/bin/missher update  (absolute path variant used in tests)
            (".venv/bin/missher" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`missher update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m missher_cli.main
        # gateway run|start|restart``). ``_spawn_missher_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp MISSHER_HOME, resolves the DEVELOPER's
        # ``missher-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … missher gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "missher gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (missher_cli.web_server_gateway."
                "_spawn_missher_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_MISSHER_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import missher_cli.voice as _voice
    except Exception:
