...
def test_two_queries_never_consume_previous_query_caches(tmp_path, monkeypatch):
    ...
    assert HonchoClientConfig.from_global_config(host="missher", config_path=path).recall_sync
    config_provider.save_config({"hosts": {"missher": {"recallSync": False}}}, str(tmp_path))
    assert not HonchoClientConfig.from_global_config(host="missher", config_path=path).recall_sync
...
def test_missing_peer_notice_surfaces_once():
    ...
    assert "missher honcho peer --user" in first
