...
from missher_cli.profiles import _get_default_missher_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "missher"
        assert config.workspace_id == "missher"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"missher": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"missher": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"missher": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="missher-agent"
        ):
            result = config.resolve_session_name("/home/user/missher-agent/subdir")
        assert result == "missher-agent"


class TestResolveConfigPath:
    def test_prefers_missher_home_when_exists(self, tmp_path):
        missher_home = tmp_path / "missher"
        missher_home.mkdir()
        local_cfg = missher_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: MISSHER_HOME points at ~/.missher/profiles/<name>, so
        # _get_default_missher_home() must resolve back to ~/.missher — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".missher"
        ...
        assert _get_default_missher_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "missher_coder"
        assert profile_host_key("default") == "missher"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"MISSHER_HONCHO_HOST": "missher.coder"}):
            assert resolve_active_host() == "missher.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("MISSHER_HONCHO_HOST", None)
            # Temporarily remove missher_cli.profiles to simulate import failure
            saved = sys.modules.get("missher_cli.profiles")
            sys.modules["missher_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "missher"
            finally:
                if saved is not None:
                    sys.modules["missher_cli.profiles"] = saved
                else:
                    sys.modules.pop("missher_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="missher_coder")
        assert config.host == "missher_coder"
        assert config.workspace_id == "missher"  # shared workspace
        assert config.ai_peer == "missher_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"missher": {"enabled": True, "aiPeer": "missher"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (missher.work)
        ...
        cfg = HonchoClientConfig(
            ...
            raw={"hosts": {"missher.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_missher_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MISSHER_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("missher_cli.profiles.get_active_profile_name", return_value="default"), \
