"""Tests for ``missher honcho peers map``: account discovery, the resolution preview, and alias writes."""
...
def _cfg(root=None, **host):
    return {"apiKey": "***", **(root or {}), "hosts": {"missher": host}}
...
def _run_map(monkeypatch, tmp_path, *, answers, cfg, db_rows=(), ws_peers=None, workspaces=None):
    ...
    # One profile per host block: "missher" is default, "missher.<name>" is profile <name>.
    profiles = [("default" if k == "missher" else k.removeprefix("missher."), k, v) for k, v in cfg["hosts"].items()]
    ...
    for name, value in {
        ...
        "_peers_map_client": lambda workspace=None: (client, SimpleNamespace(workspace_id="missher") if client else None),
...
class TestCmdPeersMap:
    @pytest.mark.parametrize("cfg, answers, kwargs", [
        ...
        (_cfg(), ["111", "eri", ""], {}),
        (_cfg(peerName="eri"), ["1", "p1", ""], dict(db_rows=[_row("111")], ws_peers=["eri", "missher"])),
    ...
    def test_writes_alias_to_host_block(self, monkeypatch, tmp_path, cfg, answers, kwargs):
        ...
        assert written["cfg"]["hosts"]["missher"]["userPeerAliases"] == {"111": "eri"}
    ...
    def test_dash_clears_alias(self, monkeypatch, tmp_path, aliases, expected_host):
        ...
        assert written["cfg"]["hosts"]["missher"] == expected_host
    ...
    def test_cleared_host_map_does_not_resurrect_root_aliases(self, monkeypatch, tmp_path, capsys):
        ...
        assert written["cfg"]["hosts"]["missher"]["userPeerAliases"] == {}
        assert "root aliases no longer apply to [missher]" in capsys.readouterr().out

        path = tmp_path / "honcho.json"
        path.write_text(json.dumps(written["cfg"]))
        assert HonchoClientConfig.from_global_config(host="missher", config_path=path).user_peer_aliases == {}

    @pytest.mark.parametrize("cfg, answers, kwargs", [
        (_cfg(), [""], {}),
        (_cfg(userPeerAliases={"111": "eri"}), ["111", "eri", ""], {}),
        (_cfg(pinUserPeer=True, peerName="eri"), ["n"], {}),
        (_cfg(peerName="eri"), ["w", "2", "n", ""], dict(ws_peers=["eri"], workspaces=["missher", "cosmania-dex"])),
    ...
    @pytest.mark.parametrize("cfg, answers, kwargs, expected", [
        ...
        ({"apiKey": "***", "userPeerAliases": {},
          "hosts": {"missher": {"workspace": "missher"}, "missher.dreamer": {"workspace": "dreamland"}}},
         ["222", "tek", "", "all"], {}, ["also apply in workspace 'dreamland'"]),
        ({"apiKey": "***", "hosts": {"missher": {"peerName": "eri", "userPeerAliases": {"111": "eri"}},
                                     "missher.dreamer": {"peerName": "eri", "userPeerAliases": {"111": "bob"}}}},
...
class TestWorkspaceSwitch:
    def test_browse_and_confirmed_switch_writes_workspace(self, monkeypatch, tmp_path):
        written = _run_map(
            ...
            cfg=_cfg(peerName="eri"), ws_peers=["eri"], workspaces=["missher", "cosmania-dex"],
        )
        assert written["cfg"]["hosts"]["missher"]["workspace"] == "cosmania-dex"
    ...
    def test_browse_client_is_built_for_the_browsed_workspace(self, monkeypatch):
        ...
        seen = []
        base = client_mod.HonchoClientConfig(host="missher", workspace_id="missher", api_key="k")
        ...
        assert own.workspace_id == "missher"
        assert browsed.workspace_id == "cosmania-dex"
        assert [c.workspace_id for c in seen] == ["missher", "cosmania-dex"]
...
class TestSaveScope:
    @pytest.mark.parametrize("hosts, answers", [
        ({"missher": {}}, ["222", "tek", ""]),
        ({"missher": {}, "missher.dreamer": {}}, ["222", "tek", "", "all"]),
    ...
    def test_root_sourced_aliases_write_back_to_root(self, monkeypatch, tmp_path, hosts, answers):
        ...
        assert "userPeerAliases" not in written["cfg"]["hosts"]["missher"]

    def test_scope_this_forks_host_block(self, monkeypatch, tmp_path):
        cfg = {"apiKey": "***", "userPeerAliases": {"111": "eri"}, "hosts": {"missher": {}, "missher.dreamer": {}}}
        written = _run_map(monkeypatch, tmp_path, answers=["222", "tek", "", "this"], cfg=cfg)
        assert written["cfg"]["hosts"]["missher"]["userPeerAliases"] == {"111": "eri", "222": "tek"}
...
def test_classify_workspace_peers_labels_from_local_config(monkeypatch):
    monkeypatch.setattr(honcho_cli, "_host_key", lambda: "missher")
    rows = [
        ("default", "missher", {"peerName": "eri", "aiPeer": "hermetika"}),
        ("dreamer", "missher.dreamer", {"aiPeer": "dreamer-ai"}),
    ]
