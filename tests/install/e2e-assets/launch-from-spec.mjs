// @ts-check
/**
 * Launch the Missher desktop app from a captured launch spec and click the
 * real update flow: Settings -> About -> "Update now".
 *
 * The spec is written by launch-capture/sitecustomize.py at `missher
...
async function main() {
  ...
  // an "update complete, reopen Missher to finish" overlay and never exits;
  ...
  const boundedClose = async (application, label) => {
    ...
    // (`missher serve` python + node helpers) survives and keeps writing
  ...
  phase('relaunch');
  log('relaunching the updated app (the "reopen Missher" step)');
