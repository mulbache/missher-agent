...
#   mock_start <workroot>      start, write config into $MISSHER_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, MISSHER_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$MISSHER_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $MISSHER_HOME (api $url/v1)"
}
