// drive-update.cjs — launch the INSTALLED Missher.exe (real Electron desktop
...
//
//   node <this file> <path-to-Missher.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Missher.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: MISSHER_HOME (isolated install) and
  ...
  // The real backend (`missher serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Missher... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.missherDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Missher — this window will close" overlay should appear,
  ...
  // the staged binary), which claims MISSHER_HOME/.missher-update-in-progress
  ...
  // proved the desktop had exited and `missher update` was already running).
  //
  ...
  const missherHome = process.env.MISSHER_HOME
  const markerPath = missherHome ? path.join(missherHome, '.missher-update-in-progress') : null
  const resultPath = missherHome ? path.join(missherHome, '.missher-update-result.json') : null
