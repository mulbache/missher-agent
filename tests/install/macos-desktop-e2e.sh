...
# (Missher-Setup.dmg from the website) can reach HEAD.
#
...
#              missher-desktop-app-update  capture `missher desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              missher-update              CLI update from the installed venv
...
#     --update-method open-app-update|missher-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://missher-assets.nousresearch.com/Missher-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|missher-desktop-app-update|missher-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${MISSHER_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/missher-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$MISSHER_HOME/missher-agent"
}
...
phase_stage() {
  ...
  touch "$MISSHER_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Missher.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Missher.app" \
    "/Applications/Missher.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Missher-Setup.dmg"

  local dmg="$WORK_ROOT/Missher-Setup.dmg"
  ...
  # "Install Missher" button with native input.
  local rc=0
  ...
  log_group "Missher-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$missher" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "missher --version failed after install"
  ok "missher --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Missher.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    missher-update)
      ...
      # update subcommand only in later releases; ask the installed missher.
      local missher="$INSTALL_DIR/venv/bin/missher"
      local update_cmd=("$missher" update)
      ...
      [ "$rc" -eq 0 ] || fail "missher update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Missher.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Missher.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Missher.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, missher update) need.
      local app app_bin
...
PYEOF
    ...
    missher-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "missher desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "missher desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.missher/logs" "$ildest/missher-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Missher"
  ...
  "$INSTALL_DIR/venv/bin/missher" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "missher --version failed after update"
  ok "missher --version works post-update"
