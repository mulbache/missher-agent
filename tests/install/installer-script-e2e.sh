...
#              install landed on OLD with a working `missher`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `missher`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method missher-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  missher-update|installer-script|installer-script+desktop|missher-desktop-app-update) ;;
  *) echo "error: --update-method must be missher-update, installer-script, installer-script+desktop or missher-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/missher-installer-script-e2e"
LOG_DIR="${MISSHER_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled missher or a
# developer config, and old installer scripts hardcode $HOME/.missher (the
# MISSHER_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export MISSHER_HOME="$HOME/.missher"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Missher" \
    "$release_dir/linux-unpacked/missher" \
    "$release_dir/mac-arm64/Missher.app" \
    "$release_dir/mac/Missher.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local missher="$INSTALL_DIR/venv/bin/missher"
  [ -x "$missher" ] || fail "no missher console script at $missher"
  "$missher" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "missher --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "missher --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `missher desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `missher update` itself makes. Probe the
  # INSTALLED missher for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `missher desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local missher="$INSTALL_DIR/venv/bin/missher"
  if ! "$missher" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "missher desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$missher" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "missher desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "missher desktop --build-only ($1) exited $rc; transcript above"
  ok "missher desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  missher-update)
    ...
    # installed missher; older ones read the prompt from stdin, so close it.
    MISSHER="$INSTALL_DIR/venv/bin/missher"
    if "$MISSHER" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$MISSHER" update --yes)
    else
      update_cmd=("$MISSHER" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "missher update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "missher update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  missher-desktop-app-update)
    # The real user surface: `missher desktop` launches the app, the user
    ...
    # suite's own) is configured into MISSHER_HOME exactly like the dev:mock
    ...
    step "capturing the missher desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      MISSHER_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$MISSHER" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "missher desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "missher desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "missher desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$MISSHER_HOME/logs" "$ildest/missher-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/missher/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$MISSHER_HOME/.missher-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$MISSHER_HOME" > "$ildest/missher-home-ls.txt" 2>/dev/null || true
