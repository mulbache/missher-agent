...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Missher the way
...
# actually touches:
#
#   INSTALL   - downloads the production Missher-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Missher.exe must appear.
...
#   installer's `git clone` and `missher update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and missher-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "missher-desktop-app-update", "missher-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Missher-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:MISSHER_E2E_WORKROOT) { $env:MISSHER_E2E_WORKROOT } else { Join-Path $env:TEMP "missher-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://missher-assets.nousresearch.com/Missher-Setup.exe",
...
$MissherHome  = Join-Path $WorkRoot "missher-home"
$InstallDir  = Join-Path $MissherHome "missher-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if missher ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit missher thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Missher.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Missher.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-MissherRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $MissherHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "missher-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $MissherHome ".missher-update-result.json"
    ...
    Get-ChildItem -LiteralPath $MissherHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "missher-home-ls.txt")
}

function Test-MissherRuns([string]$Label) {
    Save-InstallSideState $Label
    $missherExe = Join-Path $InstallDir "venv\Scripts\missher.exe"
    Assert-True (Test-Path -LiteralPath $missherExe) "$Label -- venv\Scripts\missher.exe exists"
    & $missherExe --version 2>&1 | ForEach-Object { Write-Host "    missher --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- missher --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-MissherHome", $MissherHome, "-InstallDir", $InstallDir)
...
function Invoke-MissherUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $missherExe = Join-Path $InstallDir "venv\Scripts\missher.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $missherExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $missherExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "missher update transcript" $log
    Assert-True ($updateExit -eq 0) "missher update exited $updateExit (expected 0)"
}

function Invoke-MissherDesktopAppUpdate([string]$TargetSha) {
    # The missher-desktop launch surface: `missher desktop` runs its whole
    ...
    $prevCap = $env:MISSHER_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:MISSHER_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $missherExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:MISSHER_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "missher desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "missher desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $MissherHome ".missher-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured missher desktop spec; update completed"
}
...
function Stop-MissherAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Missher.exe (Electron) -- never missher.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Missher" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Missher.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `missher update`/desktop builds use the Missher-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $MissherHome "node\node.exe"),
        (Join-Path $MissherHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Missher-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Missher-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Missher-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Missher-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:MISSHER_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $MissherHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Missher-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Missher.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Missher-Setup.exe", (Join-Path $MissherHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Missher" -ErrorAction SilentlyContinue)) "Missher.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Missher-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $MissherHome "logs\bootstrap-installer.log"
    ...
    Stop-MissherAppProcesses "post-install"
    ...
    Test-MissherRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Missher.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Missher.exe present before update"
    ...
    try {
        ...
        # `missher update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .missher-update-result.json on every exit;
        #   * older checkouts -> the staged missher-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/missher/relaunch
        ...
        $updateLog = Join-Path $MissherHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-MissherRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Missher.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Missher.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Missher" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Missher window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Missher" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $MissherHome "logs\desktop-update-handoff.log"
        ...
        Stop-MissherAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, missher runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-MissherRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-MissherRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $MissherHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "missher-desktop-app-update" {
            Invoke-MissherDesktopAppUpdate $state.current
        }
        "missher-update" {
            Invoke-MissherUpdate
        }
        "installer-script" {
        ...
        "desktop-installer@latest" {
            # A user re-downloading Missher-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $MissherHome "logs\desktop.log"))) {
