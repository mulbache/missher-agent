...
Run from the PR worktree:
    ...
    cd ~/.missher/missher-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.missher/missher-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.missher/missher-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a missher-agent checkout"
)
...
# Isolated MISSHER_HOME for the config write.
