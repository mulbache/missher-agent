...
``set_missher_home_override`` / ``reset_missher_home_override``. Because
``_plugins_dir()`` keys off ``get_missher_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import missher_cli.profiles as profiles
