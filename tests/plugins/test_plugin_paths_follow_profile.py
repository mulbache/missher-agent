"""Plugin data paths follow the active profile's MISSHER_HOME, including the ContextVar override.

Several plugins carried a ``~/.missher`` fallback (guarding an ImportError of ``missher_constants``
...
def _openviking_log(home):
    ...
    return ov.get_missher_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
