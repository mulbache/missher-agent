"""Tests for the bundled missher-achievements dashboard plugin.
...
``plugins/missher-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the missher-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.missher so state/snapshot files don't collide.
    ...
    # swap ``sys.modules['missher_state']`` with auto-restoration. Without
    # this, a raw ``sys.modules[...] = fake`` assignment would leak the
    # fake into later tests in the same xdist worker — breaking every
    # test that does ``from missher_state import SessionDB``.
    module._test_monkeypatch = monkeypatch
    yield module


class _FakeSessionDB:
    """Stand-in for missher_state.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['missher_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("missher_state")
    fake_module.SessionDB = lambda **_kw: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "missher_state", fake_module)
...
def test_scan_sessions_never_opens_a_writable_session_db(plugin_api, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(missher_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
