...
from missher_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(missher_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <missher_home>/plugins/<name> and opt it in."""
    plugin_dir = missher_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MISSHER_HOME", str(tmp_path / "missher_empty"))
