...
def test_root_session_no_compression(db):
    ...
    assert prov["currentMissherSessionId"] == "root1"
    assert prov["rootMissherSessionId"] == "root1"
    assert prov["parentMissherSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_missher_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentMissherSessionId"] == "old"
    assert prov["rootMissherSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousMissherSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootMissherSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"missher"}
    assert "sessionProvenance" in meta["missher"]
    assert meta["missher"]["sessionProvenance"]["currentMissherSessionId"] == "root1"
