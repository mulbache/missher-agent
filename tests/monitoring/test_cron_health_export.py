...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("missher.gateway.up"), _M("missher.gateway.active_agents"),
        _M("missher.gateway.busy"), _M("missher.gateway.drainable"),
        _M("missher.gateway.restart_requested"),
        _M("missher.platform.up"), _M("missher.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("missher.cron.scheduler.heartbeat_age_seconds"),
        _M("missher.cron.scheduler.last_success_age_seconds"),
        _M("missher.cron.scheduler.catch_up_occurrences"),
        _M("missher.cron.jobs.enabled"), _M("missher.cron.jobs.running"),
        _M("missher.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Missher Agent-owned Relay transport health" in text
