"""Tests for missher_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Missher entry point
(missher, missher-agent, missher-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Missher entry point imports missher_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported missher_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("missher_bootstrap", None)
    import missher_bootstrap  # noqa: WPS433
    return missher_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Missher CLI
...
class TestEntryPointsImportBootstrap:
    """Every Missher entry point must import missher_bootstrap as its
    ...
    # Entry points that invoke Missher as a process.  Each one must
    # import missher_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "missher_cli/main.py",   # missher CLI (console_script)
        "run_agent.py",          # missher-agent (console_script)
        "acp_adapter/entry.py",  # missher-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import missher_bootstrap' and that
        ...
        half-finished ``missher update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``missher_bootstrap`` as a top-level module) leaves missher
        ...
        """
        # Resolve relative to the missher-agent repo root.  Tests live
        # at tests/test_missher_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # missher start even when missher_bootstrap hasn't been
        ...
        assert first_import_name == "missher_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'missher_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import missher_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Missher's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("MISSHER_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["MISSHER_PYTHON_SRC_ROOT"] = env
            elif "MISSHER_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["MISSHER_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/missher")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MISSHER_PYTHON_SRC_ROOT", None)
            else:
                os.environ["MISSHER_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/missher", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Missher root.  The guard must relocate Missher to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/missher"])
        assert result[0] == "/opt/missher"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Missher root.
        assert result.index("/opt/missher") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["MISSHER_PYTHON_SRC_ROOT"] = "/env/missher"
            hb.harden_import_path()
            assert sys.path[0] == "/env/missher"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MISSHER_PYTHON_SRC_ROOT", None)
