...
def test_frontmatter_required_fields():
    ...
    missher = fm["metadata"]["missher"]
    assert missher["tags"]
    assert "related_skills" in missher
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Missher Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["missher"]["related_skills"]:
        hits = (
