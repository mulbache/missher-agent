...
def test_required_frontmatter_fields(p):
    ...
    missher = (fm.get("metadata") or {}).get("missher") or {}
    if not (missher.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.missher.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    missher = (fm.get("metadata") or {}).get("missher") or {}
    dangling = [
        rs for rs in (missher.get("related_skills") or []) if rs not in _all_names()
    ]
