"""Guard: Missher-owned subprocesses must not resolve managed runtimes by bare PATH.

Missher installs runtimes for itself — ``uv`` at ``$MISSHER_HOME/bin/uv``, Node at
``$MISSHER_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Missher's own
...
* when a system copy also exists, the one Missher does not own wins — which is
...
``iter_missher_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Missher provisions into MISSHER_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Missher-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("missher_cli/update_cmd_deps.py", "uv"): (
    ...
    ("missher_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (missher_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Missher-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$MISSHER_HOME/bin (uv) and $MISSHER_HOME/node are not on an "
        ...
        "  node/npm -> missher_constants.find_node_executable()\n"
        "  PATH env -> missher_constants.iter_missher_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(missher_constants, helper))


def test_managed_uv_helpers_exist():
    from missher_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
