...
Every test builds a bare ``GatewayRunner`` with stub adapters against a temp ``MISSHER_HOME`` and
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("missher_cli.profiles.profiles_to_serve", return_value=served), \
            patch("missher_cli.profiles.get_profile_dir", side_effect=lambda n: home / "profiles" / n), \
            patch("missher_cli.profiles.profile_exists", return_value=True):
