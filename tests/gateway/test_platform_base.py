...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    missher_home.mkdir()
    monkeypatch.setattr(base, "_MISSHER_HOME", missher_home)
    monkeypatch.setattr(base, "_MISSHER_ROOT", missher_home)
    path = missher_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.missher/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("MISSHER_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("MISSHER_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("MISSHER_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("MISSHER_KANBAN_HOME", str(tmp_path / "missher"))
        monkeypatch.setenv("MISSHER_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "missher" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("MISSHER_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("MISSHER_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("MISSHER_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.missher/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        missher_dir = fake_home / ".missher"
        (missher_dir / "mcp-tokens").mkdir(parents=True)
        secret = missher_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._MISSHER_HOME",
            missher_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._MISSHER_ROOT",
            missher_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the MISSHER_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        missher_dir = fake_home / ".missher"
        missher_dir.mkdir(parents=True)
        token = missher_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._MISSHER_HOME", missher_dir)
        monkeypatch.setattr("gateway.platforms.base._MISSHER_ROOT", missher_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_missher_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.missher
        ...
        missher_dir = fake_home / ".missher"
        missher_dir.mkdir(parents=True)
        artifact = missher_dir / "adhoc_report.pdf"
    ...
    def test_denylist_blocks_session_stores_but_not_neighbouring_artifacts(self, tmp_path, monkeypatch):
        ...
        pasted into a chat; ``MEDIA:~/.missher/state.db`` must not exfiltrate them (#41071). Named
        ...
        monkeypatch.setattr("gateway.platforms.base._MISSHER_HOME", missher_dir)
        monkeypatch.setattr("gateway.platforms.base._MISSHER_ROOT", missher_dir)
        board = missher_dir / "kanban" / "boards" / "team-a"
        ...
        for rel in denied + allowed:
            path = missher_dir / rel
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(b"SQLite format 3\x00")
        assert [rel for rel in denied if BasePlatformAdapter.validate_media_delivery_path(str(missher_dir / rel))] == []
        assert [rel for rel in allowed if not BasePlatformAdapter.validate_media_delivery_path(str(missher_dir / rel))] == []
    ...
    def test_denylist_covers_every_profile_home_not_just_the_launch_home(self, tmp_path, monkeypatch):
        ...
        turn is the launch (default) profile's or the secondary's own (MISSHER_HOME override), while
        ...
        self._patch_roots(monkeypatch)
        fake_home = tmp_path / "home"
        missher_root = fake_home / ".missher"
        profile_b = missher_root / "profiles" / "beta"
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._MISSHER_HOME", missher_root)
        monkeypatch.setattr("gateway.platforms.base._MISSHER_ROOT", missher_root)
        ...
        for scope in (None, profile_b):  # default profile's turn, then beta's own turn
            token = set_missher_home_override(scope)
            try:
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting MISSHER_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose MISSHER_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.missher) emits
        ...
        missher_root = denied_root / ".missher"
        prof_cache = missher_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._MISSHER_ROOT", missher_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.missher/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        missher_home = tmp_path / ".missher"
        cache = missher_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.missher/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "MISSHER_HUMAN_DELAY_MODE": "natural",
            "MISSHER_HUMAN_DELAY_MIN_MS": "oops",
            "MISSHER_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_tolerates_malformed_env_vars(self):
        env = {
            "MISSHER_HUMAN_DELAY_MODE": "custom",
            "MISSHER_HUMAN_DELAY_MIN_MS": "oops",
            "MISSHER_HUMAN_DELAY_MAX_MS": "still-bad",
        }
        with patch.dict(os.environ, env):
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"MISSHER_MEDIA_DELIVERY_STRICT": "1",
                                     "MISSHER_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MISSHER_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Missher
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../missher/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.missher/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.missher exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".missher"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.missher/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
