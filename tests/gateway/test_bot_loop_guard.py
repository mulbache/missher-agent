...
async def test_routed_bot_traffic_is_metered_by_the_transport_profiles_policy(tmp_path, monkeypatch, runner, clock):
    ...
    monkeypatch.setenv("MISSHER_HOME", str(transport))
...
def test_load_settings_reads_config_yaml(monkeypatch):
    ...
    monkeypatch.setattr(missher_config, "load_config_readonly", lambda: {"gateway": {"bot_loop_guard": {"max_events": 3}}})
    assert load_settings().max_events == 3

    def boom():
        raise RuntimeError("no config")

    monkeypatch.setattr(missher_config, "load_config_readonly", boom)
