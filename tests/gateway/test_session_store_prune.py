...
def test_session_store_default_db_uses_runtime_missher_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime MISSHER_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    missher_state before a fixture redirected MISSHER_HOME used to pin every
    default SessionDB() at the developer's real ~/.missher/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_missher_home"
    fake_home.mkdir()
    monkeypatch.setenv("MISSHER_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "missher sessions list" in raw["_README"]
