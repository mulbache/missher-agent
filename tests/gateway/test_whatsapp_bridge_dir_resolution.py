...
(/opt/missher/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable MISSHER_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_missher_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable MISSHER_HOME."""
    ...
    monkeypatch.setattr(
        "missher_constants.get_missher_home", lambda: missher_home
    )
    ...
    expected = missher_home / "scripts" / "whatsapp-bridge"
