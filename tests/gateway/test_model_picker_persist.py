...
closure the PR changed, against a real temp ``MISSHER_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "missher_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_missher_home() / config.yaml`` — point it here.
    monkeypatch.setattr("missher_constants.get_missher_home", lambda: missher_home)
    monkeypatch.setattr("missher_cli.config.get_missher_home", lambda: missher_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_missher_home", default_home)
    _stub_picker_dependencies(monkeypatch)
