...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Missher"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^missher"]}
        )
        assert adapter._message_matches_mention_patterns("missher please help") is True
        assert adapter._message_matches_mention_patterns("please missher help") is False
