...
--------------------
When a non-default profile is active (``MISSHER_HOME`` points at
...
class ProfileTripwire:
    ...
    Attributes:
        root:     the default profile's home (``<tmp>/missher``).
...
def profile_tripwire(tmp_path, monkeypatch):
    ...
    1. A temp missher root acting as the *default* profile, seeded with the
       ...
       detectable.
    2. ``MISSHER_HOME`` (env var AND the context-local override) pointed at
       the testprof home — the exact activation shape ``--profile`` uses.
    3. ``missher_state.DEFAULT_DB_PATH`` restored to its import-time snapshot.
       ...
       makes resolution flow through ``get_missher_home()`` — the production
