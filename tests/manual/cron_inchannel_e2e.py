...
Uses a throwaway MISSHER_HOME so it never touches ~/.missher. No real names.
"""
...
def _fresh_home():
    """Point MISSHER_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_missher_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["MISSHER_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER MISSHER_HOME is set.
...
# a different get_missher_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"MISSHER_HOME (throwaway): {HOME}")
