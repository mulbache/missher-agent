"""Regression tests for #34107 — Docker UID/GID handling in ensure_missher_home.

When Missher runs in Docker with ``MISSHER_UID=1000`` / ``MISSHER_GID=911``,
the entrypoint chowns the top-level ``MISSHER_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_missher_home()`` — especially
...
The fix is a ``_chown_to_missher_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_missher_uid_gid
# ---------------------------------------------------------------------------


class TestResolveMissherUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("MISSHER_UID", "1000")
        monkeypatch.setenv("MISSHER_GID", "911")
        from missher_cli.config import _resolve_missher_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_missher_uid
# ---------------------------------------------------------------------------


class TestChownToMissherUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MISSHER_UID", "1000")
        monkeypatch.setenv("MISSHER_GID", "911")
        from missher_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_missher_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_missher_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("MISSHER_UID", raising=False)
        monkeypatch.delenv("MISSHER_GID", raising=False)
        from missher_cli import config as cfg
