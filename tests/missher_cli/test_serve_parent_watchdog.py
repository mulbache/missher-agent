"""Regression tests for Desktop-owned ``missher serve`` lifecycle tracking."""

import pytest

from missher_cli.web_server_lifecycle import (
    _is_serve_orphaned,
...
def test_parent_watchdog_treats_empty_marker_env_as_absent(monkeypatch):
    """Blank inherited MISSHER_PARENT_START_MARKER/NONCE degrade to PID-only tracking."""
    from missher_cli import web_server_lifecycle

    monkeypatch.setenv("MISSHER_PARENT_PID", "4242")
    monkeypatch.setenv("MISSHER_PARENT_START_MARKER", "")
    monkeypatch.setenv("MISSHER_PARENT_NONCE", "")
    seen = {}
...
def test_parent_watchdog_warns_when_disarmed_by_unusable_marker(monkeypatch, caplog):
    ...
    monkeypatch.setenv("MISSHER_PARENT_PID", "4242")
    monkeypatch.setenv("MISSHER_PARENT_START_MARKER", "ps:Sat")
    monkeypatch.setenv("MISSHER_PARENT_NONCE", "n")
    started = []
    monkeypatch.setattr(
