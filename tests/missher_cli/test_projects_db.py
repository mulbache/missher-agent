"""Tests for the per-profile Projects store (missher_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Missher Agent", folders=["/tmp/missher"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "missher-agent"
    assert proj.name == "Missher Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/missher"
    assert [f.path for f in proj.folders] == ["/tmp/missher"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "missher-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' MISSHER_HOME.
