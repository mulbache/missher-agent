...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            missher_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(missher_constants, "get_missher_home", lambda: tmp_path)
    monkeypatch.setattr(missher_state, "SessionDB", FakeSessionDB)
    ...
    assert "missher sessions optimize-storage" in capsys.readouterr().out
    conn.close()
