...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    MISSHER_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    assert pid == 4242
    assert captured["env"]["MISSHER_HOME"] == str(profile)
    assert captured["env"]["MISSHER_KANBAN_TASK"] == "t_spawn_tools"
...
""".lstrip(),
    ...
    monkeypatch.setenv("MISSHER_KANBAN_TASK", "t_spawn_tools")
