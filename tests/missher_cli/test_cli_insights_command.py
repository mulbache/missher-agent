...
from missher_cli.main_agent_cmds import cmd_insights
...
def _run_show_insights(command: str):
    ...
    with patch("missher_state.SessionDB", return_value=db), \
...
def test_insights_skips_open_when_store_missing(capsys):
    ...
    for run in (lambda: MissherCLI.__new__(MissherCLI)._show_insights("/insights 7"),
                lambda: cmd_insights(SimpleNamespace(days=30, source=None))):
        with patch("missher_state.SessionDB") as ctor:
            run()
...
def test_show_insights_opens_read_only():
    cli_obj = MissherCLI.__new__(MissherCLI)
    db = MagicMock()
    with patch("missher_state.SessionDB", return_value=db) as ctor, \
         patch("agent.insights.InsightsEngine", _InsightsEngineStub):
        _InsightsEngineStub.calls = []
