...
The notice prints the most recent curator run summary on `missher update`,
...
session start) so users normally never see the rename map. `missher update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from missher_cli import update_cmd_maint
