...
Covers:
- Defense 1: gateway stop/restart refuse when _MISSHER_GATEWAY=1
...
from missher_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "missher gateway uninstall",
        "missher  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "MISSHER GATEWAY RESTART",           # uppercase
    ])
    def test_missher_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.missher.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.missher/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l missher-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.missher.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.missher.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.missher.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.missher/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.missher.gateway-apollo:/a.plist' "
            "'ai.missher.gateway-cronus:/c.plist' 'ai.missher.gateway-plutus:/p.plist' "
            "'ai.missher.gateway:/Users/botuser/Library/LaunchAgents/ai.missher.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated missher note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.missher.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.missher.gateway',
        "launchctl kick'start' -k gui/501/ai.missher.gateway",
        "launchctl kick\\start -k gui/501/ai.missher.gateway",
        'launchctl "kickstart" -k gui/501/ai.missher.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.missher.gateway',
        "launchctl dis\\able gui/501/ai.missher.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.missher."gateway"',
        # Same class on the systemctl and missher-CLI branches.
        'systemctl re"start" missher-gateway',
        'missher gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.missher.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.missher.update-checker',
        'systemctl re"start" missher-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "missher cron list",
        "missher update",
        "missher config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `missher gateway start` is benign — starting a gateway from inside a
        ...
        "missher gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway missher
        # services must not be falsely blocked (the old `.*missher` matched any
        # missher token).
        "launchctl unload ai.missher.update-checker.plist",
        "launchctl restart ai.missher.daemon",
        ...
        "systemctl restart missher-meta.service",
        "systemctl restart missher-cron-helper",
        ...
        "echo after the missher gateway restarted cleanly",
        "the missher gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "missher skill view gateway-notes && echo missher gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `missher` is a path component
        # there, not a command.
        "cat '/docs/missher gateway restart-notes.md'",
        "less /home/user/notes/missher gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "missher gateway restart",
        "missher gateway restart; echo done",
        "missher gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;missher gateway restart",
        "true && missher gateway stop",
        "echo $(missher gateway restart)",
        "echo `missher gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.missher.gateway"])',
        'subprocess.run(["missher", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.missher.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["missher", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: missher gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `missher -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry MISSHER_HOME/MISSHER_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("MISSHER_PROFILE", "zeus")
        monkeypatch.delenv("MISSHER_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "missher -p zeus gateway stop",
        "missher -p zeus gateway restart",
        "missher --profile zeus gateway restart",
        "missher --profile zeus gateway stop",
        "missher --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "missher -v -p zeus gateway restart",
        "missher -p zeus -v gateway restart",
        "missher --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "missher -p venus gateway stop",
        "missher -p venus gateway restart",
        "missher --profile venus gateway restart",
        "missher --profile=venus gateway stop",
        "missher -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "missher -p zeus gateway start",
        "missher -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("missher gateway restart")
        assert _contains_gateway_lifecycle_command("missher gateway stop")

    def test_missher_home_derived_profile(self, monkeypatch):
        # Without MISSHER_PROFILE the guard falls back to the MISSHER_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("MISSHER_PROFILE", raising=False)
        monkeypatch.delenv("MISSHER_PROFILE_NAME", raising=False)
        import missher_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("missher -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("missher -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("missher -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("missher gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_missher_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade missher then run missher gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_missher_gateway_once.sh). The script must live under
        # MISSHER_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("MISSHER_HOME", str(tmp_path / ".missher"))
        scripts_dir = tmp_path / ".missher" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_MISSHER_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _MISSHER_GATEWAY=1.

    Issue #37453: systemctl --user restart missher-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart missher-gateway",
        "systemctl --user restart missher-gateway",
        "systemctl stop missher-gateway.service",
        "missher gateway restart",
        "missher gateway uninstall",
        "launchctl kickstart gui/501/ai.missher.gateway",
        "launchctl bootout gui/501/ai.missher.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.missher.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.missher/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.missher.gateway.restart-once.plist",
        "pkill -f missher.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart missher-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.missher.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-missher label: label-independent detection is the point
        # (#62891 second reproduction used `ai.missher.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.missher.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _MISSHER_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _MISSHER_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_MISSHER_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="missher gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["missher gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.missher.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.missher.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-missher systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run missher gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("missher gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfemissher gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under MISSHER_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("missher gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/missher\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 missher gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_missher_home()` falls back to Path.home(), which raises
        RuntimeError when neither MISSHER_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart missher-gateway' .",
        "grep -rn 'missher gateway restart' ./logs",
        "rg 'missher gateway restart' ../archive",
        "grep -c 'systemctl stop missher-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM missher_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell missher gateway restart"',
        'sqlite3 ./db ".system systemctl restart missher-gateway"',
        'psql ./x -c "\\! systemctl restart missher-gateway"',
        "grep -r 'missher gateway restart' . | sh",
        "grep -r 'missher gateway restart' ./logs | bash",
        "grep -r 'missher gateway restart' . | sudo sh",
        "grep -r 'x' . ; missher gateway restart",
        "grep -r 'x' . && systemctl restart missher-gateway",
        'grep -r "$(missher gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart missher-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell missher gateway restart"',
        'sqlite3 db ".system systemctl restart missher-gateway"',
        'psql -c "\\! systemctl restart missher-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    missher_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run missher gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MISSHER_HOME", str(tmp_path / ".missher"))
        (tmp_path / ".missher").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run missher gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop missher-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart missher-gateway' /var/log/syslog",
        "rg 'missher gateway restart' /home/user/.missher/logs/",
        "journalctl -u missher-gateway --grep 'systemctl restart missher-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'missher_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell missher gateway restart"',
        'psql -c "\\! systemctl restart missher-gateway"',
        "grep 'systemctl restart missher-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart missher-gateway",
        'grep "$(systemctl restart missher-gateway)" f',
        "grep 'restart' log; systemctl restart missher-gateway",
        'sqlite3 db "SELECT 1"; missher gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "missher gateway restart",
        "sudo systemctl stop missher-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart missher-gateway%'\""
        )
