"""Tests for SIGHUP protection and stdout mirroring in ``missher update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``missher_cli/main.py``.  These exist so
that ``missher update`` survives a terminal disconnect mid-install
...
from missher_cli.main_dashboard import _UpdateOutputStream, _finalize_update_output, _install_hangup_protection
from missher_cli.update_cmd import _log_only_write, _print_update_completion, _run_logged_subprocess


def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("MISSHER_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("missher_cli.update_cmd._branch_head_suffix", lambda: "")

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== missher-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("MISSHER_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("missher_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import missher_cli.config as _cfg
        if hasattr(_cfg, "_MISSHER_HOME_CACHE"):
            _cfg._MISSHER_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "missher update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_missher_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "missher_cli.config.get_missher_home", _boom, raising=True
        )
