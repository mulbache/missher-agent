...
#88251: ``missher update`` treated a failed desktop pack as non-fatal, printed
...
import missher_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "missher_constants.with_missher_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "missher_constants.display_missher_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_missher_home", lambda: tmp_path)
