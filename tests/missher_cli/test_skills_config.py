"""Tests for missher_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("missher_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("missher_cli.config.load_config")
    ...
    @patch("missher_cli.config.load_config")
    @patch.dict("os.environ", {"MISSHER_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """MISSHER_SESSION_PLATFORM should be used when MISSHER_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_missher_platform_takes_precedence(self, tmp_path, monkeypatch):
        """MISSHER_PLATFORM should win over MISSHER_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from missher_cli.skills_config import _get_categories
        skills = [
