...
missher_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `missher config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_missher_home = get_missher_home()`` at import time (module
    singleton), so monkeypatching MISSHER_HOME after import doesn't move it.
    ...
    this only matters for tests that swap MISSHER_HOME.)
    """
    import cli

    cli._missher_home = home
