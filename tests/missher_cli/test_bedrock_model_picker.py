...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (MISSHER_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via MISSHER_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in MISSHER_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from missher_cli.providers import MISSHER_OVERLAYS
        assert "bedrock" in MISSHER_OVERLAYS
