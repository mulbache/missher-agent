"""Tests for kanban board export / import (``missher_cli.kanban_transfer``).
...
from missher_cli import kanban_transfer as kt
from missher_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("MISSHER_KANBAN_HOME", str(root))
        for var in ("MISSHER_KANBAN_DB", "MISSHER_KANBAN_WORKSPACES_ROOT",
                    "MISSHER_KANBAN_ATTACHMENTS_ROOT", "MISSHER_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Missher"):
