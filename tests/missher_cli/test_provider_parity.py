...
the SAME provider universe as ``missher model`` (the CLI/TUI picker).
...
def test_every_missher_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `missher model` universe."""
    ...
    assert not missing, (
        "providers shown in `missher model` but not configurable in the desktop "
