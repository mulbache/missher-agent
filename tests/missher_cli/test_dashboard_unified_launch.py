...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "missher_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # MISSHER_HOME.  For a standard install (MISSHER_HOME unset) that root is
        # the platform-native default (~/.missher), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from missher_constants import get_default_missher_root
        assert env.get("MISSHER_HOME") == str(get_default_missher_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (MISSHER_DESKTOP=1) must NOT
