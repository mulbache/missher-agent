"""Tests for missher_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $MISSHER_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Missher-managed node dir.
"""
...
def _make_missher_node(missher_home: Path) -> Path:
    """Create a fake $MISSHER_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = missher_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    missher_home = fake_home / ".missher"
    _make_missher_node(missher_home)
    ...
    removed = uninstall.remove_node_symlinks(missher_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    missher_home = fake_home / ".missher"
    node_bin = _make_missher_node(missher_home)
