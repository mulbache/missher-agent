...
def test_finite_chat_joins_parallel_children_before_final_response(tmp_path, mode):
    ...
    env.update(HOME=str(tmp_path), USERPROFILE=str(tmp_path), MISSHER_HOME=str(home),
               MISSHER_MANAGED_DIR=str(tmp_path / "managed"), TERMINAL_CWD=str(tmp_path),
    ...
    command = [
        sys.executable, "-c", "from missher_cli.main import main; main()", "chat",
...
def test_tty_seeded_chat_keeps_background_delegation(monkeypatch, query, image):
    ...
    try:
        ...
        assert "MISSHER_SINGLE_QUERY_SESSION" not in os.environ
    finally:
        reset_session_vars()
