"""Regression for #74973 — `missher update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``missher gateway restart``.
...
def launchd(monkeypatch):
    """Stub missher_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.missher.gateway", raising=False)
...
class TestLaunchdRestartAfterUpdate:
    def test_plist_present_always_restarts_without_classifying(self, launchd, capsys):
        ...
        assert update_cmd._restart_launchd_gateway_after_update(supervision_verify=False) == (["ai.missher.gateway"], [])
    ...
    def test_restart_failure_warns_that_gateway_is_down(self, launchd, capsys):
        ...
        assert "missher gateway restart" in out
...
ai.missher.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.missher.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.missher/bin/missher
}
"""
...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.missher.gateway")
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from missher_cli.gateway import _parse_launchd_pid_from_print_output
