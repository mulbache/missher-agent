"""Tests for IRC gateway configuration via `missher setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Missher install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Missher install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#missher")
            monkeypatch.setenv("IRC_NICKNAME", "missher-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `missher setup gateway` experience with IRC present."""
