"""Regression: missher doctor must not run a generic Bearer-auth health
...
with HTTP 404. The dedicated check at missher_cli/doctor.py already covers
Anthropic with the right headers, so the pluggable profile must be
skipped by `_build_apikey_providers_list()`.

See: NousResearch/missher-agent#22346
"""

from __future__ import annotations
from missher_cli import doctor_connectivity


def test_build_apikey_providers_list_skips_dedicated_check_providers():
    from missher_cli import doctor
