"""Invariants for the canonical SQLite connect/transaction layer (``missher_cli/sqlite_util.py``).
...
from missher_cli import sqlite_util
...
_STORE_OPENERS = (
    ...
    ("missher_cli.projects_db", "connect"),
...
def test_every_store_opens_through_the_canonical_open_db(monkeypatch, tmp_path, module_name, attr):
    ...
    if module_name == "missher_cli.projects_db":
...
def test_plugin_db_wal_goes_through_the_shared_fallback(monkeypatch, tmp_path):
    ...
    seen = []
    real = missher_state_wal.apply_wal_with_fallback

    def spy(conn, **kwargs):
        seen.append(kwargs["db_label"])
        return real(conn, **kwargs)

    monkeypatch.setattr(missher_state_wal, "apply_wal_with_fallback", spy)
