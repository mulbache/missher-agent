...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_missher_home(), so
        # point MISSHER_HOME at the temp dir (the _missher_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "missher_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in MISSHER_HOME/config.yaml
    ...
    install whose MISSHER_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read MISSHER_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh MISSHER_HOME with NO config.yaml (managed/desktop first launch).
        missher_home = tmp_path / ".missher"
        missher_home.mkdir()
        monkeypatch.setenv("MISSHER_HOME", str(missher_home))
