...
Regression guard for the bug where `missher model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`missher_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in MISSHER_OVERLAYS
        overlay = MISSHER_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `missher_cli/main.py` (cmd_model): read
