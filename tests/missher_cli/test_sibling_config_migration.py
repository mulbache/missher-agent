...
`missher update` migrated only the active profile's config.yaml; sibling
...
context-local MISSHER_HOME override.
...
import missher_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        missher_constants, "get_process_missher_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_missher_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_missher_home_override() == before
