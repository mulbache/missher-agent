...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``missher_cli._subprocess_compat.pid_is_missher`` fails closed on foreign
  processes and identity mismatches.
- ``missher_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsMissherLive:
    def test_foreign_real_process_is_refused(self):
        """A live non-Missher process (bare python sleeper in a temp-ish argv)
        must never be judged safe for taskkill."""
        from missher_cli._subprocess_compat import pid_is_missher

        proc = _spawn_sleeper()
        try:
            # sys.executable in CI lives under a uv/hostedtoolcache path with
            # no 'missher' token; if the checkout path itself contains one this
            # assertion is environment-dependent, so guard for it.
            if "missher" in sys.executable.lower():
                pytest.skip("interpreter path names missher; probe would match")
            assert pid_is_missher(proc.pid) is False
        finally:
            _cleanup(proc)

    def test_stale_fingerprint_is_refused_even_for_missher_argv(self):
        ...
        try:
            ...
            assert (
                pid_is_missher(proc.pid, expected_start_time=live + 12345) is False
            )
        finally:
            _cleanup(proc)
    ...
    def test_nonexistent_pid_is_refused(self):
        ...
        assert pid_is_missher(2**24) is False
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from missher_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
