"""Tests for the post-pull HEAD-movement gate in ``missher update``.
...
Before this guard ``missher update`` printed "✓ Code updated!" and
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the missher_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns missher_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/missher_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        missher_main, "_resolve_update_branch", lambda args: "main"
    )
    ...
    monkeypatch.setattr(
        missher_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        missher_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        missher_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    missher_main.cmd_update(args)  # completes normally (no SystemExit)
