"""Tests for ``missher doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from missher_cli import doctor_live
from missher_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `missher doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("missher_cli.doctor.MISSHER_HOME", tmp_path / "home")
        monkeypatch.setattr("missher_cli.doctor.PROJECT_ROOT", tmp_path / "root")
