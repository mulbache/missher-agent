...
When ``MISSHER_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.missher/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.missher            (has its own auth.json fixture)
    * Profile     -> tmp_path/.missher/profiles/coder   (active, MISSHER_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Missher is not
...
def test_codex_cooldown_clear_writes_to_the_store_that_owns_the_borrowed_pool(profile_env):
    ...
    from missher_cli.auth_codex import clear_codex_pool_quota_cooldowns
...
def test_root_write_through_is_visible_to_the_next_fallback_read(profile_env):
    ...
    import os
    from missher_cli.auth import _save_auth_store, read_credential_pool
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_missher_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (MISSHER_HOME != ~/.missher, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    missher_home = tmp_path / "classic"
    missher_home.mkdir()
