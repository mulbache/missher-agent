"""Tests for the unified provider catalog (missher_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``missher model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from missher_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
