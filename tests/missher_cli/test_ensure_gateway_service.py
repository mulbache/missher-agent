"""Tests for missher_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `missher setup` and `missher import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("missher_constants.is_container", lambda: container)
