"""Tests for the top-level `./missher` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./missher` should use `missher_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "missher"
    called = []

    fake_main_module = types.ModuleType("missher_cli.main")

    def fake_main():
        called.append("missher_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "missher_cli.main", fake_main_module)
    ...
    assert called == ["missher_cli.main"]
