...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.missher.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.missher.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("missher_constants.display_missher_home", lambda: "~/.missher-work")
    ...
    assert "~/.missher-work/logs/launchd-reload.log" in out
