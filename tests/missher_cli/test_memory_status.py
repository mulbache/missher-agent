"""Tests for `missher memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from missher_cli.memory_setup import cmd_status
    ...
    with patch("missher_cli.config.load_config", return_value=config):
        with patch("missher_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
