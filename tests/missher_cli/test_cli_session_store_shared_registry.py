...
managers acquire the same path through ``missher_state_registry`` from the REPL thread a
...
import missher_cli.goals as goals
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = missher_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(missher_state_registry, "_open_session_db", recording_open)

    cli = SimpleNamespace()
    try:
        MissherCLI._init_session_store(cli)
        ...
        missher_state_registry.close_all()
