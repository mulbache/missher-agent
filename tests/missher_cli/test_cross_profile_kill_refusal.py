...
A poisoned/contaminated ``gateway.pid`` inside one profile's MISSHER_HOME can
truthfully name ANOTHER profile's live gateway (its ``missher_home`` stamp
...
def _pid_record(proc: subprocess.Popen, script: Path, owner_home: Path) -> dict:
    ...
    return {
        ...
        "missher_home": str(owner_home),
    }


class TestRecordedGatewayHomeConflicts:
    def test_conflicting_home_detected(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MISSHER_HOME", str(tmp_path / "profiles" / "tim"))
        record = {"pid": 1, "missher_home": str(tmp_path)}
    ...
    def test_same_home_accepted(self, tmp_path, monkeypatch):
        ...
        record = {"pid": 1, "missher_home": str(home)}
    ...
    def test_legacy_record_without_home_proves_nothing(self, tmp_path, monkeypatch):
        ...
        assert recorded_gateway_home_conflicts({"pid": 1, "missher_home": "  "}) is False
...
class TestCrossProfileStopRefusal:
    def test_stop_profile_gateway_refuses_other_profiles_pid(
        ...
        monkeypatch.setenv("MISSHER_HOME", str(tim_home))

        proc = _spawn_gateway_lookalike(
            tmp_path / "bin", tim_home / "gateway.lock"
        )
        try:
            record = _pid_record(proc, tmp_path / "bin" / "missher", root_home)
            (tim_home / "gateway.pid").write_text(json.dumps(record))

            from missher_cli import gateway as gateway_cli
    ...
    def test_stop_profile_gateway_still_stops_own_gateway(
        ...
        try:
            record = _pid_record(proc, tmp_path / "bin" / "missher", tim_home)
            (tim_home / "gateway.pid").write_text(json.dumps(record))
