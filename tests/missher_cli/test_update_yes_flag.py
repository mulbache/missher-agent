"""Tests for `missher update --yes / -y` — assume yes for interactive prompts.
...
from missher_cli.main import cmd_update


@pytest.fixture(autouse=True)
def _isolate_update(isolated_update_runtime, monkeypatch):
    import shutil
    from missher_cli import managed_uv, update_cmd
...
class TestUpdateYesConfigMigration:
    ...
    @patch("missher_cli.update_cmd._reload_config_modules")
    @patch("missher_cli.update_cmd._run_migrate_config_fresh")
    @patch("missher_cli.update_cmd._run_config_check_fresh", return_value=(1, 2))
    @patch("missher_cli.config.get_missing_config_fields", return_value=[])
    @patch("missher_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``missher_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``missher_cli.main`` first could leave
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    the exception escape and crash `missher update` mid-flight.
    """
    ...
    def test_unicode_decode_error_in_tty_skips_and_prints_hint(
        ...
        out = capsys.readouterr().out
        assert "missher config migrate" in out
    ...
    def test_upstream_remote_prompt_unicode_decode_error_falls_through_to_skip(
        ...
        from missher_cli.update_cmd import _sync_with_upstream_if_needed

        with patch(
            "missher_cli.update_cmd._has_upstream_remote", return_value=False
        ), patch(
            "missher_cli.update_cmd._should_skip_upstream_prompt", return_value=False
        ), patch(
            "missher_cli.update_cmd._add_upstream_remote"
        ) as mock_add, patch(
            "builtins.input",
            side_effect=UnicodeDecodeError("utf-8", b"\xff", 0, 1, "invalid byte"),
        ):
