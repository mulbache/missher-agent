...
stale grafts break ``merge-base`` and push ``missher update`` into the
orphan-divergence reset path. ``prune_stale_shallow_grafts()`` drops grafts no
live ref points at; ``missher update --check`` calls it after its successful
...
from missher_cli.gitlock import prune_stale_shallow_grafts
...
def test_update_check_prunes_and_reports_count(tmp_path, monkeypatch, capsys):
    """`missher update --check` prunes grafts after its depth-1 fetch and reports the prune."""
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        "missher_cli.update_contract.evaluate_update_admission", lambda root: None
    )
    ...
    monkeypatch.setattr("missher_cli.banner._github_compare_behind", lambda *a, **k: 0)
    prune_calls = []
    monkeypatch.setattr(
        "missher_cli.gitlock.prune_stale_shallow_grafts",
