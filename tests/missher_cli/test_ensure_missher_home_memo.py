"""ensure_missher_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".missher"
    second = tmp_path / "b" / ".missher"

    monkeypatch.setenv("MISSHER_HOME", str(first))
    cfg.ensure_missher_home()

    # Profile switch: MISSHER_HOME moves → the new path is ensured too.
