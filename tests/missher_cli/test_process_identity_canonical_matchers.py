...
from missher_cli.dashboard_procs import _is_desktop_local_serve_cmdline
from missher_cli.update_cmd_windows import _missher_holder_subcommand, _is_backend_argv
...
# value. The updater's kill set additionally requires the Desktop's `-m missher_cli.main` spawn shape —
# a user-launched `missher serve` / `missher dashboard` is refused on, never tree-killed.
CMDLINES = [
    ("python -m missher_cli.main serve " + LOOPBACK, "serve", True, True),
    ("python -m missher_cli.main dashboard", "dashboard", False, True),
    ("/venv/bin/missher serve --isolated --host=127.0.0.1 --port=0 --ssh-owner-nonce abc", "serve", True, False),
    (r"C:\missher\.venv\Scripts\missher.exe serve --host 100.106.105.2 --port 9119", "serve", False, False),
    ("missher.exe dashboard", "dashboard", False, False),
    ("missher --profile ops serve " + LOOPBACK, "serve", True, False),
    ("missher -m serve kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("python -m missher_cli.main kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("missher --reasoning high dashboard " + LOOPBACK, "dashboard", False, False),
    ("missher gateway run --replace", "gateway", False, False),
    ("missher chat --model serve", "chat", False, False),
...
def test_kill_and_relaunch_predicates_agree_with_the_canonical_holder_matcher(
    ...
    assert _missher_holder_subcommand(cmdline) == subcommand
...
def test_desktop_local_serve_spares_fixed_port_and_remote_hosts():
    assert not _is_desktop_local_serve_cmdline("missher serve --host 100.106.105.2 --port 9119 --skip-build")
    assert not _is_desktop_local_serve_cmdline("missher serve --host 127.0.0.1 --port 9119")
    assert _is_desktop_local_serve_cmdline("missher serve --host localhost --port 0")
