...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_missher_home", lambda: missher_home)
    monkeypatch.setattr(uninstall, "_is_default_missher_home", lambda home: False)
    ...
    assert str(missher_home) in output
    assert project_root.exists()
    assert missher_home.exists()
