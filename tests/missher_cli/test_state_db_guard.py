...
from missher_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    MISSHER_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def missher_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("missher_cli.config") or mod == "missher_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, missher_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, missher_home, capsys, monkeypatch):
