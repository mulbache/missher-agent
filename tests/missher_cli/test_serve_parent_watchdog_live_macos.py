...
calls ``os._exit(0)`` ~5ms after ``MISSHER_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            MISSHER_PARENT_PID=str(parent.pid),
            MISSHER_PARENT_START_MARKER=drifted_marker,
            MISSHER_PARENT_NONCE="nonce-95693",
            MISSHER_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("MISSHER_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "MISSHER_BACKEND_READY"), "backend never announced READY"
