"""Venv ownership preflight for ``missher update`` (#83529).

A venv touched by ``sudo pip`` / ``sudo missher`` contains root-owned files
(e.g. ``site-packages/missher_agent-*.dist-info/INSTALLER``). A later normal
``missher update`` then dies mid-mutation inside ``uv pip install -e .``
("Permission denied (os error 13)") with ``venv/bin/missher`` already deleted,
...
def _make_fake_venv(tmp_path):
    ...
    (venv / "bin" / "missher").write_text("#!stub\n")
    (venv / "bin" / "python").write_text("#!stub\n")
    sp = venv / "lib" / "python3.12" / "site-packages"
    dist_info = sp / "missher_agent-1.0.0.dist-info"
...
def test_foreign_owned_dist_info_child_detected(tmp_path, monkeypatch):
    ...
    installer = str(
        ...
        / "missher_agent-1.0.0.dist-info" / "INSTALLER"
    )
...
def test_foreign_owned_refuses_with_chown_hint(tmp_path, monkeypatch, capsys):
    venv = _make_fake_venv(tmp_path)
    missher_bin = str(venv / "bin" / "missher")
    real_uid = update_cmd._path_uid
    monkeypatch.setattr(
        update_cmd,
        "_path_uid",
        lambda p: 0 if str(p) == missher_bin else real_uid(p),
    )
    monkeypatch.setattr(
        update_cmd_deps,
        "_path_uid",
