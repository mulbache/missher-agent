...
``.missher-catalog.json`` sidecar) and the install endpoint has NO kill-list bypass."""
...
from missher_cli import plugin_catalog as pc_cat
...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_missher_home):
    try:
...
def _install(name: str, sidecar: dict | None):
    from missher_constants import get_missher_home
    d = get_missher_home() / "plugins" / name
    d.mkdir(parents=True)
    (d / "plugin.yaml").write_text(yaml.safe_dump({"name": name, "version": "1.0", "description": "x"}))
    if sidecar:
        (d / ".missher-catalog.json").write_text(json.dumps(sidecar))
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    from missher_cli import plugins_cmd
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Missher plugin catalog" in resp.json()["detail"]
