"""Tests for the Ctrl+S prompt stash wiring inside MissherCLI.

The state machine itself is covered by tests/missher_cli/test_prompt_stash.py. These
tests verify the cli.py side:
  - MissherCLI.__init__ creates a PromptStash
...
tests/missher_cli/test_cli_extension_hooks.py.
"""
...
def _make_cli(**kwargs):
    """Create a MissherCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports missher_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``MissherCLI.run()``'s local ``KeyBindings`` instance, so they are not
