...
``missher*.exe`` shims aside (``missher.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``missher`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("missher", "missher-agent", "missher-acp", "missher-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "missher.exe" in names, "missher.exe must be restored after a no-op install"
    assert "missher-acp.exe" in names
    assert "missher-gateway.exe" in names
    assert (scripts / "missher.exe").read_bytes() == b"MZ-old-missher"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "missher.exe").read_bytes() == b"MZ-new-missher"
...
# ---------------------------------------------------------------------------
# missher_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "missher.exe" in names, "missher.exe must be restored after a no-op recovery install"
    assert (scripts / "missher.exe").read_bytes() == b"MZ-old-missher"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "missher.exe").write_bytes(b"MZ-new-missher")
