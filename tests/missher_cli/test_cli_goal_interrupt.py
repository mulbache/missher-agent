...
minimal ``MissherCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def missher_home(tmp_path, monkeypatch):
    """Isolated MISSHER_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".missher"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("MISSHER_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves MISSHER_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal MissherCLI stub with an active goal wired in."""
    from cli import MissherCLI
    from missher_cli.goals import GoalManager

    cli = MissherCLI.__new__(MissherCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, missher_home):
        ...
        with patch("missher_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, missher_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, missher_home):
        ...
        src = inspect.getsource(MissherCLI.chat)
