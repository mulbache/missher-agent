...
def missher_home(tmp_path, monkeypatch):
    ...
    # /etc/missher on the dev/CI box can't influence the test.
    monkeypatch.setenv("MISSHER_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(missher_home, monkeypatch):
...
def test_env_expansion_in_user_config(missher_home, monkeypatch):
    from missher_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_missher_dotenv(missher_home=str(home))
