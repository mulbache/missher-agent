"""Tests for missher_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import missher_cli.gui_uninstall as gu


def _make_agent(missher_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = missher_home / "missher-agent"
    (agent_root / "missher_cli").mkdir(parents=True)
    (agent_root / "missher_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(missher_home: Path) -> None:
    """Create the source-built GUI artifacts a `missher desktop` run produces."""
    desktop = missher_home / "missher-agent" / "apps" / "desktop"
    ...
    (missher_home / "missher-agent" / "node_modules").mkdir(parents=True)
    (missher_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(missher_home: Path) -> None:
    (missher_home / "config.yaml").write_text("x: 1\n")
    (missher_home / ".env").write_text("KEY=secret\n")
    (missher_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(missher_home)
    _make_gui_build(missher_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(missher_home)
    ...
    assert summary["missher_home"] == str(missher_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `missher desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    missher_home = tmp_path / ".missher"
    _make_agent(missher_home)
    icon = lde.icon_path(missher_home / "missher-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(missher_home)
    ...
    assert lde.icon_path(missher_home / "missher-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (missher_home / "missher-agent" / "missher_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".missher")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import missher_cli.uninstall as uninstall
