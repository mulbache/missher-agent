...
board into `MISSHER_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`missher kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `missher kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `MISSHER_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("MISSHER_KANBAN_BOARD")
    os.environ.pop("MISSHER_KANBAN_BOARD", None)
    try:
        yield
    finally:
        if prev is None:
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("MISSHER_KANBAN_BOARD") == "preset"
