...
def test_memo_expires_after_ttl(monkeypatch, tmp_path):
    ...
    cache_key = auth.missher_home_key()
...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    missher_constants.set_missher_home_override (gateway/run.py,
    tui_gateway/server.py), not the MISSHER_HOME env var — the memo must key
    ...
    reset_token = missher_constants.set_missher_home_override(str(profile_a))
    try:
        token_a = auth.resolve_nous_access_token()
    finally:
        missher_constants.reset_missher_home_override(reset_token)
    ...
    reset_token = missher_constants.set_missher_home_override(str(profile_b))
    try:
