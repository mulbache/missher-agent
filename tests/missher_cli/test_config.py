"""Tests for missher_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\missher — see
                # missher_constants._get_platform_default_missher_home.
                ...
                assert home == base / "missher"
            else:
                assert home == Path.home() / ".missher"


class TestEnsureMissherHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from missher_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Missher Agent, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``missher logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from missher_cli import config as cfg_mod
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from missher_cli.config import atomic_config_write
...
class TestLoadEnvInlineComments:
    def test_unquoted_hash_is_a_comment_quoted_hash_is_data(self, tmp_path):
        ...
        is a comment, a quoted value keeps its hash. Missher' own writer (_quote_env_value) always quotes
        values containing `#`, so a saved secret round-trips."""
        from missher_cli.config import invalidate_env_cache
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MISSHER_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MISSHER_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MISSHER_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MISSHER_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("missher_cli.config.sanitize_env_file", return_value=2),
            patch(
                "missher_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("missher_cli.config.read_raw_config", return_value={}),
            patch("missher_cli.config.get_missing_env_vars", return_value=[]),
            patch("missher_cli.config.get_missing_config_fields", return_value=[]),
            patch("missher_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from missher_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """MISSHER_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `missher setup`) lets a
        ...
        via config.yaml; MISSHER_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from missher_cli.config import OPTIONAL_ENV_VARS
        assert "MISSHER_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `missher setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `missher setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_missher_home(self):
        from missher_cli.config_migrations import support_floor_message
        from missher_constants import display_missher_home

        msg = support_floor_message()
        assert f"{display_missher_home()}/config.yaml" in msg
...
class TestRetiredMultiplexAllowlist:
    def test_v43_drops_multiplex_profile_allowlist_from_user_config(self, tmp_path, monkeypatch):
        ...
        from missher_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Missher runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Missher
    process on next startup via the dotenv → ``os.environ`` chain in
    ``missher_cli/env_loader.py``.
    ...
    def _missher_home(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MISSHER_HOME", str(tmp_path))
        ensure_missher_home()
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "MISSHER_LANGFUSE_PUBLIC_KEY",
            "MISSHER_SPOTIFY_CLIENT_ID",
            "MISSHER_QWEN_BASE_URL",
            "MISSHER_MAX_ITERATIONS",
        ],
    )
    def test_missher_integration_keys_still_writable(self, allowed_key):
        """``MISSHER_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "MISSHER_COPILOT_ACP_COMMAND",
            "MISSHER_COPILOT_ACP_ARGS",
            "MISSHER_YOLO_MODE",
            "MISSHER_ACCEPT_HOOKS",
            "MISSHER_REDACT_SECRETS",
            "MISSHER_INTERACTIVE",
            "MISSHER_EXEC_ASK",
            "MISSHER_GATEWAY_SESSION",
            "MISSHER_CRON_SESSION",
            "MISSHER_SINGLE_QUERY_SESSION",
            "MISSHER_SESSION_KEY",
            "MISSHER_SESSION_PLATFORM",
        ],
    )
    def test_missher_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        (tmp_path / ".env").write_text(
            f"MISSHER_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["MISSHER_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Missher_Yolo_Mode", "MISSHER_YOLO_MODE"),
            ("Missher_Optional_Mcps", "MISSHER_OPTIONAL_MCPS"),
            ("Missher_Copilot_Acp_Command", "MISSHER_COPILOT_ACP_COMMAND"),
            ("Missher_Copilot_Acp_Args", "MISSHER_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from missher_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from missher_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from missher_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"MISSHER_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: missher\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "missher"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import missher_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`missher config set/unset` must exit cleanly (no traceback) when the
...
def test_gateway_multiplex_keys_are_recognized_config_keys():
    """``missher config set gateway.multiplex_profiles true`` used to warn 'not a recognized config
    ...
    from missher_cli.config_defaults import DEFAULT_CONFIG
