"""Tests for missher_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from missher_cli import relaunch as relaunch_mod


class TestResolveMissherBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/missher"
        ...
        assert relaunch_mod.resolve_missher_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "missher"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'missher' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_missher_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/missher" if name == "missher" else None
        )
        assert relaunch_mod.resolve_missher_bin() == "/usr/bin/missher"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_missher_bin", lambda: "/usr/bin/missher")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/missher"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/missher", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/missher", ["/usr/bin/missher", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        missher).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_missher_bin", lambda: r"C:\Users\test\missher.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\missher.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\missher.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_missher_bin", lambda: r"C:\missher.exe")
...
class TestResolveMissherBinWindowsPyGuard:
    """On Windows, resolve_missher_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a missher.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\missher.exe" if name == "missher" else None,
        )

        bin_path = relaunch_mod.resolve_missher_bin()
        # Must NOT be the .py — must be the missher.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\missher.exe"
    ...
    def test_posix_python_launcher_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        The git installer invokes ``missher`` with the venv interpreter while the
        ...
        script = tmp_path / "missher"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        wrapper = tmp_path / "bin" / "missher"
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: str(wrapper) if name == "missher" else None,
        )
        assert relaunch_mod.resolve_missher_bin() == str(wrapper)
    ...
    def test_posix_python_launcher_on_path_falls_back_to_current_python(
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil,
            "which",
            lambda name: str(script) if name == "missher" else None,
        )

        assert relaunch_mod.resolve_missher_bin() is None

        # A console script pinned to the running interpreter keeps the venv: still exec-able.
        pinned = tmp_path / "pinned" / "missher"
        pinned.parent.mkdir()
        pinned.write_text(f"#!{sys.executable}\n")
        pinned.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(pinned), "chat"])
        assert relaunch_mod.resolve_missher_bin() == str(pinned)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_missher_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND missher.exe
