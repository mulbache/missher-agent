...
Covers:
  ...
  - Namespace spoofing (missher:, foreign, already-colon'd) is rejected
  ...
  - `missher plugins show` output includes emits/listens
"""
...
@pytest.mark.parametrize(
    "bad_event",
    [
        "missher:x",   # reserved core prefix
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "missher:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including missher: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("missher:core_event", lambda **p: got.append("missher"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "missher:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
...
def test_pending_budget_drops_new_event_without_blocking(monkeypatch, caplog):
    from missher_cli import plugins_dispatch
