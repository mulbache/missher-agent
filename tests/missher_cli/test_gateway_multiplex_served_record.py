...
- Probe: ``named_profile_served_by_running_multiplexer`` reads the pid-verified default
  ``gateway_state.json`` first (env-only opt-in on the default profile is invisible to ``missher -p X``);
...
- ``missher status`` / ``cron status`` on a satellite say running-via-multiplexer; the default lists served.
"""
...
def served_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MISSHER_HOME", str(root / "profiles" / "coder"))
    ...
    monkeypatch.setattr(status, "_read_process_cmdline", lambda pid: (
        "python -m missher_cli.main gateway run" if pid == os.getpid() else real_cmdline(pid)))
    return root
...
def test_probe_falls_back_to_config_only_without_recorded_key(served_root):
    ...
    (served_root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "missher_home": str(served_root), "gateway_state": "running"}))
...
def test_probe_survives_a_missing_default_pid_file(served_root):
    ...
    (``missher -p X status``, ``cron list``, the dashboard ladder) say "not running" about the gateway
...
def test_recycled_pid_does_not_lend_a_stale_record_its_served_profiles(served_root):
    ...
    once did, so `missher -p coder gateway start` exited 78 for a multiplexer that was long gone."""
    import subprocess
    import gateway.status as status
    from missher_cli.gateway import named_profile_served_by_running_multiplexer
    ...
    try:
        ...
        for name in ("gateway.pid", "gateway_state.json"):
            (served_root / name).write_text(json.dumps({
                "pid": child.pid, "missher_home": str(served_root), "gateway_state": "running",
...
def test_status_surfaces_agree_for_a_satellite_profile(served_root, monkeypatch):
    import missher_cli.gateway as gw
    import missher_cli.status as st
    import missher_cli.cron as cr
...
def test_dashboard_liveness_ladder_reports_served_profile_running(served_root):
    ...
    "stopped" while `missher -p X status` said running. Alpha's `<X>:<platform>` entries project as its own."""
...
def test_dashboard_lifecycle_verbs_target_the_multiplexer(served_root, monkeypatch):
    ...
    # This process's own MISSHER_HOME is coder's; the restart child must still run under the DEFAULT
    ...
    assert _profile_action_environment(restart)["MISSHER_HOME"] == str(served_root)
