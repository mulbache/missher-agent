...
Covers:
 - Normal path (tokens already in Missher auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Missher
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `missher auth openai-codex`. The old
...
@pytest.fixture()
def missher_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Missher auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(missher_auth_only_env):
    """openai-codex appears when tokens are already in Missher auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Missher
    auth store."""
    missher_home = tmp_path / ".missher"
    missher_home.mkdir()
