"""Tests for the post-pull syntax guard in ``missher update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``missher_cli/config.py``), the CLI
becomes unbootable — every ``missher`` invocation imports those files at
...
markers in ``missher_cli/config.py``; users who ran ``missher update`` in
the 7-minute window before #28458 landed could not run any ``missher``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(missher_main.subprocess, "run", fake_run)
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except missher_constants.py
    for relpath in update_cmd._UPDATE_CRITICAL_FILES:
        if relpath == "missher_constants.py":
            continue
...
# release; if a future ``missher update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
