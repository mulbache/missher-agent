...
``[diff "x"] command=/textconv=`` driver. Missher gathers workspace context by
...
context-gathering git path Missher runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from missher_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
...
def test_web_git_diff_is_safe(malicious_repo):
    from missher_cli import web_git
    repo, marker = malicious_repo
