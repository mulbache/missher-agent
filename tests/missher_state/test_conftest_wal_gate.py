"""The conftest WAL gate must agree with missher_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``missher_state``. That is deliberate: importing
``missher_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.missher`` before the per-test ``MISSHER_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_missher_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and missher_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_missher_state_at_collection():
    """The gate must stay import-free of missher_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.missher, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``missher_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "missher_state" or name.startswith("missher_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported missher_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.missher during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("missher_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["missher_state"] = saved
    assert not blocked
