...
def _reset_configured_delete_override_warned_paths():
    ...
    missher_state_wal._delete_overridden_warned_paths.clear()
    yield
    missher_state_wal._delete_overridden_warned_paths.clear()
...
def test_wal_probe_unknown_never_emits_set_pragma(monkeypatch, tmp_path, caplog):
    ...
    from missher_state_wal import WalUnsupportedError, apply_wal_with_fallback

    _configure_mode(monkeypatch, tmp_path, "wal")
    _disable_vulnerable_gate(monkeypatch)
    missher_state_wal._wal_probe_unknown_paths.clear()
    ...
    try:
        ...
        monkeypatch.setattr("missher_state_wal._on_disk_journal_mode", lambda _conn: None)
        conn = sqlite3.connect(str(db_path), factory=_SpyConnection)
        try:
            with caplog.at_level(logging.WARNING, logger="missher_state_wal"):
...
def test_resolve_journal_mode_uses_real_database_config(monkeypatch, tmp_path):
    from missher_state_wal import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "DELETE")
    assert resolve_journal_mode() == "delete"


def test_new_nonsecret_missher_env_override_is_not_exposed(monkeypatch, tmp_path):
    from missher_state_wal import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "wal")
    monkeypatch.setenv("MISSHER_JOURNAL_MODE", "delete")
...
def test_real_db_openers_honor_configured_delete(monkeypatch, tmp_path):
    ...
    from missher_cli import kanban_db, projects_db
    ...
    discord = DiscordRecoveryStore(missher_home=tmp_path)
