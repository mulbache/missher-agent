...
- files inside the Missher home tree are repaired with ``chmod u+rw``
...
from missher_state import SessionDB, preflight_db_writability
...
def missher_home(tmp_path, monkeypatch):
    """Isolated MISSHER_HOME so the repair scope covers tmp DBs."""
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, missher_home):
        db = missher_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, missher_home):
        db = missher_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, missher_home):
        sub = missher_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, missher_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, missher_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, missher_home):
        db = missher_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, missher_home):
        db_path = missher_home / "state.db"
