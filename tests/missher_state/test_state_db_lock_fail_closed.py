...
work on a file several Missher processes share (gateway service, the Desktop
app's `missher serve` backend, CLI sessions, the TUI slash worker):

* `missher_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `missher_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
