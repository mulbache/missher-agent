...
Missher instances: a main gateway (user ``ubuntu``,
MISSHER_HOME=/home/ubuntu/.missher) and a demo gateway (user ``demo``,
MISSHER_HOME=/home/demo/.missher).  The demo gateway runs as another user, so
its ``/proc/<pid>/fd`` table is unreadable from the main instance and the
holder scan falls back to ``/proc/<pid>/cmdline`` + ``_looks_like_missher``.
The demo process's argv matches the Missher patterns exactly, so the fallback
...
rebuild in ``missher_state_schema._recover_stale_fts`` was deferred 42 times
...
missher); a genuine second instance with a DIFFERENT MISSHER_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* Missher
...
# token lives under /home/demo/.missher, the binary name matches the Missher
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.missher/missher-agent/missher",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + missher launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.missher/missher-agent/venv/bin/python",
    "/home/demo/.missher/missher-agent/missher_cli/main.py",
    "gateway",
]

# A Missher-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["missher", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT Missher home → not a holder of our state.db."""
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["missher", f"--db={db_path}", "gateway"],
            ["missher", "checkpoint", f"{db_path}-wal"],
            ["missher", "--home", our_home, "gateway"],
        ):
            assert missher_state_holders._looks_like_missher(argv) or argv[0] == "missher"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
