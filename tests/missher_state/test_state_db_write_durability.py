...
The defect: missher_state already knows macOS `fsync()` does not guarantee
...
from missher_state import repair_state_db_schema
from missher_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in missher_state_repair; missher_state only
    # re-imports them.
    import missher_state_repair

    source = Path(missher_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(missher_state_repair.__file__))
