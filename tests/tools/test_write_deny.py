...
class TestWriteDenyExactPaths:
    ...
    def test_missher_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.missher/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "missher_root"
        ...
        # Sanity check: MISSHER_HOME does point to the profile dir, not the root.
        from missher_constants import get_missher_home, get_default_missher_root
...
class TestWriteAllowed:
    ...
    def test_missher_control_files_requested_writable(self):
