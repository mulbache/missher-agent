...
Older images made /opt/missher/ui-tui and /opt/missher/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV MISSHER_TUI_DIR=/opt/missher/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R missher:missher /opt/missher/ui-tui" not in text
    assert "chown -R missher:missher /opt/missher/node_modules" not in text
