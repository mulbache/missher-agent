...
``missher_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``MISSHER_`` prefix was dropped
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            out[k] = v
            continue
        if k in _MISSHER_CHILD_ALLOWED:
            out[k] = v
    return out
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # MISSHER_* handling (#27303): only the operational allowlist passes;
        # every other MISSHER_* is dropped (the broad prefix was removed).
        "MISSHER_HOME": "/home/alice/.missher",        # allowlisted → kept
        "MISSHER_PROFILE": "default",                 # allowlisted → kept
        "MISSHER_INTERACTIVE": "1",                   # not allowlisted → dropped
        "MISSHER_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "MISSHER_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``missher_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``missher_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        import tempfile, ast
        stub = generate_missher_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_missher_tools_module(["terminal"], transport="uds")
...
# let the child import missher_tools, a user script that printed non-ASCII
