...
was gated on ``[ -f "$MISSHER_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'MISSHER_HOME="{home}"\n'
        # In tests we run unprivileged; as_missher is a passthrough then.
        'as_missher() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Missher loads $MISSHER_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/missher/.env.example -> $MISSHER_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $MISSHER_HOME/.env from it on first boot"
    )
