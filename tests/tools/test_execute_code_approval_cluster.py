...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad MISSHER_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: MISSHER_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_missher_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "MISSHER_HOME": "/h", "MISSHER_PROFILE": "p",
        "MISSHER_CONFIG": "/c.yaml", "MISSHER_ENV": "/e",
        "MISSHER_DELEGATED_CHILD_CONTEXT": "1",
        # other MISSHER_* → dropped (broad prefix removed)
        "MISSHER_BASE_URL": "https://x", "MISSHER_INTERACTIVE": "1",
        "MISSHER_KANBAN_TASK": "t_parent",
    ...
    for kept in (
        ...
        "MISSHER_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "MISSHER_BASE_URL", "MISSHER_INTERACTIVE", "MISSHER_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped MISSHER_* vars."""
    import logging
