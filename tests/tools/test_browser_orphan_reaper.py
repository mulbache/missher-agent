...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning missher PID to write to <session>.owner_pid
...
class TestReapOrphanedBrowserSessions:
    ...
    def test_real_profile_attach_daemon_is_reaped_when_owner_is_dead(self, fake_tmpdir):
        """#100855: the shared ``missher-real-profile`` attach daemon is not ``<prefix>_<hex>``
        ...
        def _pid_exists(pid):
            return pid == 4242  # daemon alive, owning missher gone
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which missher process owns a daemon so that
    concurrent missher processes don't reap each other's active browser
...
class TestLeakedDaemonWithLiveOwner:
    ...
    long-lived missher process, pinning ~5 CPU cores and driving load to 100+.
...
class TestPeriodicOrphanReap:
    ...
    boot — which is exactly what happens in a missher process that stays up
    for days.
    """
