...
Covers:
- :class:`MissherTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.MissherMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _MISSHER_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# MissherTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = MissherTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MISSHER_HOME", str(tmp_path))
        storage = MissherTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (MissherMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: MissherTokenStorage, **context_attrs):
    ...
    provider = _MISSHER_PROVIDER_CLS.__new__(_MISSHER_PROVIDER_CLS)
    provider._missher_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MISSHER_HOME", str(tmp_path))
        storage = MissherTokenStorage("mgr-srv")
        ...
        with patch.object(
            _MISSHER_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MISSHER_HOME", str(tmp_path))
        storage = MissherTokenStorage("flow-srv")
        ...
        with patch.object(
            _MISSHER_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
