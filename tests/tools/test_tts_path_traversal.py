...
def test_output_path_rejects_missher_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    missher_home = tmp_path / "missher-home"
    missher_home.mkdir()
    monkeypatch.setattr(file_safety, "_missher_home_path", lambda: missher_home)
    monkeypatch.setattr(file_safety, "_missher_root_path", lambda: missher_home)

    target = missher_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    missher_home = tmp_path / "missher-home"
    token_dir = missher_home / "mcp-tokens"
