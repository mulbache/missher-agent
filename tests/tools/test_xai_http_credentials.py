...
def _set_xai_oauth_unavailable(monkeypatch):
    ...
    import missher_cli.auth_xai as auth_xai
...
def test_prefer_api_key_falls_back_to_oauth_without_explicit_key(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "missher_cli.config.get_env_value", lambda name, default=None: default
    )
    _install_fake_oauth_pool(monkeypatch, "oauth-token-x1")
...
def test_prefer_api_key_honors_missher_xai_base_url_with_validation(monkeypatch):
    """The preferred-key path reads the same override pair as the OAuth
    branch (MISSHER_XAI_BASE_URL first, then XAI_BASE_URL) behind the same
    ...
    monkeypatch.setattr(
        ...
        lambda name, default=None: {
            ...
            "MISSHER_XAI_BASE_URL": "https://staging.x.ai/v1",
