...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "missher_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "missher_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    missher_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["missher_cli"] = missher_cli
    sys.modules["missher_cli.config"] = types.SimpleNamespace(
        get_missher_home=lambda: Path(tempfile.gettempdir()) / "missher-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.missher/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
