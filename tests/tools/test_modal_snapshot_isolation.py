...
def _restore_tool_modules():
    original_missher_home = os.environ.get("MISSHER_HOME")
    original_modules = {
        name: module
        ...
        or name == "missher_cli"
        or name.startswith("missher_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_missher_home is None:
            os.environ.pop("MISSHER_HOME", None)
        else:
            os.environ["MISSHER_HOME"] = original_missher_home
        _reset_modules(("tools", "missher_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    missher_cli = types.ModuleType("missher_cli")
    missher_cli.__path__ = [str(REPO_ROOT / "missher_cli")]  # type: ignore[attr-defined]
    sys.modules["missher_cli"] = missher_cli
    missher_home = tmp_path / "missher-home"
    os.environ["MISSHER_HOME"] = str(missher_home)
    sys.modules["missher_cli.config"] = types.SimpleNamespace(
        get_missher_home=lambda: missher_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="missher-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": missher_home / "modal_snapshots.json",
