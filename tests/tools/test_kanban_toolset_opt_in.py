...
def test_saved_opt_in_roundtrip_reaches_schema_and_board(surface, tmp_path, monkeypatch):
    ...
    from missher_cli.tools_config import _apply_toolset_change, _get_platform_tools
    ...
    if surface == "http":
        ...
        from missher_cli.web_routers.tools import router
    ...
    try:
        ...
        token = set_missher_home_override(other_home)
        try:
        ...
        from missher_cli.kanban_db_connect import connect_closing
        from missher_cli.kanban_db import get_task
...
def test_selection_is_scoped_and_preserves_worker_and_deny_boundaries(legacy, tmp_path, monkeypatch):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        named, broad = list(pool.map(_names, [["kanban"], ["missher-cli"]]))
    ...
    monkeypatch.setenv("MISSHER_KANBAN_TASK", "t_worker")
    worker = _names(["file"])
