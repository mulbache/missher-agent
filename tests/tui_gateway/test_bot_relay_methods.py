...
from missher_cli.dashboard_auth.ws_tickets import INTERNAL_PROVIDER, INTERNAL_USER_ID
...
def home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MISSHER_HOME", str(h))
    return h
...
def test_outbox_drain_returns_each_envelope_once(home):
    ...
    env = bot_relay.enqueue_envelope(
        home, target=target, message="m", sender_profile="default", sender_handle="missher"
    )
...
def test_deliver_validates_profile_and_runs_transport(home, monkeypatch):
    ...
    assert argv[0].rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("missher", "missher.exe")
    assert "Bot Chat" in argv and "--query-file" in argv

    # 'missher' alias resolves to default
    _result(srv._methods["bot_relay.deliver"](2, {"profile": "missher", "message": "x"}))
...
def test_deliver_lands_in_live_bot_chat_instead_of_subprocess(home, monkeypatch):
    ...
    def _fake_run(argv, *a, **k):
        ...
        # daemon thread; only the relay's `missher` CLI spawn is under test.
...
def test_deliver_write_failure_still_removes_tempfile(home, monkeypatch, tmp_path):
    ...
    assert not glob.glob(str(tmp_path / "missher-relay-dm-*")), "tempfile leaked"
...
def test_deliver_child_env_carries_the_envelope_sender_on_every_attempt(home, monkeypatch, fake_runs, sender, expected):
    """MISSHER_TURN_AUTHOR on the child comes from the envelope's sender fields alone: the retry gets the same
    ...
    assert all(e["MISSHER_RELAY_TEST_MARKER"] == "kept" for e in envs)
...
def test_gateway_drains_the_mailbox_the_tools_write_to(tmp_path, monkeypatch, subdir):
    """Both ends of the relay mailbox derive the install root from MISSHER_HOME with ONE formula.
    The writer side (``message_agent``'s ``_missher_root``) and the drain side
    (``methods_bot_relay._relay_root``) must agree for a ``profiles/<name>`` home AND for an
    arbitrary subdir of the native ``~/.missher`` — a split here is silent non-delivery."""
    from tools.bot_mode_probe import _default_home, _missher_root
    from tui_gateway import methods_bot_relay

    monkeypatch.setenv("HOME", str(tmp_path))
    home = tmp_path / ".missher" / subdir
    home.mkdir(parents=True)
    monkeypatch.setenv("MISSHER_HOME", str(home))

    writer_root = _missher_root(Path(_default_home()))
    target = {"profile": "scout", "handle": "scout", "connection_id": "cloud-1",
              "connection_label": "", "title": "", "description": ""}
    env = bot_relay.enqueue_envelope(
        writer_root, target=target, message="m", sender_profile="default", sender_handle="missher")
