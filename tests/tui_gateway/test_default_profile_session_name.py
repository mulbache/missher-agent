...
def test_default_home_aliases_are_reported_as_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_missher_home", launch_home)

    for alias in (default_home.name, "missher"):
    ...
    # "missher" is a legal profile id: a REAL named profile of that name is never swallowed by the alias.
    (default_home / "profiles" / "missher").mkdir(parents=True)
    assert server._response_profile_name("missher") == "missher"
