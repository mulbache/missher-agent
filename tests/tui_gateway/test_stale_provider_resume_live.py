...
def live_home(monkeypatch):
    """A REAL isolated MISSHER_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="missher-live-staleprov-"))
    home = tmp / ".missher"
    ...
    # missher_constants caches the resolved home at first read — the env var
    ...
    import missher_cli.config as hconfig
    ...
    # conftest also re-pins missher_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(missher_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_missher_home", str(home), raising=False)
    yield home, server
