...
guard is covered in ``tests/missher_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``missher_cli.model_switch_providers.prewarm_picker_cache_async``
...
import missher_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from missher_cli.model_switch,
