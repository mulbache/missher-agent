    ...
    with patch.dict(
        "sys.modules",
        {
            "missher_constants": MagicMock(
                get_missher_home=MagicMock(return_value="/tmp/missher_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'missher-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'missher-release' patched"
    }
