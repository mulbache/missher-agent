...
``MISSHER_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def missher_root(tmp_path, monkeypatch):
    """A temp MISSHER_HOME root with two named profiles: 'work' and 'other'.

    Pointing MISSHER_HOME at a dir outside ~/.missher makes it the profile ROOT
    (get_default_missher_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "missher_home"
...
def test_add_lands_in_named_profile_only(missher_root):
    root = missher_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(missher_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(missher_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_does_not_mix_launch_runtime_into_another_profile(missher_root):
...
def test_status_includes_named_profile_runtime_in_multiplex(missher_root):
    ...
    work_token = set_missher_home_override(missher_root / "profiles" / "work")
    try:
        work_scope = missher_home_key()
    finally:
        reset_missher_home_override(work_token)
...
def test_set_api_key_writes_env_and_header_to_right_profile(missher_root):
    root = missher_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(missher_root):
    root = missher_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(missher_root):
    root = missher_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(missher_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(missher_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(missher_root):
    root = missher_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == MISSHER_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
...
def test_test_resolves_env_refs_from_requested_profile_secret_scope(missher_root, monkeypatch):
    ...
    work = missher_root / "profiles" / "work"
