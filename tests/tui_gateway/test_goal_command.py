...
def missher_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves MISSHER_HOME.
...
@pytest.fixture()
def server(missher_home, monkeypatch):
    ...
    # Pin config resolution to the isolated MISSHER_HOME. Sibling test
    ...
    # module-level ``_missher_home = get_missher_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_missher_home", missher_home)
...
def test_moa_bare_returns_usage(server, session, missher_home):
    _write_moa_config(missher_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_missher_home())
        started.set()
