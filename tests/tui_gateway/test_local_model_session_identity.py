...
from missher_cli.local_runtime import endpoint
...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("missher_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("missher_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_missher_home", launch)
    monkeypatch.setattr(server, "_probe_credentials", lambda agent: None)
    monkeypatch.setattr("missher_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("missher_cli.banner.get_available_skills", lambda: {})
    ...
    assert get_missher_home() == launch
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_missher_home() == secondary
    assert get_missher_home() == launch
