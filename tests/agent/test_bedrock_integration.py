...
Note: Tests that import ``missher_cli.auth`` or ``missher_cli.runtime_provider``
...
class TestProviderAliases:
    ...
    def test_aws_alias(self):
        from missher_cli.models import _PROVIDER_ALIASES
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        monkeypatch.setattr("missher_cli.anon_auth.guest_enabled", lambda: False)
        # Mock the auth store to have no active provider
        with patch("missher_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("missher_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("missher_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("missher_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
    ...
    def test_bedrock_openai_models_route_to_mantle_responses(self, monkeypatch):
        ...
        for model_id in BEDROCK_OPENAI_RESPONSES_MODEL_IDS:
            with patch("missher_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
                 patch("missher_cli.runtime_provider._get_model_config", return_value={
                     "provider": "bedrock",
                     "default": model_id,
                 }):
                result = resolve_runtime_provider(requested="bedrock")
...
class TestProvidersModule:
    """Verify bedrock is wired into missher_cli/providers.py."""
    ...
    def test_determine_api_mode_from_bedrock_url(self):
        from missher_cli.providers import determine_api_mode
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "missher-agent[bedrock]" not in extras["all"]
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_mantle_config_region_beats_env_region(self, monkeypatch):
        ...
        with patch("missher_cli.config.load_config_readonly",
