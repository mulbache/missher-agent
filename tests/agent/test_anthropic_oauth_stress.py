...
def _process_claude_code_refresh_worker(
    ...
    os.environ["MISSHER_HOME"] = profile_home
    ...
    def fake_refresh(refresh_token, *, use_json=False):
        ...
        # what must ensure that the second Missher process never calls this
...
def missher_home(tmp_path, monkeypatch):
    """Real, throwaway MISSHER_HOME so _auth_store_lock and
...
def test_high_concurrency_anthropic_refresh_no_lost_updates_no_deadlock(
    ...
    """CONCURRENCY 'Missher processes' race the same stale refresh token
    ...
    shared_stale_entry = _entry(
        id="pool-entry", refresh_token="stale-rt", source="manual:missher_pkce"
    )
    pools = [
...
def test_distinct_profiles_share_one_claude_refresh_without_duplicate_post(
    ...
    """
    shared_credentials_path = missher_home / "shared-claude-credentials.json"
    ...
    server_state_path = missher_home / "fake-token-server.json"
    server_state_path.write_text(
        json.dumps({"calls": [], "spent": [], "rotation": 0}),
        encoding="utf-8",
    )

    profile_homes = [missher_home / "profile-a", missher_home / "profile-b"]
