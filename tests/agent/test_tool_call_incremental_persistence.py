...
def _make_agent():
    missher_home = Path(tempfile.mkdtemp(prefix="missher-test-home-"))
    (missher_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._missher_home", missher_home),
