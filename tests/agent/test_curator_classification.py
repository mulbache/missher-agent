...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"missher-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "missher-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "missher-agent-dev"
...
# `missher curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `missher curator pin <umbrella>` in the rename block so
