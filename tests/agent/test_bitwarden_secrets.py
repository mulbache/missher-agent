...
binary download are exercised manually by `missher secrets bitwarden
...
def missher_home(tmp_path, monkeypatch):
    """Point Missher at an isolated home directory."""
    home = tmp_path / ".missher"
    home.mkdir()
    monkeypatch.setenv("MISSHER_HOME", str(home))
    # Some modules cache get_missher_home; clear if needed.
    import missher_constants
    if hasattr(missher_constants, "_MISSHER_HOME_CACHE"):
        missher_constants._MISSHER_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(missher_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from missher_cli.env_loader import _apply_external_secret_sources
