...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated MISSHER_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="missher_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".missher"))
    prev_home = os.environ.get("MISSHER_HOME")
    os.environ["MISSHER_HOME"] = os.path.join(test_home, ".missher")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("missher_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into missher_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["MISSHER_HOME"] = prev_home
