...
conversation-affinity hint Missher sends — ``prompt_cache_key`` on both
...
bucket it warmed. Missher cannot infer the logical conversation from the id's
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Missher-internal token; a session KEY embeds the
...
class TestSourceOverrideDomain:
    ...
    ``MISSHER_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("MISSHER_SESSION_SOURCE", "override-src")
