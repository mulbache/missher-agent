...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_missher_attribution(mock_openai):
    """Keyless requests still identify as Missher (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Missher Agent"
    assert str(headers.get("User-Agent", "")).startswith("MissherAgent/")
