...
manipulation. Missher keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point MISSHER_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_missher_home /
    get_default_missher_root) instead of monkeypatching private helpers —
    ...
    MISSHER_HOME=<root>/profiles/<name> makes get_default_missher_root()
