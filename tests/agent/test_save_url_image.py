...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate MISSHER_HOME under tmp_path."""
    monkeypatch.setenv("MISSHER_HOME", str(tmp_path / ".missher"))
    (tmp_path / ".missher").mkdir()

    # Force the constants/image cache helpers to re-read MISSHER_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("missher_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_missher_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under MISSHER_HOME — gateway cleanup
