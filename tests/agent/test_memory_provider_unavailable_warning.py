"""Regression tests for NousResearch/missher-agent#2765.
...
service that didn't inherit ``~/.missher/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "missher memory status" in msg
