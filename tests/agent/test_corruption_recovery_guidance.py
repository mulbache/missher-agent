...
3. Improved "corrupt" cause wording in _format_turn_completion_explanation
   with the full recovery path (missher doctor, sqlite3 .recover, backups)
"""
...
def test_format_turn_completion_corrupt_includes_recovery_options():
    ...
    assert "missher doctor" in explanation
    assert ".recover" in explanation
    assert "backups" in explanation
    assert "Freeing disk space will not help" in explanation


def test_gateway_corruption_banner_backups_dir_follows_missher_home(monkeypatch, tmp_path):
    """The gateway broadcast's step 3 must name the live backups dir, not ~/.missher (#104250).

    Pre-update backups live at ``<missher_root>/backups`` (``missher_cli/backup.py``); a
    custom-MISSHER_HOME gateway must not be told to restore from a directory that never
    held its backups.
    """
    import asyncio
    ...
    custom_home = tmp_path / "custom-missher-home"
    monkeypatch.setenv("MISSHER_HOME", str(custom_home / "profiles" / "research"))
    ...
    assert "~/.missher/backups" not in sent[0]
...
def test_format_turn_completion_corrupt_never_names_the_live_db():
    ...
    assert 'sqlite3 ~/.missher/state.db ".recover"' not in explanation
    # The replacement guidance names the safe command.
    assert "missher sessions recover --source" in explanation
...
def test_corrupt_guidance_pins_the_failing_profile(tmp_path, monkeypatch):
    """#105887: every `missher ...` command in the recovery guidance (turn explainer, gateway
    home-channel notice, exhausted-repair diagnostic) carries the active profile selector and
    names that profile's state.db. A bare `missher` follows the sticky ``active_profile`` file,
    so with another profile active the operator would repair the wrong database."""
    import asyncio

    import gateway.run as gateway_run
    from missher_state import _default_db_path
    from missher_state_repair import _persistent_repair_exhausted_error
    from run_agent import AIAgent
    ...
    commands = [line.strip() for line in explanation.splitlines() if "missher " in line]
    assert commands and all("missher -p research " in line for line in commands), commands
    # The conftest pins missher_state.DEFAULT_DB_PATH, so the store named is whatever the
    ...
    monkeypatch.setattr(runner, "_send_home_channel_message", _capture_send)
    asyncio.run(runner._send_session_db_warning_notifications())
    notice_commands = [line.strip() for line in sent[0].splitlines() if "missher " in line]
    assert notice_commands and all("missher -p research " in line for line in notice_commands), notice_commands
    assert f"--source {_default_db_path()} " in sent[0]

    exhausted = _persistent_repair_exhausted_error(home / "state.db")
    assert "`missher -p research sessions recover --source" in exhausted
