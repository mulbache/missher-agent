...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure MISSHER_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.missher/.env")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.missher/config.yaml", "api_key: missherConfigSecret123", "missherConfigSecret123"),
            (
                "head ~/.missher/profiles/work/config.yaml",
            ...
            (
                "grep 'foo|bar' ~/.missher/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.missher/config.example.yaml",
            "cat ~/.missher/config.template.yaml",
            "cat ~/.bashrc.example",
