...
def test_preset_resolution_is_cached_across_create_calls(monkeypatch, tmp_path):
    ...
    calls = {"n": 0}
    import missher_cli.moa_config as moa_cfg_mod
...
def test_slot_runtime_is_cached_across_create_calls(monkeypatch, tmp_path):
    ...
    import missher_cli.runtime_provider as rt_mod
...
def test_slot_runtime_cache_expires_after_ttl(monkeypatch):
    ...
    key = (missher_home_key(), "openai", "gpt-5")
...
def test_slot_runtime_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    accounts; a cached api_key/base_url must never cross the per-turn MISSHER_HOME override."""
    import agent.moa_loop as moa
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: {"provider": "openai", "model": "gpt-5",
                      "api_key": f"key-{missher_constants.get_missher_home().name}",
                      "base_url": "https://x", "api_mode": None})

    slot = {"provider": "openai", "model": "gpt-5"}
    tok = missher_constants.set_missher_home_override(str(a))
    try:
        assert moa._slot_runtime(slot)["api_key"] == "key-a"
    finally:
        missher_constants.reset_missher_home_override(tok)
    tok = missher_constants.set_missher_home_override(str(b))
    try:
