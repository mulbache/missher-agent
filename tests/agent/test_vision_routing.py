...
def isolated_home(monkeypatch):
    """Temp MISSHER_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="missher_test_31179_")
    missher_home = os.path.join(test_home, ".missher")
...
def _fresh_modules():
    """Drop cached missher modules so each test reloads against current env."""
    _drop_reload_targets()
