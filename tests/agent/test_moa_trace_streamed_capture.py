...
path via ``missher chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp MISSHER_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point MISSHER_HOME at a temp dir and turn moa.save_traces on."""
    missher_home = tmp_path / ".missher"
    missher_home.mkdir()
    monkeypatch.setenv("MISSHER_HOME", str(missher_home))

    # save_moa_turn reads config via missher_cli.config.load_config; stub it to
    ...
    return missher_home / "moa-traces"
