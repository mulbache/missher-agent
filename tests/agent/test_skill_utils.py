...
class TestParseConfigStringList:
    """#86661: `missher config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `missher config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    missher_home = tmp_path / "da\\ta"
    subprocess_home = missher_home / "home"
    subprocess_home.mkdir(parents=True)
    (missher_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("MISSHER_HOME", str(missher_home))
    monkeypatch.setenv("HOME", str(missher_home))
