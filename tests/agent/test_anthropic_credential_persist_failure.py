...
``~/.missher/.anthropic_oauth.json`` for ``missher_pkce`` ones.  Those singletons
...
def missher_home(tmp_path, monkeypatch):
    """Real on-disk MISSHER_HOME so ``load_pool()`` re-reads what we persisted."""
    ...
    monkeypatch.setattr(
        "missher_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_missher_oauth_writer_raises_instead_of_swallowing(missher_home, monkeypatch):
    ...
    with pytest.raises(CredentialPersistError):
        AA._write_missher_oauth_credentials(
            _ROTATED_ACCESS, _ROTATED_REFRESH, _EXPIRED_MS + 3_600_000
        )
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    missher_home, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
# ---------------------------------------------------------------------------
# Pool path: missher_pkce
# ---------------------------------------------------------------------------


def test_pool_missher_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "missher_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
