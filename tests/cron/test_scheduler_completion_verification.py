...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(missher_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "missher_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("MISSHER_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("MISSHER_TEST_RUNTIME_KEY", ""),
            "base_url": None,
