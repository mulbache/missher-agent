...
These exercise the real store against a temp MISSHER_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    ...
    # cron.jobs caches no home at import; get_missher_home() reads the env live.
    yield tmp_path
...
def test_fresh_claim_from_a_dead_same_host_owner_is_reclaimable(temp_home):
    ...
    ``missher cron run`` killed mid-flight must not block the next manual run for the whole TTL
