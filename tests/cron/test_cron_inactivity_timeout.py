...
- Unlimited timeout (MISSHER_CRON_TIMEOUT=0)
- Backward compat: MISSHER_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """MISSHER_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """MISSHER_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("MISSHER_CRON_TIMEOUT", "1200")
        raw = os.getenv("MISSHER_CRON_TIMEOUT", "").strip()
...
class TestSysPathOrdering:
    ...
    def test_missher_time_importable(self):
        """missher_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _missher_now
        assert callable(_missher_now)
