...
the shared /blueprint command handler (missher_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated MISSHER_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("missher://blueprint/morning-brief?")
