...
def cron_store(tmp_path, monkeypatch):
    ...
    (missher_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("MISSHER_HOME", str(missher_home))
    monkeypatch.setattr(J, "MISSHER_DIR", missher_home)
    monkeypatch.setattr(J, "CRON_DIR", missher_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", missher_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", missher_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_missher_now", lambda: now):
            due = J.get_due_jobs()
