...
profile's MISSHER_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.missher/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active MISSHER_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever MISSHER_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    MISSHER_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import missher_constants

    monkeypatch.setattr(
        missher_constants, "_get_platform_default_missher_home", lambda: root
    )
    monkeypatch.setenv("MISSHER_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile MISSHER_HOME (<root>/profiles/<name>), the cron store
    ...
    assert missher_constants.get_missher_home().resolve() == profile_home.resolve()
    assert missher_constants.get_default_missher_root().resolve() == root.resolve()

    # cron/jobs.py computes MISSHER_DIR from get_missher_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.MISSHER_DIR.resolve() == profile_home.resolve()
        assert (
