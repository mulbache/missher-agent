...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.missher/missher-agent/utils.py)
...
2. ``missher update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``missher_cli/main.py`` (the ``missher update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``missher_constants`` after the
